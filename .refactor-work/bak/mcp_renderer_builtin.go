...
func renderAgenticWorkflowsMCPConfigWithOptions(yaml *strings.Builder, isLast bool, includeCopilotFields bool, actionMode ActionMode, guardPolicies map[string]any) {
	...
	renderAgenticWorkflowsMCPRuntimeFields(yaml, actionMode)
	yaml.WriteString("                \"env\": {\n")
	renderMCPEnvVars(yaml, envVars, "                  ")
	renderMCPEnvCloseAndGuardPolicies(yaml, guardPolicies)
...
func renderAgenticWorkflowsMCPRuntimeFields(yaml *strings.Builder, actionMode ActionMode) {
	containerImage, entrypoint, entrypointArgs, mounts := agenticWorkflowsMCPRuntime(actionMode)
	yaml.WriteString("                \"container\": \"" + containerImage + "\",\n")
	if entrypoint != "" {
		yaml.WriteString("                \"entrypoint\": \"" + entrypoint + "\",\n")
	}
...
func renderMCPEnvCloseAndGuardPolicies(yaml *strings.Builder, guardPolicies map[string]any) {
	if len(guardPolicies) > 0 {
		yaml.WriteString("                },\n")
		renderGuardPoliciesJSON(yaml, guardPolicies, "                ")
		return
...
func renderMCPEnvVars(yaml *strings.Builder, envVars []mcpEnvVar, indent string) {
	for i, envVar := range envVars {
		comma := ","
		if i == len(envVars)-1 {
			comma = ""
		}
		yaml.WriteString(indent + "\"" + envVar.name + "\": \"" + renderMCPEnvValue(envVar) + "\"" + comma + "\n")
	}
}

func renderMCPEnvValue(envVar mcpEnvVar) string {
	if envVar.isLiteral {
		return envVar.value
	}
	return "\\${" + envVar.value + "}"
...
func agenticWorkflowsMCPRuntime(actionMode ActionMode) (string, string, []string, []string) {
	if actionMode.IsDev() {
		mcpRendererBuiltinLog.Print("Using dev mode configuration with locally built Docker image")
		return constants.DevModeGhAwImage, "", nil, []string{constants.DefaultWorkspaceMount, constants.DefaultTmpGhAwMount}
	}
	return constants.DefaultAlpineImage, GhAwBinaryPath, []string{"mcp-server", "--validate-actor"}, []string{
		constants.DefaultGhAwMount,
		constants.DefaultGhBinaryMount,
