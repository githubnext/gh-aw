...
func (r *MCPConfigRendererUnified) RenderGitHubMCP(yaml *strings.Builder, githubTool map[string]any, workflowData *WorkflowData) {
	...
	r.renderGitHubMCPJSONBody(yaml, githubTool, githubType, readOnly, lockdown, shouldUseStepOutputForGuardPolicy, toolsets, features, explicitGuardPolicies)
	if r.options.IsLast {
		yaml.WriteString("              }\n")
	} else {
		yaml.WriteString("              },\n")
...
func (r *MCPConfigRendererUnified) renderGitHubMCPJSONBody(yaml *strings.Builder, githubTool map[string]any, githubType GitHubMCPMode, readOnly bool, lockdown bool, guardPoliciesFromStep bool, toolsets string, features string, guardPolicies map[string]any) {
	if githubType == GitHubMCPModeRemote {
		r.renderGitHubMCPRemoteJSON(yaml, githubTool, readOnly, lockdown, guardPoliciesFromStep, toolsets, features, guardPolicies)
		return
	}
	r.renderGitHubMCPLocalJSON(yaml, githubTool, readOnly, lockdown, guardPoliciesFromStep, toolsets, features, guardPolicies)
}

func (r *MCPConfigRendererUnified) renderGitHubMCPRemoteJSON(yaml *strings.Builder, githubTool map[string]any, readOnly bool, lockdown bool, guardPoliciesFromStep bool, toolsets string, features string, guardPolicies map[string]any) {
	...
	RenderGitHubMCPRemoteConfig(yaml, GitHubMCPRemoteOptions{
		ReadOnly: readOnly, Lockdown: lockdown, LockdownFromStep: false,
		GuardPoliciesFromStep: guardPoliciesFromStep, Toolsets: toolsets, Features: features,
		AuthorizationValue: authValue, IncludeToolsField: r.options.IncludeCopilotFields,
		AllowedTools: getGitHubAllowedTools(githubTool), IncludeEnvSection: r.options.IncludeCopilotFields,
		GuardPolicies: guardPolicies,
	})
}

func (r *MCPConfigRendererUnified) renderGitHubMCPLocalJSON(yaml *strings.Builder, githubTool map[string]any, readOnly bool, lockdown bool, guardPoliciesFromStep bool, toolsets string, features string, guardPolicies map[string]any) {
	...
	RenderGitHubMCPDockerConfig(yaml, GitHubMCPDockerOptions{
		ReadOnly: readOnly, Lockdown: lockdown, LockdownFromStep: false,
		GuardPoliciesFromStep: guardPoliciesFromStep, Toolsets: toolsets, Features: features,
		DockerImageVersion: githubDockerImageVersion, CustomArgs: customArgs,
		IncludeTypeField: r.options.IncludeCopilotFields, AllowedTools: getGitHubAllowedTools(githubTool),
		EffectiveToken: "", GuardPolicies: guardPolicies,
	})
}
...
func renderGitHubTOMLLocal(yaml *strings.Builder, githubTool map[string]any, readOnly bool, lockdown bool, toolsets string, features string) {
	githubDockerImageVersion := getGitHubDockerImageVersion(githubTool)
	customArgs := getGitHubCustomArgs(githubTool)
	yaml.WriteString("          container = \"ghcr.io/github/github-mcp-server:" + githubDockerImageVersion + "\"\n")
	writeGitHubTOMLCustomArgs(yaml, customArgs)
	envVars := buildGitHubMCPEnvVars("$GH_AW_GITHUB_TOKEN", "$GITHUB_SERVER_URL", readOnly, lockdown, toolsets, features)
	envKeys := sliceutil.SortedKeys(envVars)
	writeTOMLInlineStringMapSection(yaml, "          ", "env", envVars)
	writeGitHubTOMLEnvVars(yaml, envKeys)
}

func writeGitHubTOMLCustomArgs(yaml *strings.Builder, customArgs []string) {
	if len(customArgs) == 0 {
		return
	}
...
func writeGitHubTOMLEnvVars(yaml *strings.Builder, envKeys []string) {
