...
var gitLog = logger.New("gitutil:gitutil")
...
func IsAuthError(errMsg string) bool {
	gitLog.Printf("Checking if error is auth-related: %s", errMsg)
	lowerMsg := strings.ToLower(errMsg)
	...
	if isAuth {
		gitLog.Print("Detected authentication error")
	}
	return isAuth
}
...
func FindGitRoot() (string, error) {
	...
	dir, err := os.Getwd()
	if err != nil {
		gitLog.Printf("Failed to get current directory: %v", err)
	...
	gitLog.Printf("Found git root: %s", root)
	return root, nil
}
...
func ReadFileFromHEAD(filePath, gitRoot string) (string, error) {
	...
	gitLog.Printf("Reading %q from git HEAD (relative path: %s)", filePath, relPath)

	cmd := exec.Command("git", "-C", gitRoot, "show", "HEAD:"+relPath)
	output, err := cmd.Output()
	if err != nil {
		gitLog.Printf("File %q not found in HEAD commit: %v", filePath, err)
