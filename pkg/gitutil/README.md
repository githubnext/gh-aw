...
| `ReadFileFromHEAD` | `func(filePath, gitRoot string) (string, error)` | Reads a file's content from the `HEAD` commit by resolving a literal tree entry with `git ls-tree` and then reading the blob with `git cat-file`; rejects paths that escape the repository; requires `git` on `PATH` |
...
- `ReadFileFromHEAD` requires `git` on `PATH` because resolving a literal tree entry and reading the resulting blob with `git ls-tree`/`git cat-file` is more reliable than re-implementing pack-file parsing in pure Go.
