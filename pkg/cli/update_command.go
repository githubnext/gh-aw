...
Examples:
  ...
  ` + string(constants.CLIExtensionPrefix) + ` update --cool-down 3d          # Apply a custom 3-day cooldown period`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			noReleaseBump, _ := cmd.Flags().GetBool("no-release-bump")
			disableReleaseBumpLegacy, _ := cmd.Flags().GetBool("disable-release-bump")
			disableReleaseBump := noReleaseBump || disableReleaseBumpLegacy
	...
	cmd.Flags().Bool("disable-release-bump", false, "Alias for --no-release-bump")
