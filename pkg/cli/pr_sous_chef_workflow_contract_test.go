...
func TestPRSousChefWorkflowAddCommentTargetContract(t *testing.T) {
	...
	assert.Contains(t, text, "Make at most 4 tool calls total.", "Sub-agent should have a hard tool-call budget")
	assert.Contains(t, text, "skip_reason: \"sub_agent_error\"", "Workflow should skip failed sub-agent responses without retry")
	assert.Contains(t, text, "echo \"eligible_count=$(jq '.prs | length' /tmp/gh-aw/agent/pr-sous-chef-candidates-compact.json)\" >> \"$GITHUB_OUTPUT\"", "Workflow should export eligible_count for early-exit guards")
}
