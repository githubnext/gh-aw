...
func TestPRSousChefWorkflowAddCommentTargetContract(t *testing.T) {
	...
	assert.NotContains(t, text, "filtered_last_comment_from_sous_chef=", "Workflow should not permanently skip PRs solely because the latest comment is from sous-chef")
	assert.NotContains(t, text, "skipped_last_comment_from_sous_chef", "Run summary should no longer report a last-comment skip counter")
	assert.NotContains(t, text, "latest PR comment is from pr-sous-chef itself", "Skip rules should rely on cooldown instead of latest-comment-only skipping")
