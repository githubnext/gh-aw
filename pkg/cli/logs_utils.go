...
func getAgenticWorkflowNames(verbose bool) ([]string, error) {
	...
	for _, file := range files {
		name, ok := getAgenticWorkflowNamesFromFile(file, verbose)
		if ok {
...
func getAgenticWorkflowNamesFromFile(file string, verbose bool) (string, bool) {
	if verbose {
...
func findAgentLogFile(logDir string, engine workflow.CodingAgentEngine) (string, bool) {
	...
	if logFileForParsing != "" && logFileForParsing != defaultAgentStdioLogPath {
		if found, ok := findAgentLogFileNonDefault(logDir, logFileForParsing); ok {
			return found, true
		}
	}

	return findAgentLogFileDefault(logDir)
}

func findAgentLogFileNonDefault(logDir, logFileForParsing string) (string, bool) {
	if found, ok := findAgentLogFileAgentOutputDir(logDir); ok {
		return found, true
	}
	if found, ok := findAgentLogFileFlattened(logDir, logFileForParsing); ok {
		return found, true
	}
	return findAgentLogFileRecursive(logDir)
}

func findAgentLogFileAgentOutputDir(logDir string) (string, bool) {
	...
	if walkErr := filepath.Walk(agentOutputDir, func(path string, info os.FileInfo, err error) error {
		...
		if info != nil && !info.IsDir() && foundFile == "" {
			foundFile = path
			return errWalkStop
		}
		return nil
...
func findAgentLogFileFlattened(logDir, logFileForParsing string) (string, bool) {
	// Check for flattened location (after flattening). Strip /tmp/gh-aw/ prefix to
	// get the relative path under logDir.
	const tmpGhAwPrefix = "/tmp/gh-aw/"
	after, ok := strings.CutPrefix(logFileForParsing, tmpGhAwPrefix)
	if !ok {
		return "", false
	}
	flattenedPath := filepath.Join(logDir, after)
	...
	if fileutil.DirExists(flattenedPath) {
		if found, ok := findAgentLogFileFlattenedDir(flattenedPath); ok {
			return found, true
		}
	}
	return findAgentLogFileByBase(logDir, logFileForParsing)
}

func findAgentLogFileFlattenedDir(flattenedPath string) (string, bool) {
	...
	return foundLogFile, foundLogFile != ""
}

func findAgentLogFileByBase(logDir, logFileForParsing string) (string, bool) {
	...
	if walkErr := filepath.Walk(logDir, func(path string, info os.FileInfo, err error) error {
		...
		if info != nil && !info.IsDir() && info.Name() == targetBase && foundByBase == "" {
			foundByBase = path
			return errWalkStop
		}
		return nil
...
func findAgentLogFileRecursive(logDir string) (string, bool) {
...
func findAgentLogFileDefault(logDir string) (string, bool) {
