...
func TestValidEngineNames(t *testing.T) {
	...
	expectedEngines := []string{"copilot", "claude", "codex", "gemini", "antigravity", "opencode", "cursor", "pi"}
...
func TestCompleteEngineNames(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			toComplete: "",
			wantLen:    8, // antigravity, claude, codex, copilot, cursor, gemini, opencode, pi
		},
		{
			name:       "c prefix returns claude, codex, copilot, cursor",
			toComplete: "c",
			wantLen:    4,
		},
		{
