	...
	for _, req := range requirements {
		if req.Optional {
			...
			if err := checkOptionalSecret(req, config); err != nil && config.Verbose {
				_, _ = console.PrintWarningMessage(fmt.Sprintf("Optional secret %s: %v", req.Name, err))
			}
			continue
		}
...
func ensureSecretAvailable(req SecretRequirement, config EngineSecretConfig) error {
	...
	if setutil.Contains(config.ExistingSecrets, req.Name) {
		if mustValidateExistingSecretValue(req) {
			_, _ = console.PrintWarningMessage(req.Name + " already exists, but GitHub does not expose stored secret values for validation.")
			_, _ = console.PrintInfoMessage("Paste the current or replacement fine-grained PAT so gh aw can validate it and update the repository secret.")
			revalidateConfig := config
			revalidateConfig.OverwriteExistingSecret = true
			return engineSecretsPromptFn(req, revalidateConfig)
		}
		_, _ = console.PrintSuccessMessage(fmt.Sprintf("Using existing %s secret in repository", req.Name))
		return nil
	}
	...
	for _, alt := range req.AlternativeEnvVars {
		if setutil.Contains(config.ExistingSecrets, alt) {
			if mustValidateExistingSecretValue(req) {
				_, _ = console.PrintWarningMessage(alt + " already exists in the repository, but GitHub does not expose stored secret values for validation.")
				_, _ = console.PrintInfoMessage(fmt.Sprintf("Paste the current or replacement fine-grained PAT so gh aw can validate it and store it as %s.", req.Name))
				revalidateConfig := config
				revalidateConfig.OverwriteExistingSecret = true
				return engineSecretsPromptFn(req, revalidateConfig)
			}
			_, _ = console.PrintSuccessMessage(fmt.Sprintf("Using existing %s secret in repository (alternative for %s)", alt, req.Name))
			return nil
		}
	}
	...
	if envValue != "" {
		...
		if req.IsEngineSecret && req.EngineName == string(constants.CopilotEngine) {
			if err := stringutil.ValidateCopilotPAT(envValue); err != nil {
				_, _ = console.PrintWarningMessage(fmt.Sprintf("%s in environment is not a valid fine-grained PAT: %s", req.Name, stringutil.GetPATTypeDescription(envValue)))
				_, _ = console.PrintErrorMessage(err.Error())
				// Continue to prompt for a new token
			} else {
				_, _ = console.PrintSuccessMessage(fmt.Sprintf("Found valid %s in environment", req.Name))
			...
			_, _ = console.PrintSuccessMessage(fmt.Sprintf("Found %s in environment", req.Name))
			// Upload to repository if we have a repo slug
			if config.RepoSlug != "" {
				return engineSecretsUploadFn(req.Name, envValue, config.RepoSlug, config.Verbose, config.OverwriteExistingSecret)
			}
...
func promptForCopilotPATUnified(req SecretRequirement, config EngineSecretConfig) error {
	...
	_, _ = console.PrintCommandMessage("  " + preconfiguredPATURL)
	fmt.Fprintln(os.Stderr, "")
	...
	if err := confirmForm.RunWithContext(config.ctx()); err != nil {
		...
		if openBootstrapBrowser(preconfiguredPATURL) {
			_, _ = console.PrintSuccessMessage("Opened the preconfigured Copilot PAT page in your browser.")
		} else {
			_, _ = console.PrintWarningMessage("Couldn't open your browser automatically (no supported opener found) — open the URL above manually.")
		}
	}
	...
	_, _ = console.PrintSuccessMessage("Valid fine-grained Copilot token received")
...
func promptForSystemTokenUnified(req SecretRequirement, config EngineSecretConfig) error {
	...
	fmt.Fprintln(os.Stderr, "")
	_, _ = console.PrintInfoMessage("When needed: " + req.WhenNeeded)
	_, _ = console.PrintInfoMessage("Recommended scopes: " + req.Description)
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Create a token at:")
	_, _ = console.PrintCommandMessage("  " + patURL)
	fmt.Fprintln(os.Stderr, "")
	...
	_, _ = console.PrintSuccessMessage(req.Name + " token received")
...
func promptForGenericAPIKeyUnified(req SecretRequirement, config EngineSecretConfig) error {
	...
	if req.KeyURL != "" {
		...
		_, _ = console.PrintCommandMessage("  " + req.KeyURL)
		fmt.Fprintln(os.Stderr, "")
	}
	...
	_, _ = console.PrintSuccessMessage(label + " API key received")
...
func checkOptionalSecret(req SecretRequirement, config EngineSecretConfig) error {
	...
	if setutil.Contains(config.ExistingSecrets, req.Name) {
		if config.Verbose {
			_, _ = console.PrintInfoMessage(fmt.Sprintf("Optional secret %s exists in repository", req.Name))
		}
		return nil
	}
	...
	if os.Getenv(req.Name) != "" { //nolint:osgetenvlibrary
		if config.Verbose {
			_, _ = console.PrintInfoMessage(fmt.Sprintf("Optional secret %s found in environment", req.Name))
		}
		return nil
	}
...
func uploadSecretToRepo(secretName, secretValue, repoSlug string, verbose bool, overwriteExisting bool) error {
	...
	if err == nil && stringContainsSecretName(string(output), secretName) {
		if !overwriteExisting {
			if verbose {
				_, _ = console.PrintInfoMessage(fmt.Sprintf("Secret %s already exists, skipping upload", secretName))
			}
			return nil
		}
		if verbose {
			_, _ = console.PrintInfoMessage(fmt.Sprintf("Secret %s already exists, replacing it with the validated value", secretName))
		}
	}

	// Upload the secret
	if verbose {
		_, _ = console.PrintInfoMessage(fmt.Sprintf("Uploading %s secret to repository", secretName))
	}
	...
	_, _ = console.PrintSuccessMessage(fmt.Sprintf("Uploaded %s secret to repository", secretName))
	return nil
}
...
func displayMissingSecrets(requirements []SecretRequirement, repoSlug string, existingSecrets map[string]struct {
	...
	if len(requiredMissing) > 0 {
		_, _ = console.PrintErrorMessage("Required secrets are missing:")
		for _, req := range requiredMissing {
			fmt.Fprintln(os.Stderr, "")
			_, _ = console.PrintInfoMessage("Secret: " + req.Name)
			_, _ = console.PrintInfoMessage("When needed: " + req.WhenNeeded)
			_, _ = console.PrintInfoMessage("Recommended scopes: " + req.Description)
			_, _ = console.PrintCommandMessage(fmt.Sprintf("gh aw secrets set %s --owner %s --repo %s", req.Name, cmdOwner, cmdRepo))
		}
	}

	if len(optionalMissing) > 0 {
		fmt.Fprintln(os.Stderr, "")
		_, _ = console.PrintWarningMessage("Optional secrets are missing:")
		for _, req := range optionalMissing {
			fmt.Fprintln(os.Stderr, "")
			_, _ = console.PrintInfoMessage(fmt.Sprintf("Secret: %s (optional)", req.Name))
			_, _ = console.PrintInfoMessage("When needed: " + req.WhenNeeded)
			_, _ = console.PrintInfoMessage("Recommended scopes: " + req.Description)
			_, _ = console.PrintCommandMessage(fmt.Sprintf("gh aw secrets set %s --owner %s --repo %s", req.Name, cmdOwner, cmdRepo))
		}
...
func displaySecretsSummaryTable(requirements []SecretRequirement, existingSecrets map[string]struct {
	...
	fmt.Fprintln(os.Stderr, "")
	_, _ = console.PrintInfoMessage("Required secrets summary:")
	fmt.Fprintln(os.Stderr, "")
