...
// TestWorkflowStatus_JSONMarshalingWithCommand tests that WorkflowStatus with command field can be marshaled to JSON
func TestWorkflowStatus_JSONMarshalingWithCommand(t *testing.T) {
	// Test that WorkflowStatus with command can be marshaled to JSON
	status := WorkflowStatus{
		Workflow:      "test-workflow",
		EngineID:      "copilot",
		Compiled:      "Yes",
	...
	if unmarshaled["command"] != "test-command" {
		t.Errorf("Expected command='test-command', got %v", unmarshaled["command"])
	}
}
