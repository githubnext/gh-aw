...
func extractExperimentData(logsPath string) *ExperimentData {
	...
	if data := extractExperimentDataFromState(logsPath); data != nil {
		return data
	}
	if data := extractExperimentDataFromUsageSummary(logsPath); data != nil {
		return data
	}
...
func extractExperimentDataFromState(logsPath string) *ExperimentData {
	...
	if data := extractExperimentDataFromRuns(state); data != nil {
		return data
	}
	return deriveExperimentDataFromCounts(state.Counts)
}

func extractExperimentDataFromRuns(state *ExperimentState) *ExperimentData {
	...
	return &ExperimentData{Assignments: lastRun.Assignments, CumulativeCounts: state.Counts}
}

func deriveExperimentDataFromCounts(counts map[string]map[string]int) *ExperimentData {
	assignments := make(map[string]string, len(counts))
	for _, name := range sliceutil.SortedKeys(counts) {
		selected := deriveLastSelectedVariant(counts[name])
		assignments[name] = selected
		experimentDataLog.Printf("Experiment %q: selected variant=%q", name, selected)
	}
	return &ExperimentData{Assignments: assignments, CumulativeCounts: counts}
}
...
func extractExperimentDataFromUsageSummary(logsPath string) *ExperimentData {
	...
	return &ExperimentData{Assignments: usageSummary.Experiments.Assignments}
}
