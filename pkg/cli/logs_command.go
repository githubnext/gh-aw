...
All available artifact sets: activation, agent, all, detection, experiment, firewall,
github-api, mcp, usage.
...
` + WorkflowIDExplanation,
  ...
  ` + string(constants.CLIExtensionPrefix) + ` logs --cache-before 2024-01-01   # Evict local cache older than 2024-01-01 before downloading runs`,
	...
	logsCmd.Flags().StringSlice("artifacts", []string{"usage"}, "Artifact sets to download (default: usage). Use 'all' for everything, or comma-separate sets. Valid sets: "+strings.Join(ValidArtifactSetNames(), ", "))
