  ...
  ` + string(constants.CLIExtensionPrefix) + ` logs --cache-before 2024-01-01   # Evict local cache older than 2024-01-01 before downloading runs`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdin {
				...
				excludeStaged, _ := cmd.Flags().GetBool("exclude-staged")
				noStagedLegacy, _ := cmd.Flags().GetBool("no-staged")
				noStaged := excludeStaged || noStagedLegacy
	...
	logsCmd.Flags().Bool("no-staged", false, "Alias for --exclude-staged")
	_ = logsCmd.Flags().MarkHidden("no-staged")
	_ = logsCmd.Flags().MarkDeprecated("no-staged", "use --exclude-staged instead")
