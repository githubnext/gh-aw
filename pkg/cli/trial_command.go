...
Repository modes:
- Default mode (no flags): Creates a temporary trial repository and simulates execution as if running against the current repository (github.repository context points to current repository)
...
`,
	...
	cmd.Flags().BoolP("yes", "y", false, "Auto-accept confirmations (required in CI)")
	cmd.Flags().Bool("dry-run", false, "Preview execution without applying any changes")
	...
	cmd.Flags().Bool("auto-merge-prs", false, "Auto-merge any pull requests created during execution")
	addEngineFlag(cmd)
	addJSONFlag(cmd)
