...
func TestAddWizardCommandMentionsSupportedEngines(t *testing.T) {
	cmd := NewAddWizardCommand(func(string) error { return nil })
	require.NotNil(t, cmd, "Add wizard command should be created")
	assert.Contains(t, cmd.Long, "Copilot, Claude, Codex, Gemini, Antigravity, or Pi", "Add wizard help should mention all interactive engine options")
}
