...
func addWorkflowsWithPR(ctx context.Context, workflows []*ResolvedWorkflow, opts AddOptions) (int, string, error) {
	...
	currentBranch, branchName, tracker, err := addWorkflowsWithPRPrepare(workflows, opts)
	if err != nil {
		return 0, "", err
	}
	...
	if err := addWorkflowsWithPRStage(tracker, opts); err != nil {
		return 0, "", err
	}

	commitMessage, prTitle, prBody := addWorkflowsWithPRMessages(workflows)

	if err := commitChanges(commitMessage, opts.Verbose); err != nil {
		return 0, "", addWorkflowsWithPRCommitError(err, commitMessage, branchName, prTitle)
	}

	if err := addWorkflowsWithPRPushBranch(branchName, prTitle, opts); err != nil {
		return 0, "", err
	}

	prNumber, prURL, err := addWorkflowsWithPRCreatePR(branchName, prTitle, prBody, tracker, opts)
	if err != nil {
		return 0, "", err
	}
...
func addWorkflowsWithPRPushBranch(branchName, prTitle string, opts AddOptions) error {
	// Push branch
...
func addWorkflowsWithPRCreatePR(branchName, prTitle, prBody string, tracker *FileTracker, opts AddOptions) (int, string, error) {
	// Create PR
...
func addWorkflowsWithPRPrepare(workflows []*ResolvedWorkflow, opts AddOptions) (string, string, *FileTracker, error) {
...
func addWorkflowsWithPRStage(tracker *FileTracker, opts AddOptions) error {
...
func addWorkflowsWithPRMessages(workflows []*ResolvedWorkflow) (string, string, string) {
	var joinedNames string
	if len(workflows) == 1 {
		joinedNames = workflows[0].Spec.WorkflowName
		return "Add agentic workflow " + joinedNames, "Add agentic workflow " + joinedNames, "Add agentic workflow " + joinedNames
	}
	...
	return "Add agentic workflows: " + joinedNames, "Add agentic workflows: " + joinedNames, "Add agentic workflows: " + joinedNames
}

func addWorkflowsWithPRCommitError(err error, commitMessage, branchName, prTitle string) error {
