...
// TestCompileWithActionlint tests the compile command with --actionlint flag
func TestCompileWithActionlint(t *testing.T) {
	setup := setupIntegrationTest(t)
	defer setup.cleanup()

	// Initialize git repository for actionlint to work (it needs git root)
	gitInitCmd := exec.Command("git", "init")
	gitInitCmd.Dir = setup.tempDir
	if output, err := gitInitCmd.CombinedOutput(); err != nil {
		t.Fatalf("Failed to initialize git repository: %v\nOutput: %s", err, string(output))
...
# Actionlint Test Workflow

This workflow tests the actionlint linter integration.
`

	testWorkflowPath := filepath.Join(setup.workflowsDir, "actionlint-test.md")
	if err := os.WriteFile(testWorkflowPath, []byte(testWorkflow), 0644); err != nil {
		t.Fatalf("Failed to write test workflow file: %v", err)
	}

	// First compile without actionlint to create the lock file
	compileCmd := exec.Command(setup.binaryPath, "compile", testWorkflowPath)
	if output, err := compileCmd.CombinedOutput(); err != nil {
		t.Fatalf("Initial compile failed: %v\nOutput: %s", err, string(output))
	}
	...
	// Check that the lock file was created
	lockFilePath := filepath.Join(setup.workflowsDir, "actionlint-test.lock.yml")
	if _, err := os.Stat(lockFilePath); os.IsNotExist(err) {
		t.Fatalf("Expected lock file %s was not created", lockFilePath)
	}

	// Now compile with --actionlint flag
	actionlintCmd := exec.Command(setup.binaryPath, "compile", testWorkflowPath, "--actionlint", "--verbose")
	output, err := actionlintCmd.CombinedOutput()

	// The command should succeed even if actionlint finds issues
	if err != nil {
		t.Fatalf("Compile with --actionlint failed: %v\nOutput: %s", err, string(output))
	}

	outputStr := string(output)

	// Note: actionlint may find warnings/errors in the generated workflow
	// The test just verifies that the command succeeds with --actionlint flag
	// If there are findings, they will be shown in the standard compiler error format

	// The lock file should still exist after actionlint scan
	if _, err := os.Stat(lockFilePath); os.IsNotExist(err) {
		t.Fatalf("Lock file was removed after actionlint scan")
	}

	t.Logf("Integration test passed - actionlint flag works correctly\nOutput: %s", outputStr)
}
