		...
		Apply: func(content string, frontmatter map[string]any) (string, bool, error) {
			...
			for i, line := range result.FrontmatterLines {
				...
				if !strings.Contains(trimmedLine, "cron:") && !strings.Contains(trimmedLine, "schedule:") {
					frontmatterLines[i] = originalLine
					continue
				}
				...
				if strings.HasPrefix(scheduleValue, "daily at") && !strings.Contains(scheduleValue, "around") && !strings.Contains(scheduleValue, "between") {
					...
					frontmatterLines[i] = fmt.Sprintf("%s%s%s: %s", leadingSpace, listMarker, fieldName, newSchedule)
					modified = true
					codemodsLog.Printf("Converted 'daily at' to 'daily around' on line %d: %s -> %s", i+1, scheduleValue, newSchedule)
					continue
				}
				...
				if strings.Contains(scheduleValue, "weekly on") && strings.Contains(scheduleValue, " at ") && !strings.Contains(scheduleValue, "around") {
					...
					frontmatterLines[i] = fmt.Sprintf("%s%s%s: %s", leadingSpace, listMarker, fieldName, newSchedule)
					modified = true
					codemodsLog.Printf("Converted 'weekly on DAY at' to 'weekly on DAY around' on line %d: %s -> %s", i+1, scheduleValue, newSchedule)
					continue
				}
				...
				if strings.HasPrefix(scheduleValue, "monthly on") {
					...
					if day != "" {
						...
						modified = true
						codemodsLog.Printf("Converted 'monthly on' to cron on line %d: %s -> %s", i+1, scheduleValue, cronExpr)
						continue
					}
				}
