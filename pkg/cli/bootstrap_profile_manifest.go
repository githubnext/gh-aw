...
func extractManifestBootstrap(value any, manifestPath string) (*repositoryPackageBootstrap, error) {
	...
	if !ok {
		return nil, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap must be a mapping. Example: bootstrap: { config: [{ type: repo-variable, name: EXAMPLE, prompt: Enter a value }] }", manifestPath)
	}

	configValue, ok := root["config"]
	if !ok {
		return nil, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config is required. Example: bootstrap: { config: [{ type: repo-variable, name: EXAMPLE, prompt: Enter a value }] }", manifestPath)
	}

	configItems, ok := configValue.([]any)
	if !ok {
		return nil, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config must be a list. Example: bootstrap: { config: [{ type: repo-variable, name: EXAMPLE, prompt: Enter a value }] }", manifestPath)
	}
	if len(configItems) == 0 {
		return nil, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config must not be empty. Example: bootstrap: { config: [{ type: repo-variable, name: EXAMPLE, prompt: Enter a value }] }", manifestPath)
	}
...
func parseManifestBootstrapAction(actionType string, actionMap map[string]any, manifestPath string, index int) (repositoryPackageBootstrapAction, error) {
	...
	case "repo-secret":
		if action.Name == "" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].name is required when type=repo-secret. Example: { type: repo-secret, name: EXAMPLE_SECRET, prompt: Enter a secret }", manifestPath, index)
		}
		if action.Prompt == "" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].prompt is required when type=repo-secret. Example: { type: repo-secret, name: EXAMPLE_SECRET, prompt: Enter a secret }", manifestPath, index)
		}
	case "github-app":
		...
		if action.ExistingOnly && action.Mode != "" && action.Mode != "existing" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].existing-only requires mode to be 'existing' or unset. Remove mode=%q or set it to 'existing'", manifestPath, index, action.Mode)
		}
		...
		if action.Mode != "create-or-existing" && action.Mode != "existing" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].mode must be one of: create-or-existing, existing. Example: { type: github-app, mode: existing, app-id-variable: APP_ID, private-key-secret: APP_PRIVATE_KEY }", manifestPath, index)
		}
		if action.Owner != "" && action.Owner != "repo" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].owner must be 'repo' when type=github-app. Example: { type: github-app, owner: repo, app-id-variable: APP_ID, private-key-secret: APP_PRIVATE_KEY }", manifestPath, index)
		}
		if action.AppIDVariable == "" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].app-id-variable is required when type=github-app. Example: { type: github-app, app-id-variable: APP_ID, private-key-secret: APP_PRIVATE_KEY }", manifestPath, index)
		}
		if action.PrivateKeySecret == "" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].private-key-secret is required when type=github-app. Example: { type: github-app, app-id-variable: APP_ID, private-key-secret: APP_PRIVATE_KEY }", manifestPath, index)
		}
	case "copilot-auth":
		if action.Secret == "" {
		...
		if action.Strategy != "prompt-if-actions-auth-unavailable" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].strategy must be 'prompt-if-actions-auth-unavailable'. Example: { type: copilot-auth, strategy: prompt-if-actions-auth-unavailable }", manifestPath, index)
		}
	case "handoff":
		if action.Message == "" {
			return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].message is required when type=handoff. Example: { type: handoff, message: Continue with repository-specific setup. }", manifestPath, index)
		}
	default:
		return repositoryPackageBootstrapAction{}, fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].type %q is not supported. Example: use one of %s", manifestPath, index, actionType, bootstrapActionTypeExample)
	}
...
func manifestBootstrapFieldError(manifestPath string, index int, field string, err error) error {
	if example, ok := manifestBootstrapFieldExample(field); ok {
		return fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].%s %s. Example: bootstrap.config[%d].%s: %s", manifestPath, index, field, err.Error(), index, field, example)
	}
	return fmt.Errorf("invalid Agentic Workflow manifest %q: bootstrap.config[%d].%s %s", manifestPath, index, field, err.Error())
}
