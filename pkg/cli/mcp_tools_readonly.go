...
- compiled_file: Path to the generated .lock.yml file`,
		...
		if args.Zizmor || args.Poutine || args.Actionlint || args.RunnerGuard || args.Grype || args.Yamllint {
			// Check if Docker images are available; if not, start downloading and return retry message
			if err := CheckAndPrepareDockerImages(ctx, args.Zizmor, args.Poutine, args.Actionlint, args.RunnerGuard, args.Grype, args.Yamllint); err != nil {
		...
		mcpLog.Printf("Executing compile tool: workflows=%v, strict=%v, fix=%v, zizmor=%v, poutine=%v, actionlint=%v, runner-guard=%v, grype=%v, yamllint=%v",
			args.Workflows, args.Strict, args.Fix, args.Zizmor, args.Poutine, args.Actionlint, args.RunnerGuard, args.Grype, args.Yamllint)
