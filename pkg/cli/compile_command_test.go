...
// TestCompileWorkflows_PurgeCampaignOrchestrators tests that --purge removes orphaned campaign orchestrator files
func TestCompileWorkflows_PurgeCampaignOrchestrators(t *testing.T) {
	// Create temporary directory structure for testing
	tempDir := testutil.TempDir(t, "test-purge-campaign-*")
	workflowsDir := filepath.Join(tempDir, ".github/workflows")
	if err := os.MkdirAll(workflowsDir, 0755); err != nil {
		t.Fatalf("Failed to create workflows directory: %v", err)
	}
	...
	validCampaignMd := filepath.Join(workflowsDir, "valid-campaign.campaign.md")
...
This is a valid campaign definition.`

	if err := os.WriteFile(validCampaignMd, []byte(validCampaignContent), 0644); err != nil {
		t.Fatalf("Failed to create valid campaign: %v", err)
	}

	// Create orphaned campaign orchestrator files (without source .campaign.md)
	orphanedOrchestratorMd := filepath.Join(workflowsDir, "orphaned-campaign.campaign.g.md")
	orphanedOrchestratorContent := `---
name: Orphaned Campaign Orchestrator
on:
  schedule:
    - cron: "0 0 * * *"
---
...
This orchestrator has no source .campaign.md file.`

	if err := os.WriteFile(orphanedOrchestratorMd, []byte(orphanedOrchestratorContent), 0644); err != nil {
		t.Fatalf("Failed to create orphaned orchestrator: %v", err)
	}
...
jobs:
  test:
	...
	if err := os.WriteFile(orphanedOrchestratorLockYml, []byte(orphanedOrchestratorLockContent), 0644); err != nil {
		t.Fatalf("Failed to create orphaned orchestrator lock file: %v", err)
	}

	// Create a valid orchestrator that SHOULD exist (has a source .campaign.md)
	validOrchestratorMd := filepath.Join(workflowsDir, "valid-campaign.campaign.g.md")
	validOrchestratorContent := `---
name: Valid Campaign Orchestrator
on:
  schedule:
    - cron: "0 0 * * *"
---

# Valid Campaign Orchestrator

This orchestrator has a source .campaign.md file.`

	if err := os.WriteFile(validOrchestratorMd, []byte(validOrchestratorContent), 0644); err != nil {
		t.Fatalf("Failed to create valid orchestrator: %v", err)
	}

	// Verify files exist before purge
	if _, err := os.Stat(orphanedOrchestratorMd); os.IsNotExist(err) {
		t.Fatal("Orphaned orchestrator should exist before purge")
	}
	if _, err := os.Stat(orphanedOrchestratorLockYml); os.IsNotExist(err) {
		t.Fatal("Orphaned orchestrator lock file should exist before purge")
	}
	if _, err := os.Stat(validOrchestratorMd); os.IsNotExist(err) {
		t.Fatal("Valid orchestrator should exist before purge")
	}
	...
	// Verify orphaned orchestrator files were deleted
	if _, err := os.Stat(orphanedOrchestratorMd); !os.IsNotExist(err) {
		t.Error("Orphaned campaign orchestrator should have been purged")
	}
	if _, err := os.Stat(orphanedOrchestratorLockYml); !os.IsNotExist(err) {
	...
	if _, err := os.Stat(validOrchestratorMd); os.IsNotExist(err) {
		t.Error("Valid campaign orchestrator should still exist (has source .campaign.md)")
	}

	// Verify the valid campaign definition still exists
	if _, err := os.Stat(validCampaignMd); os.IsNotExist(err) {
		t.Error("Valid campaign definition should still exist")
	}
}
