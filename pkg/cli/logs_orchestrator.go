...
// All configuration is passed via the cfg DownloadConfig parameter.
func DownloadWorkflowLogs(ctx context.Context, cfg DownloadConfig) error {
	...
	engine := cfg.Engine
	ref := cfg.Ref
	beforeRunID := cfg.BeforeRunID
	afterRunID := cfg.AfterRunID
	repoOverride := cfg.RepoOverride
	verbose := cfg.Verbose
	toolGraph := cfg.ToolGraph
	noStaged := cfg.NoStaged
	firewallOnly := cfg.FirewallOnly
	noFirewall := cfg.NoFirewall
	parse := cfg.Parse
	jsonOutput := cfg.JSONOutput
	timeout := cfg.Timeout
	summaryFile := cfg.SummaryFile
	safeOutputType := cfg.SafeOutputType
	filteredIntegrity := cfg.FilteredIntegrity
	train := cfg.Train
	format := cfg.Format
	artifactSets := cfg.ArtifactSets
