...
func findWorkflowsWithSource(workflowsDir string, filterNames []string, verbose bool) ([]*workflowWithSource, error) {
	...
	var fileCount int
	walkErr := filepath.WalkDir(workflowsDir, func(workflowPath string, d os.DirEntry, entryErr error) error {
		if entryErr != nil {
			return entryErr
		}
		if d.IsDir() {
		...
		fileCount++
		workflowName := normalizeWorkflowID(d.Name())
	...
	updateLog.Printf("Scanned %d markdown file(s) recursively in workflows directory", fileCount)
