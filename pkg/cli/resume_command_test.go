...
func TestNewResumeCommand(t *testing.T) {
	...
	assert.Equal(t, "Resume a Copilot CLI session from a workflow run", cmd.Short)
...
func TestFindResumeSessionID(t *testing.T) {
	t.Run("rejects missing sessions", func(t *testing.T) {
		...
		require.EqualError(t, err, "workflow run does not contain a Copilot session")
	})
	...
	t.Run("rejects multiple sessions", func(t *testing.T) {
		...
		require.EqualError(t, err, "workflow run contains multiple Copilot sessions ([session-a session-b]); unable to select one automatically")
	})
}
...
func TestRunResumeCommandRejectsNonCopilotEngine(t *testing.T) {
	...
	require.EqualError(t, err, `resume currently supports Copilot CLI runs only; run 123 used engine "claude"`)
}

func TestWithResumeCopilotHome(t *testing.T) {
	environment := withResumeCopilotHome(
