...
func TestPRCodeQualityReviewerWorkflowSubAgentModelContract(t *testing.T) {
	...
	// The timeout must be exactly 20 minutes to give grumpy-coder sub-agent (SDK idle-timeout ~14.5 min)
	// adequate headroom before the step-level timeout fires. Update this assertion when changing the timeout.
	assert.Contains(t, text, "timeout-minutes: 20", "Workflow timeout should be exactly 20 minutes to prevent sub-agent timeout cascade")
}
