...
func TestConfigureProjectTimezone_UsesRepoConfigOverEnv(t *testing.T) {
	...
	t.Setenv(compilerenv.DefaultUTC, "UTC")
	...
	loadRepoConfigForProjectTimezone = func(string) (*workflow.RepoConfig, error) {
		return &workflow.RepoConfig{UTC: "America/New_York"}, nil
	}
	...
	assert.Contains(t, consoleOutput, "2025-10-28 10:30:45 EDT (UTC-04:00)")
}
...
func TestConfigureProjectTimezone_UsesEnvFallback(t *testing.T) {
	...
	t.Setenv(compilerenv.DefaultUTC, "Europe/London")
	...
	assert.Contains(t, consoleOutput, "2025-01-28 14:30:45 GMT (UTC+00:00)")
}

func TestConfigureProjectTimezone_InvalidTimezoneResetsOverride(t *testing.T) {
	t.Setenv(compilerenv.DefaultUTC, "Invalid/Timezone")
	console.SetTimeLocation(time.FixedZone("PDT", -7*60*60))
