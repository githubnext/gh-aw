...
func renderSingleAuditDiffPretty(diff *AuditDiff) {
	...
	renderAuditDiffPrettySummary(diff)
...
func renderAuditDiffPrettySummary(diff *AuditDiff) {
	summaryParts, anomalyCount := collectAuditDiffPrettySummary(diff)
...
func collectAuditDiffPrettySummary(diff *AuditDiff) ([]string, int) {
	var summaryParts []string
	anomalyCount := 0
	summaryParts, anomalyCount = appendFirewallPrettySummary(summaryParts, anomalyCount, diff.FirewallDiff)
	return appendMCPPrettySummary(summaryParts, anomalyCount, diff.MCPToolsDiff)
}

func appendFirewallPrettySummary(summaryParts []string, anomalyCount int, diff *FirewallDiff) ([]string, int) {
	if diff == nil || isEmptyFirewallDiff(diff) {
		return summaryParts, anomalyCount
	}
	parts := make([]string, 0, 4)
	parts = appendCountSummary(parts, len(diff.NewDomains), "new domains")
	parts = appendCountSummary(parts, len(diff.RemovedDomains), "removed domains")
	parts = appendCountSummary(parts, len(diff.StatusChanges), "status changes")
	parts = appendCountSummary(parts, len(diff.VolumeChanges), "volume changes")
	if len(parts) > 0 {
		summaryParts = append(summaryParts, "Firewall: "+strings.Join(parts, ", "))
	}
	return summaryParts, anomalyCount + diff.Summary.AnomalyCount
}

func appendMCPPrettySummary(summaryParts []string, anomalyCount int, diff *MCPToolsDiff) ([]string, int) {
	if diff == nil || isEmptyMCPToolsDiff(diff) {
		return summaryParts, anomalyCount
	}
	parts := make([]string, 0, 3)
	parts = appendCountSummary(parts, diff.Summary.NewToolCount, "new tools")
	parts = appendCountSummary(parts, diff.Summary.RemovedToolCount, "removed tools")
	parts = appendCountSummary(parts, diff.Summary.ChangedToolCount, "changed tools")
	if len(parts) > 0 {
		summaryParts = append(summaryParts, "MCP tools: "+strings.Join(parts, ", "))
	}
	return summaryParts, anomalyCount + diff.Summary.AnomalyCount
}

func appendCountSummary(parts []string, count int, label string) []string {
	if count > 0 {
...
func renderFirewallDiffPrettySection(diff *FirewallDiff) {
	...
	fmt.Fprintln(os.Stderr)
	renderFirewallNewDomainsTable(diff.NewDomains)
	renderFirewallRemovedDomainsTable(diff.RemovedDomains)
	renderFirewallStatusChangesTable(diff.StatusChanges)
	renderFirewallVolumeChangesTable(diff.VolumeChanges)
}

func renderFirewallNewDomainsTable(entries []DomainDiffEntry) {
	if len(entries) == 0 {
		return
	}
...
func renderFirewallRemovedDomainsTable(entries []DomainDiffEntry) {
	if len(entries) == 0 {
		return
	}
...
func renderFirewallStatusChangesTable(entries []DomainDiffEntry) {
	if len(entries) == 0 {
		return
	}
...
func renderFirewallVolumeChangesTable(entries []DomainDiffEntry) {
	if len(entries) == 0 {
		return
	}
...
func renderMCPToolsDiffPrettySection(diff *MCPToolsDiff) {
	...
	fmt.Fprintln(os.Stderr)
	renderMCPNewToolsTable(diff.NewTools)
	renderMCPRemovedToolsTable(diff.RemovedTools)
	renderMCPChangedToolsTable(diff.ChangedTools)
}

func renderMCPNewToolsTable(entries []MCPToolDiffEntry) {
	if len(entries) == 0 {
		return
	}
...
func renderMCPRemovedToolsTable(entries []MCPToolDiffEntry) {
	if len(entries) == 0 {
		return
	}
...
func renderMCPChangedToolsTable(entries []MCPToolDiffEntry) {
	if len(entries) == 0 {
		return
	}
...
func renderRunMetricsDiffPrettySection(run1ID, run2ID int64, diff *RunMetricsDiff) {
	...
	fmt.Fprintln(os.Stderr)
	if config := buildRunMetricsPrettyTable(run1ID, run2ID, diff); len(config.Rows) > 0 {
		fmt.Fprint(os.Stderr, console.RenderTable(config))
	}
	renderRunMetricsDetailSections(run1ID, run2ID, diff)
}

func buildRunMetricsPrettyTable(run1ID, run2ID int64, diff *RunMetricsDiff) console.TableConfig {
	config := console.TableConfig{Headers: []string{"Metric", fmt.Sprintf("Run #%d", run1ID), fmt.Sprintf("Run #%d", run2ID), "Change"}, Rows: make([][]string, 0)}
	appendRunMetricsPrettyRow(&config, diff.Run1TokenUsage > 0 || diff.Run2TokenUsage > 0, []string{"Token usage", strconv.Itoa(diff.Run1TokenUsage), strconv.Itoa(diff.Run2TokenUsage), diff.TokenUsageChange})
	appendRunMetricsPrettyRow(&config, diff.Run1Duration != "" || diff.Run2Duration != "", []string{"Duration", diff.Run1Duration, diff.Run2Duration, diff.DurationChange})
	appendRunMetricsPrettyRow(&config, diff.Run1Turns > 0 || diff.Run2Turns > 0, []string{"Turns", strconv.Itoa(diff.Run1Turns), strconv.Itoa(diff.Run2Turns), fmt.Sprintf("%+d", diff.TurnsChange)})
	appendRunMetricsPrettyRow(&config, diff.Run1TokensPerTurn > 0 || diff.Run2TokensPerTurn > 0, []string{"Tokens / turn", strconv.Itoa(diff.Run1TokensPerTurn), strconv.Itoa(diff.Run2TokensPerTurn), diff.TokensPerTurnChange})
	return config
}

func appendRunMetricsPrettyRow(config *console.TableConfig, include bool, row []string) {
	if include {
		config.Rows = append(config.Rows, row)
	}
}

func renderRunMetricsDetailSections(run1ID, run2ID int64, diff *RunMetricsDiff) {
	renderRunMetricsTokenUsageSection(run1ID, run2ID, diff.TokenUsageDetails)
	renderRunMetricsRateLimitSection(run1ID, run2ID, diff.GitHubRateLimitDetails)
	renderRunMetricsToolCallsSection(run1ID, run2ID, diff.ToolCallsDiff)
}

func renderRunMetricsTokenUsageSection(run1ID, run2ID int64, diff *TokenUsageDiff) {
	if diff == nil {
		return
	}
	fmt.Fprintln(os.Stderr)
	renderTokenUsageDiffPrettySection(run1ID, run2ID, diff)
}

func renderRunMetricsRateLimitSection(run1ID, run2ID int64, diff *GitHubRateLimitDiff) {
	if diff == nil {
		return
	}
	fmt.Fprintln(os.Stderr)
	renderGitHubRateLimitDiffPrettySection(run1ID, run2ID, diff)
}

func renderRunMetricsToolCallsSection(run1ID, run2ID int64, diff *ToolCallsDiff) {
	if diff == nil {
		return
	}
	fmt.Fprintln(os.Stderr)
	renderToolCallsDiffPrettySection(run1ID, run2ID, diff)
}
...
func renderTokenUsageDiffPrettySection(run1ID, run2ID int64, diff *TokenUsageDiff) {
	...
	fmt.Fprintln(os.Stderr)
	config := buildTokenUsagePrettyTable(run1ID, run2ID, diff)
...
func buildTokenUsagePrettyTable(run1ID, run2ID int64, diff *TokenUsageDiff) console.TableConfig {
	config := console.TableConfig{Headers: []string{"Token Type", fmt.Sprintf("Run #%d", run1ID), fmt.Sprintf("Run #%d", run2ID), "Change"}, Rows: make([][]string, 0)}
	appendRunMetricsPrettyRow(&config, diff.Run1InputTokens > 0 || diff.Run2InputTokens > 0, []string{"Input", strconv.Itoa(diff.Run1InputTokens), strconv.Itoa(diff.Run2InputTokens), diff.InputTokensChange})
	appendRunMetricsPrettyRow(&config, diff.Run1OutputTokens > 0 || diff.Run2OutputTokens > 0, []string{"Output", strconv.Itoa(diff.Run1OutputTokens), strconv.Itoa(diff.Run2OutputTokens), diff.OutputTokensChange})
	appendRunMetricsPrettyRow(&config, diff.Run1CacheReadTokens > 0 || diff.Run2CacheReadTokens > 0, []string{"Cache read", strconv.Itoa(diff.Run1CacheReadTokens), strconv.Itoa(diff.Run2CacheReadTokens), diff.CacheReadTokensChange})
	appendRunMetricsPrettyRow(&config, diff.Run1CacheWriteTokens > 0 || diff.Run2CacheWriteTokens > 0, []string{"Cache write", strconv.Itoa(diff.Run1CacheWriteTokens), strconv.Itoa(diff.Run2CacheWriteTokens), diff.CacheWriteTokensChange})
	appendRunMetricsPrettyRow(&config, diff.Run1AIC > 0 || diff.Run2AIC > 0, []string{"AI Credits", fmt.Sprintf("%.3f", diff.Run1AIC), fmt.Sprintf("%.3f", diff.Run2AIC), diff.AICChange})
	appendRunMetricsPrettyRow(&config, diff.Run1TotalRequests > 0 || diff.Run2TotalRequests > 0, []string{"API requests", strconv.Itoa(diff.Run1TotalRequests), strconv.Itoa(diff.Run2TotalRequests), diff.RequestsDelta})
	appendRunMetricsPrettyRow(&config, diff.Run1CacheEfficiency > 0 || diff.Run2CacheEfficiency > 0, []string{"Cache efficiency", fmt.Sprintf("%.1f%%", diff.Run1CacheEfficiency*100), fmt.Sprintf("%.1f%%", diff.Run2CacheEfficiency*100), diff.CacheEfficiencyChange})
	return config
}
