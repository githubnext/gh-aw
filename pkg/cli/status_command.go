...
func StatusWorkflows(pattern string, verbose bool, jsonOutput bool, ref string, labelFilter string, repoOverride string) error {
	...
	if verbose && !jsonOutput {
		if tty.IsStderrTerminal() {
			...
			fmt.Fprintln(os.Stderr, "Checking status of workflow files")
			if pattern != "" {
	...
	if verbose && !jsonOutput {
		if tty.IsStderrTerminal() {
			fmt.Fprintln(os.Stderr, console.FormatCountMessage(fmt.Sprintf("Found %d markdown workflow files", len(mdFiles))))
			fmt.Fprintln(os.Stderr, console.FormatProgressMessage("Fetching GitHub workflow status..."))
		} else {
			fmt.Fprintf(os.Stderr, "Found %d markdown workflow files\n", len(mdFiles))
			fmt.Fprintln(os.Stderr, "Fetching GitHub workflow status...")
		}
	}
	...
	if err != nil {
		...
		if verbose && !jsonOutput {
			if tty.IsStderrTerminal() {
				...
				fmt.Fprintf(os.Stderr, "Verbose: Failed to fetch GitHub workflows: %v\n", err)
			}
		}
		if !jsonOutput {
			if tty.IsStderrTerminal() {
				...
				fmt.Fprintf(os.Stderr, "Warning: Could not fetch GitHub workflow status: %v\n", err)
			}
		}
		...
		if verbose && !jsonOutput {
			if tty.IsStderrTerminal() {
				...
				fmt.Fprintf(os.Stderr, "Successfully fetched %d GitHub workflows\n", len(githubWorkflows))
			}
		}
	}
	...
	if ref != "" {
		if verbose && !jsonOutput {
			if tty.IsStderrTerminal() {
				...
				fmt.Fprintf(os.Stderr, "Fetching latest runs for ref: %s\n", ref)
			}
		}
		...
		if err != nil {
			...
			if verbose && !jsonOutput {
				if tty.IsStderrTerminal() {
					...
					fmt.Fprintf(os.Stderr, "Verbose: Failed to fetch workflow runs for ref: %v\n", err)
				}
			}
			if !jsonOutput {
				if tty.IsStderrTerminal() {
					...
					fmt.Fprintf(os.Stderr, "Warning: Could not fetch workflow runs for ref '%s': %v\n", ref, err)
				}
			}
			...
			if verbose && !jsonOutput {
				if tty.IsStderrTerminal() {
					...
					fmt.Fprintf(os.Stderr, "Successfully fetched %d workflow runs for ref\n", len(latestRunsByWorkflow))
				}
			}
		}
	}
