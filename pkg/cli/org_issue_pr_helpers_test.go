...
func TestCloseExistingOrgIssuesByMarkerSkipsPRsAndPaginates(t *testing.T) {
	...
	assert.Contains(t, logText, "api --hostname "+expectedHost+" /repos/octo/repo/issues?state=open&per_page=100&page=1", "first page should be queried")
	assert.Contains(t, logText, "api --hostname "+expectedHost+" /repos/octo/repo/issues?state=open&per_page=100&page=2", "second page should be queried")
	assert.Contains(t, logText, "api --hostname "+expectedHost+" --method PATCH /repos/octo/repo/issues/1 -f state=closed -f state_reason=not_planned", "matching issues should be closed")
	assert.Contains(t, logText, "api --hostname "+expectedHost+" --method PATCH /repos/octo/repo/issues/101 -f state=closed -f state_reason=not_planned", "matching issues on later pages should also be closed")
