	...
	if result.PRNumber == 0 {
		...
		for !mergeDone {
			...
			case mergeActionEditTitle:
				...
				} else if editErr := editPRTitle(result.PRNumber, newTitle, c.RepoOverride, c.ghHost); editErr != nil {
...
func (c *AddInteractiveConfig) updateLocalBranch() error {
	...
	output, err := c.runGHCombined("Getting default branch...", "repo", "view", "--repo", c.RepoOverride, "--json", "defaultBranchRef", "--jq", ".defaultBranchRef.name")
	defaultBranch := ""
	if err == nil {
...
func (c *AddInteractiveConfig) mergePullRequest(prNumber int) error {
	...
	squashOutput, squashErr := c.runGHCombined("Merging pull request (squash)...", "pr", "merge", prArg, "--repo", c.RepoOverride, "--squash")
	if squashErr == nil {
		return nil
	}
	...
	if strings.Contains(combinedText, squashMergeNotAllowedErr) {
		...
		mergeOutput, mergeErr := c.runGHCombined("Merging pull request...", "pr", "merge", prArg, "--repo", c.RepoOverride, "--merge")
		if mergeErr != nil {
...
func editPRTitle(prNumber int, newTitle, repoOverride, ghHost string) error {
	args := []string{"pr", "edit", strconv.Itoa(prNumber), "--title", newTitle}
	if repoOverride != "" {
		args = append(args, "--repo", repoOverride)
	}
	output, err := workflow.RunGHCombinedWithHost("Updating PR title...", ghHost, args...)
	if err != nil {
