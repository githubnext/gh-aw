...
func renderObservabilityInsights(insights []ObservabilityInsight) {
	for _, insight := range insights {
		...
		fmt.Fprintln(os.Stderr, console.FormatInfoMessage(fmt.Sprintf("  %s %s [%s]", icon, insight.Title, insight.Category)))
		fmt.Fprintln(os.Stderr, console.FormatInfoMessage(fmt.Sprintf("     %s", insight.Summary)))
		if strings.TrimSpace(insight.Evidence) != "" {
			fmt.Fprintln(os.Stderr, console.FormatInfoMessage(fmt.Sprintf("     Evidence: %s", insight.Evidence)))
		}
