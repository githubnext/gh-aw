...
func UpdateActions(ctx context.Context, allowMajor, verbose, disableReleaseBump bool) error {
	...
	for _, s := range snapshot {
		...
		if currentVer := parseVersion(entry.Version); currentVer != nil {
			if latestVer := parseVersion(latestVersion); latestVer != nil && currentVer.IsNewer(latestVer) {
				...
				fmt.Fprintln(os.Stderr, console.FormatWarningMessage(fmt.Sprintf("%s: skipping proposed update from %s to %s (would be a downgrade)", entry.Repo, entry.Version, latestVersion)))
