...
type cleanupOldRunFoldersTestCase struct {
	name            string
...
func cleanupOldRunFoldersTestCases(now time.Time) []cleanupOldRunFoldersTestCase {
	return []cleanupOldRunFoldersTestCase{
		{
			name: "removes folders older than cutoff",
			setup: func(t *testing.T, dir string) {
				makeRunDir(t, dir, 1, now.Add(-14*24*time.Hour), true)
				makeRunDir(t, dir, 2, now.Add(-3*24*time.Hour), true)
			},
			wantRemoved:     1,
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				makeRunDir(t, dir, 10, now.Add(-1*24*time.Hour), true)
				makeRunDir(t, dir, 11, now.Add(-2*24*time.Hour), true)
			},
			wantRemoved:  0,
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				makeRunDir(t, dir, 20, now.Add(-30*24*time.Hour), true)
				makeRunDir(t, dir, 21, now.Add(-14*24*time.Hour), true)
			},
			wantRemoved:     2,
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				require.NoError(t, os.MkdirAll(filepath.Join(dir, "summary"), 0755))
				makeRunDir(t, dir, 30, now.Add(-30*24*time.Hour), true)
			},
			wantRemoved:     1,
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				...
				makeRunDir(t, dir, 50, now.Add(-30*24*time.Hour), true)
			},
			wantRemoved:     1,
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				makeRunDir(t, dir, 40, time.Time{}, false)
				makeRunDir(t, dir, 41, now.Add(-30*24*time.Hour), true)
			},
			wantRemoved:     1,
			wantDirsLeft:    []string{"run-40"},
			wantDirsRemoved: []string{"run-41"},
		},
		{name: "empty output directory returns zero removed", setup: func(t *testing.T, dir string) {}, wantRemoved: 0},
		{
...
func assertRunFoldersState(t *testing.T, tmpDir string, tt cleanupOldRunFoldersTestCase, removed int) {
	t.Helper()
...
func TestCleanupOldRunFolders(t *testing.T) {
	...
	for _, tt := range cleanupOldRunFoldersTestCases(now) {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertRunFoldersState(t, tmpDir, tt, removed)
		})
	}
}
