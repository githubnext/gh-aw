...
func TestAddWorkflowWithDispatchWorkflowFromSharedImport(t *testing.T) {
	...
	//
	// Note: pinned to commit 0b16355b which includes:
	// - strict: false in smoke-copilot.md (required since sandbox.mcp.container is blocked in strict mode)
	// - the fix to serena-go.md to use ./serena.md (explicitly-relative path) so that the
	//   sibling import resolves correctly to shared/mcp/serena.md instead of .github/workflows/serena.md
