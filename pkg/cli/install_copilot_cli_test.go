...
func TestInstallCopilotCLIScriptUsesCompatWindowForExplicitVersionToolcacheFallback(t *testing.T) {
	const explicitVersion = "1.0.75"
	const cachedCompatibleVersion = "1.0.56"
	const cachedBoundaryMinVersion = "1.0.21"
	const cachedTooOldVersion = "1.0.20"
	...
	require.NoError(t, os.WriteFile(filepath.Join(minBoundaryToolcacheBin, "copilot"), []byte("#!/usr/bin/env bash\necho 'copilot "+cachedBoundaryMinVersion+"'\n"), 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(tooOldToolcacheBin, "copilot"), []byte("#!/usr/bin/env bash\necho 'copilot "+cachedTooOldVersion+"'\n"), 0o755))
...
`), 0o755))
	...
	output, err := cmd.CombinedOutput()
	require.NoError(t, err, "install_copilot_cli.sh should use compat window fallback for explicit versions: %s", output)

	assert.Contains(t, string(output), "Explicit Copilot CLI version argument provided ("+explicitVersion+")")
	assert.Contains(t, string(output), "Using compat-resolved Copilot CLI window: 1.0.21..1.0.75")
	assert.Contains(t, string(output), "Will prefer an exact toolcache hit for "+explicitVersion+", then fall back to the compat window before downloading.")
	assert.Contains(t, string(output), "Exact version mismatch (want "+explicitVersion+", got "+cachedCompatibleVersion+"); checking compat window fallback")
	assert.Contains(t, string(output), "Skipping candidate (below compat minimum: "+cachedTooOldVersion+" < 1.0.21)")
	...
	require.NoError(t, err, "Expected curl to fetch compatibility matrix")
	assert.Contains(t, string(curlLogContent), "/compat.json", "compat matrix should be downloaded")
	assert.NotContains(t, string(curlLogContent), "SHA256SUMS.txt", "release downloads should not run when compat toolcache fallback succeeds")
}
