...
- **`tools:`** - Tool configuration for coding agent
  - `github:` - GitHub API tools
    ...
    - `toolset:` - Enable specific GitHub toolset groups (array only)
      ...
      **GitHub Toolsets Overview:**
      
      Toolsets organize GitHub API functions into logical groups. You can enable specific toolsets based on your workflow needs, use `[default]` for recommended toolsets, or use `[all]` to enable everything.
      
      **Default Toolsets** (enabled when `toolset` is not specified or set to `[default]`):
      - `context` - GitHub Actions context and environment information
      - `repos` - Repository operations (read/write files, branches, commits)
      - `issues` - Issue management (create, update, comment on issues)
      - `pull_requests` - Pull request operations (create, review, merge PRs)
      ...
      **Usage Examples:**
      ```yaml
      # Use default toolsets (recommended for most workflows)
      tools:
        github:
          toolset: [default]
      
      # Use default toolsets plus discussions
      tools:
        github:
          toolset: [default, discussions]
      
      # Use specific toolsets only
      tools:
        github:
          toolset: [repos, issues, pull_requests]
      
      # Enable all toolsets (use with caution)
      tools:
        github:
          toolset: [all]
      ```
      
      **When to Enable Additional Toolsets:**
      - Add `discussions` when working with GitHub Discussions
      - Add `actions` when managing GitHub Actions workflows
      - Add `labels` when programmatically managing repository labels
      - Add `code_security` or `dependabot` for security-related workflows
      - Add `projects` when working with GitHub Projects (beta)
      - Add `gists` when managing code snippets
      
      **Note**: If `toolset` is not specified, it defaults to `[default]` which includes `context`, `repos`, `issues`, `pull_requests`, and `users`.
