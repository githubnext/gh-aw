...
- **`engine:`** - AI processor configuration
  - String format: `"claude"` (default), `"codex"`, `"opencode"`, `"gemini"`, `"genaiscript"`
  - Object format for extended configuration:
    ```yaml
    engine:
      id: claude                        # Required: agent CLI identifier (claude, codex, opencode, gemini, genaiscript)
...
- **Invalid enum values** - e.g., `engine` must be "claude", "codex", "opencode", "gemini", or "genaiscript"
