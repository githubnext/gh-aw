...
When migrating traditional GitHub Actions workflows to agentic workflows, the conversion strategy depends on the nature of your existing steps:

#### Conversion Decision Tree

**For AI-powered tasks** (code analysis, issue triage, documentation generation):
- Use **Claude engine** (recommended) or **Codex engine** for experimental features
- Replace multiple deterministic steps with natural language instructions
- Leverage AI reasoning capabilities for complex decision-making

**For traditional automation** (builds, deployments, standardized processes):
- Use **Custom engine** to preserve existing GitHub Actions steps
- Maintains deterministic behavior while gaining agentic workflow benefits
- Ideal for hybrid workflows combining AI and traditional automation

#### Conversion Examples

##### Example 1: AI-Powered Issue Triage
...
tools:
  github:
    allowed: [get_issue, update_issue, add_issue_comment]
---

# Intelligent Issue Triage

Analyze issue #${{ github.event.issue.number }} and:
1. Categorize the issue type based on content and context
2. Apply appropriate labels (bug, enhancement, documentation, question)
3. Set priority based on severity and impact
4. Add a helpful triage comment with next steps
5. Assign to appropriate team member if clear ownership exists

Use sophisticated reasoning to understand the issue beyond simple keyword matching.
```

##### Example 2: Traditional Build with Custom Engine
...
**After (Agentic Workflow with Custom Engine):**
```markdown
---
on: [push, pull_request]
permissions:
...
This workflow maintains the original deterministic build steps while providing access to agentic workflow features like enhanced logging and safe outputs.
```

##### Example 3: Hybrid Workflow (Custom + AI Analysis)

**Converting a workflow with both traditional and AI components:**
...
safe-outputs:
  create-issue:
    ...
    labels: [coverage, automation]
---

# Build with AI Coverage Analysis

Execute the standard build and test process, then analyze the coverage report:

1. Review the generated coverage report in detail
2. Identify critical areas with low coverage
3. Suggest specific test improvements
4. Create an issue with actionable recommendations if coverage drops below thresholds
5. Analyze trends compared to previous runs
```

#### Migration Guidelines

**1. Identify Conversion Strategy:**
- **AI Tasks**: Convert to Claude/Codex with natural language instructions
- **Traditional Tasks**: Migrate to Custom engine preserving existing steps
- **Hybrid**: Use Custom engine with AI analysis in the markdown content

**2. Engine Selection:**
- **Claude**: Best for reasoning, analysis, and content generation
- **Codex**: For code-specific tasks and experimental features
- **Custom**: When you need to preserve exact GitHub Actions behavior

**3. Convert Frontmatter:**
```yaml
# Traditional workflow triggers and permissions remain largely the same
on: [original triggers]
permissions: [minimal permissions + safe-outputs]

# Add engine configuration
engine: claude  # or codex, or custom with steps

# Replace direct permissions with safe-outputs when possible
safe-outputs:
  create-issue:    # Instead of issues: write
  add-comment:     # Instead of issues: write + pull-requests: write
```

**4. Handle Secrets and Environment Variables:**
- Existing secrets work the same way: `${{ secrets.SECRET_NAME }}`
- Environment variables can be added to engine config:
```yaml
engine:
  id: claude
  env:
...
**5. Preserve Context:**
- GitHub context expressions work the same: `${{ github.event.issue.number }}`
- Use `${{ needs.task.outputs.text }}` for sanitized content access

#### Common Conversion Patterns

**Pattern 1: Simple Label Assignment → AI Categorization**
```yaml
# Before: Basic keyword matching
- run: echo "bug" | grep -q "${{ github.event.issue.title }}" && gh issue edit --add-label bug

# After: AI reasoning
engine: claude
# In markdown: "Intelligently categorize this issue and apply appropriate labels"
```

**Pattern 2: Multi-step Scripts → Natural Language**
```yaml
...
# After: AI decision making
engine: claude  
# In markdown: "Analyze the situation and take appropriate action based on these criteria..."
```

**Pattern 3: Preserving Deterministic Steps**
```yaml
# Before: Critical build steps
- run: npm ci && npm run build

# After: Keep exact behavior with Custom engine
engine:
  id: custom
  steps:
    - run: npm ci
...
**Common Issues:**
1. **Missing Permissions**: Add required permissions or use safe-outputs
2. **Complex Logic**: Consider if AI reasoning can replace complex scripts
3. **Secrets Access**: Ensure secrets are accessible to the chosen engine
4. **Step Dependencies**: Custom engine maintains step order, AI engines are single-execution

**Testing Conversion:**
1. Start with `gh aw compile --verbose` to validate syntax
2. Test with `workflow_dispatch` trigger for manual testing
3. Use `safe-outputs` to avoid permission issues during testing
4. Compare outputs with original workflow behavior
