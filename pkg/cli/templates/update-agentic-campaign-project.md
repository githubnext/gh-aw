...
| `priority` | single-select | `High` / `Medium` / `Low` (ensure Medium option exists in project) |
| `size` | single-select | `Small` / `Medium` / `Large` (ensure Medium option exists in project) |
...
- `repo`: extract `owner/repo` from the issue/PR URL
- `priority`: default `Medium` unless explicitly known (ensure Medium option exists in project)
- `size`: default `Medium` unless explicitly known (ensure Medium option exists in project)
...
- `content_type`, `content_number`, `repo`
...
- [ ] `fields.repo` matches `owner/repo`
- [ ] `fields.priority` ∈ {`High`, `Medium`, `Low`} (Medium option must exist in project)
- [ ] `fields.size` ∈ {`Small`, `Medium`, `Large`} (Medium option must exist in project)
