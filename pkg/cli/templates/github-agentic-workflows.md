...
- **`tools:`** - Tool configuration for coding agent
  ...
  - `serena:` - AI-powered code intelligence with language service integration
    - Array format: `serena: ["go", "typescript"]` (enable specific languages)
    - Object format for custom configuration:
      ...
      serena:
        ...
        args: ["--custom-arg"]
        languages:
          go:              # Enable Go language service
          typescript:      # Enable TypeScript language service
          python:          # Enable Python language service
      ```
...
- **`safe-outputs:`** - Safe output processing configuration (preferred way to handle GitHub API write operations)
  ...
  - `upload-assets:` - Publish assets to orphaned git branch
    ```yaml
    safe-outputs:
      upload-assets:
        branch: "assets/${{ github.workflow }}"  # Optional: branch name
        max-size: 10240                          # Optional: max file size in KB (default: 10MB)
        allowed-exts: [".png", ".jpg", ".json"]  # Optional: allowed extensions
        max: 10                                  # Optional: max assets (default: 10)
    ```
    Publishes files to a separate assets branch for embedding in issues, PRs, and discussions.
  ...
  **Global Safe Output Configuration:**
  ...
  - `messages:` - Customize workflow status messages
    ```yaml
    safe-outputs:
      messages:
    ...
    ```
    Available placeholders: `{workflow_name}`, `{run_url}`, `{triggering_number}`, `{event_type}`, `{status}`.
