...
// SafeOutputsProcessLogFilename is the filename of the diagnostic log written by the
// Process Safe Outputs step (safe_output_handler_manager.cjs) on failure.
// It captures the failure reason and per-item error details so that hard safe-output
// failures (BUNDLE hardfails) are diagnosable from run artifacts without live-log access.
// It is uploaded alongside the safe-output-items.jsonl manifest in the safe-outputs-items artifact.
const SafeOutputsProcessLogFilename = "safe-outputs-process.log"
