...
var ghcliLog = logger.New("ghcli:ghcli")
...
func ExecGH(args ...string) *exec.Cmd {
	...
	if ghToken != "" {
		ghcliLog.Printf("GH_TOKEN is set, using it for gh CLI")
		return cmd
	}
	...
	if githubToken != "" {
		ghcliLog.Printf("GH_TOKEN not set, using GITHUB_TOKEN as fallback for gh CLI")
		// Set GH_TOKEN in the command's environment
		cmd.Env = append(os.Environ(), "GH_TOKEN="+githubToken)
	} else {
		ghcliLog.Printf("Neither GH_TOKEN nor GITHUB_TOKEN is set, gh CLI will use default authentication")
	}
