...
func TestComputeAllowedToolsWithSafeOutputs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: "Read,Write(${{ env.GITHUB_AW_SAFE_OUTPUTS }})",
		},
		{
		...
		},
		{
			...
			expected: "Bash,Write(${{ env.GITHUB_AW_SAFE_OUTPUTS }})",
		},
		{
			...
			expected: "Read,Write(${{ env.GITHUB_AW_SAFE_OUTPUTS }}),mcp__github__create_issue,mcp__github__create_pull_request",
		},
	}
