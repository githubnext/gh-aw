...
func (c *Compiler) buildPrepareDetectionFilesStep() []string {
	return []string{
		...
		"          if [ -d /tmp/gh-aw/comment-memory ]; then\n",
		"            mkdir -p /tmp/gh-aw/threat-detection/comment-memory\n",
		"            cp /tmp/gh-aw/comment-memory/*.md /tmp/gh-aw/threat-detection/comment-memory/ 2>/dev/null || true\n",
		"          fi\n",
