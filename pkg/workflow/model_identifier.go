...
func ParseModelIdentifier(s string) (*ParsedModelIdentifier, error) {
	...
	if s == "" {
		return nil, errors.New("model identifier must not be empty. Example: openai/gpt-5")
	}
...
func validateProviderToken(token string) error {
	if token == "" {
		return errors.New("model identifier: provider token must not be empty (segment type: provider). Example: openai/gpt-5")
	}
	if !reProviderToken.MatchString(token) {
		...
		if !isAlpha(rune(token[0])) {
			return fmt.Errorf("model identifier: provider token %q must start with a letter (segment type: provider). Example: openai/gpt-5", token)
		}
	}
	// Provider token must not end with "-".
	if strings.HasSuffix(token, "-") {
		return fmt.Errorf("model identifier: provider token %q must not end with '-' (segment type: provider). Example: openai/gpt-5", token)
	}
	return nil
}
...
func validateModelToken(token string) error {
	if token == "" {
		return errors.New("model identifier: model token must not be empty (segment type: model). Example: openai/gpt-5")
	}
	if !reModelToken.MatchString(token) {
		...
		return fmt.Errorf("model identifier: model token %q is syntactically invalid (segment type: model). Example: openai/gpt-5", token)
	}
	return nil
}
...
func validateModelGlobToken(token string) error {
	if token == "" {
		return errors.New("model identifier: model glob token must not be empty (segment type: model). Example: openai/gpt-*")
	}
	for _, r := range token {
		switch {
...
func validateBareName(name string) error {
	if name == "" {
		return errors.New("model identifier: bare name must not be empty (segment type: alias). Example: small")
	}
	// Must not start with "-" or ".".
	if name[0] == '-' || name[0] == '.' {
		return fmt.Errorf("model identifier: bare name %q must not start with '-' or '.' (segment type: alias). Example: small", name)
	}
	if !reBareName.MatchString(name) {
		...
		return fmt.Errorf("model identifier: bare name %q is syntactically invalid (segment type: alias). Example: small", name)
	}
	return nil
}
...
func parseQueryString(raw string) (map[string]string, error) {
	...
	for pair := range strings.SplitSeq(raw, "&") {
		...
		if !found {
			return nil, fmt.Errorf("model identifier: parameter %q is missing '=' separator. Example: openai/gpt-5?effort=high", pair)
		}
...
func validateParamKey(key string) error {
	if key == "" {
		return errors.New("model identifier: parameter key must not be empty (segment type: parameter key). Example: openai/gpt-5?effort=high")
	}
	if !reParamKey.MatchString(key) {
		...
		if !isAlpha(rune(key[0])) {
			return fmt.Errorf("model identifier: parameter key %q must start with a letter (segment type: parameter key). Example: openai/gpt-5?effort=high", key)
		}
	}
	return nil
}
...
func validateParamValue(value string) error {
	if value == "" {
		return errors.New("model identifier: parameter value must not be empty (segment type: parameter value). Example: openai/gpt-5?effort=high")
	}
