...
func TestGenerateCopilotInstallerSteps(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			shouldContain: []string{
				"${{ runner.temp }}/gh-aw/actions/install_copilot_cli.sh 0.0.369",
		...
		},
		{
			...
			shouldContain: []string{
				"${{ runner.temp }}/gh-aw/actions/install_copilot_cli.sh v0.0.370",
				"GH_HOST: github.com",
			},
			shouldNotContain: []string{
		...
		},
		{
			...
			shouldContain: []string{
				"${{ runner.temp }}/gh-aw/actions/install_copilot_cli.sh 1.2.3",
		...
		},
		{
			...
			shouldContain: []string{
				"${{ runner.temp }}/gh-aw/actions/install_copilot_cli.sh " + string(constants.DefaultCopilotVersion),
				"GH_HOST: github.com",
			},
			shouldNotContain: []string{
				"gh.io/copilot-install | sudo bash",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedVersionLine := "${{ runner.temp }}/gh-aw/actions/install_copilot_cli.sh " + tt.expectedVersion
...
func TestCopilotInstallerCustomVersion(t *testing.T) {
	...
	expectedVersionLine := "${{ runner.temp }}/gh-aw/actions/install_copilot_cli.sh " + customVersion
