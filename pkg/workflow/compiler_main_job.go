...
func (c *Compiler) buildMainJob(data *WorkflowData, activationJobCreated bool) (*Job, error) {
	...
	// Add inference_access_error, mcp_policy_error, and copilot_timeout outputs for Copilot engine only
	...
	if engineErr == nil {
		if _, ok := engine.(*CopilotEngine); ok {
			...
			outputs["copilot_timeout"] = "${{ steps.detect-copilot-errors.outputs.copilot_timeout || 'false' }}"
			compilerMainJobLog.Print("Added copilot_timeout output (Copilot engine)")
		}
	}
