...
func generatePlaceholderSubstitutionStep(yaml *strings.Builder, expressionMappings []*ExpressionMapping, indent string, data *WorkflowData) {
	...
	writePlaceholderSubstitutionStepHeader(yaml, indent, data)
...
func writePlaceholderSubstitutionStepHeader(yaml *strings.Builder, indent string, data *WorkflowData) {
...
func writePlaceholderSubstitutionEnv(yaml *strings.Builder, expressionMappings []*ExpressionMapping, indent string) {
	for _, mapping := range expressionMappings {
		writePlaceholderEnvVar(yaml, indent, mapping)
	}
}

func writePlaceholderEnvVar(yaml *strings.Builder, indent string, mapping *ExpressionMapping) {
	content := mapping.Content
	if !isQuotedPlaceholderValue(content) {
		fmt.Fprintf(yaml, indent+"    %s: ${{ %s }}\n", mapping.EnvVar, content)
		return
	}
	innerValue := content[1 : len(content)-1]
	if strings.Contains(innerValue, "\n") {
		fmt.Fprintf(yaml, indent+"    %s: \"%s\"\n", mapping.EnvVar, escapeMultilinePlaceholderValue(innerValue))
		return
	}
	fmt.Fprintf(yaml, indent+"    %s: %s\n", mapping.EnvVar, content)
}

func isQuotedPlaceholderValue(content string) bool {
	return (strings.HasPrefix(content, "'") && strings.HasSuffix(content, "'")) ||
		(strings.HasPrefix(content, "\"") && strings.HasSuffix(content, "\""))
}

func escapeMultilinePlaceholderValue(value string) string {
