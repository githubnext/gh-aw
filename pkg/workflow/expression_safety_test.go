...
func TestValidateExpressionSafety(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "unauthorized_github_workflow",
			content:        "The workflow name is ${{ github.workflow }}",
			expectError:    true,
			expectedErrors: []string{"github.workflow"},
		},
		{
		...
		{
			name:           "multiple_expressions_with_unauthorized",
		...
		},
		{
			...
			content:        "Token: ${{ secrets.GITHUB_TOKEN }}, Invalid: ${{ github.actor }}, Env: ${{ env.TEST }}",
			expectError:    true,
			expectedErrors: []string{"secrets.GITHUB_TOKEN", "github.actor"},
		},
		{
			name:           "expressions_with_whitespace_unauthorized",
		...
		{
			name:           "expressions_in_code_blocks_unauthorized",
...
func TestValidateExpressionSafetyEdgeCases(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "unauthorized expression with functions - github.workflow",
			content:     "Function: ${{ toJson(github.workflow) }}",
			expectError: true,
			description: "Expressions with functions should be unauthorized when the base expression is not allowed",
		},
		{
			name:        "multiline_expression_unauthorized",
			content:     "Multi:\n${{ github.workflow\n}}",
			expectError: true,
...
func TestValidateExpressionSafetyWithParser(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "unauthorized AND expression",
			content:        `${{ github.workflow && github.repository }}`,
			wantErr:        true,
			errContains:    "github.workflow",
		},
		{
			name:           "unauthorized OR expression",
			content:        `${{ github.workflow || github.repository }}`,
			wantErr:        true,
			errContains:    "github.workflow",
		},
		{
			name:           "unauthorized NOT expression",
			content:        `${{ !github.workflow }}`,
			wantErr:        true,
			errContains:    "github.workflow",
		},
		{
			name:           "complex unauthorized expression with parentheses",
			content:        `${{ (github.workflow && github.repository) || github.run_id }}`,
			wantErr:        true,
			errContains:    "github.workflow",
		},
		{
			name:        "mixed allowed and unauthorized - github.workflow",
			content:     `${{ github.workflow && secrets.TOKEN }}`,
			wantErr:     true,
			errContains: "secrets.TOKEN",
		},
		{
			name:        "unauthorized in complex expression - both workflow and secret",
			content:     `${{ (github.workflow || secrets.TOKEN) && github.repository }}`,
			wantErr:     true,
			errContains: "secrets.TOKEN",
		},
		{
			name:        "nested complex unauthorized expression",
		...
		{
			name:           "unparseable but unauthorized literal - github.workflow",
			content:        `${{ github.workflow }}`,
			wantErr:        true,
			errContains:    "github.workflow",
		},
