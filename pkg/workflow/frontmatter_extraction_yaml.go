...
func (c *Compiler) commentOutProcessedFieldsInOnSection(yamlStr string, frontmatter map[string]any) string {
	...
	for _, line := range lines {
		...
		if !inPullRequest && !inIssues && !inDiscussion && !inIssueComment {
			...
			} else if inSkipIfMatch && (strings.HasPrefix(trimmedLine, "query:") || strings.HasPrefix(trimmedLine, "max:") || strings.HasPrefix(trimmedLine, "scope:") || strings.HasPrefix(trimmedLine, "github-token:") || strings.HasPrefix(trimmedLine, "github-app:")) {
			...
			} else if inSkipIfNoMatch && (strings.HasPrefix(trimmedLine, "query:") || strings.HasPrefix(trimmedLine, "min:") || strings.HasPrefix(trimmedLine, "scope:") || strings.HasPrefix(trimmedLine, "github-token:") || strings.HasPrefix(trimmedLine, "github-app:")) {
