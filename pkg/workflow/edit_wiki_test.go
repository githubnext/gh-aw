...
func TestEditWikiPageConfigParsing(t *testing.T) {
	compiler := NewCompiler(false, "", "test")

	t.Run("Should parse basic edit-wiki-page configuration", func(t *testing.T) {
		...
		if config.EditWikiPage == nil {
			t.Fatal("Expected EditWiki to be parsed")
		}

		// Check defaults
		if config.EditWikiPage.Max != 1 {
			t.Errorf("Expected default max to be 1, got %d", config.EditWikiPage.Max)
		}

		if len(config.EditWikiPage.Path) != 0 {
			t.Errorf("Expected default path to be empty, got %v", config.EditWikiPage.Path)
		}
	})

	t.Run("Should parse edit-wiki-page configuration with all options", func(t *testing.T) {
		frontmatter := map[string]any{
			"safe-outputs": map[string]any{
				"edit-wiki-page": map[string]any{
					"path":         []any{"docs/", "wiki/"},
					"max":          3,
		...
		if config.EditWikiPage.Max != 3 {
			t.Errorf("Expected max to be 3, got %d", config.EditWikiPage.Max)
		}

		expectedPaths := []string{"docs/", "wiki/"}
		if len(config.EditWikiPage.Path) != len(expectedPaths) {
			t.Errorf("Expected path length to be %d, got %d", len(expectedPaths), len(config.EditWikiPage.Path))
		}

		for i, expectedPath := range expectedPaths {
			if i < len(config.EditWikiPage.Path) && config.EditWikiPage.Path[i] != expectedPath {
				t.Errorf("Expected path[%d] to be %q, got %q", i, expectedPath, config.EditWikiPage.Path[i])
			}
		}

		if config.EditWikiPage.GitHubToken != "${{ secrets.WIKI_PAT }}" {
			t.Errorf("Expected github-token to be %q, got %q", "${{ secrets.WIKI_PAT }}", config.EditWikiPage.GitHubToken)
		}
	})
	...
	t.Run("Should handle numeric max values of different types", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if config.EditWikiPage.Max != tc.expected {
					t.Errorf("Expected max to be %d, got %d", tc.expected, config.EditWikiPage.Max)
				}
			})
		}
	})
...
func TestEditWikiJobCreation(t *testing.T) {
	...
	t.Run("Should build edit-wiki-page job with basic configuration", func(t *testing.T) {
		workflowData := &WorkflowData{
			Name:        "test-workflow",
			Description: "Test workflow",
			SafeOutputs: &SafeOutputsConfig{
				EditWikiPage: &EditWikiPageConfig{
					Max: 1,
				},
			},
		}

		job, err := compiler.buildEditWikiPageJob(workflowData, "main_job", false, nil)
		if err != nil {
			t.Fatalf("Expected no error building edit-wiki-page job, got: %v", err)
		}
	...
	t.Run("Should build edit-wiki-page job with path restrictions", func(t *testing.T) {
		workflowData := &WorkflowData{
			Name:        "test-workflow",
			Description: "Test workflow",
			SafeOutputs: &SafeOutputsConfig{
	...
	t.Run("Should build edit-wiki-page job with staged mode", func(t *testing.T) {
		staged := true
		workflowData := &WorkflowData{
			Name:        "test-workflow",
			Description: "Test workflow",
	...
	t.Run("Should build edit-wiki-page job with custom GitHub token", func(t *testing.T) {
		workflowData := &WorkflowData{
			Name:        "test-workflow",
			Description: "Test workflow",
			SafeOutputs: &SafeOutputsConfig{
	...
	t.Run("Should fail when SafeOutputs.EditWikiPage is nil", func(t *testing.T) {
		workflowData := &WorkflowData{
			Name:        "test-workflow",
			Description: "Test workflow",
			SafeOutputs: &SafeOutputsConfig{
		...
		_, err := compiler.buildEditWikiPageJob(workflowData, "main_job", false, nil)
		if err == nil {
			t.Fatal("Expected error when EditWiki config is nil")
		}

		if !strings.Contains(err.Error(), "safe-outputs.edit-wiki-page configuration is required") {
