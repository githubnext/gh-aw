...
func TestParseAndBuildSafeJobsRunsOnVariants(t *testing.T) {
	tests := []struct {
		name     string
		job      map[string]any
		expected string
		...
		{
			name: "object runs-on with group only",
			job: map[string]any{
				"runs-on": map[string]any{"group": "sj-group"},
				"steps":   []any{map[string]any{"run": "echo hi"}},
			},
			expected: "runs-on:\n      group: sj-group",
		},
		{
			name: "object runs-on with group and labels",
			job: map[string]any{
				"runs-on": map[string]any{"group": "sj-group", "labels": []any{"linux"}},
				"steps":   []any{map[string]any{"run": "echo hi"}},
			},
			expected: "runs-on:\n      group: sj-group\n      labels:\n        - linux",
		},
		{
			name: "object runner alias",
			job: map[string]any{
				"runner": map[string]any{"group": "runner-group", "labels": []any{"linux"}},
				"steps":  []any{map[string]any{"run": "echo hi"}},
			},
			expected: "runs-on:\n      group: runner-group\n      labels:\n        - linux",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := NewCompiler()
			safeJobs := c.parseSafeJobsConfig(map[string]any{"deploy": tt.job})
			workflowData := &WorkflowData{
				Name:        "test-workflow",
				SafeOutputs: &SafeOutputsConfig{Jobs: safeJobs},
			}
...
func TestCompileWorkflowSafeJobRunsOnObject(t *testing.T) {
	tmpDir := testutil.TempDir(t, "safe-jobs-runs-on-object")
	testContent := `---
...
`
	...
	testFile := filepath.Join(tmpDir, "test-safe-jobs-runs-on-object.md")
	require.NoError(t, os.WriteFile(testFile, []byte(testContent), 0o644))

	compiler := NewCompiler()
	require.NoError(t, compiler.CompileWorkflow(testFile))

	lockFile := filepath.Join(tmpDir, "test-safe-jobs-runs-on-object.lock.yml")
	compiled, err := os.ReadFile(lockFile)
	require.NoError(t, err)
