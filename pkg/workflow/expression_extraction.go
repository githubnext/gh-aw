...
func (e *ExpressionExtractor) ExtractExpressions(markdown string) ([]*ExpressionMapping, error) {
	...
	expressionExtractionLog.Printf("Extracted %d unique expressions with environment variable mappings", len(result))
	return result, nil
}
...
func (e *ExpressionExtractor) ReplaceExpressionsWithEnvVars(markdown string) string {
	expressionExtractionLog.Printf("Replacing expressions with env vars: mappings=%d", len(e.mappings))
	result := markdown
