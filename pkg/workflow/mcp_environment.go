...
func collectMCPEnvironmentVariables(tools map[string]any, mcpTools []string, workflowData *WorkflowData, hasAgenticWorkflows bool) map[string]string {
	...
	// Also check tools["github"] directly so that mode:gh-proxy workflows — where GitHub
	// is excluded from mcpTools but determine-automatic-lockdown still runs — also get
	// the GH_AW_SINK_VISIBILITY env var.
