...
type CreateIssuesConfig struct {
	...
	Idempotent           *string  `yaml:"idempotent,omitempty"`         // When true, skip creation if an open issue with the same key was already created today (UTC). Prevents same-day duplicate issues on reruns. Works best with close-older-issues: true.
...
func (c *Compiler) parseIssuesConfig(outputMap map[string]any) *CreateIssuesConfig {
	...
	for _, field := range []string{"close-older-issues", "group", "footer", "idempotent"} {
