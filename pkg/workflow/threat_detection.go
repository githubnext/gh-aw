...
func (c *Compiler) buildDetectionEngineExecutionStep(data *WorkflowData) []string {
	...
	for _, step := range executionSteps {
		// Detect whether the engine already emits continue-on-error for this step.
		// Copilot engine adds it; other engines (Claude, Gemini, …) do not.
		...
		for i, line := range step {
			...
			// Inject the if condition (and continue-on-error when not already present) after the
			// first line (- name:)
			if i == 0 {
