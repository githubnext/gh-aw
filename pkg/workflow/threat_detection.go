	...
	for _, step := range executionSteps {
		for i, line := range step {
			...
			if i == 0 {
				...
				// Note: continue-on-error is already present in the step lines generated by
				// GetExecutionSteps (added in copilot_engine_execution.go). No need to inject again.
			}
		}
	}
