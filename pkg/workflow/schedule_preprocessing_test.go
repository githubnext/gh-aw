...
type scheduleWorkflowDispatchTestCase struct {
	name                   string
...
var scheduleWorkflowDispatchTestCases = []scheduleWorkflowDispatchTestCase{
	{
...
func TestScheduleWorkflowDispatchAutomatic(t *testing.T) {
	for _, tt := range scheduleWorkflowDispatchTestCases {
		t.Run(tt.name, func(t *testing.T) {
			assertScheduleWorkflowDispatchCase(t, tt)
		})
	}
}

func assertScheduleWorkflowDispatchCase(t *testing.T, tt scheduleWorkflowDispatchTestCase) {
	t.Helper()
	...
	assertScheduleAndCron(t, onMap, tt.expectedCron)
...
func assertScheduleAndCron(t *testing.T, onMap map[string]any, expectedCron string) {
	t.Helper()
	...
	if expectedCron != "" && actualCron != expectedCron {
		t.Errorf("expected cron '%s', got '%s'", expectedCron, actualCron)
	}
}
