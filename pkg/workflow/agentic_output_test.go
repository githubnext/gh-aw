...
`
	...
	t.Log("Codex workflow correctly includes GITHUB_AW_SAFE_OUTPUTS functionality and engine output collection for logs")
}
...
func TestEngineOutputFileDeclarations(t *testing.T) {
	...
	if len(codexOutputFiles) > 0 && codexOutputFiles[0] != "/tmp/gh-aw/mcp-config/log/" {
		t.Errorf("Codex engine should declare /tmp/gh-aw/mcp-config/log/, got: %v", codexOutputFiles[0])
	}
