...
`
	...
	// Verify GITHUB_AW_SAFE_OUTPUT functionality (should be present for all engines)
	...
	// Verify job output declaration for GITHUB_AW_SAFE_OUTPUT
	if !strings.Contains(lockContent, "outputs:\n      output: ${{ steps.collect_output.outputs.output }}") {
		t.Error("Expected job output declaration for 'output'")
	}

	// Verify GITHUB_AW_SAFE_OUTPUT is passed to Claude
	if !strings.Contains(lockContent, "GITHUB_AW_SAFE_OUTPUT: ${{ env.GITHUB_AW_SAFE_OUTPUT }}") {
		t.Error("Expected GITHUB_AW_SAFE_OUTPUT environment variable to be passed to engine")
	}
	...
	if !strings.Contains(lockContent, fmt.Sprintf("name: %s", OutputArtifactName)) {
		t.Errorf("Expected GITHUB_AW_SAFE_OUTPUT artifact name to be '%s'", OutputArtifactName)
	}

	t.Log("Claude workflow correctly includes both GITHUB_AW_SAFE_OUTPUT and engine output collection")
}
...
func TestCodexEngineNoOutputSteps(t *testing.T) {
	...
	// Test case with Codex engine (should have GITHUB_AW_SAFE_OUTPUT but no engine output collection)
	testContent := `---
on: push
permissions:
  contents: read
...
This workflow tests that Codex engine gets GITHUB_AW_SAFE_OUTPUT but not engine output collection.
`
	...
	// Verify that Codex workflow DOES have GITHUB_AW_SAFE_OUTPUT functionality
	if !strings.Contains(lockContent, "- name: Setup agent output") {
		t.Error("Codex workflow should have 'Setup agent output' step (GITHUB_AW_SAFE_OUTPUT functionality)")
	}

	if !strings.Contains(lockContent, "- name: Collect agent output") {
		t.Error("Codex workflow should have 'Collect agent output' step (GITHUB_AW_SAFE_OUTPUT functionality)")
	}

	if !strings.Contains(lockContent, "- name: Upload agentic output file") {
		t.Error("Codex workflow should have 'Upload agentic output file' step (GITHUB_AW_SAFE_OUTPUT functionality)")
	}

	if !strings.Contains(lockContent, "GITHUB_AW_SAFE_OUTPUT") {
		t.Error("Codex workflow should reference GITHUB_AW_SAFE_OUTPUT environment variable")
	}

	if !strings.Contains(lockContent, fmt.Sprintf("name: %s", OutputArtifactName)) {
		t.Errorf("Codex workflow should reference %s artifact (GITHUB_AW_SAFE_OUTPUT)", OutputArtifactName)
	}

	// Verify that job outputs section includes output for GITHUB_AW_SAFE_OUTPUT
	if !strings.Contains(lockContent, "outputs:\n      output: ${{ steps.collect_output.outputs.output }}") {
		t.Error("Codex workflow should have job output declaration for 'output' (GITHUB_AW_SAFE_OUTPUT)")
	}
	...
	t.Log("Codex workflow correctly includes GITHUB_AW_SAFE_OUTPUT functionality but excludes engine output collection")
}
