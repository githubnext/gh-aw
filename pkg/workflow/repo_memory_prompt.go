...
func buildRepoMemoryPromptSection(config *RepoMemoryConfig) *PromptSection {
	...
	if len(config.Memories) == 1 && config.Memories[0].ID == "default" {
		...
		// When wiki mode is disabled, use a GitHub expression that evaluates to empty string
		// (${{ '' }}) so that expression interpolation always substitutes __GH_AW_WIKI_NOTE__
		// with an empty value, even if the compiled lock file was generated by an older version
		// that did not include this variable in the substitution step.
		wikiNoteText := "${{ '' }}"
		if memory.Wiki {
