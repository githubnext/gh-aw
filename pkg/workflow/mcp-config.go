...
func renderSafeOutputsMCPConfigTOML(yaml *strings.Builder) {
	...
	yaml.WriteString("          ]\n")
	// Use environment variable references instead of GitHub Actions expressions to prevent template injection
	// The actual values are set in the Setup MCPs step's env block
	yaml.WriteString("          env = { \"GH_AW_SAFE_OUTPUTS\" = \"$GH_AW_SAFE_OUTPUTS\", \"GH_AW_SAFE_OUTPUTS_CONFIG\" = $GH_AW_SAFE_OUTPUTS_CONFIG, \"GH_AW_ASSETS_BRANCH\" = \"$GH_AW_ASSETS_BRANCH\", \"GH_AW_ASSETS_MAX_SIZE_KB\" = \"$GH_AW_ASSETS_MAX_SIZE_KB\", \"GH_AW_ASSETS_ALLOWED_EXTS\" = \"$GH_AW_ASSETS_ALLOWED_EXTS\", \"GITHUB_REPOSITORY\" = \"$GITHUB_REPOSITORY\", \"GITHUB_SERVER_URL\" = \"$GITHUB_SERVER_URL\" }\n")
}
