...
func TestCustomAWFConfiguration(t *testing.T) {
	...
	t.Run("root-mode: false agent config passes --rootless to install script", func(t *testing.T) {
		agentConfig := &AgentSandboxConfig{
			ID:               "awf",
		...
		if len(step) == 0 {
			t.Error("Expected installation step to be generated when root-mode is false (network isolation mode)")
		}
		...
		if !strings.Contains(stepStr, "--rootless") {
			t.Error("Should contain --rootless flag when root-mode is false (network isolation mode)")
		}
	})
	...
	t.Run("non-isolation agent config does not pass --rootless", func(t *testing.T) {
		...
		if strings.Contains(stepStr, "--rootless") {
			t.Error("Should not contain --rootless flag when root-mode is true (normal mode)")
		}
	})
}
