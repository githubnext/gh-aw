...
// TestParseOTLPHeaders verifies the comma-separated key=value header string parser.
func TestParseOTLPHeaders(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    "api-key=myApiKey",
			expected: map[string]string{"api-key": "myApiKey"},
		},
		{
			name:     "multiple key=value pairs",
			input:    "api-key=abc,Authorization=Bearer token",
			expected: map[string]string{"api-key": "abc", "Authorization": "Bearer token"},
		},
		{
			name:     "pairs with spaces trimmed",
			input:    "  api-key = abc  ,  other = val  ",
			expected: map[string]string{"api-key": "abc", "other": "val"},
		},
		{
			name:     "GitHub Actions expression skipped (returns nil)",
			input:    "${{ secrets.OTEL_HEADERS }}",
			expected: nil,
		},
		{
			name:     "value with equals sign preserves after first =",
			input:    "Authorization=Bearer abc=def",
			expected: map[string]string{"Authorization": "Bearer abc=def"},
		},
		{
			name:     "pairs without = ignored",
			input:    "noequals,api-key=abc",
			expected: map[string]string{"api-key": "abc"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expected, got, "parseOTLPHeaders(%q)", tt.input)
		})
	}
}
