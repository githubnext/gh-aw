...
func (c *Compiler) addAllSafeOutputConfigEnvVars(steps *[]string, data *WorkflowData) {
	consolidatedSafeOutputsStepsLog.Print("Adding safe output config environment variables")
	if data.SafeOutputs == nil {
		consolidatedSafeOutputsStepsLog.Print("No safe outputs configured, skipping env var addition")
		return
	}
	...
	if !c.trialMode && data.SafeOutputs.Staged && hasAnySafeOutputEnabled(data.SafeOutputs) {
		*steps = append(*steps, "          GH_AW_SAFE_OUTPUTS_STAGED: \"true\"\n")
		consolidatedSafeOutputsStepsLog.Print("Added staged flag")
	}
	...
	if (data.SafeOutputs.CreateIssues != nil && hasCopilotAssignee(data.SafeOutputs.CreateIssues.Assignees)) ||
		...
		*steps = append(*steps, "          GH_AW_ASSIGN_COPILOT: \"true\"\n")
		consolidatedSafeOutputsStepsLog.Print("Copilot assignment requested - enabled for create-issue or create-pull-request fallback issues")
	}
