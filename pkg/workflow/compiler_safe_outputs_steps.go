...
func buildExtractBaseBranchStep() []string {
	return []string{
		...
		"                const workflowRepo = (process.env.GITHUB_REPOSITORY || '').trim();\n",
		"                const item = (data.items || []).find(i => {\n",
		"                  const itemRepo = (i.repo || '').trim();\n",
		"                  const sameRepo = !itemRepo || (workflowRepo && (itemRepo === workflowRepo || (!itemRepo.includes('/') && workflowRepo.endsWith('/' + itemRepo))));\n",
