...
func (c *Compiler) buildHandlerManagerStep(data *WorkflowData) ([]string, error) {
	...
	steps = append(steps, "            const path = require('path');\n")
	steps = append(steps, "            const { setupGlobals } = require('"+SetupActionDestination+"/setup_globals.cjs');\n")
	steps = append(steps, "            setupGlobals(core, github, context, exec, io, getOctokit);\n")
	steps = append(steps, "            const logsDir = '/tmp/gh-aw';\n")
	steps = append(steps, "            fs.mkdirSync(logsDir, { recursive: true });\n")
	steps = append(steps, "            const stdoutPath = path.join(logsDir, 'process-safe-outputs.stdout.log');\n")
	steps = append(steps, "            const stderrPath = path.join(logsDir, 'process-safe-outputs.stderr.log');\n")
	steps = append(steps, "            const stdoutStream = fs.createWriteStream(stdoutPath, { flags: 'a' });\n")
	steps = append(steps, "            const stderrStream = fs.createWriteStream(stderrPath, { flags: 'a' });\n")
	steps = append(steps, "            const originalStdoutWrite = process.stdout.write.bind(process.stdout);\n")
	steps = append(steps, "            const originalStderrWrite = process.stderr.write.bind(process.stderr);\n")
	steps = append(steps, "            process.stdout.write = (chunk, encoding, callback) => {\n")
	steps = append(steps, "              stdoutStream.write(chunk);\n")
	steps = append(steps, "              return originalStdoutWrite(chunk, encoding, callback);\n")
	steps = append(steps, "            };\n")
	steps = append(steps, "            process.stderr.write = (chunk, encoding, callback) => {\n")
	steps = append(steps, "              stderrStream.write(chunk);\n")
	steps = append(steps, "              return originalStderrWrite(chunk, encoding, callback);\n")
	steps = append(steps, "            };\n")
	steps = append(steps, "            const { main } = require('"+SetupActionDestination+"/safe_output_handler_manager.cjs');\n")
	steps = append(steps, "            try {\n")
	steps = append(steps, "              await main();\n")
	steps = append(steps, "            } finally {\n")
	steps = append(steps, "              process.stdout.write = originalStdoutWrite;\n")
	steps = append(steps, "              process.stderr.write = originalStderrWrite;\n")
	steps = append(steps, "              await Promise.all([\n")
	steps = append(steps, "                new Promise((resolve) => stdoutStream.end(resolve)),\n")
	steps = append(steps, "                new Promise((resolve) => stderrStream.end(resolve)),\n")
	steps = append(steps, "              ]);\n")
	steps = append(steps, "            }\n")
