	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectStep {
				...
				if !strings.Contains(string(yaml), "bash ${{ runner.temp }}/gh-aw/actions/download_docker_images.sh") {
					t.Error("Expected to find 'bash ${{ runner.temp }}/gh-aw/actions/download_docker_images.sh' script call in generated YAML")
				}
