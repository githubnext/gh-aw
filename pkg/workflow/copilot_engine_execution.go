...
// nodePathSetupCommand resolves the global node_modules directory from the node executable
// and exports it via NODE_PATH without requiring npm on PATH.
const nodePathSetupCommand = `GH_AW_NPM_GLOBAL_ROOT=""; if [ -n "${GH_AW_NODE_EXEC:-}" ]; then GH_AW_NPM_GLOBAL_ROOT="$(dirname "$(dirname "$GH_AW_NODE_EXEC")")/lib/node_modules"; fi; if [ -n "$GH_AW_NPM_GLOBAL_ROOT" ] && [ -d "$GH_AW_NPM_GLOBAL_ROOT" ]; then export NODE_PATH="${GH_AW_NPM_GLOBAL_ROOT}${NODE_PATH:+:${NODE_PATH}}"; fi`
