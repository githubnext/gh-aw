...
const nodePathSetupCommandForCopilotSDK = `GH_AW_WORKSPACE_NODE_MODULES="${GITHUB_WORKSPACE:-$PWD}/node_modules"; if [ -d "$GH_AW_WORKSPACE_NODE_MODULES" ]; then export NODE_PATH="${GH_AW_WORKSPACE_NODE_MODULES}${NODE_PATH:+:${NODE_PATH}}"; GH_AW_WORKSPACE_NODE_MODULES_BIN="$GH_AW_WORKSPACE_NODE_MODULES/.bin"; if [ -d "$GH_AW_WORKSPACE_NODE_MODULES_BIN" ]; then export PATH="$GH_AW_WORKSPACE_NODE_MODULES_BIN:$PATH"; fi; fi; ` + nodePathSetupCommand
