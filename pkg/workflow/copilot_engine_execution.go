...
var copilotEngineExecutionLog = logger.New("workflow:copilot_engine_execution")

// This file contains the execution steps generation logic for the Copilot engine.
// Extracted from copilot_engine.go to reduce file size.

func (e *CopilotEngine) GetExecutionSteps(workflowData *WorkflowData, logFile string) []GitHubActionStep {
	copilotEngineExecutionLog.Printf("Generating execution steps for Copilot: workflow=%s, firewall=%v", workflowData.Name, isFirewallEnabled(workflowData))
	...
	if sandboxEnabled {
		...
		copilotEngineExecutionLog.Print("Added workspace directory to --add-dir")

		copilotEngineExecutionLog.Print("Using firewall mode with simplified arguments")
	} else {
		...
		copilotEngineExecutionLog.Print("Using standard mode with full arguments")
	}
	...
	if modelConfigured {
		copilotEngineExecutionLog.Printf("Using custom model: %s", workflowData.EngineConfig.Model)
	...
	if workflowData.AgentFile != "" {
		...
		copilotEngineExecutionLog.Printf("Using custom agent: %s (identifier: %s)", workflowData.AgentFile, agentIdentifier)
	...
	if len(toolArgs) > 0 {
		copilotEngineExecutionLog.Printf("Adding %d tool permission arguments", len(toolArgs))
	}
	...
	if isSRTEnabled(workflowData) {
		...
		copilotEngineExecutionLog.Print("Using Sandbox Runtime (SRT) for execution")
		...
		if err != nil {
			copilotEngineExecutionLog.Printf("Error generating SRT config: %v", err)
		...
		if agentConfig != nil && agentConfig.Command != "" {
			...
			copilotEngineExecutionLog.Printf("Using custom SRT command: %s", agentConfig.Command)

			// Build args list with custom args appended
			var srtArgs []string
			if len(agentConfig.Args) > 0 {
				srtArgs = append(srtArgs, agentConfig.Args...)
				copilotEngineExecutionLog.Printf("Added %d custom args from agent config", len(agentConfig.Args))
			}
...
%s %s -- %s 2>&1 | tee %s`, agentConfig.Command, shellJoinArgs(srtArgs), copilotCommand, shellEscapeArg(logFile))
		...
		copilotEngineExecutionLog.Print("Set container working directory to GITHUB_WORKSPACE")
		...
		copilotEngineExecutionLog.Print("Added workspace mount to AWF")
		...
		copilotEngineExecutionLog.Print("Added gh CLI and copilot binary mounts to AWF container")
		...
		if agentConfig != nil && len(agentConfig.Mounts) > 0 {
			...
			copilotEngineExecutionLog.Printf("Added %d custom mounts from agent config", len(sortedMounts))
		}
		...
		if agentConfig != nil && agentConfig.Command != "" {
			...
			copilotEngineExecutionLog.Printf("Using custom AWF command: %s", awfCommand)
		} else {
			awfCommand = "sudo -E awf"
			copilotEngineExecutionLog.Print("Using standard AWF command")
		}
...
%s%s 2>&1 | tee %s`, mkdirCommands.String(), copilotCommand, logFile)
	...
	if agentConfig != nil && len(agentConfig.Env) > 0 {
		...
		copilotEngineExecutionLog.Printf("Added %d custom env vars from agent config", len(agentConfig.Env))
	}
