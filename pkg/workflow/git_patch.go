...
func (c *Compiler) generateGitPatchStep(yaml *strings.Builder) {
	...
	yaml.WriteString("                  fi\n")
	yaml.WriteString("                # Extract branch from push-to-pull-request-branch line using simple grep and sed\n")
	yaml.WriteString("                elif echo \"$line\" | grep -q '\"type\"[[:space:]]*:[[:space:]]*\"push-to-pull-request-branch\"'; then\n")
	yaml.WriteString("                  echo \"Found push-to-pull-request-branch line: $line\"\n")
	yaml.WriteString("                  # Extract branch value using sed\n")
	yaml.WriteString("                  BRANCH_NAME=$(echo \"$line\" | sed -n 's/.*\"branch\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p')\n")
	yaml.WriteString("                  if [ -n \"$BRANCH_NAME\" ]; then\n")
	yaml.WriteString("                    echo \"Extracted branch name from push-to-pull-request-branch: $BRANCH_NAME\"\n")
	yaml.WriteString("                    break\n")
	yaml.WriteString("                  fi\n")
	yaml.WriteString("                fi\n")
	yaml.WriteString("              fi\n")
