...
func TestLabelTriggerIntegrationDiscussion(t *testing.T) {
	...
	// Check names
	names, ok := discussion["names"].([]string)
	if !ok {
		t.Fatalf("discussion.names is not a string array")
	}
	if len(names) != 2 || names[0] != "question" || names[1] != "announcement" {
		t.Errorf("discussion.names = %v, want [question announcement]", names)
	}
