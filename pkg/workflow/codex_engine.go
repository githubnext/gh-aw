...
type codexRunParams struct {
	modelParam              string
	webSearchParam          string
	webFetchParam           string
	executionPolicyParam    string
	customArgsParam         string
	structuredOutputParam   string
...
func (e *CodexEngine) GetInstallationSteps(workflowData *WorkflowData) []GitHubActionStep {
	...
	return buildCodexFirewallInstallationSteps(buildCodexCLIInstallationSteps(workflowData), workflowData)
}
...
func buildCodexCLIInstallationSteps(workflowData *WorkflowData) []GitHubActionStep {
	...
	return append(steps, GenerateDockerSbxNpmCLIInstallStep(
		"@openai/codex",
		version,
...
func buildCodexFirewallInstallationSteps(steps []GitHubActionStep, workflowData *WorkflowData) []GitHubActionStep {
...
func getCodexModelEnvVar(workflowData *WorkflowData) string {
...
func buildCodexRunParams(workflowData *WorkflowData, modelEnvVar string, firewallEnabled bool) codexRunParams {
	...
	if workflowData.ParsedTools != nil && workflowData.ParsedTools.WebSearch != nil {
		params.webSearchParam = ""
	}
	if workflowData.ParsedTools != nil && workflowData.ParsedTools.WebFetch != nil {
		params.webFetchParam = ""
	}
	if firewallEnabled {
		params.executionPolicyParam = " --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check "
	}
	if workflowData.EngineConfig != nil && len(workflowData.EngineConfig.Args) > 0 {
		...
		params.customArgsParam = sb.String()
	}
	if workflowData.IsDetectionRun {
		params.structuredOutputParam = fmt.Sprintf(` --output-schema %s -o %s`, detectionSchemaFilePath, detectionResultFilePath)
		params.detectionSchemaWriteCmd = fmt.Sprintf("mkdir -p /tmp/gh-aw/threat-detection && printf '%%s' '%s' > %s", detectionResponseSchema, detectionSchemaFilePath)
		codexEngineLog.Printf("Enabling structured outputs for Codex detection run")
	}
	return params
}

func (e *CodexEngine) resolveCodexCommandConfig(workflowData *WorkflowData) codexCommandConfig {
	config := codexCommandConfig{
		commandName:       "codex",
		harnessScriptName: e.GetHarnessScriptName(),
	}
	if workflowData.EngineConfig != nil && workflowData.EngineConfig.Command != "" {
		config.commandName = workflowData.EngineConfig.Command
		codexEngineLog.Printf("Using custom command: %s", config.commandName)
	}
	if workflowData.EngineConfig != nil && workflowData.EngineConfig.HarnessScript != "" {
		config.harnessScriptName = workflowData.EngineConfig.HarnessScript
		codexEngineLog.Printf("Using custom harness script: %s", config.harnessScriptName)
	}
	return config
}

func (e *CodexEngine) buildCodexBaseCommand(workflowData *WorkflowData, config codexCommandConfig, params codexRunParams) string {
	if config.harnessScriptName != "" {
		execPrefix := fmt.Sprintf(`%s %s/%s %s`, nodeRuntimeResolutionCommand, SetupActionDestinationShell, config.harnessScriptName, config.commandName)
		return fmt.Sprintf("%s exec%s%s%s%s%s%s --prompt-file /tmp/gh-aw/aw-prompts/prompt.txt",
			execPrefix, params.modelParam, params.webSearchParam, params.webFetchParam, params.executionPolicyParam, params.structuredOutputParam, params.customArgsParam)
	}
	return getWorkspaceCommandPrefixFor(workflowData.EngineConfig) + fmt.Sprintf("%s exec%s%s%s%s%s%s \"$INSTRUCTION\"",
		config.commandName, params.modelParam, params.webSearchParam, params.webFetchParam, params.executionPolicyParam, params.structuredOutputParam, params.customArgsParam)
}

func (e *CodexEngine) buildCodexCommand(workflowData *WorkflowData, logFile, codexCommand string, config codexCommandConfig, params codexRunParams) string {
	if isFirewallEnabled(workflowData) {
		return e.buildCodexFirewallCommand(workflowData, logFile, codexCommand, config.harnessScriptName, params.detectionSchemaWriteCmd)
	}
	return e.buildCodexNonFirewallCommand(workflowData, logFile, codexCommand, config.harnessScriptName, params.detectionSchemaWriteCmd)
}

func getCodexAllowedDomains(workflowData *WorkflowData) string {
...
func buildCodexCommandWithSetup(workflowData *WorkflowData, codexCommand, harnessScriptName string) string {
	command := fmt.Sprintf(`%s && %s`, GetNpmBinPathSetup(), codexCommand)
	if harnessScriptName == "" {
		command = fmt.Sprintf(`%s && INSTRUCTION="$(cat /tmp/gh-aw/aw-prompts/prompt.txt)" && %s`, GetNpmBinPathSetup(), codexCommand)
	}
	if dockerSbxCLIPath := GetDockerSbxNpmCLIPathSetup(workflowData); dockerSbxCLIPath != "" {
		command = fmt.Sprintf("%s && %s", dockerSbxCLIPath, command)
	}
	if mcpCLIPath := GetMCPCLIPathSetup(workflowData); mcpCLIPath != "" {
		command = fmt.Sprintf("%s && %s", mcpCLIPath, command)
	}
	return command
}

func buildCodexAWFPathSetup(workflowData *WorkflowData, detectionSchemaWriteCmd string) string {
...
func (e *CodexEngine) buildCodexFirewallCommand(workflowData *WorkflowData, logFile, codexCommand, harnessScriptName, detectionSchemaWriteCmd string) string {
	return BuildAWFCommand(AWFCommandConfig{
		EngineName:         "codex",
		EngineCommand:      buildCodexCommandWithSetup(workflowData, codexCommand, harnessScriptName),
		LogFile:            logFile,
		WorkflowData:       workflowData,
		UsesTTY:            false,
		AllowedDomains:     getCodexAllowedDomains(workflowData),
		PathSetup:          buildCodexAWFPathSetup(workflowData, detectionSchemaWriteCmd),
		ExcludeEnvVarNames: ComputeAWFExcludeEnvVarNames(workflowData, []string{"CODEX_API_KEY", "OPENAI_API_KEY"}),
	})
}

func buildCodexSchemaWritePrefix(workflowData *WorkflowData, detectionSchemaWriteCmd string) string {
	if workflowData.IsDetectionRun {
		return detectionSchemaWriteCmd + " && "
	}
	return ""
}

func (e *CodexEngine) buildCodexNonFirewallCommand(workflowData *WorkflowData, logFile, codexCommand, harnessScriptName, detectionSchemaWriteCmd string) string {
	schemaWritePrefix := buildCodexSchemaWritePrefix(workflowData, detectionSchemaWriteCmd)
	if harnessScriptName != "" {
...
func (e *CodexEngine) buildCodexExecutionEnv(workflowData *WorkflowData, modelConfigured, firewallEnabled bool, modelEnvVar string) map[string]string {
...
func (e *CodexEngine) GetExecutionSteps(workflowData *WorkflowData, logFile string) []GitHubActionStep {
	...
	modelEnvVar := getCodexModelEnvVar(workflowData)
	runParams := buildCodexRunParams(workflowData, modelEnvVar, firewallEnabled)
	commandConfig := e.resolveCodexCommandConfig(workflowData)
	codexCommand := e.buildCodexBaseCommand(workflowData, commandConfig, runParams)
	command := e.buildCodexCommand(workflowData, logFile, codexCommand, commandConfig, runParams)
	env := e.buildCodexExecutionEnv(workflowData, modelConfigured, firewallEnabled, modelEnvVar)
	return []GitHubActionStep{e.buildCodexExecutionStep(workflowData, command, env)}
}
...
func applyCodexPlaywrightTool(result, toolsConfig *ToolsConfig) {
	if toolsConfig == nil || toolsConfig.Playwright == nil {
		return
	}
...
func (e *CodexEngine) expandNeutralToolsToCodexTools(toolsConfig *ToolsConfig) *ToolsConfig {
	...
	applyCodexPlaywrightTool(result, toolsConfig)
	return result
}
