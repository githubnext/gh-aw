...
func ResolveWorkflowName(workflowInput string) (string, error) {
	...
	// Check for .lock.yaml first (user preference), then fall back to .lock.yml
	lockFile := filepath.Join(workflowsDir, normalizedName+constants.LockExtensionYAML)
	if _, err := os.Stat(lockFile); err != nil {
		lockFile = filepath.Join(workflowsDir, normalizedName+constants.LockExtensionYML)
	}
...
func GetWorkflowLockFileName(input string) (string, error) {
	...
	lockFile := filepath.Join(workflowsDir, normalizedName+constants.LockExtensionYML)
	if _, err := os.Stat(lockFile); err == nil {
		return normalizedName + constants.LockExtensionYML, nil
	}
	...
	for _, wf := range workflows {
		if strings.ToLower(wf.DisplayName) == lowerInput {
			return wf.WorkflowID + constants.LockExtensionYML, nil
		}
	}
...
func GetAllWorkflows() ([]WorkflowNameMatch, error) {
	...
	lockFiles, err := filepath.Glob(filepath.Join(workflowsDir, "*"+constants.LockExtensionYML))
	if err != nil {
	...
	for _, lockFile := range lockFiles {
		...
		workflowID := strings.TrimSuffix(base, constants.LockExtensionYML)
