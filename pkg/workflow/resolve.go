...
func ResolveWorkflowName(workflowInput string) (string, error) {
	...
	if _, err := os.Stat(mdFile); err != nil {
		return "", missingWorkflowMarkdownError(normalizedName, mdFile)
	}
	...
	if _, err := os.Stat(lockFile); err != nil {
		return "", missingWorkflowLockError(normalizedName, lockFile)
	}

	// Read and parse the lock file to extract the workflow name
	workflowName, err := readWorkflowNameFromLockFile(lockFile)
	if err != nil {
		return "", err
	}

	if workflowName == "" {
		return "", missingWorkflowNameInLockError(normalizedName, lockFile)
	}

	resolveLog.Printf("Successfully resolved workflow name: %s", workflowName)
	return workflowName, nil
}

func missingWorkflowMarkdownError(normalizedName, mdFile string) error {
...
func missingWorkflowLockError(normalizedName, lockFile string) error {
	suggestions := []string{
...
func readWorkflowNameFromLockFile(lockFile string) (string, error) {
...
func missingWorkflowNameInLockError(normalizedName, lockFile string) error {
...
func FindWorkflowName(input string) (string, error) {
	...
	if displayName := findWorkflowByID(workflows, input, normalizedInput, false); displayName != "" {
		return displayName, nil
	}

	// Strategy 3: Try case-insensitive match with workflow ID
	if displayName := findWorkflowByID(workflows, input, normalizedInput, true); displayName != "" {
		return displayName, nil
	}

	// Strategy 4: Try exact match with display name (case-sensitive)
	if displayName := findWorkflowByDisplayName(workflows, input, false); displayName != "" {
		return displayName, nil
	}

	// Strategy 5: Try case-insensitive match with display name
	if displayName := findWorkflowByDisplayName(workflows, input, true); displayName != "" {
		return displayName, nil
	}
...
func findWorkflowByID(workflows []WorkflowNameMatch, input, normalizedInput string, fold bool) string {
	for _, wf := range workflows {
		matches := wf.WorkflowID == input || wf.WorkflowID == normalizedInput
		if fold {
			matches = strings.EqualFold(wf.WorkflowID, input) || strings.EqualFold(wf.WorkflowID, normalizedInput)
		}
		if matches {
			if fold {
			...
			return wf.DisplayName
		}
	}
	return ""
}

func findWorkflowByDisplayName(workflows []WorkflowNameMatch, input string, fold bool) string {
	for _, wf := range workflows {
		matches := wf.DisplayName == input
		if fold {
			matches = strings.EqualFold(wf.DisplayName, input)
		}
		if matches {
			if fold {
