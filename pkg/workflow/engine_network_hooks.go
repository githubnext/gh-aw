...
// GenerateNetworkHookScript generates a JavaScript script for network permissions using actions/github-script
func (g *NetworkHookGenerator) GenerateNetworkHookScript(allowedDomains []string) string {
	// Return the embedded JavaScript content
	return networkPermissionsScript
}

// GenerateNetworkHookWorkflowStep generates a GitHub Actions workflow step that uses actions/github-script for network permissions
func (g *NetworkHookGenerator) GenerateNetworkHookWorkflowStep(allowedDomains []string) GitHubActionStep {
	// Convert domain list to JSON for environment variable
	...
	var lines []string
	lines = append(lines, "      - name: Network Permissions Hook")
	lines = append(lines, "        uses: actions/github-script@v7")
	lines = append(lines, "        env:")
	lines = append(lines, fmt.Sprintf("          GITHUB_AW_NETWORK_DOMAINS: '%s'", string(domainsJSON)))
	lines = append(lines, "          GITHUB_AW_TOOL_NAME: ${{ env.TOOL_NAME }}")
	lines = append(lines, "          GITHUB_AW_TOOL_INPUT: ${{ env.TOOL_INPUT }}")
	lines = append(lines, "        with:")
	lines = append(lines, "          script: |")

	// Add the JavaScript content with proper indentation
	jsLines := FormatJavaScriptForYAML(networkPermissionsScript)
