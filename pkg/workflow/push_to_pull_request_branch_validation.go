...
//
//  2. No constraints — target: "*" without require-title-prefix or labels means the agent may
...
func (c *Compiler) validatePushToPullRequestBranchWarnings(safeOutputs *SafeOutputsConfig, checkoutConfigs []*CheckoutConfig) {
	...
	if cfg.TitlePrefix == "" && len(cfg.Labels) == 0 {
		msg := strings.Join([]string{
			...
			"Consider adding require-title-prefix: or labels: to restrict which PRs can receive pushes.",
			"",
			"Example:",
			"",
			"  push-to-pull-request-branch:",
			"    target: \"*\"",
			"    require-title-prefix: \"[bot] \"  # only PRs whose title starts with this prefix",
