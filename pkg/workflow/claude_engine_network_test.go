...
func TestClaudeEngineNetworkPermissions(t *testing.T) {
	...
	t.Run("InstallationSteps without network permissions", func(t *testing.T) {
		...
		if len(steps) != 3 {
			t.Errorf("Expected 3 installation steps without network permissions (Node.js setup, cache, install), got %d", len(steps))
		}
	})
	...
	t.Run("InstallationSteps with network permissions", func(t *testing.T) {
		...
		steps := engine.GetInstallationSteps(workflowData)
		if len(steps) != 5 {
			t.Errorf("Expected 5 installation steps with network permissions (Node.js, cache, install, settings, hook), got %d", len(steps))
		}
...
func TestNetworkPermissionsIntegration(t *testing.T) {
	t.Run("Full workflow generation", func(t *testing.T) {
		...
		if len(steps) != 5 {
			t.Fatalf("Expected 5 installation steps (Node.js, cache, install, settings, hook), got %d", len(steps))
		}
