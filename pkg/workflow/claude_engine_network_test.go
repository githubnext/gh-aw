...
func TestClaudeEngineNetworkPermissions(t *testing.T) {
	...
	t.Run("InstallationSteps with network permissions and firewall enabled", func(t *testing.T) {
		...
		steps := engine.GetInstallationSteps(workflowData)
		// With AWF enabled: Node.js setup + AWF cache restore + AWF install + Claude install = 4 steps
		// (secret validation is now in the activation job)
		if len(steps) != 4 {
			t.Errorf("Expected 4 installation steps with network permissions and AWF (Node.js setup + AWF cache + AWF install + Claude install), got %d", len(steps))
		}
...
func TestNetworkPermissionsIntegration(t *testing.T) {
	t.Run("Full workflow generation with AWF", func(t *testing.T) {
		...
		// (secret validation is now in the activation job)
		if len(steps) != 4 {
			t.Fatalf("Expected 4 installation steps (Node.js setup + AWF cache + AWF install + Claude install), got %d", len(steps))
		}
