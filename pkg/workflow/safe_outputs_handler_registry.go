...
func createPullRequestHandlerConfig(cfg *SafeOutputsConfig) map[string]any {
	...
	addCreatePullRequestHandlerDefaults(builder, cfg, c)
	addCreatePullRequestHeadToken(builder, c)
	return builder.Build()
}

func addCreatePullRequestHandlerDefaults(builder *handlerConfigBuilder, cfg *SafeOutputsConfig, c *CreatePullRequestsConfig) {
	builder.
		AddDefault("max_patch_size", resolveCreatePullRequestMaxPatchSize(cfg, c)).
		AddDefault("max_patch_files", resolveCreatePullRequestMaxPatchFiles(cfg, c)).
		...
		AddIfNotEmpty("base_branch", c.BaseBranch).
		AddDefault("protected_files_policy", resolveCreatePullRequestProtectedFilesPolicy(c)).
		AddStringSlice("protected_files", getAllManifestFiles()).
		AddStringSlice("protected_path_prefixes", getProtectedPathPrefixes()).
		AddDefault("protect_top_level_dot_folders", true).
		AddStringSlice("_protected_files_exclude", c.ProtectedFilesExclude).
...
func addCreatePullRequestHeadToken(builder *handlerConfigBuilder, c *CreatePullRequestsConfig) {
	if c.HeadGitHubApp != nil {
		//nolint:gosec // G101: False positive - this is a GitHub Actions expression template, not a hardcoded credential
		builder.AddIfNotEmpty("head-github-token", "${{ steps.safe-outputs-head-app-token.outputs.token }}")
		return
...
func resolveCreatePullRequestProtectedFilesPolicy(c *CreatePullRequestsConfig) string {
	if c.ManifestFilesPolicy != nil {
		return *c.ManifestFilesPolicy
	}
	return "request_review"
}

func resolveCreatePullRequestMaxPatchSize(cfg *SafeOutputsConfig, c *CreatePullRequestsConfig) int {
	maxPatchSize := 4096
	if cfg.MaximumPatchSize > 0 {
		maxPatchSize = cfg.MaximumPatchSize
	}
...
func resolveCreatePullRequestMaxPatchFiles(cfg *SafeOutputsConfig, c *CreatePullRequestsConfig) int {
	maxPatchFiles := 100
