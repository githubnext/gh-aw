...
type RepoConfig struct {
	...
	// references. Enterprises running in a private cloud can use this to
	// redirect container images to internal mirrors. Keys are source image
	// references (e.g. "ghcr.io/owner/image:tag") and values are replacement
	// image references. Digest-pinned replacement images are supported.
...
func (r *RepoConfig) UnmarshalJSON(data []byte) error {
	...
	var raw struct {
		...
		ContainerPins map[string]string `json:"container_pins,omitempty"`
	}
