...
func (c *Compiler) setupEngineAndImports(result *parser.FrontmatterResult, cleanPath string, content []byte, markdownDir string) (*engineSetupResult, error) {
	...
	for _, importedFile := range importsResult.ImportedFiles {
		...
		if resolveErr != nil {
			...
			fmt.Fprintln(os.Stderr, console.FormatWarningMessage(fmt.Sprintf("Skipping security scan for unresolvable import '%s': %v", importedFile, resolveErr)))
			continue
		}
		...
		if readErr != nil {
			...
			fmt.Fprintln(os.Stderr, console.FormatWarningMessage(fmt.Sprintf("Skipping security scan for unreadable import '%s' (resolved path: %s): %v", importedFile, fullPath, readErr)))
			continue
		}
