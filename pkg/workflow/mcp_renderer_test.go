...
func TestRenderSafeOutputsMCP_JSON_Copilot(t *testing.T) {
	...
	if !strings.Contains(output, `"mounts": ["\${GITHUB_WORKSPACE}:\${GITHUB_WORKSPACE}:rw", "${RUNNER_TEMP}/gh-aw/safeoutputs:${RUNNER_TEMP}/gh-aw/safeoutputs:rw", "/tmp/gh-aw/mcp-logs/safeoutputs:/tmp/gh-aw/mcp-logs/safeoutputs:rw", "/tmp/gh-aw:/tmp/gh-aw:rw"]`) {
