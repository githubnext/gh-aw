...
func GenerateTraceSetupStep(yaml *strings.Builder) {
	...
	yaml.WriteString("          \n")
	yaml.WriteString("          # Initialize checkpoint counter\n")
	yaml.WriteString("          echo \"0\" > trace/.checkpoint_counter\n")
	yaml.WriteString("          \n")
	yaml.WriteString("          # Copy checkpoint recording script to temp location\n")
	yaml.WriteString("          cat > /tmp/gh-aw/record_checkpoint.sh <<'EOF_SCRIPT'\n")
	yaml.WriteString("          #!/usr/bin/env bash\n")
	yaml.WriteString("          set -euo pipefail\n")
	yaml.WriteString("          TRACE_DIR=\"${TRACE_DIR:-trace}\"\n")
	yaml.WriteString("          CHECKPOINT_TYPE=\"${1:-tool_call}\"\n")
	yaml.WriteString("          CHECKPOINT_DESC=\"${2:-Agent action}\"\n")
	yaml.WriteString("          CHECKPOINT_COUNTER_FILE=\"$TRACE_DIR/.checkpoint_counter\"\n")
	yaml.WriteString("          if [[ -f \"$CHECKPOINT_COUNTER_FILE\" ]]; then\n")
	yaml.WriteString("              COUNTER=$(cat \"$CHECKPOINT_COUNTER_FILE\")\n")
	yaml.WriteString("          else\n")
	yaml.WriteString("              COUNTER=0\n")
	yaml.WriteString("          fi\n")
	yaml.WriteString("          COUNTER=$((COUNTER + 1))\n")
	yaml.WriteString("          echo \"$COUNTER\" > \"$CHECKPOINT_COUNTER_FILE\"\n")
	yaml.WriteString("          CHECKPOINT_ID=$(printf \"c%03d\" \"$COUNTER\")\n")
	yaml.WriteString("          REPO_SHA=\"$(git rev-parse HEAD 2>/dev/null || echo 'unknown')\"\n")
	yaml.WriteString("          cat >> \"$TRACE_DIR/checkpoints.jsonl\" <<'CHECKPOINT_EOF'\n")
	yaml.WriteString("          {\"id\":\"$CHECKPOINT_ID\",\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"kind\":\"$CHECKPOINT_TYPE\",\"name\":\"$CHECKPOINT_DESC\",\"repo_sha\":\"$REPO_SHA\"}\n")
	yaml.WriteString("          CHECKPOINT_EOF\n")
	yaml.WriteString("          echo \"✅ Checkpoint $CHECKPOINT_ID: $CHECKPOINT_TYPE - $CHECKPOINT_DESC\"\n")
	yaml.WriteString("          EOF_SCRIPT\n")
	yaml.WriteString("          chmod +x /tmp/gh-aw/record_checkpoint.sh\n")
	yaml.WriteString("          \n")
