...
func collectPackagesFromWorkflow(
	...
	if toolCommand != "" && workflowData.ParsedTools != nil {
		for _, toolConfig := range workflowData.ParsedTools.Custom {
			if toolConfig.Command == toolCommand {
				// Extract package from args, skipping flags
				for _, arg := range toolConfig.Args {
					// Skip flags (arguments starting with - or --)
					if !strings.HasPrefix(arg, "-") && !seen[arg] {
