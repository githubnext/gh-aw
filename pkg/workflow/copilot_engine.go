...
func (e *CopilotEngine) generateCopySessionStateStep() GitHubActionStep {
	stepLines := []string{
		...
		"        if: always()",
		"        run: |",
		"          # Search for session-state file in multiple locations",
		fmt.Sprintf("          if [ -f %s ]; then", shellEscapeArg(sessionStateSource)),
		fmt.Sprintf("            SESSION_STATE_PATH=%s", shellEscapeArg(sessionStateSource)),
		fmt.Sprintf("            echo \"Found session-state file at: $SESSION_STATE_PATH\""),
		"          else",
		"            # Search in home directory",
		"            SESSION_STATE_PATH=$(find ~ -maxdepth 3 -name 'session-state' -type f 2>/dev/null | head -1)",
		"            if [ -z \"$SESSION_STATE_PATH\" ]; then",
		"              # Search in /tmp directory",
		"              SESSION_STATE_PATH=$(find /tmp -maxdepth 3 -name 'session-state' -type f 2>/dev/null | head -1)",
		"            fi",
		"            if [ -z \"$SESSION_STATE_PATH\" ]; then",
		"              echo \"Error: session-state file not found in expected locations\"",
		"              echo \"Searched locations:\"",
		fmt.Sprintf("              echo \"  - %s\"", shellEscapeArg(sessionStateSource)),
		"              echo \"  - ~ (home directory)\"",
		"              echo \"  - /tmp\"",
		"              exit 1",
		"            else",
		"              echo \"Found session-state file at: $SESSION_STATE_PATH\"",
		"            fi",
		"          fi",
		"          echo \"Copying Copilot session-state file\"",
		fmt.Sprintf("          mkdir -p \"$(dirname %s)\"", shellEscapeArg(sessionStateFile)),
		fmt.Sprintf("          cp \"$SESSION_STATE_PATH\" %s", shellEscapeArg(sessionStateFile)),
	}
