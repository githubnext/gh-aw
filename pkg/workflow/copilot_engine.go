...
func (e *CopilotEngine) getDockerComposeExecutionSteps(workflowData *WorkflowData, logFile string) []GitHubActionStep {
	var stepLines []string
	...
	stepLines = append(stepLines, "          # Execute containerized GitHub Copilot CLI with proxy")
	stepLines = append(stepLines, "          ")
	stepLines = append(stepLines, "          # Create necessary directories")
	stepLines = append(stepLines, "          mkdir -p mcp-config prompts logs safe-outputs .copilot")
	stepLines = append(stepLines, "          ")
	...
	stepLines = append(stepLines, "          ")
	stepLines = append(stepLines, "          # Copy Copilot logs from container if they exist")
	stepLines = append(stepLines, "          docker compose -f docker-compose-engine.yml cp agent:"+logsFolder+" logs/ || true")
	stepLines = append(stepLines, "          ")
	...
	// Add environment variables
	stepLines = append(stepLines, "        env:")
	stepLines = append(stepLines, "          XDG_CONFIG_HOME: /home/runner")
	stepLines = append(stepLines, "          COPILOT_AGENT_RUNNER_TYPE: STANDALONE")
	stepLines = append(stepLines, "          GITHUB_TOKEN: ${{ secrets.COPILOT_CLI_TOKEN }}")
	...
	if HasMCPServers(workflowData) {
		stepLines = append(stepLines, "          GITHUB_AW_MCP_CONFIG: /home/runner/.copilot/mcp-config.json")
	}
