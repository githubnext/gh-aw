...
func (c *Compiler) validateStepsSectionSecrets(frontmatter map[string]any, sectionName string) error {
	...
	if c.strictMode {
		return fmt.Errorf(
			"strict mode: secrets expressions detected in '%s' section will be leaked to the agent job. Found: %s. "+
				"Move secrets into the engine configuration or use environment-level secrets instead",
	...
	warningMsg := fmt.Sprintf(
		"Warning: secrets expressions detected in '%s' section will be leaked to the agent job. Found: %s. "+
			"Consider moving secrets into the engine configuration or using environment-level secrets instead.",
		sectionName, strings.Join(secretRefs, ", "),
	)
