...
- Call #1 is required in Test Requirement 7 (comment on latest discussion).
- Call #2 depends on trigger:
  - `pull_request` event: post the brief PR summary comment in Output step 2, and **skip Output step 3**.
  - non-`pull_request` event: **skip Output step 2** and post the fun discussion comment in Output step 3.
