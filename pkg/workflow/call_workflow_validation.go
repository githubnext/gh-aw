...
func validateYAMLWorkflowHasCallTrigger(path, workflowName string) error {
	...
	if !hasOn {
		return fmt.Errorf("call-workflow: workflow '%s' has no 'on' trigger section, expected an 'on' section with a 'workflow_call' trigger. Example: on: workflow_call", workflowName)
	}
	if containsWorkflowCall(onSection) {
		return nil
	}
	return fmt.Errorf("call-workflow: workflow '%s' does not support the workflow_call trigger, expected 'workflow_call' in the 'on' section. Example: on: workflow_call", workflowName)
}
