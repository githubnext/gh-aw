...
func (c *Compiler) generateMCPSetup(yaml *strings.Builder, tools map[string]any, engine CodingAgentEngine, workflowData *WorkflowData) error {
	...
	// Pass GITHUB_MCP_HOST (set by detect-github-host step) so the gateway container can
	// expand $GITHUB_MCP_HOST references inside the MCP server env block.
	containerCmd.WriteString(" -e GITHUB_MCP_HOST")
	...
	if len(mcpEnvVars) > 0 {
		...
		standardEnvVars := []string{
			...
			"DEFAULT_BRANCH", "GITHUB_MCP_SERVER_TOKEN", "GITHUB_MCP_LOCKDOWN", "GITHUB_MCP_HOST",
