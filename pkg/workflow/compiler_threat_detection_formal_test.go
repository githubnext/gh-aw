...
func TestFormal_CTR016_NilManifestSkipsEnforcement(t *testing.T) {
	err := EnforceSafeUpdate(nil, []string{"MY_SECRET"}, []string{"evil-org/action@deadbeef # v1"}, "", false, false)
	require.NoError(t, err)
}

func TestFormal_CTR016_EmptyManifestRejectsNewSecret(t *testing.T) {
	err := EnforceSafeUpdate(&GHAWManifest{Version: currentGHAWManifestVersion}, []string{"MY_SECRET"}, nil, "", false, false)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "MY_SECRET")
}

func TestFormal_CTR016_GitHubTokenExempt_BareForm(t *testing.T) {
	err := EnforceSafeUpdate(&GHAWManifest{Version: currentGHAWManifestVersion}, []string{"GITHUB_TOKEN"}, nil, "", false, false)
	require.NoError(t, err)
}

func TestFormal_CTR016_GitHubTokenExempt_PrefixedForm(t *testing.T) {
	err := EnforceSafeUpdate(&GHAWManifest{Version: currentGHAWManifestVersion}, []string{"secrets.GITHUB_TOKEN"}, nil, "", false, false)
	require.NoError(t, err)
}

func TestFormal_CTR016_GhAwInternalSecretExempt(t *testing.T) {
	err := EnforceSafeUpdate(&GHAWManifest{Version: currentGHAWManifestVersion}, []string{"GH_AW_GITHUB_TOKEN"}, nil, "", false, false)
	require.NoError(t, err)
}
...
func TestFormal_CTR016_SecretPrefixNormalization(t *testing.T) {
	...
	err := EnforceSafeUpdate(manifest, []string{"secrets.MY_SECRET"}, nil, "", false, false)
	require.NoError(t, err)
}
...
func TestFormal_CTR016_NewActionDriftRejected(t *testing.T) {
	...
	err := EnforceSafeUpdate(manifest, nil, []string{"actions/checkout@abc1234 # v4", "evil-org/steal@deadbeef # v1"}, "", false, false)
	require.Error(t, err)
...
func TestFormal_CTR016_RemovedActionDriftRejected(t *testing.T) {
	...
	err := EnforceSafeUpdate(manifest, nil, []string{}, "", false, false)
	require.Error(t, err)
...
func TestFormal_CTR016_KnownActionPinUpdateAllowed(t *testing.T) {
	...
	err := EnforceSafeUpdate(manifest, nil, []string{"my-org/action@def5678 # v2"}, "", false, false)
	require.NoError(t, err)
}
...
func TestFormal_CTR016_RedirectWhitespaceNormalization(t *testing.T) {
	...
	err := EnforceSafeUpdate(manifest, nil, nil, "  owner/repo/workflows/new.md@main  ", false, false)
	require.NoError(t, err)
}
