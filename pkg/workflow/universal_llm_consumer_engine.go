...
func (e *UniversalLLMConsumerEngine) BuildCLIEngineExecutionSteps(
	...
	engineCommand := e.buildUniversalCLICommand(workflowData, cfg)
	command := e.buildUniversalCLIExecutionCommand(workflowData, logFile, cfg, engineCommand, firewallEnabled, modelConfigured)
	env := e.buildUniversalCLIExecutionEnv(workflowData, cfg, firewallEnabled, modelConfigured)
	step := e.buildUniversalCLIExecutionStep(workflowData, cfg, command, env)
	return prependUniversalCLIConfigStep(cfg.ConfigStep, step)
}

func prependUniversalCLIConfigStep(configStep, step GitHubActionStep) []GitHubActionStep {
	if len(configStep) == 0 {
		return []GitHubActionStep{step}
	}
	return []GitHubActionStep{configStep, step}
}

func (e *UniversalLLMConsumerEngine) buildUniversalCLICommand(workflowData *WorkflowData, cfg UniversalCLIEngineExecutionConfig) string {
	cliArgs := append([]string{}, cfg.ExtraCLIArgs...)
	commandName := cfg.DefaultCommandName
	if workflowData.EngineConfig != nil && workflowData.EngineConfig.Command != "" {
		commandName = workflowData.EngineConfig.Command
	}
	engineCommand := fmt.Sprintf("%s run %s \"$(cat %s)\"", commandName, shellJoinArgs(cliArgs), constants.AwPromptsFile)
	return getWorkspaceCommandPrefixFor(workflowData.EngineConfig) + engineCommand
}

func (e *UniversalLLMConsumerEngine) buildUniversalCLIExecutionCommand(workflowData *WorkflowData, logFile string, cfg UniversalCLIEngineExecutionConfig, engineCommand string, firewallEnabled, modelConfigured bool) string {
	if firewallEnabled {
		...
		return BuildAWFCommand(AWFCommandConfig{
			...
			WorkflowData:   workflowData,
			UsesTTY:        false,
			AllowedDomains: resolveUniversalCLIAllowedDomains(workflowData, cfg, modelConfigured),
		})
	}
...
func resolveUniversalCLIAllowedDomains(workflowData *WorkflowData, cfg UniversalCLIEngineExecutionConfig, modelConfigured bool) string {
...
func (e *UniversalLLMConsumerEngine) buildUniversalCLIExecutionEnv(workflowData *WorkflowData, cfg UniversalCLIEngineExecutionConfig, firewallEnabled, modelConfigured bool) map[string]string {
	env := map[string]string{
...
func (e *UniversalLLMConsumerEngine) buildUniversalCLIExecutionStep(workflowData *WorkflowData, cfg UniversalCLIEngineExecutionConfig, command string, env map[string]string) GitHubActionStep {
	stepLines := []string{
		"      - name: " + cfg.StepName,
		"        id: agentic_execution",
	}
	filteredEnv := FilterEnvForSecrets(env, e.GetUniversalRequiredSecretNames(workflowData))
