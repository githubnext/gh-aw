...
func ResolveSetupActionReference(ctx context.Context, actionMode ActionMode, version string, actionTag string, resolver ActionSHAResolver) string {
...
func resolveSetupActionRef(ctx context.Context, actionMode ActionMode, version string, actionTag string, resolver ActionSHAResolver, actionsOrgRepo string) string {
	if actionsOrgRepo == "" {
...
func (c *Compiler) resolveActionReference(localActionPath string, data *WorkflowData) string {
	...
	if localActionPath == "./actions/setup" {
		...
		if c.actionTag != "" {
			return resolveSetupActionRef(c.context(), c.actionMode, c.version, c.actionTag, resolver, c.effectiveActionsRepo())
		}
		if !hasActionTag {
			return resolveSetupActionRef(c.context(), c.actionMode, c.version, "", resolver, c.effectiveActionsRepo())
		}
		// hasActionTag is true and no compiler actionTag: use action mode with the frontmatter tag
		return resolveSetupActionRef(c.context(), ActionModeAction, c.version, frontmatterActionTag, resolver, c.effectiveActionsRepo())
	}
