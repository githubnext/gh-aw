...
// It creates jobs for threat detection, issue creation, PR creation, comments, labels, and other
// safe output operations. Each job is added to the job manager and their names are tracked for
// dependency management in the conclusion job.
func (c *Compiler) buildSafeOutputsJobs(data *WorkflowData, jobName, markdownPath string) error {
	...
	// This replaces the individual job creation for each safe output type
