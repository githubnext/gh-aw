...
func TestValidateGitHubGuardPolicy(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid guard policy with disapproval-labels",
			toolsMap: map[string]any{
				"github": map[string]any{
					"allowed-repos":      "all",
					"min-integrity":      "approved",
					"disapproval-labels": []string{"needs-work", "unsafe-for-agent"},
				},
			},
			shouldError: false,
		},
		{
			name: "valid guard policy with both approval-labels and disapproval-labels",
			toolsMap: map[string]any{
				"github": map[string]any{
					"allowed-repos":      "all",
					"min-integrity":      "approved",
					"approval-labels":    []string{"human-reviewed"},
					"disapproval-labels": []string{"needs-work"},
				},
			},
			shouldError: false,
		},
		{
			name: "disapproval-labels without min-integrity fails",
			toolsMap: map[string]any{
				"github": map[string]any{
					"disapproval-labels": []string{"needs-work"},
				},
		...
		{
			name: "disapproval-labels with empty string entry fails",
			toolsMap: map[string]any{
				"github": map[string]any{
					"allowed-repos":      "all",
					"min-integrity":      "approved",
					"disapproval-labels": []string{""},
				},
			},
			shouldError: true,
			errorMsg:    "'github.disapproval-labels' entries must not be empty strings",
		},
		{
			name: "disapproval-labels as GitHub Actions expression is valid",
			toolsMap: map[string]any{
				"github": map[string]any{
					"allowed-repos":      "all",
					"min-integrity":      "approved",
		...
		{
			name: "disapproval-labels expression without min-integrity fails",
			toolsMap: map[string]any{
				"github": map[string]any{
					"disapproval-labels": "${{ vars.DISAPPROVAL_LABELS }}",
				},
