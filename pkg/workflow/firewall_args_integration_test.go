...
// TestCleanupScriptWorkspaceIntegration tests that cleanup scripts use $GITHUB_WORKSPACE
func TestCleanupScriptWorkspaceIntegration(t *testing.T) {
	t.Run("compiled workflow uses GITHUB_WORKSPACE for cleanup script", func(t *testing.T) {
		// Create temporary directory for test
		tmpDir := t.TempDir()
		workflowsDir := filepath.Join(tmpDir, ".github", "workflows")
		err := os.MkdirAll(workflowsDir, 0755)
		...
		// Create test workflow with firewall enabled
		workflowContent := `---
on: workflow_dispatch
permissions:
  contents: read
...
`
		...
		workflowPath := filepath.Join(workflowsDir, "test-cleanup-workspace.md")
		err = os.WriteFile(workflowPath, []byte(workflowContent), 0644)
		if err != nil {
			t.Fatalf("Failed to write workflow file: %v", err)
		}
		...
		// Compile the workflow
		compiler := NewCompiler(false, "", "test-cleanup-workspace")
		compiler.SetSkipValidation(true)
		...
		// Read the compiled workflow
		lockPath := filepath.Join(workflowsDir, "test-cleanup-workspace.lock.yml")
		lockContent, err := os.ReadFile(lockPath)
		if err != nil {
			t.Fatalf("Failed to read compiled workflow: %v", err)
		}
		...
		// Verify cleanup script uses $GITHUB_WORKSPACE with proper quoting
		if !strings.Contains(lockYAML, "\"${GITHUB_WORKSPACE}\"/scripts/ci/cleanup.sh") &&
			...
			t.Error("Compiled workflow should use quoted $GITHUB_WORKSPACE or ${GITHUB_WORKSPACE} for cleanup script path")
		}

		// Verify it does NOT use compile-time relative path
		if strings.Contains(lockYAML, "run: ./scripts/ci/cleanup.sh") {
			t.Error("Compiled workflow should not use compile-time relative path './scripts/ci/cleanup.sh'")
		}
	})
}
