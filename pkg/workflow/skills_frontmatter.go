...
var githubTokenExpressionRegexp = regexp.MustCompile(`^\$\{\{\s*(secrets\.[A-Za-z_][A-Za-z0-9_]*(\s*\|\|\s*secrets\.[A-Za-z_][A-Za-z0-9_]*)*|needs\.[A-Za-z_][A-Za-z0-9_]*\.outputs\.[A-Za-z_][A-Za-z0-9_]*|steps\.[A-Za-z_][A-Za-z0-9_]*\.outputs\.[A-Za-z_][A-Za-z0-9_]*)\s*\}\}$`)
		...
		case map[string]any:
			...
			if tokenValue, hasToken := typed["github-token"]; hasToken {
				...
				if !githubTokenExpressionRegexp.MatchString(token) {
					return fmt.Errorf(
						"skills[%d].github-token must be a valid GitHub token expression. Example: skills[%d].github-token: \"${{ secrets.NAME }}\", \"${{ needs.auth.outputs.token }}\", or \"${{ steps.fetch_token.outputs.token }}\"",
						i,
						i,
					)
				}
