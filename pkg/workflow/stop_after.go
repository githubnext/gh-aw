...
func (c *Compiler) buildStopTimeCheckJob(data *WorkflowData, activationJobCreated bool) (*Job, error) {
	...
	steps = append(steps, "          printf 'Checking stop-time limit: %s\\n' \"$STOP_TIME\"\n")
	steps = append(steps, "          \n")
	...
	steps = append(steps, "            printf 'Warning: Invalid stop-time format: %s. Expected format: YYYY-MM-DD HH:MM:SS\\n' \"$STOP_TIME\"\n")
	steps = append(steps, "          else\n")
	steps = append(steps, "            CURRENT_EPOCH=$(date +%s)\n")
	steps = append(steps, "            echo \"Current time: $(date)\"\n")
	steps = append(steps, "            printf 'Stop time: %s\\n' \"$STOP_TIME\"\n")
	steps = append(steps, "            \n")
