...
func (c *Compiler) validateDispatchRepository(data *WorkflowData, workflowPath string) error {
	...
	for toolKey, tool := range config.Tools {
		...
		complete, err := validateDispatchRepositoryTool(toolKey, tool, collector)
		if err != nil {
			return err
		}
		if complete {
...
func validateDispatchRepositoryTool(toolKey string, tool *DispatchRepositoryToolConfig, collector *ErrorCollector) (bool, error) {
	if strings.TrimSpace(tool.Workflow) == "" {
		...
		return false, collector.Add(workflowErr)
	}
	if strings.TrimSpace(tool.EventType) == "" {
		...
		return false, collector.Add(eventTypeErr)
	}
	...
	if !hasRepository && !hasAllowedRepos {
		...
		return false, collector.Add(repoErr)
	}
	if err := validateDispatchRepositoryTarget(toolKey, tool.Repository, collector); err != nil {
		return true, err
	}
	return true, validateDispatchAllowedRepositories(toolKey, tool.AllowedRepositories, collector)
}

func validateDispatchRepositoryTarget(toolKey string, repo string, collector *ErrorCollector) error {
	if strings.TrimSpace(repo) == "" || hasExpressionMarker(repo) || repoSlugPattern.MatchString(repo) {
		return nil
	}
	repoFmtErr := fmt.Errorf("dispatch_repository: tool %q has invalid 'repository' format %q (expected 'owner/repo')", toolKey, repo)
	return collector.Add(repoFmtErr)
}

func validateDispatchAllowedRepositories(toolKey string, repos []string, collector *ErrorCollector) error {
	for _, repo := range repos {
		if hasExpressionMarker(repo) || strings.Contains(repo, "*") || repoSlugPattern.MatchString(repo) {
			continue
		}
