...
func TestFirewallArgsInCopilotEngine(t *testing.T) {
	...
	t.Run("AWF includes --add-host when MCP gateway is enabled", func(t *testing.T) {
		workflowData := &WorkflowData{
			Name: "test-workflow",
			EngineConfig: &EngineConfig{
				ID: "copilot",
		...
		// Check that --add-host is included when MCP gateway is enabled
		if !strings.Contains(stepContent, "--add-host host.docker.internal:host-gateway") {
			t.Error("Expected AWF command to contain '--add-host host.docker.internal:host-gateway' when MCP gateway is enabled")
		}
	})

	t.Run("AWF does not include --add-host when MCP gateway is not enabled", func(t *testing.T) {
		workflowData := &WorkflowData{
			Name: "test-workflow",
			EngineConfig: &EngineConfig{
				ID: "copilot",
			...
			// No SandboxConfig.MCP, so gateway is not enabled
		}
		...
		// Check that --add-host is not included when MCP gateway is not enabled
		if strings.Contains(stepContent, "--add-host host.docker.internal:host-gateway") {
			t.Error("AWF command should not contain '--add-host host.docker.internal:host-gateway' when MCP gateway is not enabled")
		}
	})
}
