...
// tmpSafePrefix is the only /tmp subtree that the agent should write to.
const tmpSafePrefix = "/tmp/gh-aw/agent"

// warnPromptTmpPaths returns a non-empty advisory message when content contains
// /tmp/ or /tmp/gh-aw/ references that are not under /tmp/gh-aw/agent/.
...
func warnPromptTmpPaths(content string) string {
	...
	// Walk every /tmp/ occurrence; warn as soon as one is not under /tmp/gh-aw/agent.
	rest := content
	for {
		...
		if !strings.HasPrefix(segment, tmpSafePrefix) {
			return "Prompt references /tmp/ or /tmp/gh-aw/ directly. " +
