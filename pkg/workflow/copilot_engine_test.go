...
func TestCopilotSessionFileCopyStep(t *testing.T) {
	...
	// Verify step name
	if !strings.Contains(stepContent, "Copy Copilot session files to logs") {
		t.Error("Expected step name to contain 'Copy Copilot session files to logs'")
	}
	...
	// Verify session-state directory reference
	...
	// Verify logs directory reference
