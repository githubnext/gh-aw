...
func TestCopilotEngineBYOKOmitsCopilotGitHubToken(t *testing.T) {
	...
	t.Run("AWF command omits --exclude-env COPILOT_GITHUB_TOKEN in copilot-sdk mode", func(t *testing.T) {
		workflowData := &WorkflowData{
			Name: "test-workflow",
			EngineConfig: &EngineConfig{
				CopilotSDK: true,
		...
		stepContent := strings.Join([]string(steps[0]), "\n")
		if strings.Contains(stepContent, "--exclude-env COPILOT_GITHUB_TOKEN") {
			t.Errorf("AWF command should not exclude COPILOT_GITHUB_TOKEN in copilot-sdk mode, got:\n%s", stepContent)
		}
	})
}
