...
func TestCopilotEngineExecutionSteps(t *testing.T) {
	...
	if !strings.Contains(stepContent, "copilot --add-dir /tmp/ --add-dir \"$GITHUB_WORKSPACE\" --log-level all --log-dir") {
		t.Errorf("Expected command to contain 'copilot --add-dir /tmp/ --add-dir \"$GITHUB_WORKSPACE\" --log-level all --log-dir' in step content:\n%s", stepContent)
	}
