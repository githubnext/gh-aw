...
func (c *Compiler) buildJobs(data *WorkflowData, markdownPath string) error {
	...
	c.resolveSafeOutputActionsForJobs(data, markdownPath)
	_, activationJobCreated, err := c.buildPreActivationAndActivationJobs(data, frontmatter, lockFilename)
	if err != nil {
		return err
	}
	if err := c.buildCoreWorkflowJobs(data, markdownPath, activationJobCreated); err != nil {
		return err
	}
	if err := c.finalizeWorkflowJobs(data); err != nil {
		return err
	}
...
func (c *Compiler) resolveSafeOutputActionsForJobs(data *WorkflowData, markdownPath string) {
	if data.SafeOutputs != nil && len(data.SafeOutputs.Actions) > 0 {
		c.resolveAllActions(data, markdownPath)
	}
}

func (c *Compiler) buildCoreWorkflowJobs(data *WorkflowData, markdownPath string, activationJobCreated bool) error {
	...
	if err := c.addEvalsJob(data); err != nil {
		return err
	}
	...
	return c.buildMemoryManagementJobs(data)
}

func (c *Compiler) addEvalsJob(data *WorkflowData) error {
...
func (c *Compiler) finalizeWorkflowJobs(data *WorkflowData) error {
	...
	return c.ensureConclusionIsLastJob()
}
