...
func (c *Compiler) hasSafeEventsOnly(data *WorkflowData, frontmatter map[string]any) bool {
	...
	if onValue, exists := frontmatter["on"]; exists {
		if onMap, ok := onValue.(map[string]any); ok {
			...
			// workflow_dispatch is always considered safe since the external user check
			// is skipped for manually triggered workflows (GitHub's native workflow_dispatch
			// permission model handles access control)
