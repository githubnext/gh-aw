...
// extractLabelNames extracts the 'label-names' field from frontmatter.
...
func (c *Compiler) extractLabelNames(frontmatter map[string]any) []string {
	if onValue, exists := frontmatter["on"]; exists {
		if onMap, ok := onValue.(map[string]any); ok {
			if labelNamesValue, hasLabelNames := onMap["label-names"]; hasLabelNames {
				names := parseOptionalStringSliceField(labelNamesValue, "on.label-names")
				if len(names) > 0 {
					roleLog.Printf("Extracted %d label-names: %v", len(names), names)
					return names
				}
			}
		}
	...
	if onValue, exists := frontmatter["on"]; exists {
		if onMap, ok := onValue.(map[string]any); ok {
			...
			for eventName := range onMap {
				...
				// Skip roles, bots, and label-names as they are configuration, not event types
				if eventName == "command" || eventName == "stop-after" || eventName == "reaction" || eventName == "roles" || eventName == "bots" || eventName == "label-names" {
					continue
				}
			...
			if _, hasLabelNames := onMap["label-names"]; hasLabelNames {
				eventCount--
			}
