...
---`,
			...
			expectedDockerImage: "ghcr.io/github/github-mcp-server:v0.19.1",
		},
	}
...
`
			...
			case "docker":
				...
				if !strings.Contains(lockContent, `"GITHUB_MCP_SERVER_TOKEN": "${{ secrets.GH_AW_GITHUB_TOKEN || secrets.GITHUB_TOKEN }}"`) {
					t.Errorf("Expected GITHUB_MCP_SERVER_TOKEN env var but didn't find it in:\n%s", lockContent)
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "docker":
				...
				if !strings.Contains(result, `"ghcr.io/github/github-mcp-server:v0.19.1"`) {
...
---`,
			...
			expectedDockerImage: "v0.19.1", // Default version
		},
		{
