...
describe("add_comment.cjs", () => {
  ...
  it("should include workflow source in footer when GH_AW_WORKFLOW_SOURCE is provided", async () => {
    ...
    expect(callArgs.body).toContain("Generated by [Test Workflow]");
    expect(callArgs.body).toContain("https://github.com/testowner/testrepo/actions/runs/12345");
  ...
  it("should use header level 4 for related items in comments", async () => {
    ...
    // Check that the related items are formatted with italic text and dots
    expect(callArgs.body).toContain("_Related:");
    // Check that it does NOT use header format
    expect(callArgs.body).not.toMatch(/####\s+Related Items/);
    expect(callArgs.body).not.toMatch(/^##\s+Related Items/m);
    expect(callArgs.body).not.toMatch(/\*\*Related Items:\*\*/);

    // Check that the references are included in new format
    expect(callArgs.body).toContain("[#456](https://github.com/testowner/testrepo/issues/456)");
    expect(callArgs.body).toContain("[Discussion #789](https://github.com/testowner/testrepo/discussions/789)");
    expect(callArgs.body).toContain("[PR #101](https://github.com/testowner/testrepo/pull/101)");
  ...
  it("should use header level 4 for related items in staged mode preview", async () => {
    ...
    // Check that the related items are formatted with italic text and dots
    expect(summaryContent).toContain("_Related:");
    // Check that it does NOT use header format
    expect(summaryContent).not.toMatch(/####\s+Related Items/);
    expect(summaryContent).not.toMatch(/^##\s+Related Items/m);
    expect(summaryContent).not.toMatch(/\*\*Related Items:\*\*/);

    // Check that the references are included with new format
    expect(summaryContent).toContain("[#456](https://github.com/testowner/testrepo/issues/456)");
    expect(summaryContent).toContain("[Discussion #789](https://github.com/testowner/testrepo/discussions/789)");
    expect(summaryContent).toContain("[PR #101](https://github.com/testowner/testrepo/pull/101)");
