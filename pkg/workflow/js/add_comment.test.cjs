...
describe("add_comment.cjs", () => {
  ...
  it("should create comment on discussion successfully", async () => {
    process.env.GITHUB_AW_AGENT_OUTPUT = JSON.stringify({
      items: [
        {
          type: "add-comment",
          body: "Test discussion comment content",
        },
      ],
    });
    ...
    // Mock the GraphQL response for getting discussion ID
    ...
    // Mock the GraphQL response for creating the comment
    mockGithub.graphql.mockResolvedValueOnce({
      addDiscussionComment: {
        comment: {
          id: "DC_kwDOABC5678",
          url: "https://github.com/testowner/testrepo/discussions/42#discussioncomment-5678",
          databaseId: 5678,
        },
      },
    });
    ...
    // Second call should be to create the comment
    expect(mockGithub.graphql.mock.calls[1][0]).toContain("mutation($discussionId: ID!, $body: String!)");
    expect(mockGithub.graphql.mock.calls[1][1].discussionId).toBe("D_kwDOABC1234");
    expect(mockGithub.graphql.mock.calls[1][1].body).toContain("Test discussion comment content");
  ...
  it("should skip discussion_comment when not in discussion context", async () => {
    ...
    global.context.eventName = "push"; // Not a discussion event
