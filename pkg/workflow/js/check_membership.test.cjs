...
describe("check_membership.cjs", () => {
  ...
  (beforeEach(() => {
    ((mockCore = { debug: vi.fn(), info: vi.fn(), warning: vi.fn(), error: vi.fn(), setFailed: vi.fn(), setOutput: vi.fn(), summary: { addRaw: vi.fn().mockReturnThis(), write: vi.fn().mockResolvedValue() } }),
      (mockGithub = { rest: { repos: { getCollaboratorPermissionLevel: vi.fn() } } }),
      (mockContext = { eventName: "issues", actor: "testuser", repo: { owner: "testorg", repo: "testrepo" } }),
      (global.core = mockCore),
      (global.github = mockGithub),
      (global.context = mockContext));
  }),
    afterEach(() => {
      (delete global.core, delete global.github, delete global.context, delete process.env.GH_AW_REQUIRED_ROLES);
    }));
  const runScript = async () => {
    const fs = await import("fs"),
      path = await import("path"),
      scriptPath = path.join(import.meta.dirname, "check_membership.cjs"),
      scriptContent = fs.readFileSync(scriptPath, "utf8"),
      utilsPath = path.join(import.meta.dirname, "check_permissions_utils.cjs"),
      utilsContent = fs.readFileSync(utilsPath, "utf8"),
      scriptWithoutMain = scriptContent.replace("await main();", ""),
      scriptFunction = new Function("core", "github", "context", "process", "require", scriptWithoutMain + "\nreturn main();");
    await scriptFunction(mockCore, mockGithub, mockContext, process, modulePath => {
      if ("./check_permissions_utils.cjs" === modulePath) {
        const moduleExports = {},
          mockModule = { exports: moduleExports };
        return (new Function("core", "github", "context", "process", "module", "exports", utilsContent)(mockCore, mockGithub, mockContext, process, mockModule, moduleExports), mockModule.exports);
      }
    ...
    describe("configuration validation", () => {
      (it("should fail when no required permissions are specified", async () => {
        ...
        it("should filter out empty permission values", async () => {
          ((process.env.GH_AW_REQUIRED_ROLES = "admin, , write, "), mockGithub.rest.repos.getCollaboratorPermissionLevel.mockResolvedValue({ data: { permission: "admin" } }), await runScript(), expect(mockCore.info).toHaveBeenCalled());
          const permissionsLog = mockCore.info.mock.calls.map(call => call[0]).find(log => log.includes("Required permissions:"));
