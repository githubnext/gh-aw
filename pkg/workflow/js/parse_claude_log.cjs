        ...
        if (trimmedLine.startsWith("[")) {
          ...
          try {
            ...
            if (Array.isArray(arr)) {
              logEntries = logEntries.concat(arr);
              continue;
            }
            ...
            const closeIdx = buf.lastIndexOf("]");
            if (openIdx !== -1 && closeIdx !== -1 && closeIdx > openIdx) {
              const sub = buf.substring(openIdx, closeIdx + 1);
              try {
                const arr2 = JSON.parse(sub);
                if (Array.isArray(arr2)) {
                  logEntries = logEntries.concat(arr2);
                  continue;
                }
              } catch (subParseError) {
                // Continue to individual JSON line parsing
              }
            }
          }
        }
