...
function sanitizeContent(content) {
  ...
  function sanitizeUrlProtocols(s) {
    // Match protocol patterns while avoiding false positives like "**word:**"
    // This covers URLs like https://example.com, javascript:alert(), mailto:user@domain.com, etc.
    // Uses negative lookbehind to prevent matching when preceded by **
    return s.replace(
      /(?<!\*\*)\b(?:(?:javascript|vbscript|data|file|mailto|tel|ftp|ssh):[^\s\])}'"<>&\x00-\x1f]+|\w+:\/\/[^\s\])}'"<>&\x00-\x1f]+)/gi,
      match => {
        // Extract protocol name to check if it's HTTPS
        const protocolMatch = match.match(/^(\w+):/);
        if (protocolMatch) {
          const protocol = protocolMatch[1].toLowerCase();
          // Allow https (case insensitive), redact everything else
          return protocol === "https" ? match : "(redacted)";
        }
        return "(redacted)";
      }
    );
