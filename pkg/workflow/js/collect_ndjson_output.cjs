      ...
      switch (itemType) {
        ...
        case "commit_status":
          if (!item.state || typeof item.state !== "string") {
            errors.push(`Line ${i + 1}: commit_status requires a 'state' field (string)`);
            continue;
          }
          ...
          if (!item.description || typeof item.description !== "string") {
            errors.push(`Line ${i + 1}: commit_status requires a 'description' field (string)`);
            continue;
          }
          item.state = sanitizeContent(item.state, 64);
          item.description = sanitizeContent(item.description, 140);
          if (item.context !== undefined) {
            if (typeof item.context !== "string") {
              errors.push(`Line ${i + 1}: commit_status 'context' must be a string`);
              continue;
            }
            item.context = sanitizeContent(item.context, 100);
          }
          break;
        default:
