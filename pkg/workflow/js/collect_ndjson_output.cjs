  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      switch (itemType) {
        ...
        case "remove_labels":
          if (!item.labels || !Array.isArray(item.labels)) {
            errors.push(`Line ${i + 1}: remove_labels requires a 'labels' array field`);
            continue;
          }
          if (item.labels.some(label => typeof label !== "string")) {
            errors.push(`Line ${i + 1}: remove_labels labels array must contain only strings`);
            continue;
          }
          const removeLabelsItemNumberValidation = validateIssueOrPRNumber(item.item_number, "remove_labels 'item_number'", i + 1);
          if (!removeLabelsItemNumberValidation.isValid) {
            if (removeLabelsItemNumberValidation.error) errors.push(removeLabelsItemNumberValidation.error);
            continue;
          }
          item.labels = item.labels.map(label => sanitizeContent(label, 128));
          break;
