...
describe("checkout_pr_branch.cjs", () => {
  let mockCore, mockExec, mockContext;
  (beforeEach(() => {
    ((mockCore = { info: vi.fn(), setFailed: vi.fn() }),
      (mockExec = { exec: vi.fn().mockResolvedValue(0) }),
      (mockContext = { eventName: "pull_request", payload: { pull_request: { number: 123, head: { ref: "feature-branch" } } } }),
    ...
    afterEach(() => {
      (delete global.core, delete global.exec, delete global.context, delete process.env.GITHUB_TOKEN, vi.clearAllMocks());
    }));
  const runScript = async () => {
    const { execFileSync } = await import("child_process"),
      fs = await import("fs"),
      scriptPath = (await import("path")).join(import.meta.dirname, "checkout_pr_branch.cjs"),
      scriptContent = fs.readFileSync(scriptPath, "utf8"),
      wrappedScript = new (0, Object.getPrototypeOf(async function () {}).constructor)("core", "exec", "context", scriptContent.replace(/main\(\)\.catch.*$/s, "await main();"));
    try {
  ...
  (describe("pull_request events", () => {
    (it("should checkout PR branch using git fetch and checkout", async () => {
      ...
      it("should handle git fetch errors", async () => {
        (mockExec.exec.mockRejectedValueOnce(new Error("git fetch failed")), await runScript(), expect(mockCore.setFailed).toHaveBeenCalledWith("Failed to checkout PR branch: git fetch failed"));
      }),
    ...
    describe("comment events on PRs", () => {
      (beforeEach(() => {
        ...
        it("should handle gh pr checkout errors", async () => {
          (mockExec.exec.mockRejectedValueOnce(new Error("gh pr checkout failed")), await runScript(), expect(mockCore.setFailed).toHaveBeenCalledWith("Failed to checkout PR branch: gh pr checkout failed"));
        }),
        it("should pass environment variables to gh command", async () => {
          ((process.env.CUSTOM_VAR = "custom-value"), await runScript(), expect(mockExec.exec).toHaveBeenCalledWith("gh", ["pr", "checkout", "123"]), delete process.env.CUSTOM_VAR);
        }));
    }),
    describe("no pull request context", () => {
      (it("should skip checkout when no pull request context", async () => {
        ...
        it("should skip checkout for push events", async () => {
          ((mockContext.eventName = "push"), (mockContext.payload = {}), await runScript(), expect(mockCore.info).toHaveBeenCalledWith("No pull request context available, skipping checkout"), expect(mockExec.exec).not.toHaveBeenCalled());
        }),
    ...
    describe("different event types", () => {
      (it("should handle pull_request_target event", async () => {
        ((mockContext.eventName = "pull_request_target"), await runScript(), expect(mockCore.info).toHaveBeenCalledWith("Event: pull_request_target"), expect(mockExec.exec).toHaveBeenCalledWith("gh", ["pr", "checkout", "123"]));
      }),
        it("should handle pull_request_review event", async () => {
          ((mockContext.eventName = "pull_request_review"), await runScript(), expect(mockCore.info).toHaveBeenCalledWith("Event: pull_request_review"), expect(mockExec.exec).toHaveBeenCalledWith("gh", ["pr", "checkout", "123"]));
        }),
        it("should handle pull_request_review_comment event", async () => {
          ((mockContext.eventName = "pull_request_review_comment"), await runScript(), expect(mockExec.exec).toHaveBeenCalledWith("gh", ["pr", "checkout", "123"]));
        }));
    }),
    describe("error handling", () => {
      (it("should handle non-Error exceptions", async () => {
        (mockExec.exec.mockRejectedValueOnce("string error"), await runScript(), expect(mockCore.setFailed).toHaveBeenCalledWith("Failed to checkout PR branch: string error"));
      }),
        it("should handle errors with custom messages", async () => {
          ...
          (mockExec.exec.mockRejectedValueOnce(customError), await runScript(), expect(mockCore.setFailed).toHaveBeenCalledWith("Failed to checkout PR branch: Permission denied: unable to access repository"));
        }));
    }),
    describe("branch name variations", () => {
      (it("should handle branches with slashes", async () => {
        ...
        it("should handle very long branch names", async () => {
          ...
          ((mockContext.payload.pull_request.head.ref = longBranchName), await runScript(), expect(mockExec.exec).toHaveBeenCalledWith("git", ["fetch", "origin", longBranchName]));
        }));
    }));
});
