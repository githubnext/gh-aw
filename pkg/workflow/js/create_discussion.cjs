...
async function main() {
  ...
  if (createdDiscussions.length > 0) {
    ...
    for (const discussion of createdDiscussions) {
      summaryContent += `- [${discussion.title} #${discussion.number}](${discussion.url})\n`;
    }
    await core.summary.addRaw(summaryContent).write();
  }
