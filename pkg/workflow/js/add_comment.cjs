async function main() {
  ...
  for (let i = 0; i < commentItems.length; i++) {
    ...
    // Add discussion reference if available
    const discussionNumber = process.env.GITHUB_AW_DISCUSSION_NUMBER;
    const discussionUrl = process.env.GITHUB_AW_DISCUSSION_URL;
    let footer = `> AI generated by [${workflowName}](${runUrl})`;
    if (discussionNumber && discussionUrl) {
      footer += ` | [Discussion #${discussionNumber}](${discussionUrl})`;
    }
