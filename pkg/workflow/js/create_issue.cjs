 ...
 * @param {object} [triggeringContext] - Context about what triggered the workflow (optional, can be null)
 * @param {string} triggeringContext.type - Type of trigger: 'issue', 'pull_request', or 'discussion'
 * @param {number} triggeringContext.number - Number of the triggering item
 * @param {string} [triggeringContext.url] - URL for discussions (optional)
 * @returns {string} Footer text
 */
function generateFooter(workflowName, runUrl, workflowSource, workflowSourceURL, triggeringContext) {
...
async function main() {
  ...
  if (isIssueContext && context.payload.issue) {
    ...
    triggeringContext = {
      ...
      url: `${repoUrl}/discussions/${context.payload.discussion.number}`
    };
  }
  ...
  for (let i = 0; i < createIssueItems.length; i++) {
    ...
    bodyLines.push(``, ``, generateFooter(workflowName, runUrl, workflowSource, workflowSourceURL, triggeringContext).trimEnd(), "");
