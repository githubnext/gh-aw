  ...
  describe("create_pr_review_comment.cjs", () => {
    ...
    (beforeEach(() => {
      ...
      it("should include workflow source in footer when GH_AW_WORKFLOW_SOURCE is provided", async () => {
        ...
        (mockGithub.rest.pulls.createReviewComment.mockResolvedValue({ data: mockComment }), await eval(`(async () => { ${createPRReviewCommentScript}; await main(); })()`), expect(mockGithub.rest.pulls.createReviewComment).toHaveBeenCalled());
