...
// Copilot suffix markers - content between these should be treated as ephemeral
// but content AFTER the end marker should be preserved
const COPILOT_SUFFIX_START_MARKER = "<!-- START COPILOT CODING AGENT TIPS -->";
const COPILOT_SUFFIX_END_MARKER = "<!-- END COPILOT CODING AGENT TIPS -->";

/**
 * Removes the copilot suffix section from a body and returns the preserved parts
 * The copilot suffix section is between START and END markers
 * Content before and after the section is preserved
 * @param {string} body - The original body content
 * @returns {{before: string, after: string}} The body parts without the copilot suffix section
 */
function extractPartsWithoutCopilotSuffix(body) {
  if (!body) return { before: "", after: "" };

  // Search for the entire start XML comment marker
  const startIndex = body.indexOf(COPILOT_SUFFIX_START_MARKER);
  if (startIndex === -1) {
    return { before: body, after: "" };
  }

  // Content before the copilot suffix section
  const before = body.substring(0, startIndex).trimEnd();

  // Search for the end marker after the start marker
  const endIndex = body.indexOf(COPILOT_SUFFIX_END_MARKER, startIndex);
  if (endIndex === -1) {
    // No end marker found, so everything after start is the copilot suffix
    return { before, after: "" };
  }

  // Content after the copilot suffix section (after the end marker)
  const afterEndMarker = body.substring(endIndex + COPILOT_SUFFIX_END_MARKER.length);
  const after = afterEndMarker.trimStart();
      ...
      if (typeof updateItem.body === "string") {
        ...
        if (operation === "append" || operation === "prepend") {
          try {
            ...
            // Extract parts before and after the copilot suffix section
            // Content before and after the copilot tips section is preserved
            const { before: preservedBefore, after: preservedAfter } = extractPartsWithoutCopilotSuffix(currentPR.body || "");
            ...
            if (operation === "prepend") {
              ...
              // preservedBefore + preservedAfter forms the user's content to keep
              const prependSection = `${updateItem.body}${aiFooter}\n\n---\n\n`;
              const userContent = preservedAfter ? `${preservedBefore}\n\n${preservedAfter}` : preservedBefore;
              updateData.body = prependSection + userContent;
              core.info("Operation: prepend (add to start with separator, copilot suffix removed, other content preserved)");
            } else {
              // Append: add horizontal line, content, and AI footer at the end
              const appendSection = `\n\n---\n\n${updateItem.body}${aiFooter}`;
              const userContent = preservedAfter ? `${preservedBefore}\n\n${preservedAfter}` : preservedBefore;
              updateData.body = userContent + appendSection;
              core.info("Operation: append (add to end with separator, copilot suffix removed, other content preserved)");
            }
          } catch (error) {
