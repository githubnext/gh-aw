      ...
      if (typeof updateItem.body === "string") {
        ...
        if (operation === "append" || operation === "prepend") {
          try {
            ...
            // Remove any existing copilot suffix from the current body
            const preservedBody = removeCopilotSuffix(currentPR.body || "");

            // Build the AI footer with copilot suffix marker
            const aiFooter = `\n\n> AI generated by [${workflowName}](${runUrl})`;

            if (operation === "prepend") {
              // Prepend: add content, AI footer, and horizontal line at the start
              const prependSection = `${updateItem.body}${aiFooter}\n\n---\n\n`;
              updateData.body = prependSection + preservedBody;
              core.info("Operation: prepend (add to start with separator, preserved content after copilot suffix removed)");
            } else {
              // Append: add horizontal line, content, and AI footer at the end
              const appendSection = `\n\n---\n\n${updateItem.body}${aiFooter}`;
              updateData.body = preservedBody + appendSection;
              core.info("Operation: append (add to end with separator, content after copilot suffix removed)");
            }
          } catch (error) {
