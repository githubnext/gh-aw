...
async function main() {
  ...
  if (isStaged) {
    ...
    summaryContent += "_Would create the following PR review comments:_\n\n";
    ...
    for (let i = 0; i < reviewCommentItems.length; i++) {
      ...
      if (item.pull_request_number) {
        ...
        summaryContent += `_Target: [PR #${item.pull_request_number}](${pullUrl})_\n\n`;
      } else {
        summaryContent += `_Target: Current PR_\n\n`;
      }
      summaryContent += `_File: ${item.path || "No path provided"}_\n\n`;
      summaryContent += `_Line: ${item.line || "No line provided"}_`;
      if (item.start_line) {
        summaryContent += ` _(start: ${item.start_line})_`;
      }
      summaryContent += ` · _Side: ${item.side || "RIGHT"}_\n\n`;
  ...
  if (createdComments.length > 0) {
    let summaryContent = "\n\n## Created PR Review Comments\n";
    for (const comment of createdComments) {
      summaryContent += `- [View Review Comment #${comment.id}](${comment.html_url})\n`;
    }
    await core.summary.addRaw(summaryContent).write();
  }
