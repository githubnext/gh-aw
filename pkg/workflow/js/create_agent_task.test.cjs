...
describe("create_agent_task.cjs", () => {
  ...
  describe("basic functionality", () => {
    ...
    it("should handle missing output file", async () => {
      process.env.GITHUB_AW_AGENT_OUTPUT = "/nonexistent/file.json";

      await runScript();

      expect(mockCore.setFailed).toHaveBeenCalled();
      expect(mockCore.setFailed.mock.calls[0][0]).toContain("Error reading agent output file");
    });
    ...
    it("should handle no create_agent_task items", async () => {
      ...
      expect(mockCore.info).toHaveBeenCalledWith("No create-agent-task items found in agent output");
    });
  });
  ...
  describe("staged mode", () => {
    beforeEach(() => {
      ...
      process.env.GITHUB_AW_AGENT_TASK_BASE = "main";
  ...
  describe("agent task creation", () => {
    beforeEach(() => {
      process.env.GITHUB_AW_AGENT_TASK_BASE = "develop";
      process.env.GITHUB_AW_SAFE_OUTPUTS_STAGED = "false";
    });
  ...
  describe("edge cases", () => {
    it("should handle output with no items array", async () => {
      fs.writeFileSync(testOutputFile, JSON.stringify({}));
      process.env.GITHUB_AW_AGENT_OUTPUT = testOutputFile;
