  ...
  describe("update_issue.cjs", () => {
    ...
    (beforeEach(() => {
      ...
      it('should handle explicit issue number with target "*"', async () => {
        (setAgentOutput({ items: [{ type: "update_issue", issue_number: 456, title: "Updated title" }] }), (process.env.GH_AW_UPDATE_TITLE = "true"), (process.env.GH_AW_UPDATE_TARGET = "*"), (global.context.eventName = "push")); // Not an issue event, but should work with explicit target
