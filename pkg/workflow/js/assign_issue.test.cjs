...
summary:{addRaw:vi.fn().mockReturnThis(),write:vi.fn().mockResolvedValue()}},mockExec={exec:vi.fn()},mockGithub={graphql:vi.fn()},mockContext={repo:{owner:"testowner",repo:"testrepo"}};
// Set up global variables
global.core=mockCore,global.exec=mockExec,global.github=mockGithub,global.context=mockContext,describe("assign_issue.cjs",()=>{let assignIssueScript;beforeEach(()=>{vi.clearAllMocks(),
// Reset environment variables
delete process.env.GH_TOKEN,delete process.env.ASSIGNEE,delete process.env.ISSUE_NUMBER;
// Read the script content
const scriptPath=path.join(process.cwd(),"assign_issue.cjs");assignIssueScript=fs.readFileSync(scriptPath,"utf8")}),describe("Environment variable validation",()=>{it("should fail when GH_TOKEN is not set",async()=>{process.env.ASSIGNEE="test-user",process.env.ISSUE_NUMBER="123",delete process.env.GH_TOKEN,
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),expect(mockCore.setFailed).toHaveBeenCalledWith(expect.stringContaining("GH_TOKEN environment variable is required but not set")),expect(mockCore.setFailed).toHaveBeenCalledWith(expect.stringContaining("https://githubnext.github.io/gh-aw/reference/safe-outputs/#assigning-issues-to-copilot")),expect(mockExec.exec).not.toHaveBeenCalled()}),it("should fail when GH_TOKEN is empty string",async()=>{process.env.GH_TOKEN="   ",process.env.ASSIGNEE="test-user",process.env.ISSUE_NUMBER="123",
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),expect(mockCore.setFailed).toHaveBeenCalledWith(expect.stringContaining("GH_TOKEN environment variable is required but not set")),expect(mockExec.exec).not.toHaveBeenCalled()}),it("should fail when ASSIGNEE is not set",async()=>{process.env.GH_TOKEN="ghp_test123",process.env.ISSUE_NUMBER="123",delete process.env.ASSIGNEE,
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),expect(mockCore.setFailed).toHaveBeenCalledWith("ASSIGNEE environment variable is required but not set"),expect(mockExec.exec).not.toHaveBeenCalled()}),it("should fail when ASSIGNEE is empty string",async()=>{process.env.GH_TOKEN="ghp_test123",process.env.ASSIGNEE="   ",process.env.ISSUE_NUMBER="123",
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),expect(mockCore.setFailed).toHaveBeenCalledWith("ASSIGNEE environment variable is required but not set"),expect(mockExec.exec).not.toHaveBeenCalled()}),it("should fail when ISSUE_NUMBER is not set",async()=>{process.env.GH_TOKEN="ghp_test123",process.env.ASSIGNEE="test-user",delete process.env.ISSUE_NUMBER,
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),expect(mockCore.setFailed).toHaveBeenCalledWith("ISSUE_NUMBER environment variable is required but not set"),expect(mockExec.exec).not.toHaveBeenCalled()}),it("should fail when ISSUE_NUMBER is empty string",async()=>{process.env.GH_TOKEN="ghp_test123",process.env.ASSIGNEE="test-user",process.env.ISSUE_NUMBER="   ",
// Execute the script
...
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),expect(mockCore.error).toHaveBeenCalledWith("Failed to assign issue: User not found"),expect(mockCore.setFailed).toHaveBeenCalledWith("Failed to assign issue #999 to test-user: User not found")}),it("should handle non-Error objects in catch block",async()=>{process.env.GH_TOKEN="ghp_test123",process.env.ASSIGNEE="test-user",process.env.ISSUE_NUMBER="999";const stringError="Command failed";mockExec.exec.mockRejectedValue(stringError),
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),expect(mockCore.error).toHaveBeenCalledWith("Failed to assign issue: Command failed"),expect(mockCore.setFailed).toHaveBeenCalledWith("Failed to assign issue #999 to test-user: Command failed")}),it("should handle top-level errors with catch handler",async()=>{process.env.GH_TOKEN="ghp_test123",process.env.ASSIGNEE="test-user",process.env.ISSUE_NUMBER="123";
// Mock exec to throw an error that isn't caught in main
const uncaughtError=new Error("Uncaught error");mockExec.exec.mockRejectedValue(uncaughtError),
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),
// The error should be caught in the main catch handler
expect(mockCore.setFailed).toHaveBeenCalled()})}),describe("Edge cases for regular users",()=>{it("should handle numeric issue number",async()=>{process.env.GH_TOKEN="ghp_test123",process.env.ASSIGNEE="test-user",process.env.ISSUE_NUMBER="123",mockExec.exec.mockResolvedValue(0),
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`),expect(mockExec.exec).toHaveBeenCalledWith("gh",["issue","edit","123","--add-assignee","test-user"],expect.any(Object))}),it("should pass through GH_TOKEN in exec environment",async()=>{process.env.GH_TOKEN="ghp_test123",process.env.ASSIGNEE="test-user",process.env.ISSUE_NUMBER="123",process.env.OTHER_VAR="other_value",mockExec.exec.mockResolvedValue(0),
// Execute the script
...
// Execute the script
await eval(`(async () => { ${assignIssueScript} })()`);const failedCall=mockCore.setFailed.mock.calls[0][0];expect(failedCall).toContain("https://githubnext.github.io/gh-aw/reference/safe-outputs/#assigning-issues-to-copilot")})})});