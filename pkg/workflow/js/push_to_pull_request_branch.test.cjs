      ...
      describe("Commit title suffix", () => {
        (beforeEach(() => {
          ...
          it("should append bracketed suffix (for [skip-ci] support)", async () => {
            (setAgentOutput({ items: [{ type: "push_to_pull_request_branch", content: "some changes to push" }] }), (process.env.GH_AW_COMMIT_TITLE_SUFFIX = " [skip-ci]"), mockFs.existsSync.mockReturnValue(!0));
          ...
          it("should append suffix without brackets as-is", async () => {
            (setAgentOutput({ items: [{ type: "push_to_pull_request_branch", content: "some changes to push" }] }), (process.env.GH_AW_COMMIT_TITLE_SUFFIX = " (automated)"), mockFs.existsSync.mockReturnValue(!0));
            (mockPatchContent("From abc123 Mon Sep 17 00:00:00 2001\nFrom: Test User <test@example.com>\nDate: Mon, 1 Jan 2024 00:00:00 +0000\nSubject: [PATCH] Add new feature\n\n---\n file.txt | 1 +\n 1 file changed, 1 insertion(+)\n"),
              // Execute the script
              await executeScript(),
              // Verify that writeFileSync was called with modified patch
          ...
          it("should append suffix with brackets", async () => {
            (setAgentOutput({ items: [{ type: "push_to_pull_request_branch", content: "some changes to push" }] }), (process.env.GH_AW_COMMIT_TITLE_SUFFIX = " [bot]"), mockFs.existsSync.mockReturnValue(!0));
            (mockPatchContent("From abc123 Mon Sep 17 00:00:00 2001\nFrom: Test User <test@example.com>\nDate: Mon, 1 Jan 2024 00:00:00 +0000\nSubject: [PATCH] Add new feature\n\n---\n file.txt | 1 +\n 1 file changed, 1 insertion(+)\n"),
              // Execute the script
              await executeScript(),
              // Verify that writeFileSync was called with modified patch
          ...
          it("should not modify patch when no commit title suffix is set", async () => {
            (setAgentOutput({ items: [{ type: "push_to_pull_request_branch", content: "some changes to push" }] }), delete process.env.GH_AW_COMMIT_TITLE_SUFFIX, mockFs.existsSync.mockReturnValue(!0));
            (mockPatchContent("From abc123 Mon Sep 17 00:00:00 2001\nFrom: Test User <test@example.com>\nDate: Mon, 1 Jan 2024 00:00:00 +0000\nSubject: [PATCH] Add new feature\n\n---\n file.txt | 1 +\n 1 file changed, 1 insertion(+)\n"),
              // Execute the script
              await executeScript(),
