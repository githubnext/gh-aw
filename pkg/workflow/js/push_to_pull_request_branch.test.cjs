...
`;
      ...
      // Verify that writeFileSync WAS called to add AI generated footer
      expect(mockFs.writeFileSync).toHaveBeenCalled();
      const writtenPatch = mockFs.writeFileSync.mock.calls[0][1];

      // Check that the patch has the AI generated footer but no title suffix
      expect(writtenPatch).toContain("AI generated by");
      expect(writtenPatch).not.toContain("Subject: [PATCH] Add new feature [");
      expect(writtenPatch).toContain("Subject: [PATCH] Add new feature");
    });
...
`;
  ...
  describe("AI generated footer", () => {
    beforeEach(() => {
      // Add writeFileSync to mockFs
      mockFs.writeFileSync = vi.fn();
    });

    it("should add AI generated footer to commit message", async () => {
      const validOutput = {
        items: [
          {
            type: "push_to_pull_request_branch",
...
This is the commit body with some details.
---
 file.txt | 1 +
 1 file changed, 1 insertion(+)
`;
      ...
      // Check that the patch has the AI generated footer
      expect(writtenPatch).toContain("AI generated by Workflow:");
      expect(writtenPatch).toMatch(/AI generated by Workflow:.*\/actions\/runs\/undefined/);

      // Check that the footer is before the --- separator
      const footerIndex = writtenPatch.indexOf("AI generated by");
      const separatorIndex = writtenPatch.indexOf("\n---\n");
      expect(footerIndex).toBeGreaterThan(0);
      expect(footerIndex).toBeLessThan(separatorIndex);
    });

    it("should add AI generated footer with workflow name and run URL", async () => {
      const validOutput = {
        items: [
          {
            type: "push_to_pull_request_branch",
...
`;
      ...
      // Check that the patch has the AI generated footer with correct workflow name
      expect(writtenPatch).toContain("AI generated by Test Workflow:");
      expect(mockCore.info).toHaveBeenCalledWith(expect.stringContaining("AI generated footer:"));
    });

    it("should work with both AI footer and commit title suffix", async () => {
      const validOutput = {
        items: [
          {
            type: "push_to_pull_request_branch",
      ...
      setAgentOutput(validOutput);
      process.env.GH_AW_COMMIT_TITLE_SUFFIX = " [bot]";
      process.env.GH_AW_WORKFLOW_NAME = "Bot Workflow";

      mockFs.existsSync.mockReturnValue(true);
      const originalPatch = `From abc123 Mon Sep 17 00:00:00 2001
From: Test User <test@example.com>
Date: Mon, 1 Jan 2024 00:00:00 +0000
Subject: [PATCH] Auto-generated changes
...
`;
      ...
      // Check that both the title suffix and AI footer are present
      expect(writtenPatch).toContain("Subject: [PATCH] Auto-generated changes [bot]");
      expect(writtenPatch).toContain("AI generated by Bot Workflow:");
    });

    it("should handle commit messages containing --- in the body", async () => {
      const validOutput = {
        items: [
          {
            type: "push_to_pull_request_branch",
...
This commit adds documentation with markdown separators.

Here's a markdown separator:
---
...
`;
      ...
      // Check that the AI footer is inserted before the diffstat separator, not the --- in the commit message
      expect(writtenPatch).toContain("AI generated by Test Workflow:");

      // The footer should appear right before the diffstat line
      expect(writtenPatch).toMatch(/AI generated by Test Workflow:.*\n---\n README\.md \| 10 \+{10}/);

      // The --- in the commit message should remain unchanged
      expect(writtenPatch).toContain("Here's a markdown separator:\n---\n\nMore content after the separator.");
    });
  });
