...
summary:{addRaw:vi.fn().mockReturnThis(),write:vi.fn().mockResolvedValue()}},mockContext={eventName:"pull_request",payload:{pull_request:{number:123},repository:{html_url:"https://github.com/testowner/testrepo"}},repo:{owner:"testowner",repo:"testrepo"}},mockGithub={graphql:vi.fn(),request:vi.fn()};
// Set up global variables
global.core=mockCore,global.context=mockContext,global.github=mockGithub,describe("push_to_pull_request_branch.cjs",()=>{let pushToPrBranchScript,mockFs,mockExec,tempFilePath;
// Helper function to set agent output via file
const setAgentOutput=data=>{tempFilePath=path.join("/tmp",`test_agent_output_${Date.now()}_${Math.random().toString(36).slice(2)}.json`);const content="string"==typeof data?data:JSON.stringify(data);fs.writeFileSync(tempFilePath,content),process.env.GH_AW_AGENT_OUTPUT=tempFilePath},mockPatchContent=patchContent=>{mockFs.readFileSync.mockImplementation((filepath,encoding)=>{
// If reading the agent output file, always read the actual temp file
const agentOutputPath=process.env.GH_AW_AGENT_OUTPUT;return agentOutputPath&&filepath===agentOutputPath?fs.readFileSync(filepath,encoding||"utf8"):patchContent;
// For patch files, return the mocked content
})},executeScript=async()=>(
// Set globals just before execution
// Set globals just before execution
global.core=mockCore,global.context=mockContext,global.github=mockGithub,global.mockFs=mockFs,global.exec=mockExec,await eval(`(async () => { ${pushToPrBranchScript} })()`));
// Helper function to mock patch file content while preserving agent output file reading
beforeEach(()=>{vi.clearAllMocks(),
// Clear environment variables
delete process.env.GH_AW_PUSH_TARGET,delete process.env.GH_AW_AGENT_OUTPUT,delete process.env.GH_AW_PUSH_IF_NO_CHANGES,delete process.env.GH_AW_PR_TITLE_PREFIX,delete process.env.GH_AW_PR_LABELS,
// Create fresh mock objects for each test
mockFs={existsSync:vi.fn(),readFileSync:vi.fn().mockImplementation((filepath,encoding)=>{
// If reading the agent output file, always read the actual temp file
const agentOutputPath=process.env.GH_AW_AGENT_OUTPUT;return agentOutputPath&&filepath===agentOutputPath?fs.readFileSync(filepath,encoding||"utf8"):"diff --git a/file.txt b/file.txt\n+new content";
...
mockExec={exec:vi.fn().mockResolvedValue(0),// For commands that don't read output
getExecOutput:vi.fn().mockImplementation((command,args)=>{
// Handle the gh pr view command specifically
if("gh"===command&&args&&"pr"===args[0]&&"view"===args[1]&&args.includes("--json")&&args.includes("headRefName,title,labels")){const prData=JSON.stringify({headRefName:"feature-branch",title:"Test PR Title",labels:["bug","enhancement"]});return Promise.resolve({exitCode:0,stdout:prData+"\n",stderr:""})}
...
mockCore.setFailed.mockReset(),mockCore.setOutput.mockReset(),mockCore.warning.mockReset(),mockCore.error.mockReset();
// Read the script content
const scriptPath=path.join(process.cwd(),"push_to_pull_request_branch.cjs");pushToPrBranchScript=fs.readFileSync(scriptPath,"utf8"),
// Modify the script to inject our mocks and make core available
pushToPrBranchScript=pushToPrBranchScript.replace(/\/\*\* @type \{typeof import\("fs"\)\} \*\/\nconst fs = require\("fs"\);/,"const core = global.core;\nconst context = global.context || {};\nconst fs = global.mockFs;\nconst exec = global.exec;")}),afterEach(()=>{
...
"undefined"!=typeof global&&(delete global.core,delete global.context,delete global.mockFs,delete global.exec)}),describe("Script execution",()=>{it("should skip when no agent output is provided",async()=>{
// Remove the output content environment variable
// Remove the output content environment variable
delete process.env.GH_AW_AGENT_OUTPUT,
// Execute the script
await executeScript(),expect(mockCore.info).toHaveBeenCalledWith("Agent output content is empty"),expect(mockCore.setFailed).not.toHaveBeenCalled()}),it("should skip when agent output is empty",async()=>{setAgentOutput(""),
// Execute the script
await executeScript(),expect(mockCore.info).toHaveBeenCalledWith("Agent output content is empty"),expect(mockCore.setFailed).not.toHaveBeenCalled()}),it("should handle missing patch file with default 'warn' behavior",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"test"}]}),mockFs.existsSync.mockReturnValue(!1),
// Execute the script
await executeScript(),expect(mockCore.info).toHaveBeenCalledWith("No patch file found - cannot push without changes"),expect(mockCore.setFailed).not.toHaveBeenCalled()}),it("should fail when patch file missing and if-no-changes is 'error'",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"test"}]}),process.env.GH_AW_PUSH_IF_NO_CHANGES="error",mockFs.existsSync.mockReturnValue(!1),
// Execute the script
await executeScript(),expect(mockCore.setFailed).toHaveBeenCalledWith("No patch file found - cannot push without changes")}),it("should silently succeed when patch file missing and if-no-changes is 'ignore'",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"test"}]}),process.env.GH_AW_PUSH_IF_NO_CHANGES="ignore",mockFs.existsSync.mockReturnValue(!1),
// Execute the script
await executeScript(),expect(mockCore.info).not.toHaveBeenCalled(),expect(mockCore.setFailed).not.toHaveBeenCalled()}),it("should fail when patch file contains error content",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"test"}]}),mockFs.existsSync.mockReturnValue(!0),mockPatchContent("Failed to generate patch: some error"),
// Execute the script
await executeScript(),
...
expect(mockCore.error).toHaveBeenCalledWith("Patch file generation failed - this is an error condition that requires investigation"),expect(mockCore.error).toHaveBeenCalledWith("Patch file location: /tmp/gh-aw/aw.patch"),expect(mockCore.error).toHaveBeenCalledWith(expect.stringMatching(/Patch file size: \d+ bytes/)),expect(mockCore.error).toHaveBeenCalledWith(expect.stringMatching(/Patch file preview \(first \d+ characters\):/)),expect(mockCore.error).toHaveBeenCalledWith(expect.stringContaining("Failed to generate patch: some error"))}),it("should fail when patch file contains error content regardless of if-no-changes config",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"test"}]}),
// Set if-no-changes to 'ignore' to verify it still fails
process.env.GH_AW_PUSH_IF_NO_CHANGES="ignore",mockFs.existsSync.mockReturnValue(!0),mockPatchContent("Failed to generate patch: git diff failed"),
// Execute the script
await executeScript(),
// Should always fail regardless of if-no-changes configuration
expect(mockCore.setFailed).toHaveBeenCalledWith("Patch file contains error message - cannot push without changes"),
// Should log diagnostic information
expect(mockCore.error).toHaveBeenCalledWith("Patch file generation failed - this is an error condition that requires investigation")}),it("should handle empty patch file with default 'warn' behavior",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"test"}]}),mockFs.existsSync.mockReturnValue(!0),mockPatchContent(""),
// Mock the git command to return a branch name
// Execute the script
await executeScript(),expect(mockCore.info).toHaveBeenCalledWith("Patch file is empty - no changes to apply (noop operation)"),expect(mockCore.info).toHaveBeenCalledWith(expect.stringMatching(/Agent output content length: \d+/)),expect(mockCore.setFailed).not.toHaveBeenCalled()}),it("should fail when empty patch and if-no-changes is 'error'",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"test"}]}),process.env.GH_AW_PUSH_IF_NO_CHANGES="error",mockFs.existsSync.mockReturnValue(!0),mockPatchContent("   "),
// Execute the script
...
const invalidJsonPath=path.join("/tmp",`test_invalid_${Date.now()}.json`);fs.writeFileSync(invalidJsonPath,"invalid json content"),process.env.GH_AW_AGENT_OUTPUT=invalidJsonPath,mockFs.existsSync.mockReturnValue(!0),mockPatchContent("some patch content"),
// Execute the script
await executeScript(),expect(mockCore.setFailed).toHaveBeenCalledWith(expect.stringMatching(/Error parsing agent output JSON:/)),
// Clean up
fs.existsSync(invalidJsonPath)&&fs.unlinkSync(invalidJsonPath)}),it("should handle agent output without valid items array",async()=>{setAgentOutput({items:"not an array"}),mockFs.existsSync.mockReturnValue(!0),mockPatchContent("some patch content"),
// Execute the script
await executeScript(),expect(mockCore.info).toHaveBeenCalledWith("No valid items found in agent output"),expect(mockCore.setFailed).not.toHaveBeenCalled()}),it("should use custom target configuration",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"test"}]}),process.env.GH_AW_PUSH_TARGET="custom-target",mockFs.existsSync.mockReturnValue(!0),mockPatchContent("some patch content"),
// Mock the git commands
// Execute the script
await executeScript(),expect(mockCore.info).toHaveBeenCalledWith("Target configuration: custom-target")})}),describe("Script validation",()=>{it("should have valid JavaScript syntax",()=>{const scriptPath=path.join(__dirname,"push_to_pull_request_branch.cjs"),scriptContent=fs.readFileSync(scriptPath,"utf8");
// Basic syntax validation - should not contain obvious errors
expect(scriptContent).toContain("async function main()"),expect(scriptContent).toContain("core.setFailed"),expect(scriptContent).toContain("/tmp/gh-aw/aw.patch"),expect(scriptContent).toContain("await main()")}),it("should export a main function",()=>{const scriptPath=path.join(__dirname,"push_to_pull_request_branch.cjs"),scriptContent=fs.readFileSync(scriptPath,"utf8");
// Check that the script has the expected structure
expect(scriptContent).toMatch(/async function main\(\) \{[\s\S]*\}/)}),it("should validate patch size within limit",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"some changes to push"}]}),process.env.GH_AW_MAX_PATCH_SIZE="10",// 10 KB limit
mockFs.existsSync.mockReturnValue(!0);
// Create patch content under 10 KB (approximately 5 KB)
const patchContent="diff --git a/file.txt b/file.txt\n+new content\n".repeat(100);mockPatchContent(patchContent),
// Mock the git commands that will be called
// Execute the script
...
mockFs.existsSync.mockReturnValue(!0),mockPatchContent("diff --git a/file.txt b/file.txt\n+new content\n"),
// Mock the git commands that will be called
// Execute the script
...
mockFs.existsSync.mockReturnValue(!0),mockPatchContent(""),// Empty patch
// Mock the git commands that will be called
// Execute the script
await executeScript(),
...
mockExec.getExecOutput.mockImplementation((command,args)=>{if("gh"===command&&args&&"pr"===args[0]&&"view"===args[1]){const prData={headRefName:"feature-branch",title:"[bot] Add new feature",labels:[]};return Promise.resolve({exitCode:0,stdout:JSON.stringify(prData),stderr:""})}return"git"===command&&args&&"rev-parse"===args[0]&&"HEAD"===args[1]?Promise.resolve({exitCode:0,stdout:"abc123def456\n",stderr:""}):Promise.resolve({exitCode:0,stdout:"",stderr:""})}),
// Execute the script
...
mockExec.getExecOutput.mockImplementation((command,args)=>{if("gh"===command&&args&&"pr"===args[0]&&"view"===args[1]){const prData={headRefName:"feature-branch",title:"Add new feature",labels:[]};return Promise.resolve({exitCode:0,stdout:JSON.stringify(prData),stderr:""})}return"git"===command&&args&&"rev-parse"===args[0]&&"HEAD"===args[1]?Promise.resolve({exitCode:0,stdout:"abc123def456\n",stderr:""}):Promise.resolve({exitCode:0,stdout:"",stderr:""})}),
// Execute the script
...
mockExec.getExecOutput.mockImplementation((command,args)=>{if("gh"===command&&args&&"pr"===args[0]&&"view"===args[1]){const prData={headRefName:"feature-branch",title:"Add new feature",labels:["automation","enhancement","feature"]};return Promise.resolve({exitCode:0,stdout:JSON.stringify(prData),stderr:""})}return"git"===command&&args&&"rev-parse"===args[0]&&"HEAD"===args[1]?Promise.resolve({exitCode:0,stdout:"abc123def456\n",stderr:""}):Promise.resolve({exitCode:0,stdout:"",stderr:""})}),
// Execute the script
...
mockExec.getExecOutput.mockImplementation((command,args)=>{if("gh"===command&&args&&"pr"===args[0]&&"view"===args[1]){const prData={headRefName:"feature-branch",title:"Add new feature",labels:["feature"]};return Promise.resolve({exitCode:0,stdout:JSON.stringify(prData),stderr:""})}return"git"===command&&args&&"rev-parse"===args[0]&&"HEAD"===args[1]?Promise.resolve({exitCode:0,stdout:"abc123def456\n",stderr:""}):Promise.resolve({exitCode:0,stdout:"",stderr:""})}),
// Execute the script
...
mockExec.getExecOutput.mockImplementation((command,args)=>{if("gh"===command&&args&&"pr"===args[0]&&"view"===args[1]){const prData={headRefName:"feature-branch",title:"[automated] Add new feature",labels:["bot","feature","enhancement"]};return Promise.resolve({exitCode:0,stdout:JSON.stringify(prData),stderr:""})}return"git"===command&&args&&"rev-parse"===args[0]&&"HEAD"===args[1]?Promise.resolve({exitCode:0,stdout:"abc123def456\n",stderr:""}):Promise.resolve({exitCode:0,stdout:"",stderr:""})}),
// Execute the script
await executeScript(),expect(mockCore.info).toHaveBeenCalledWith('✓ Title prefix validation passed: "[automated] "'),expect(mockCore.info).toHaveBeenCalledWith("✓ Labels validation passed: bot,feature"),expect(mockCore.setFailed).not.toHaveBeenCalled()})}),describe("Commit title suffix",()=>{beforeEach(()=>{
// Add writeFileSync to mockFs
mockFs.writeFileSync=vi.fn()}),it("should append bracketed suffix (for [skip-ci] support)",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"some changes to push"}]}),process.env.GH_AW_COMMIT_TITLE_SUFFIX=" [skip-ci]",mockFs.existsSync.mockReturnValue(!0),mockPatchContent("From abc123 Mon Sep 17 00:00:00 2001\nFrom: Test User <test@example.com>\nDate: Mon, 1 Jan 2024 00:00:00 +0000\nSubject: [PATCH] Add new feature\n\n---\n file.txt | 1 +\n 1 file changed, 1 insertion(+)\n"),
// Execute the script
await executeScript(),
// Verify that writeFileSync was called with modified patch
expect(mockFs.writeFileSync).toHaveBeenCalled();const writtenPatch=mockFs.writeFileSync.mock.calls[0][1];
// Check that the patch appends [skip-ci] at the end
expect(writtenPatch).toContain("Subject: [PATCH] Add new feature [skip-ci]"),expect(mockCore.info).toHaveBeenCalledWith(expect.stringContaining('commit title suffix: " [skip-ci]"'))}),it("should append suffix without brackets as-is",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"some changes to push"}]}),process.env.GH_AW_COMMIT_TITLE_SUFFIX=" (automated)",mockFs.existsSync.mockReturnValue(!0),mockPatchContent("From abc123 Mon Sep 17 00:00:00 2001\nFrom: Test User <test@example.com>\nDate: Mon, 1 Jan 2024 00:00:00 +0000\nSubject: [PATCH] Add new feature\n\n---\n file.txt | 1 +\n 1 file changed, 1 insertion(+)\n"),
// Execute the script
await executeScript(),
// Verify that writeFileSync was called with modified patch
expect(mockFs.writeFileSync).toHaveBeenCalled();const writtenPatch=mockFs.writeFileSync.mock.calls[0][1];
...
expect(mockExec.exec).toHaveBeenCalledWith("git am /tmp/gh-aw/aw.patch")})}),describe("Patch failure investigation",()=>{beforeEach(()=>{
// Add writeFileSync to mockFs
mockFs.writeFileSync=vi.fn()}),it("should investigate patch failure by logging git status and failed patch",async()=>{setAgentOutput({items:[{type:"push_to_pull_request_branch",content:"some changes to push"}]}),mockFs.existsSync.mockReturnValue(!0),mockPatchContent("diff --git a/file.txt b/file.txt\n+new content");
// Mock git am to fail
let gitAmCalled=!1;mockExec.exec.mockImplementation(async(cmd,args)=>{if("string"==typeof cmd&&cmd.includes("git am"))throw gitAmCalled=!0,new Error("Patch does not apply");return 0}),
// Mock getExecOutput for investigation commands
mockExec.getExecOutput.mockImplementation(async(command,args)=>{if("gh"===command&&args&&"pr"===args[0]&&"view"===args[1]){const prData={headRefName:"feature-branch",title:"Test PR Title",labels:["bug","enhancement"]};return Promise.resolve({exitCode:0,stdout:JSON.stringify(prData)+"\n",stderr:""})}
// Handle git status investigation
return"git"===command&&args&&"status"===args[0]?Promise.resolve({exitCode:0,stdout:"On branch feature-branch\nYour branch is up to date\n",stderr:""}):// Handle git log investigation
"git"===command&&args&&"log"===args[0]&&"--oneline"===args[1]&&"-5"===args[2]?Promise.resolve({exitCode:0,stdout:"abc123 Latest commit\ndef456 Previous commit\n",stderr:""}):// Handle git diff HEAD investigation
"git"===command&&args&&"diff"===args[0]&&"HEAD"===args[1]?Promise.resolve({exitCode:0,stdout:"diff --git a/modified.txt b/modified.txt\n+modified content\n",stderr:""}):// Handle git am --show-current-patch=diff investigation
"git"===command&&args&&"am"===args[0]&&"--show-current-patch=diff"===args[1]?Promise.resolve({exitCode:0,stdout:"diff --git a/conflicting.txt b/conflicting.txt\n+conflicting line\n",stderr:""}):// Handle git am --show-current-patch investigation
"git"===command&&args&&"am"===args[0]&&"--show-current-patch"===args[1]?Promise.resolve({exitCode:0,stdout:"From abc123 Mon Sep 17 00:00:00 2001\nSubject: [PATCH] Add feature\n",stderr:""}):Promise.resolve({exitCode:0,stdout:"",stderr:""})}),
// Execute the script
await executeScript(),
// Verify git am was called and failed
expect(gitAmCalled).toBe(!0),
// Verify investigation commands were called
expect(mockExec.getExecOutput).toHaveBeenCalledWith("git",["status"]),expect(mockExec.getExecOutput).toHaveBeenCalledWith("git",["log","--oneline","-5"]),expect(mockExec.getExecOutput).toHaveBeenCalledWith("git",["diff","HEAD"]),expect(mockExec.getExecOutput).toHaveBeenCalledWith("git",["am","--show-current-patch=diff"]),expect(mockExec.getExecOutput).toHaveBeenCalledWith("git",["am","--show-current-patch"]),
// Verify investigation logs
expect(mockCore.info).toHaveBeenCalledWith("Investigating patch failure..."),expect(mockCore.info).toHaveBeenCalledWith("Git status output:"),expect(mockCore.info).toHaveBeenCalledWith("On branch feature-branch\nYour branch is up to date\n"),expect(mockCore.info).toHaveBeenCalledWith("Recent commits (last 5):"),expect(mockCore.info).toHaveBeenCalledWith("abc123 Latest commit\ndef456 Previous commit\n"),expect(mockCore.info).toHaveBeenCalledWith("Uncommitted changes:"),expect(mockCore.info).toHaveBeenCalledWith("diff --git a/modified.txt b/modified.txt\n+modified content\n"),expect(mockCore.info).toHaveBeenCalledWith("Failed patch diff:"),expect(mockCore.info).toHaveBeenCalledWith("diff --git a/conflicting.txt b/conflicting.txt\n+conflicting line\n"),expect(mockCore.info).toHaveBeenCalledWith("Failed patch (full):"),expect(mockCore.info).toHaveBeenCalledWith("From abc123 Mon Sep 17 00:00:00 2001\nSubject: [PATCH] Add feature\n"),
...
// Mock git am to fail
mockExec.exec.mockImplementation(async(cmd,args)=>{if("string"==typeof cmd&&cmd.includes("git am"))throw new Error("Patch does not apply");return 0}),
// Mock investigation commands to also fail
mockExec.getExecOutput.mockImplementation(async(command,args)=>{if("gh"===command&&args&&"pr"===args[0]&&"view"===args[1]){const prData={headRefName:"feature-branch",title:"Test PR Title",labels:[]};return Promise.resolve({exitCode:0,stdout:JSON.stringify(prData)+"\n",stderr:""})}
// Make git status and git am --show-current-patch fail
if("git"===command)throw new Error("Git command failed");return Promise.resolve({exitCode:0,stdout:"",stderr:""})}),
// Execute the script
await executeScript(),
