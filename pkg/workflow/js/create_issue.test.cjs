...
describe("create_issue.cjs", () => {
  ...
  it("should assign issue to bot when assign-to-bot is configured", async () => {
    process.env.GITHUB_AW_AGENT_OUTPUT = JSON.stringify({
      items: [
        {
          type: "create_issue",
          title: "Issue to be assigned",
          body: "This issue should be assigned to a bot",
        },
      ],
    });
    ...
    // Mock GraphQL calls for assignment using counter
    let graphqlCallCount = 0;
    mockGithub.graphql.mockImplementation(() => {
      graphqlCallCount++;
      if (graphqlCallCount === 1) {
        // Get issue node ID
        return Promise.resolve({ repository: { issue: { id: "issue-node-id-123" } } });
      }
      if (graphqlCallCount === 2) {
        // Get bot user node ID
        return Promise.resolve({ user: { id: "bot-node-id-copilot" } });
      }
      if (graphqlCallCount === 3) {
        // Assign mutation
        return Promise.resolve({ addAssigneesToAssignable: { assignable: { id: "issue-node-id-123", number: 123 } } });
      }
      return Promise.reject(new Error("Unexpected graphql call"));
    });
  ...
  it("should handle assignment failure gracefully and continue", async () => {
    process.env.GITHUB_AW_AGENT_OUTPUT = JSON.stringify({
      items: [
        {
          type: "create_issue",
          title: "Issue with failed assignment",
          body: "Assignment should fail but issue should still be created",
        },
      ],
    });
    process.env.GITHUB_AW_ISSUE_ASSIGN_TO_BOT = "nonexistent-bot";
    ...
    // Mock GraphQL to fail on getting bot user using counter
    let graphqlCallCount = 0;
    mockGithub.graphql.mockImplementation(() => {
      graphqlCallCount++;
      if (graphqlCallCount === 1) {
        // Get issue node ID succeeds
        return Promise.resolve({ repository: { issue: { id: "issue-node-id-456" } } });
      }
      // Bot user lookup fails on second call
    ...
    try {
      ...
      // Script may throw due to test environment limitations
      console.log("Script execution error (expected in test):", error);
    }
  ...
  it("should not attempt assignment when assign-to-bot is not configured", async () => {
    process.env.GITHUB_AW_AGENT_OUTPUT = JSON.stringify({
      items: [
        {
          type: "create_issue",
          title: "Issue without assignment",
          body: "No assignment configured",
        },
      ],
    });
