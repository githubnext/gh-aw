  ...
  describe("create_issue.cjs", () => {
    ...
    (beforeEach(() => {
      ...
      it("should skip when agent output is empty", async () => {
        (setAgentOutput(""), await eval(`(async () => { ${createIssueScript} })()`), expect(mockCore.info).toHaveBeenCalledWith("Agent output content is empty"), expect(mockGithub.rest.issues.create).not.toHaveBeenCalled());
      }),
      ...
      it("should extract title from markdown heading", async () => {
        ...
        (mockGithub.rest.issues.create.mockResolvedValue({ data: mockIssue }), await eval(`(async () => { ${createIssueScript} })()`));
      ...
      it("should include workflow source in footer when GH_AW_WORKFLOW_SOURCE is provided", async () => {
        ...
        const mockIssue = { number: 101, html_url: "https://github.com/testowner/testrepo/issues/101" };
        (mockGithub.rest.issues.create.mockResolvedValue({ data: mockIssue }), await eval(`(async () => { ${createIssueScript} })()`), expect(mockGithub.rest.issues.create).toHaveBeenCalled());
        const callArgs = mockGithub.rest.issues.create.mock.calls[0][0];
      ...
      it("should replace #aw_ID references in issue body with real issue numbers", async () => {
        ...
        (mockGithub.rest.issues.create.mockResolvedValueOnce({ data: parentIssue }).mockResolvedValueOnce({ data: subIssue }), await eval(`(async () => { ${createIssueScript} })()`));
