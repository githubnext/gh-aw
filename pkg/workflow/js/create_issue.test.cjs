...
describe("create_issue.cjs", () => {
  ...
  it("should include triggering issue reference in footer when triggered by issue", async () => {
      ...
      items: [
        {
          type: "create_issue",
          title: "Related Issue",
          body: "This issue is related to the triggering issue",
        },
      ],
    });
    ...
    // Check that the footer includes the triggering issue reference
    expect(callArgs.body).toContain("AI generated by");
    expect(callArgs.body).toContain("for #42");
  });

  it("should include triggering PR reference in footer when triggered by pull request", async () => {
    // Set up PR context
    ...
    process.env.GITHUB_AW_AGENT_OUTPUT = JSON.stringify({
      items: [
        {
          type: "create_issue",
          title: "PR Related Issue",
          body: "This issue is related to the triggering PR",
        },
      ],
    });
    ...
    // Check that the footer includes the triggering PR reference
    expect(callArgs.body).toContain("AI generated by");
    expect(callArgs.body).toContain("for #123");
  });

  it("should not include triggering reference when not triggered by issue/PR/discussion", async () => {
    // Set up non-triggering context (e.g., push event)
      ...
      items: [
        {
          ...
          body: "This issue is not triggered by an issue/PR",
        },
      ],
    });
    ...
    // Check that the footer does NOT include triggering reference
    expect(callArgs.body).toContain("AI generated by");
