  ...
  describe("create_issue.cjs", () => {
    ...
    (beforeEach(() => {
      ...
      it("should not include workflow source footer when GH_AW_WORKFLOW_SOURCE is not provided", async () => {
        (setAgentOutput({ items: [{ type: "create_issue", title: "Test Issue", body: "Test issue without source" }] }), (process.env.GH_AW_WORKFLOW_NAME = "Test Workflow"), delete process.env.GH_AW_WORKFLOW_SOURCE); // Ensure it's not set
      ...
      it("should include triggering issue number in footer when in issue context", async () => {
        (setAgentOutput({ items: [{ type: "create_issue", title: "New issue from issue context", body: "Created from issue #42" }] }),
          (process.env.GH_AW_WORKFLOW_NAME = "Test Workflow"),
