...
if (!toolsEnv) {
  throw new Error("GITHUB_AW_SAFE_OUTPUTS_TOOLS environment variable is required");
}
...
try {
  ...
  throw new Error(`Failed to parse GITHUB_AW_SAFE_OUTPUTS_TOOLS: ${error instanceof Error ? error.message : String(error)}`);
}
...
// Use pre-filtered tools from environment (provided by Go)
...
Object.keys(safeOutputsConfig).forEach(configKey => {
  ...
  // Check if this is a safe-job (not in PREDEFINED_TOOLS)
  const isKnownTool = PREDEFINED_TOOLS[normalizedKey];
  if (!isKnownTool) {
