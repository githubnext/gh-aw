...
/**
 * Handler for generate_git_patch tool
 * Generates a git patch file from the branch specified in safe outputs
 * @param {any} args - Tool arguments
 * @returns {Object} MCP response with patch file path or error
 */
const generateGitPatchHandler = args => {
  const cwd = process.env.GITHUB_WORKSPACE || process.cwd();
  ...
  try {
        ...
        try {
          ...
          if (entry.type === "create_pull_request" || entry.type === "push_to_pull_request_branch") {
            if (entry.branch) {
              ...
              debug(`Found branch name from ${entry.type}: ${branchName}`);
              break;
            }
          }
        } catch (parseError) {
    ...
    if (!branchName) {
      debug("No branch found in safe outputs, no patch generation");
      return {
        content: [
          {
            type: "text",
    ...
    debug(`Looking for branch: ${branchName}`);
    ...
    try {
      ...
      debug(`Branch ${branchName} does not exist`);
      return {
        content: [
          {
            type: "text",
    ...
    try {
      ...
      debug(`Failed to count commits: ${countError instanceof Error ? countError.message : String(countError)}`);
    }
    ...
    if (!patchContent || !patchContent.trim()) {
      debug("Generated patch is empty - no changes between base and branch");
      return {
        content: [
          {
            type: "text",
    ...
    throw new Error(`Failed to generate git patch: ${errorMessage}`);
  }
};
...
const ALL_TOOLS = [
  ...
  },
  {
    ...
    handler: generateGitPatchHandler,
  },
  {
    name: "missing_tool",
...
// Auto-enable generate_git_patch when create-pull-request or push-to-pull-request-branch is configured
const hasPRTools = Object.keys(safeOutputsConfig).find(config => {
  const normalized = normTool(config);
  return normalized === "create_pull_request" || normalized === "push_to_pull_request_branch";
});
if (hasPRTools) {
  const patchTool = ALL_TOOLS.find(t => t.name === "generate_git_patch");
  if (patchTool && !TOOLS["generate_git_patch"]) {
    TOOLS["generate_git_patch"] = patchTool;
    debug("Auto-enabled generate_git_patch tool for PR operations");
  }
}
