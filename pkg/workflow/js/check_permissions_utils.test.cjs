...
((global.core = mockCore),
  ...
  describe("check_permissions_utils", () => {
    let parseRequiredPermissions, checkRepositoryPermission, originalEnv;
    (beforeEach(async () => {
      (vi.clearAllMocks(), (originalEnv = process.env.GH_AW_REQUIRED_ROLES));
      const module = await import("./check_permissions_utils.cjs");
      ((parseRequiredPermissions = module.parseRequiredPermissions), (checkRepositoryPermission = module.checkRepositoryPermission));
    }),
      afterEach(() => {
      ...
      describe("checkRepositoryPermission", () => {
        (it("should return authorized when user has exact permission match", async () => {
          ...
          (expect(result).toEqual({ authorized: !0, permission: "admin" }),
          ...
          it("should return authorized for maintain when maintainer is required", async () => {
            ...
            (expect(result).toEqual({ authorized: !0, permission: "maintain" }), expect(mockCore.info).toHaveBeenCalledWith("✅ User has maintain access to repository"));
          }),
          it("should return unauthorized when user has insufficient permissions", async () => {
            ...
            const result = await checkRepositoryPermission("testuser", "testowner", "testrepo", ["admin", "write"]);
            (expect(result).toEqual({ authorized: !1, permission: "read" }), expect(mockCore.warning).toHaveBeenCalledWith("User permission 'read' does not meet requirements: admin, write"));
          }),
          it("should return error on API failure", async () => {
            ...
            (expect(result).toEqual({ authorized: !1, error: "API Error: Not Found" }), expect(mockCore.warning).toHaveBeenCalledWith("Repository permission check failed: API Error: Not Found"));
          }),
          it("should handle non-Error API failures", async () => {
            ...
            const result = await checkRepositoryPermission("testuser", "testowner", "testrepo", ["admin"]);
            (expect(result).toEqual({ authorized: !1, error: "String error" }), expect(mockCore.warning).toHaveBeenCalledWith("Repository permission check failed: String error"));
          }),
          it("should check multiple permissions and return true for any match", async () => {
            ...
            (expect(result).toEqual({ authorized: !0, permission: "write" }), expect(mockCore.info).toHaveBeenCalledWith("✅ User has write access to repository"));
          }),
          it("should handle triage permission", async () => {
            ...
            expect(result).toEqual({ authorized: !0, permission: "triage" });
          }),
          it("should check permissions in order and stop at first match", async () => {
            ...
            (expect(result.authorized).toBe(!0), expect(result.permission).toBe("write"));
