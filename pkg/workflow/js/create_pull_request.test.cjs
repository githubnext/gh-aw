...
const createTestableFunction = scriptContent => {
  ...
  scriptBody = scriptBody.replace(/const \{ getCampaign \} = require\("\.\/get_campaign\.cjs"\);?\s*/g, "");

  // Create a testable function that has the same logic but can be called with dependencies
  return new Function(`
    const { fs, crypto, github, core, context, process, console, updateActivationComment, getCampaign } = arguments[0];
