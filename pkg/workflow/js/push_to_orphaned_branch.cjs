...
const maxCount = parseInt(process.env.GITHUB_AW_ORPHANED_BRANCH_MAX_COUNT || '1');
...
try {
  ...
  core.setFailed(`Failed to parse agent output: ${error instanceof Error ? error.message : String(error)}`);
  return;
}

// Extract push-to-orphaned-branch items
const orphanedBranchItems = (parsedOutput.items || []).filter(item => 
  item.type === 'push-to-orphaned-branch'
);
...
if (orphanedBranchItems.length > maxCount) {
  core.setFailed(`Too many files to upload: ${orphanedBranchItems.length} (max: ${maxCount})`);
  return;
}

core.info(`Found ${orphanedBranchItems.length} file(s) to upload to orphaned branch`);
...
if (isStaged) {
  ...
  core.summary.addRaw('The following files would be uploaded to an orphaned branch:\n\n');
  ...
  for (const item of orphanedBranchItems) {
    ...
    fileUrls.push(`https://raw.githubusercontent.com/${owner}/${repo.repo}/orphaned-uploads/staged/${item.filename}`);
  }
  ...
  try {
    ...
    for (const item of orphanedBranchItems) {
      ...
      core.info(`Created file: ${timestampedFilename} (${fileBuffer.length} bytes)`);
    ...
    core.summary.addRaw(`Successfully uploaded ${uploadedFiles.length} file(s) to orphaned branch \`${branchName}\`\n\n`);
    ...
    core.setFailed(`Failed to upload files to orphaned branch: ${error instanceof Error ? error.message : String(error)}`);
    return;
  }
}
...
core.info(`Successfully processed ${uploadedFiles.length} file(s) for orphaned branch upload`);