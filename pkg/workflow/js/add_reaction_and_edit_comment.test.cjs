...
summary:{addRaw:vi.fn().mockReturnThis(),write:vi.fn().mockResolvedValue()}},mockGithub={request:vi.fn(),graphql:vi.fn(),rest:{issues:{createComment:vi.fn()}}},mockContext={eventName:"issues",runId:12345,repo:{owner:"testowner",repo:"testrepo"},payload:{issue:{number:123},repository:{html_url:"https://github.com/testowner/testrepo"}}};
// Set up global variables
global.core=mockCore,global.github=mockGithub,global.context=mockContext,describe("add_reaction_and_edit_comment.cjs",()=>{let reactionScript;beforeEach(()=>{vi.clearAllMocks(),
// Reset environment variables
delete process.env.GH_AW_REACTION,delete process.env.GH_AW_COMMAND,delete process.env.GH_AW_WORKFLOW_NAME,
// Reset context to default state
global.context.eventName="issues",global.context.payload={issue:{number:123},repository:{html_url:"https://github.com/testowner/testrepo"}};
...
// Verify outputs
expect(mockCore.setOutput).toHaveBeenCalledWith("reaction-id","789"),expect(mockCore.setOutput).toHaveBeenCalledWith("comment-id","999"),expect(mockCore.setOutput).toHaveBeenCalledWith("comment-url","https://github.com/testowner/testrepo/pull/456#issuecomment-999")})}),describe("Discussion reactions",()=>{it("should add reaction to discussion using GraphQL",async()=>{process.env.GH_AW_REACTION="rocket",global.context.eventName="discussion",global.context.payload={discussion:{number:10},repository:{html_url:"https://github.com/testowner/testrepo"}},
// Mock GraphQL query to get discussion ID
mockGithub.graphql.mockResolvedValueOnce({repository:{discussion:{id:"D_kwDOABcD1M4AaBbC",url:"https://github.com/testowner/testrepo/discussions/10"}}}).mockResolvedValueOnce({addReaction:{reaction:{id:"MDg6UmVhY3Rpb24xMjM0NTY3ODk=",content:"ROCKET"}}}),
// Execute the script
await eval(`(async () => { ${reactionScript} })()`),
...
global.context.eventName="issue_comment",global.context.payload={issue:{number:123},comment:{id:456},repository:{html_url:"https://github.com/testowner/testrepo"}},
// Mock reaction call
...
// NO GH_AW_COMMAND needed - all workflows with reactions create comments
global.context.eventName="pull_request_review_comment",global.context.payload={pull_request:{number:456},comment:{id:789},repository:{html_url:"https://github.com/testowner/testrepo"}},
// Mock reaction call
...
// NO GH_AW_COMMAND set - this is NOT a command workflow
global.context.eventName="discussion",global.context.payload={discussion:{number:10},repository:{html_url:"https://github.com/testowner/testrepo"}},
// Mock GraphQL query to get discussion ID
mockGithub.graphql.mockResolvedValueOnce({repository:{discussion:{id:"D_kwDOABcD1M4AaBbC",url:"https://github.com/testowner/testrepo/discussions/10"}}}).mockResolvedValueOnce({addReaction:{reaction:{id:"MDg6UmVhY3Rpb24xMjM0NTY3ODk=",content:"EYES"}}}).mockResolvedValueOnce({repository:{discussion:{id:"D_kwDOABcD1M4AaBbC"}}}).mockResolvedValueOnce({addDiscussionComment:{comment:{id:"DC_kwDOABcD1M4AaBbE",url:"https://github.com/testowner/testrepo/discussions/10#discussioncomment-999"}}}),
// Execute the script
await eval(`(async () => { ${reactionScript} })()`),
...
// Verify outputs
expect(mockCore.setOutput).toHaveBeenCalledWith("reaction-id","MDg6UmVhY3Rpb24xMjM0NTY3ODk="),expect(mockCore.setOutput).toHaveBeenCalledWith("comment-id","DC_kwDOABcD1M4AaBbE"),expect(mockCore.setOutput).toHaveBeenCalledWith("comment-url","https://github.com/testowner/testrepo/discussions/10#discussioncomment-999")}),it("should create new comment for discussion_comment events",async()=>{process.env.GH_AW_REACTION="eyes",
// NO GH_AW_COMMAND needed - all workflows with reactions create comments
process.env.GH_AW_WORKFLOW_NAME="Discussion Bot",global.context.eventName="discussion_comment",global.context.payload={discussion:{number:10},comment:{id:123,node_id:"DC_kwDOABcD1M4AaBbC"},repository:{html_url:"https://github.com/testowner/testrepo"}},
// Mock GraphQL mutation to add reaction
mockGithub.graphql.mockResolvedValueOnce({addReaction:{reaction:{id:"MDg6UmVhY3Rpb24xMjM0NTY3ODk=",content:"EYES"}}}).mockResolvedValueOnce({repository:{discussion:{id:"D_kwDOABcD1M4AaBbC"}}}).mockResolvedValueOnce({addDiscussionComment:{comment:{id:"DC_kwDOABcD1M4AaBbE",url:"https://github.com/testowner/testrepo/discussions/10#discussioncomment-789"}}}),
// Execute the script
await eval(`(async () => { ${reactionScript} })()`),
// Verify comment was created with replyToId parameter (threaded comment)
// Uses default pirate-themed message from messages.cjs
expect(mockGithub.graphql).toHaveBeenCalledWith(expect.stringContaining("addDiscussionComment"),expect.objectContaining({dId:"D_kwDOABcD1M4AaBbC",body:expect.stringContaining("be settin' sail on this discussion comment"),replyToId:"DC_kwDOABcD1M4AaBbC"})),expect(mockCore.setOutput).toHaveBeenCalledWith("comment-id","DC_kwDOABcD1M4AaBbE"),expect(mockCore.setOutput).toHaveBeenCalledWith("comment-url","https://github.com/testowner/testrepo/discussions/10#discussioncomment-789"),expect(mockCore.setOutput).toHaveBeenCalledWith("comment-repo","testowner/testrepo")})}),describe("Error handling",()=>{it("should handle missing discussion number",async()=>{process.env.GH_AW_REACTION="eyes",global.context.eventName="discussion",global.context.payload={
// discussion number is missing
repository:{html_url:"https://github.com/testowner/testrepo"}},
// Execute the script
await eval(`(async () => { ${reactionScript} })()`),expect(mockCore.setFailed).toHaveBeenCalledWith("Discussion number not found in event payload")}),it("should handle missing discussion or comment info for discussion_comment",async()=>{process.env.GH_AW_REACTION="eyes",global.context.eventName="discussion_comment",global.context.payload={discussion:{number:10},
// comment is missing
repository:{html_url:"https://github.com/testowner/testrepo"}},
// Execute the script
await eval(`(async () => { ${reactionScript} })()`),expect(mockCore.setFailed).toHaveBeenCalledWith("Discussion or comment information not found in event payload")}),it("should handle unsupported event types",async()=>{process.env.GH_AW_REACTION="eyes",global.context.eventName="push",global.context.payload={repository:{html_url:"https://github.com/testowner/testrepo"}},
// Execute the script
await eval(`(async () => { ${reactionScript} })()`),expect(mockCore.setFailed).toHaveBeenCalledWith("Unsupported event type: push")})})});