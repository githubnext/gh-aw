async function safeOutputsMcpServerMain(): Promise<void> {
  const fs = require("fs");
  ...
  interface Tool {
    ...
    handler?: (args: any) => { content: Array<{ type: string; text: string }> };
  }
  ...
  interface UploadAssetEntry extends SafeOutputEntry {
    type: "upload_asset";
    path: string;
    fileName: string;
    sha: string;
  ...
  const safeOutputsConfig: SafeOutputsConfig = Object.fromEntries(
  ...
  const SERVER_INFO: ServerInfo = { name: "safe-outputs-mcp-server", version: "1.0.0" };
  const debug = (msg: string) => process.stderr.write(`[${SERVER_INFO.name}] ${msg}\n`);
  
  function writeMessage(obj: JsonRpcMessage): void {
  ...
  function replyResult(id: string | number | null | undefined, result: any): void {
    if (id === undefined || id === null) return; // notification
    const res: JsonRpcMessage = { jsonrpc: "2.0", id, result };
    writeMessage(res);
  }
  
  function replyError(id: string | number | null | undefined, code: number, message: string, data?: any): void {
  ...
  function appendSafeOutput(entry: SafeOutputEntry): void {
  ...
  const defaultHandler = (type: string) => (args: any) => {
    const entry: SafeOutputEntry = { ...(args || {}), type };
  ...
  const uploadAssetHandler = (args: UploadAssetArgs) => {
  ...
  const normTool = (toolName: string): string => (toolName ? toolName.replace(/-/g, "_").toLowerCase() : "");
  ...
  const TOOLS: { [key: string]: Tool } = Object.fromEntries(
  ...
  function handleMessage(req: JsonRpcMessage): void {
