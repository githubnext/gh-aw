...
function readConfig(): SafeOutputConfigs {
  let safeOutputsConfigRaw: SafeOutputConfigs;
  try {
    const configFile = readFileSync("/tmp/safe-outputs/config.json", "utf8");
    core.info(`found config.json: ${configFile.length} chars`);
  ...
  return Object.fromEntries(Object.entries(safeOutputsConfigRaw).map(([k, v]) => [k.replace(/-/g, "_"), v])) as SafeOutputConfigs;
}
