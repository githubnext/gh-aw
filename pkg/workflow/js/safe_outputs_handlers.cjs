...
function createHandlers(server, appendSafeOutput, config = {}) {
  ...
  /**
   * Handler for dispatch_workflow tool
   * Validates workflow_id against allowed list and prevents recursive calls
   */
  const dispatchWorkflowHandler = args => {
    const entry = { ...args, type: "dispatch_workflow" };

    // Get allowed workflows from configuration
    const allowedWorkflows = config.dispatch_workflow?.allowed_workflows || [];
    const currentWorkflowName = process.env.GH_AW_WORKFLOW_NAME || "";
    const strictMode = process.env.GH_AW_STRICT_MODE === "true";
    const maxCount = config.dispatch_workflow?.max || 1;
    ...
    if (entry.workflow_id === currentWorkflowName) {
      throw new Error(`Recursive workflow dispatch not allowed. Cannot dispatch workflow '${entry.workflow_id}' from itself.`);
    }

    // In strict mode, enforce max=1
    if (strictMode && maxCount > 1) {
      throw new Error(`In strict mode, dispatch-workflow max must be 1 (current: ${maxCount})`);
    }
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            result: "success",
            message: `Workflow '${entry.workflow_id}' dispatch prepared`,
