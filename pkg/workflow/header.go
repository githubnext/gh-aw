...
func GenerateWorkflowHeader(sourceFile string, generatedBy string, customInstructions string) string {
	...
	writeWorkflowHeaderDisclaimer(&header, generatedBy)
	header.WriteString("#\n")

	// Add ASCII logo
	writeWorkflowHeaderLogo(&header)
	header.WriteString("#\n")
	header.WriteString("#\n")

	// Add regeneration instructions
	writeWorkflowHeaderRegeneration(&header, sourceFile)

	// Add custom instructions if provided
	writeWorkflowHeaderCustomInstructions(&header, customInstructions)
...
func writeWorkflowHeaderDisclaimer(header *strings.Builder, generatedBy string) {
...
func writeWorkflowHeaderLogo(header *strings.Builder) {
...
func writeWorkflowHeaderRegeneration(header *strings.Builder, sourceFile string) {
	if sourceFile != "" {
...
func writeWorkflowHeaderCustomInstructions(header *strings.Builder, customInstructions string) {
