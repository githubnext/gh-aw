...
func TestCheckNetworkSupport_WithRestrictions(t *testing.T) {
	...
	t.Run("claude engine with restrictions - no warning", func(t *testing.T) {
		compiler := NewCompiler(false, "", "test")
		engine := NewClaudeEngine()
		perms := &NetworkPermissions{
		...
		if compiler.warningCount != initialWarnings {
			t.Error("Should not emit warning for claude engine with network restrictions (it supports firewall now)")
		}
	})
...
func TestCheckNetworkSupport_StrictMode(t *testing.T) {
	...
	t.Run("strict mode: claude engine with restrictions - no error", func(t *testing.T) {
		compiler := NewCompiler(false, "", "test")
		compiler.strictMode = true
		engine := NewClaudeEngine()
		perms := &NetworkPermissions{
		...
		err := compiler.checkNetworkSupport(engine, perms)
		if err != nil {
			t.Errorf("Expected no error for claude in strict mode (it supports firewall now), got: %v", err)
		}
	})
...
func TestCheckFirewallDisable(t *testing.T) {
	...
	t.Run("strict mode: firewall disabled with codex engine - error", func(t *testing.T) {
		compiler := NewCompiler(false, "", "test")
		compiler.strictMode = true
		engine := NewCodexEngine()
		perms := &NetworkPermissions{
