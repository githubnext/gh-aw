...
func (e *ClaudeEngine) getDockerComposeExecutionSteps(workflowData *WorkflowData, logFile string) []GitHubActionStep {
	var stepLines []string
	...
	// Add allowed tools comment before the timeout
	allowedToolsComment := e.generateAllowedToolsComment(e.computeAllowedClaudeToolsString(workflowData.Tools, workflowData.SafeOutputs), "        ")
	if allowedToolsComment != "" {
		// Split the comment into lines and add each line
		commentLines := strings.Split(strings.TrimSuffix(allowedToolsComment, "\n"), "\n")
	...
	stepLines = append(stepLines, "          # Execute containerized Claude Code with proxy")
	stepLines = append(stepLines, "          ")
	stepLines = append(stepLines, "          # Create necessary directories")
	stepLines = append(stepLines, "          mkdir -p mcp-config prompts logs safe-outputs .claude")
	stepLines = append(stepLines, "          ")
	...
	stepLines = append(stepLines, "          cp -r /tmp/gh-aw/.claude/* .claude/ 2>/dev/null || true")
	stepLines = append(stepLines, "          ")
...
func (e *ClaudeEngine) createMissingToolEntry(toolName, reason string, verbose bool) {
	...
	defer func() {
		if cerr := file.Close(); cerr != nil && verbose {
			fmt.Printf("Failed to close safe outputs file: %v\n", cerr)
		}
	}()
