...
//   - repos="all" or repos="public": accept=[] (write-sink not required, agent secrecy is empty)
...
func deriveSafeOutputsGuardPolicyFromGitHub(githubTool any) map[string]any {
	...
	case string:
		...
		switch r {
		case "all", "public":
			// For "all" or "public", agent secrecy is empty, so accept=[] (write-sink not required)
			acceptList = []string{}
		default:
