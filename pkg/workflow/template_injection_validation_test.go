...
func TestValidateNoTemplateInjection(t *testing.T) {
  ...
  test:
    ...
    steps:
      - name: Unsafe usage
        run: |
          bash ${{ runner.temp }}/gh-aw/actions/stop_mcp_gateway.sh ${{ steps.start-mcp-gateway.outputs.gateway-pid }}`,
			shouldError: true,
  ...
  agent:
    steps:
      - name: Stop MCP Gateway
        ...
        run: |
          bash ${{ runner.temp }}/gh-aw/actions/stop_mcp_gateway.sh "$GATEWAY_PID"`
