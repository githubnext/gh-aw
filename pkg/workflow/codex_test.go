...
`
			...
			if tt.expectCodex {
				...
				// Check that --additional-mcp-config argument is present (Copilot now uses base64-encoded config)
				if !strings.Contains(lockContent, "--additional-mcp-config") {
					t.Errorf("Expected lock file to contain --additional-mcp-config argument for copilot but it didn't.\nContent:\n%s", lockContent)
				}
				...
				if strings.Contains(lockContent, "cat > /home/runner/.copilot/mcp-config.json") {
					t.Errorf("Expected lock file to NOT contain file-based mcp-config.json generation (Copilot uses --additional-mcp-config now).\nContent:\n%s", lockContent)
				}
				// Ensure it does NOT contain codex
				if strings.Contains(lockContent, "codex exec") {
