...
`
			...
			if tt.expectCodex {
				...
				if !strings.Contains(lockContent, "cat \"$GH_AW_PROMPT\"") || !strings.Contains(lockContent, "} >> \"$GITHUB_STEP_SUMMARY\"") {
					t.Errorf("Expected lock file to contain prompt printing command with grouped redirects but it didn't.\nContent:\n%s", lockContent)
				}
