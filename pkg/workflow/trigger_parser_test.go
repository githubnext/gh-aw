...
func TestParseTriggerShorthand(t *testing.T) {
	tests := []struct {
		...
		// Deployment Patterns
		{
			...
			wantConds: []string{"github.event.deployment_status.state == 'failure'"},
		},
		{
			name:      "deployment error",
			input:     "deployment error",
			wantEvent: "deployment_status",
			wantConds: []string{"github.event.deployment_status.state == 'error'"},
		},
		{
			name:      "deployment failed or error",
			input:     "deployment failed or error",
			wantEvent: "deployment_status",
			wantConds: []string{"github.event.deployment_status.state == 'failure' || github.event.deployment_status.state == 'error'"},
		},
		{
			name:      "deployment error or failure",
			input:     "deployment error or failure",
			wantEvent: "deployment_status",
			wantConds: []string{"github.event.deployment_status.state == 'error' || github.event.deployment_status.state == 'failure'"},
		},
		{
