...
func GetNpmBinPathSetup() string {
	// Prepend ${RUNNER_TEMP:-/tmp}/npm-global/bin first so that CLIs installed
	// with a writable npm prefix (see GenerateNpmInstallStepsWithScope) are found
	// before the read-only hostedtoolcache paths.
	// RUNNER_TEMP is the official GitHub Actions temp directory; the /tmp fallback
	// is used when the variable is not set (e.g. inside AWF containers that do not
	...
	return `export PATH="${RUNNER_TEMP:-/tmp}/npm-global/bin:$(find /opt/hostedtoolcache -maxdepth 4 -type d -name bin 2>/dev/null | tr '\n' ':')$PATH"; [ -n "$GOROOT" ] && export PATH="$GOROOT/bin:$PATH" || true`
}
...
func GenerateNpmInstallStepsWithScope(packageName, version, stepName, cacheKeyPrefix string, includeNodeSetup bool, isGlobal bool, runInstallScripts bool) []GitHubActionStep {
	...
	if ExpressionPattern.MatchString(version) {
		...
		if isGlobal {
			// Use $RUNNER_TEMP (official GitHub Actions temp dir) as the npm prefix to
			// avoid EROFS errors when /opt/hostedtoolcache is read-only after setup-node.
			// Falls back to /tmp when RUNNER_TEMP is not set.
			// Add the bin dir to GITHUB_PATH so subsequent steps can find the binary.
			multiLineCmd := fmt.Sprintf("npm config set prefix \"${RUNNER_TEMP:-/tmp}/npm-global\"\n%s\necho \"${RUNNER_TEMP:-/tmp}/npm-global/bin\" >> \"$GITHUB_PATH\"", installCmd)
