...
func TestCustomEngineRenderPlaywrightMCPConfigWithDomainConfiguration(t *testing.T) {
	...
	// Check that it contains Playwright MCP npx configuration with the version from tools
	if !strings.Contains(output, "@playwright/mcp@v1.40.0") {
		t.Errorf("Expected Playwright MCP npx package with v1.40.0 in output, got: %s", output)
	}
