...
// It also verifies the checkout uses a path outside $GITHUB_WORKSPACE (absolute /tmp path)
// so the files are never inside the caller's git repository.
func TestCheckoutActionsFolderDevModeHasRepository(t *testing.T) {
	...
	if !strings.Contains(combined, "path: /tmp/gh-aw/dev-actions") {
		t.Error("Dev mode Checkout actions folder should checkout to outside-workspace path '/tmp/gh-aw/dev-actions' to prevent accidental commits")
	}
}
