...
func (c *Compiler) buildUploadAssetsJob(data *WorkflowData, mainJobName string) (*Job, error) {
	...
	steps = append(steps, "          # Remove all characters that are not a-z, A-Z, 0-9, -, _, or / and convert to lowercase\n")
	steps = append(steps, "          NORMALIZED_BRANCH=$(echo \"$RAW_BRANCH\" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9/_-]//g' | sed 's#/\\+#/#g' | sed 's/^[-\\/]*//;s/[-\\/]*$//')\n")
