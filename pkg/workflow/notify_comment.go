...
func (c *Compiler) buildUpdateReactionJob(data *WorkflowData, mainJobName string, safeOutputJobNames []string) (*Job, error) {
	notifyCommentLog.Printf("Building update reaction job: main_job=%s, safe_output_jobs=%d", mainJobName, len(safeOutputJobNames))
	...
	notifyCommentLog.Printf("Update reaction job config: has_add_comment=%v, has_command=%v, has_reaction=%v", hasAddComment, hasCommand, hasReaction)
	...
	if !hasReaction {
		notifyCommentLog.Print("No reaction configured, skipping update reaction job")
		return nil, nil // No reaction configured or explicitly disabled, no comment to update
	}

	if !hasAddComment && !hasCommand {
		notifyCommentLog.Print("Neither add-comment nor command configured, skipping update reaction job")
	...
	notifyCommentLog.Printf("Update reaction job built: dependencies=%d", len(needs))
