...
func generateToolsMetaJSON(data *WorkflowData, markdownPath string) (string, error) {
	if data.SafeOutputs == nil {
		...
		if err != nil {
			return "", fmt.Errorf("unable to marshal empty tools metadata to JSON for default safe-outputs behavior: %w", err)
		}
	...
	if err != nil {
		...
		return "", fmt.Errorf("unable to marshal tools metadata to JSON for runtime schema injection; verify safe-outputs values are JSON-serializable: %w", err)
	}
