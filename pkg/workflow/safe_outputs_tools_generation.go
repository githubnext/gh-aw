...
type ToolsMeta struct {
	...
	// body-allowed: false is configured for close_discussion or close_issue).
...
// based on the safe-outputs configuration. Currently handles body-allowed: false for
...
func computeRequiredFieldRemovals(safeOutputs *SafeOutputsConfig) map[string][]string {
	...
	if safeOutputs.CloseDiscussions != nil && safeOutputs.CloseDiscussions.BodyAllowed != nil && !*safeOutputs.CloseDiscussions.BodyAllowed {
		removals["close_discussion"] = []string{"body"}
	}
	if safeOutputs.CloseIssues != nil && safeOutputs.CloseIssues.BodyAllowed != nil && !*safeOutputs.CloseIssues.BodyAllowed {
...
func generateToolsMetaJSON(data *WorkflowData, markdownPath string) (string, error) {
	...
	// Compute required field removals (e.g. body when body-allowed: false).
