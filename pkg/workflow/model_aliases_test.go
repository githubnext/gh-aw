...
func TestBuildAWFConfigJSON_ModelsSection(t *testing.T) {
	...
	t.Run("no models section when ModelMappings is nil", func(t *testing.T) {
		...
		// Check the parsed apiProxy.models section rather than a raw substring: the
		// builtin copilot/auto pricing overlay also emits a nested "models" key under
		// apiProxy.providers.
		assert.Empty(t, parsed.APIProxy.Models, "apiProxy.models section should be absent when ModelMappings is nil")
	})
}
