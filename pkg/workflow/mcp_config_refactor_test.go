	...
	tests := []struct {
	...
	}{
		{
			...
			expectedContent: []string{
				...
				`"entrypoint": "sh"`,
				`"entrypointArgs": ["-c", "bash ${RUNNER_TEMP}/gh-aw/safeoutputs/configure_git_credentials.sh --no-auth && exec node ${RUNNER_TEMP}/gh-aw/safeoutputs/safe_outputs_mcp_server.cjs"]`,
				`"env": {`,
