...
type AddCommentsConfig struct {
	...
	HideOlderComments    bool   `yaml:"hide-older-comments,omitempty"` // Minimize (hide) all previous comments from the same workflow before adding new comment
}
...
func (c *Compiler) parseCommentsConfig(outputMap map[string]any) *AddCommentsConfig {
	if configData, exists := outputMap["add-comment"]; exists {
		...
		if configMap, ok := configData.(map[string]any); ok {
			...
			if hideOlderComments, exists := configMap["hide-older-comments"]; exists {
				if hideOlderCommentsBool, ok := hideOlderComments.(bool); ok {
					commentsConfig.HideOlderComments = hideOlderCommentsBool
				}
			}
