...
func TestGenerateDefaultCheckoutStep(t *testing.T) {
	...
	t.Run("clean-git-credentials enables persist true and cleanup step", func(t *testing.T) {
		...
		assert.Contains(t, combined, "Warning: Git credential cleanup skipped - runtime cleaner and all pre-setup helper paths are unavailable", "cleanup should include explicit warning when no helper is available")
