...
func TestGenerateDefaultCheckoutStep(t *testing.T) {
	...
	t.Run("clean-git-credentials enables persist true and cleanup step", func(t *testing.T) {
		...
		assert.Contains(t, combined, "Warning: Git credential cleanup skipped - checked ${RUNNER_TEMP}/gh-aw/actions/clean_git_credentials.sh, ${RUNNER_TEMP}/gh-aw/actions/clean_git_credentials_pre_setup.sh, and ${GITHUB_WORKSPACE}/actions/setup/sh/clean_git_credentials_pre_setup.sh", "cleanup should include explicit warning when no helper is available")
