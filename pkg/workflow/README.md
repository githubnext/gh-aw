...
| `tools_types.go` | `const` | `GitHubMCPModeCLI` | `const GitHubMCPModeCLI GitHubMCPMode = "cli"` | GitHubMCPModeCLI routes GitHub operations through the pre-authenticated `gh` CLI (no MCP server). Canonical recommended default; `gh-proxy` is a legacy alias. |
| `tools_types.go` | `const` | `GitHubMCPModeGHProxy` | `const GitHubMCPModeGHProxy GitHubMCPMode = "gh-proxy"` | GitHubMCPModeGHProxy is a deprecated legacy alias for GitHubMCPModeCLI (`cli`). Migrated by `gh aw fix`. |
| `tools_types.go` | `const` | `GitHubMCPModeLocal` | `const GitHubMCPModeLocal GitHubMCPMode = "local"` | GitHubMCPModeLocal is a deprecated legacy alias for `mcp-local`. Runs the GitHub MCP server as a Docker container on the runner. Migrated by `gh aw fix`. |
| `tools_types.go` | `const` | `GitHubMCPModeRemote` | `const GitHubMCPModeRemote GitHubMCPMode = "remote"` | GitHubMCPModeRemote is a deprecated legacy alias for `mcp-remote`. Connects to the hosted GitHub MCP service. Migrated by `gh aw fix`. |
