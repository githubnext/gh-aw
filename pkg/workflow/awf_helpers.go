...
func BuildAWFCommand(config AWFCommandConfig) string {
	...
	arcDindDockerHostProbe, arcDindPrefixProbe, arcDindDockerHostRef := buildAWFArcDindProbes(config, firewallConfig)
	toolCacheMountProbe, toolCacheMountRef := buildAWFToolCacheMountProbe()
	expandableArgs, arcDindProbeExtra := buildAWFExpandableArgs(config, isArcDind)
	arcDindDockerHostProbe += arcDindProbeExtra
	configFileSetup, configArgsPrefix := buildAWFConfigSetupScript(config)
	if configArgsPrefix != "" {
		expandableArgs = configArgsPrefix + expandableArgs
	}
	...
	// Pre-create the log file with restrictive permissions (0600) before AWF starts.
	// tee would otherwise create it with the default umask (0644), leaving secrets world-readable.
	preCreateLog := fmt.Sprintf("(umask 177 && touch %s)", shellEscapeArg(config.LogFile))
	// Capture epoch-millisecond timestamp before AWF launches for span timing.
	writeAgentCLIStartMs := "printf '%s' \"$(date +%s%3N)\" > " + shellEscapeArg(AgentCLIStartMsPath)
	command := assembleAWFCommandVariants(awfCommandParts{
...
// buildAWFArcDindProbes constructs the shell probe strings for ARC/DinD Docker host detection.
// Returns the DOCKER_HOST probe script, optional chroot prefix probe, and the docker-host ref.
func buildAWFArcDindProbes(config AWFCommandConfig, firewallConfig *FirewallConfig) (dockerHostProbe, prefixProbe, dockerHostRef string) {
	prefixProbe = ""
...
if [[ "${DOCKER_HOST:-}" =~ %s ]]; then
  %s="${DOCKER_HOST}"
	...
	if awfSupportsDockerHostPathPrefix(firewallConfig) {
		...
		// NOTE: --docker-host-path-prefix is intentionally NOT passed (see gh-aw#34896).
		// The probe block is preserved for the chroot config patch which needs the guard.
		if chrootPatchBody != "" {
...
// buildAWFToolCacheMountProbe constructs shell probe strings for the runner tool-cache mount.
// Returns the probe script and the conditional --mount reference expression.
func buildAWFToolCacheMountProbe() (mountProbe, mountRef string) {
...
// buildAWFExpandableArgs builds the expandable shell args string for AWF invocation.
// Returns the expandable args and any extra shell lines to append to the arc-dind docker-host probe.
func buildAWFExpandableArgs(config AWFCommandConfig, isArcDind bool) (expandableArgs, arcDindProbeExtra string) {
	...
	if isArcDind {
		...
		// Pre-create rw mount source directories before AWF validates them.
		arcDindProbeExtra += fmt.Sprintf("\nmkdir -p \"%s\" \"%s\"",
			awfArcDindHomePathExpr,
			awfArcDindRootPathExpr+"/sandbox/agent",
		)
		// Copy prompts from /tmp/gh-aw/ to the daemon-visible path on ARC/DinD.
		arcDindProbeExtra += fmt.Sprintf("\nif [ -d /tmp/gh-aw/aw-prompts ]; then cp -a /tmp/gh-aw/aw-prompts \"%s/aw-prompts\"; fi",
			awfArcDindRootPathExpr,
		)
	}
	// upload_artifact: add rw mount for staging dir; parent is already :ro above.
	...
	// --allow-host-service-ports requires --legacy-security; value contains ${{ ... }} expressions.
...
// buildAWFMaxAICreditsConfig injects a maxAiCredits runtime expression into the config JSON
// when no compile-time value was set. Returns the (possibly updated) configJSON and the shell
// export line for GH_AW_MAX_AI_CREDITS.
func buildAWFMaxAICreditsConfig(config AWFCommandConfig, configJSON string) (updatedConfigJSON, exportLine string) {
	...
	if config.WorkflowData == nil || config.WorkflowData.EngineConfig == nil || config.WorkflowData.EngineConfig.MaxAICredits == 0 {
		...
		updatedConfigJSON = injectMaxAICreditsExpression(configJSON, fmt.Sprintf("${%s}", awfMaxAICreditsVarName))
...
// buildAWFConfigSetupScript generates the shell script that writes the AWF config JSON to
// ${RUNNER_TEMP}/gh-aw/awf-config.json immediately before the AWF invocation.
// Returns the shell setup script and the --config flag prefix for expandableArgs.
func buildAWFConfigSetupScript(config AWFCommandConfig) (configFileSetup, expandableArgsPrefix string) {
	awfConfigJSON, err := BuildAWFConfigJSON(config)
	if err != nil {
		awfHelpersLog.Printf("Warning: failed to build AWF config JSON: %v", err)
		return
	}
	awfConfigJSON, maxAICreditsExportLine := buildAWFMaxAICreditsConfig(config, awfConfigJSON)
	...
	// SC2016 is only triggered when printfArg is single-quoted (no runtime vars injected).
	printfLine := "printf '%%s\\n' %s > %q"
	...
	expandableArgsPrefix = fmt.Sprintf("--config %q ", awfConfigRuntimePathExpr)
	awfHelpersLog.Print("Using AWF config file (--config flag)")
	return
}

// awfCommandParts holds all pre-assembled parts needed to build the final AWF command string.
...
// assembleAWFCommandVariants builds the complete shell command string for AWF execution.
// Conditionally includes pathSetup and configFileSetup only when non-empty.
func assembleAWFCommandVariants(parts awfCommandParts) string {
	scriptLines := []string{"set -o pipefail", parts.writeAgentCLIStartMs}
	if parts.pathSetup != "" {
		scriptLines = append(scriptLines, parts.pathSetup)
	}
	scriptLines = append(scriptLines, parts.preCreateLog)
	if parts.configFileSetup != "" {
		scriptLines = append(scriptLines, parts.configFileSetup)
	}
	scriptLines = append(scriptLines,
		parts.modelsJSONPathExport,
		parts.arcDindDockerHostProbe,
		parts.arcDindPrefixProbe,
		parts.toolCacheMountProbe,
		awfShellcheckDirective,
	)
	scriptLines = append(scriptLines, fmt.Sprintf("%s %s %s %s %s \\\n  -- %s 2>&1 | tee -a %s",
		parts.awfCommand, parts.expandableArgs, parts.toolCacheMountRef,
		parts.arcDindDockerHostRef, shellJoinArgs(parts.awfArgs),
		parts.shellWrappedCommand, shellEscapeArg(parts.logFile),
	))
...
// buildAWFContainerAndEnvArgs returns AWF args for container runtime, env-all, exclude-env,
// and custom mounts.
func buildAWFContainerAndEnvArgs(config AWFCommandConfig, firewallConfig *FirewallConfig, agentConfig *AgentSandboxConfig) []string {
	var args []string
	...
	if isDockerSbxRuntime(config.WorkflowData) && awfSupportsContainerRuntime(firewallConfig) {
		args = append(args, "--container-runtime", "sbx")
...
// buildAWFLegacySecurityArgs returns legacy security related AWF args when legacy-security is enabled.
// Returns nil when not in legacy mode.
func buildAWFLegacySecurityArgs(firewallConfig *FirewallConfig, agentConfig *AgentSandboxConfig, workflowData *WorkflowData) []string {
	...
	if awfSupportsLegacySecurity(firewallConfig) {
		args = append(args, "--legacy-security")
...
// buildAWFCliProxyArgs returns CLI proxy AWF args when GitHub CLI mode is enabled.
// Returns nil when not applicable.
func buildAWFCliProxyArgs(firewallConfig *FirewallConfig, workflowData *WorkflowData) []string {
	...
	return []string{"--difc-proxy-host", difcProxyHost, "--difc-proxy-ca-cert", constants.TmpDIFCProxyTLSCACert}
}

// buildAWFApiBasePathArgs returns AWF args for API base paths when the endpoint URL contains a path component.
func buildAWFApiBasePathArgs(workflowData *WorkflowData) []string {
	var args []string
	if openaiBasePath := extractAPIBasePath(workflowData, "OPENAI_BASE_URL"); openaiBasePath != "" {
		args = append(args, "--openai-api-base-path", openaiBasePath)
		awfHelpersLog.Printf("Added --openai-api-base-path=%s", openaiBasePath)
	}
	if anthropicBasePath := extractAPIBasePath(workflowData, "ANTHROPIC_BASE_URL"); anthropicBasePath != "" {
	...
	if geminiBasePath := extractAPIBasePath(workflowData, "GEMINI_API_BASE_URL"); geminiBasePath != "" {
...
func BuildAWFArgs(config AWFCommandConfig) []string {
	...
	awfArgs := buildAWFContainerAndEnvArgs(config, firewallConfig, agentConfig)
	...
	awfArgs = append(awfArgs, buildAWFLegacySecurityArgs(firewallConfig, agentConfig, config.WorkflowData)...)
	...
	awfArgs = append(awfArgs, buildAWFCliProxyArgs(firewallConfig, config.WorkflowData)...)
	awfArgs = append(awfArgs, buildAWFApiBasePathArgs(config.WorkflowData)...)
...
// addMCPExcludeVars adds MCP-related environment variable names to the exclusion list.
// This covers the MCP gateway API key, GitHub MCP server token, HTTP header secrets,
// and mcp-scripts env vars that reference secrets or job outputs.
func addMCPExcludeVars(workflowData *WorkflowData, addUnique func(string)) {
...
func ComputeAWFExcludeEnvVarNames(workflowData *WorkflowData, coreSecretVarNames []string) []string {
	...
	addMCPExcludeVars(workflowData, addUnique)
