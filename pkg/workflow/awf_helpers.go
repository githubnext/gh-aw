...
func buildArcDindProbes(config AWFCommandConfig, firewallConfig *FirewallConfig) (string, string, string) {
	arcDindPrefixProbe := ""
...
func buildAWFExpandableArgs(isArcDind bool, arcDindDockerHostProbe string) (string, string) {
...
func buildMaxAICreditsExportLine(config AWFCommandConfig) string {
	...
	if config.ResolveMaxAICreditsFromEnv {
		return fmt.Sprintf(`%s="${%s:-%s}"`, awfMaxAICreditsVarName, awfMaxAICreditsVarName, defaultMaxAICredits)
	}
	...
	return fmt.Sprintf(`%s="%s"`, awfMaxAICreditsVarName, expr)
}

func buildAWFConfigPrintfArg(awfConfigJSON, maxAICreditsExportLine string) string {
...
func buildAWFConfigFileSetup(config AWFCommandConfig, expandableArgs string) (string, string) {
	...
	maxAICreditsExportLine := buildMaxAICreditsExportLine(config)
	if maxAICreditsExportLine != "" {
	...
	configFileSetup := fmt.Sprintf(
		"printf '%%s\\n' %s > %q",
		buildAWFConfigPrintfArg(awfConfigJSON, maxAICreditsExportLine),
...
func appendAWFUploadArtifactMount(expandableArgs string, workflowData *WorkflowData) string {
...
func appendAWFServicePortArgs(expandableArgs string, workflowData *WorkflowData) string {
	agentCfg := getAgentConfig(workflowData)
	isLegacyMode := agentCfg != nil && agentCfg.LegacySecurity
	if workflowData != nil && workflowData.ServicePortExpressions != "" && isLegacyMode {
	...
	} else if workflowData != nil && workflowData.ServicePortExpressions != "" {
		awfHelpersLog.Print("Skipping --allow-host-service-ports: requires legacy-security mode")
	}
	return expandableArgs
}

func buildAWFInvocationCommand(config AWFCommandConfig, commandParts awfInvocationParts) string {
	commonArgs := buildAWFInvocationArgs(config, commandParts)
	if config.PathSetup != "" && commandParts.configFileSetup != "" {
...
%s
%s %s %s %s %s \
  -- %s 2>&1 | tee -a %s`, append([]any{commonArgs[0], config.PathSetup}, commonArgs[1:]...)...)
	}
	if config.PathSetup != "" {
		args := append([]any{commonArgs[0], config.PathSetup, commonArgs[1]}, commonArgs[3:]...)
		return fmt.Sprintf(`set -o pipefail
%s
%s
%s
...
%s %s %s %s %s \
  -- %s 2>&1 | tee -a %s`, args...)
	...
	if commandParts.configFileSetup != "" {
		return fmt.Sprintf(`set -o pipefail
%s
%s
%s
...
%s
%s %s %s %s %s \
  -- %s 2>&1 | tee -a %s`, append(commonArgs[:2], commonArgs[3:]...)...)
}

func buildAWFInvocationArgs(config AWFCommandConfig, commandParts awfInvocationParts) []any {
	return []any{
		commandParts.writeAgentCLIStartMs,
		commandParts.preCreateLog,
		commandParts.configFileSetup,
		commandParts.modelsJSONPathExport,
		commandParts.arcDindDockerHostProbe,
		commandParts.arcDindPrefixProbe,
		commandParts.toolCacheMountProbe,
		awfShellcheckDirective,
		commandParts.awfCommand,
		commandParts.expandableArgs,
		commandParts.toolCacheMountRef,
		commandParts.arcDindDockerHostRef,
		shellJoinArgs(commandParts.awfArgs),
		commandParts.shellWrappedCommand,
		shellEscapeArg(config.LogFile),
	}
}

type awfInvocationParts struct {
	awfCommand             string
	awfArgs                []string
...
func BuildAWFCommand(config AWFCommandConfig) string {
	...
	arcDindPrefixProbe, arcDindDockerHostProbe, arcDindDockerHostRef := buildArcDindProbes(config, firewallConfig)
	toolCacheMountProbe, toolCacheMountRef := buildToolCacheMountProbe()
	expandableArgs, arcDindDockerHostProbe := buildAWFExpandableArgs(isArcDind, arcDindDockerHostProbe)
	...
	expandableArgs = appendAWFUploadArtifactMount(expandableArgs, config.WorkflowData)
	expandableArgs = appendAWFServicePortArgs(expandableArgs, config.WorkflowData)
	...
	return buildAWFInvocationCommand(config, awfInvocationParts{
...
func BuildAWFArgs(config AWFCommandConfig) []string {
	...
	var awfArgs []string
	awfArgs = appendAWFRuntimeArgs(awfArgs, config, firewallConfig)
	awfArgs = appendAWFEnvArgs(awfArgs, config, firewallConfig)
	awfArgs = appendAWFMountArgs(awfArgs, agentConfig)
	awfArgs = appendAWFLogArgs(awfArgs, config, firewallConfig)
	...
	awfArgs = appendAWFCLIProxyArgs(awfArgs, config, firewallConfig)
	awfArgs = appendAWFBasePathArgs(awfArgs, config.WorkflowData)
...
func appendAWFRuntimeArgs(awfArgs []string, config AWFCommandConfig, firewallConfig *FirewallConfig) []string {
...
func appendAWFLogArgs(awfArgs []string, config AWFCommandConfig, firewallConfig *FirewallConfig) []string {
...
func appendAWFCLIProxyArgs(awfArgs []string, config AWFCommandConfig, firewallConfig *FirewallConfig) []string {
...
func appendAWFBasePathArgs(awfArgs []string, workflowData *WorkflowData) []string {
	openaiBasePath := extractAPIBasePath(workflowData, "OPENAI_BASE_URL")
	if openaiBasePath != "" {
	...
	geminiBasePath := extractAPIBasePath(workflowData, "GEMINI_API_BASE_URL")
	if geminiBasePath != "" {
...
func ComputeAWFExcludeEnvVarNames(workflowData *WorkflowData, coreSecretVarNames []string) []string {
	...
	addAWFMCPScriptSecretEnvVars(workflowData, addUnique)
	addAWFConfigSecretEnvVars(workflowData, addUnique)
...
func addAWFMCPScriptSecretEnvVars(workflowData *WorkflowData, addUnique func(string)) {
...
func addAWFConfigSecretEnvVars(workflowData *WorkflowData, addUnique func(string)) {
