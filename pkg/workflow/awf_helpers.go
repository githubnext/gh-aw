...
func BuildAWFCommand(config AWFCommandConfig) string {
	...
	toolCacheMountProbe, toolCacheMountRef := buildToolCacheMountSettings()
	expandableArgs, arcDindDockerHostProbe := buildAWFExpandableArgs(config, isArcDind, arcDindDockerHostProbe)
	configFileSetup := buildAWFConfigFileSetupOrEmpty(config, expandableArgs, &expandableArgs)
	modelsJSONPathExport := buildModelsJSONPathExportScript(isArcDind)
	expandableArgs = addUploadArtifactMount(config, expandableArgs)
	expandableArgs = addLegacyHostServicePortArgs(config, expandableArgs)
	...
	command := buildAWFExecutionCommand(
...
func buildToolCacheMountSettings() (string, string) {
	probe := fmt.Sprintf(`%s=""
...
func buildAWFExpandableArgs(config AWFCommandConfig, isArcDind bool, arcDindDockerHostProbe string) (string, string) {
	ghAwDir := constants.GhAwRootDirShell
	expandableArgs := fmt.Sprintf(`--container-workdir "${GITHUB_WORKSPACE}" --mount "%s:%s:ro" --mount "%s:/host%s:ro"`,
	...
	expandableArgs += fmt.Sprintf(` --mount "%s:%s:rw" --mount "%s:%s:rw"`,
...
func buildAWFConfigFileSetupOrEmpty(config AWFCommandConfig, expandableArgs string, updatedExpandableArgs *string) string {
	...
	configFileSetup, expandableWithConfig := buildAWFConfigFileSetup(config, awfConfigJSON, expandableArgs)
	*updatedExpandableArgs = expandableWithConfig
	return configFileSetup
}

func buildAWFConfigFileSetup(config AWFCommandConfig, awfConfigJSON string, expandableArgs string) (string, string) {
	awfConfigJSON, maxAICreditsExportLine := injectDefaultMaxAICredits(config, awfConfigJSON)
	...
	return configFileSetup, fmt.Sprintf("--config %q %s", awfConfigRuntimePathExpr, expandableArgs)
}

func injectDefaultMaxAICredits(config AWFCommandConfig, awfConfigJSON string) (string, string) {
	if config.WorkflowData != nil && config.WorkflowData.EngineConfig != nil && config.WorkflowData.EngineConfig.MaxAICredits != 0 {
		return awfConfigJSON, ""
	}

	defaultMaxAICredits := defaultAWFMaxAICredits(config.WorkflowData)
	...
	if config.ResolveMaxAICreditsFromEnv {
		...
		return awfConfigJSON, fmt.Sprintf(`%s="${%s:-%s}"`, awfMaxAICreditsVarName, awfMaxAICreditsVarName, defaultMaxAICredits)
	}

	expr := defaultAWFMaxAICreditsExpression(config.WorkflowData, defaultMaxAICredits)
	awfHelpersLog.Printf("Injected maxAiCredits local var reference into AWF config JSON")
	return awfConfigJSON, fmt.Sprintf(`%s="%s"`, awfMaxAICreditsVarName, expr)
}

func defaultAWFMaxAICredits(workflowData *WorkflowData) string {
	switch {
...
func defaultAWFMaxAICreditsExpression(workflowData *WorkflowData, defaultMaxAICredits string) string {
	switch {
	case workflowData != nil && workflowData.IsEvalsRun:
...
func buildAWFConfigPrintfArg(awfConfigJSON string, maxAICreditsExportLine string) string {
...
func addUploadArtifactMount(config AWFCommandConfig, expandableArgs string) string {
	...
	return expandableArgs + fmt.Sprintf(` --mount "%s:%s:rw"`, SafeOutputsUploadArtifactsDir, SafeOutputsUploadArtifactsDir)
}

func addLegacyHostServicePortArgs(config AWFCommandConfig, expandableArgs string) string {
	...
	return expandableArgs + fmt.Sprintf(` --allow-host-service-ports "%s"`, config.WorkflowData.ServicePortExpressions)
}

func buildAWFExecutionCommand(
	...
	if pathSetup != "" {
		lines = append(lines, pathSetup)
	}
	lines = append(lines, preCreateLog)
	if configFileSetup != "" {
		lines = append(lines, configFileSetup)
	}
	lines = append(lines, modelsJSONPathExport, arcDindDockerHostProbe, toolCacheMountProbe)
	if arcDindPrefixProbe != "" {
		lines = append(lines, arcDindPrefixProbe)
	}
	lines = append(lines, awfShellcheckDirective)
