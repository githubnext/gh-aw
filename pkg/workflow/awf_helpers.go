...
func BuildAWFArgs(config AWFCommandConfig) []string {
	...
	if isAWFNetworkIsolationEnabled(config.WorkflowData) {
		awfHelpersLog.Print("Skipping host-access flags: sandbox.agent.root-mode is false (network isolation mode)")
	} else {
...
func GetAWFCommandPrefix(workflowData *WorkflowData) string {
	...
	// When root-mode is false (network isolation mode), AWF runs rootless: no sudo needed.
	// Strip the "sudo -E " prefix from the default command to get the base binary name.
	if isAWFNetworkIsolationEnabled(workflowData) {
		awfHelpersLog.Print("Using rootless AWF command (root-mode: false, network isolation mode)")
