...
type CreatePullRequestsConfig struct {
	...
	PreserveBranchName             bool     `yaml:"preserve-branch-name,omitempty"`                // When true, skips lowercase conversion and random salt suffix on agent-specified branch names. Invalid characters are still replaced for security. Useful when CI enforces branch naming conventions (e.g. Jira keys in uppercase).
}
