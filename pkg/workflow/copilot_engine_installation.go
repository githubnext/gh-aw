...
func generateAWFInstallationStep(version string, agentConfig *AgentSandboxConfig) GitHubActionStep {
	...
	stepLines := []string{
		...
		"          # Download and install AWF with retry logic for transient network errors",
		"          for i in {1..5}; do",
		fmt.Sprintf("            if curl -sSL https://raw.githubusercontent.com/githubnext/gh-aw-firewall/main/install.sh | sudo AWF_VERSION=%s bash; then", version),
		"              echo \"Successfully installed awf\"",
		"              break",
		"            else",
		"              if [ $i -eq 5 ]; then",
		"                echo \"Failed to install awf after 5 attempts\"",
		"                exit 1",
		"              fi",
		"              echo \"Installation failed, retrying in 2 seconds... (attempt $i/5)\"",
		"              sleep 2",
		"            fi",
		"          done",
		"          which awf",
