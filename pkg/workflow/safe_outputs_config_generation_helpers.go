...
// generateAssignToAgentConfig creates a config with optional max and default_agent
func generateAssignToAgentConfig(max int, defaultAgent string) map[string]any {
	config := make(map[string]any)
	if max > 0 {
		config["max"] = max
	}
