...
func (c *Compiler) buildDetectionJob(data *WorkflowData) (*Job, error) {
	...
	// GH_AW_COMPILED_VERSION is emitted only for release builds so that dev/CI builds
	// do not accidentally enable toolcache range matching against a stale compat window.
	if IsRelease() {
