...
func generateCacheMemoryArtifactUpload(builder *strings.Builder, data *WorkflowData) {
	...
	// Only upload artifacts when detection is enabled (needed for update_cache_memory job)
	// When detection is disabled, cache is saved automatically by actions/cache post-action
	threatDetectionEnabled := data.SafeOutputs != nil && (data.SafeOutputs.ThreatDetection != nil ||
		(data.SafeOutputs.Detection != nil && len(data.SafeOutputs.Detection.Steps) > 0))
	if !threatDetectionEnabled {
		cacheLog.Print("Skipping cache-memory artifact upload (detection disabled)")
		return
	}
