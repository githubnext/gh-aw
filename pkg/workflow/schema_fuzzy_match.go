...
// extractInvalidFieldName extracts the invalid field name from an error message
func extractInvalidFieldName(errorMsg string) string {
	// Try to extract from "Unknown property: field_name" format
	if idx := strings.Index(errorMsg, "Unknown property: "); idx != -1 {
		rest := errorMsg[idx+len("Unknown property: "):]
		// Extract until first space, period, or end of string
		for i, ch := range rest {
			if ch == ' ' || ch == '.' || ch == ',' {
				return rest[:i]
			}
	...
	// Try to extract from "Unknown properties: field1, field2" format
	if idx := strings.Index(errorMsg, "Unknown properties: "); idx != -1 {
		rest := errorMsg[idx+len("Unknown properties: "):]
		// Extract first field name (before comma or space)
		for i, ch := range rest {
			if ch == ',' || ch == ' ' || ch == '.' {
				return rest[:i]
			}
...
// extractValidFieldsFromSchema extracts the list of valid property names from a JSON schema
func extractValidFieldsFromSchema(schema map[string]interface{}) []string {
	if schema == nil {
		return nil
	}
