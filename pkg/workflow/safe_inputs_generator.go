...
func generateSafeInputJavaScriptToolScript(toolConfig *SafeInputToolConfig) string {
	...
	// Add main execution block so the script works when run as a subprocess
	sb.WriteString("// Run when executed directly (as a subprocess by the MCP handler)\n")
	sb.WriteString("if (require.main === module) {\n")
	sb.WriteString("  let inputJson = \"\";\n")
	sb.WriteString("  process.stdin.setEncoding(\"utf8\");\n")
	sb.WriteString("  process.stdin.on(\"data\", chunk => { inputJson += chunk; });\n")
	sb.WriteString("  process.stdin.on(\"end\", () => {\n")
	sb.WriteString("    let inputs = {};\n")
	sb.WriteString("    try {\n")
	sb.WriteString("      if (inputJson.trim()) {\n")
	sb.WriteString("        inputs = JSON.parse(inputJson.trim());\n")
	sb.WriteString("      }\n")
	sb.WriteString("    } catch (e) {\n")
	sb.WriteString("      process.stderr.write(\"Warning: Failed to parse inputs: \" + e.message + \"\\n\");\n")
	sb.WriteString("    }\n")
	sb.WriteString("    Promise.resolve(execute(inputs))\n")
	sb.WriteString("      .then(result => {\n")
	sb.WriteString("        process.stdout.write(JSON.stringify(result));\n")
	sb.WriteString("      })\n")
	sb.WriteString("      .catch(err => {\n")
	sb.WriteString("        process.stderr.write(String(err));\n")
