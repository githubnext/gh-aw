...
func (e *PiEngine) GetExecutionSteps(workflowData *WorkflowData, logFile string) []GitHubActionStep {
	...
	piCommand, piModelsJSONSetup := buildPiExecutionCommand(workflowData, modelConfigured, backend, profile, firewallEnabled, driverConfigured)
	command := buildPiWrappedCommand(workflowData, logFile, piCommand, modelConfigured, backend, profile, firewallEnabled)
	env := buildPiExecutionEnv(workflowData, modelConfigured, backend, profile, firewallEnabled, piModelsJSONSetup != "")
	stepLines := []string{
		"      - name: Execute Pi CLI",
...
func buildPiExecutionCommand(
	...
	piModelsJSONSetup := configurePiModelArgs(workflowData, modelConfigured, backend, profile, firewallEnabled, driverConfigured, &piArgs)
	piCommand := buildPiBaseCommand(workflowData, commandName, piArgs, driverConfigured)
	if piModelsJSONSetup != "" {
		piCommand = piModelsJSONSetup + piCommand
	}
	return getWorkspaceCommandPrefixFor(workflowData.EngineConfig) + piCommand, piModelsJSONSetup
}

func configurePiModelArgs(
	workflowData *WorkflowData,
	modelConfigured bool,
	backend UniversalLLMBackend,
	profile universalLLMBackendProfile,
	...
	if firewallEnabled {
		return configurePiFirewallModelArgs(modelID, backend, profile, driverConfigured, piArgs)
	}
	if !driverConfigured {
...
func configurePiFirewallModelArgs(
	modelID string,
	backend UniversalLLMBackend,
	profile universalLLMBackendProfile,
	driverConfigured bool,
...
func buildPiBaseCommand(workflowData *WorkflowData, commandName string, piArgs []string, driverConfigured bool) string {
	if driverConfigured {
...
%s 2>&1 | tee -a %s`, AgentCLIStartMsPath, AgentStepSummaryPath, logFile, piCommand, logFile)
	...
	allowedDomains := resolvePiAllowedDomains(workflowData, modelConfigured)
...
func resolvePiAllowedDomains(workflowData *WorkflowData, modelConfigured bool) string {
...
func buildPiExecutionEnv(
	workflowData *WorkflowData,
	modelConfigured bool,
	backend UniversalLLMBackend,
	profile universalLLMBackendProfile,
	firewallEnabled bool,
	hasModelsJSONSetup bool,
) map[string]string {
	...
	applyPiExecutionPhaseEnv(env, workflowData, firewallEnabled)
	applyPiCustomEnv(env, workflowData)
	return env
}

func applyPiExecutionPhaseEnv(env map[string]string, workflowData *WorkflowData, firewallEnabled bool) {
...
func applyPiCustomEnv(env map[string]string, workflowData *WorkflowData) {
