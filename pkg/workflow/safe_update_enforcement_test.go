...
func TestEnforceSafeUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := EnforceSafeUpdate(tt.manifest, tt.secretNames, tt.actionRefs, SafeUpdateEnforcementOptions{
...
func TestMemoryValidationScriptChangesRequireSafeUpdateReview(t *testing.T) {
	...
	err := EnforceSafeUpdate(manifest, nil, nil, SafeUpdateEnforcementOptions{
		CurrentMemoryValidationScripts: current,
	})
	require.Error(t, err)
