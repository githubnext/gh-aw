...
func (c *Compiler) generateYAML(data *WorkflowData, markdownPath string) (string, error) {
	...
	yaml.WriteString("# For more information: https://github.com/githubnext/gh-aw/blob/main/.github/aw/github-agentic-workflows.md\n")
...
func (c *Compiler) generateMainJobSteps(yaml *strings.Builder, data *WorkflowData) {
	...
	// Generate and upload git patch if needed for safe-outputs
	// Patch generation must happen in the agent job (where child_process is available),
	// not in the MCP server (where it isn't). The patch is then uploaded as an artifact
	// for safe-output jobs to download and use.
	if data.SafeOutputs != nil && (data.SafeOutputs.CreatePullRequests != nil || data.SafeOutputs.PushToPullRequestBranch != nil) {
		c.generateGitPatchGenerationStep(yaml)
...
// generateGitPatchGenerationStep generates a step that creates the git patch file
// This must run in the agent job where child_process is available (not in MCP server)
func (c *Compiler) generateGitPatchGenerationStep(yaml *strings.Builder) {
	...
	yaml.WriteString("            // The branch name will be determined from current branch or environment\n")
	yaml.WriteString("            const result = generateGitPatch(null);\n")
	yaml.WriteString("            if (result.success) {\n")
	yaml.WriteString("              core.info(`Patch generated: ${result.patchPath} (${result.patchSize} bytes, ${result.patchLines} lines)`);\n")
	yaml.WriteString("            } else {\n")
	yaml.WriteString("              core.info(`No patch generated: ${result.error || 'No changes found'}`);\n")
	yaml.WriteString("            }\n")
}

// generateGitPatchUploadStep generates a step that uploads a git patch artifact
// The patch is generated in a previous step (generateGitPatchGenerationStep)
