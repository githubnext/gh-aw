...
func (c *Compiler) generateYAML(data *WorkflowData, markdownPath string) (string, error) {
	...
	// Add metadata block
	dependencies := append(data.ImportedFiles, data.IncludedFiles...)
	metadataComment, err := GenerateMetadataComment(markdownPath, c.version, dependencies)
	if err != nil {
		compilerYamlLog.Printf("Warning: Failed to generate metadata: %v", err)
	} else {
		yaml.WriteString(metadataComment)
		yaml.WriteString("#\n")
	}
