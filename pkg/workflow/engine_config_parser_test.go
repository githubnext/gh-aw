...
func TestParseIntOrExpressionValues(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "non-negative accepts zero", parse: parseNonNegativeIntOrExpressionValue, raw: 0, expected: "0"},
		{name: "non-negative accepts expression", parse: parseNonNegativeIntOrExpressionValue, raw: "${{ inputs.max_retries }}", expected: "${{ inputs.max_retries }}"},
