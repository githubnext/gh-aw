...
// safe_outputs checkout app-token steps remain valid YAML when a PR-producing
// safe output injects a shared condition onto mirrored checkout steps.
...
`
	...
	compiler := NewCompiler()
	err = compiler.CompileWorkflow(mdFile)
	require.NoError(t, err, "Regression: checkout github-app + create-pull-request should compile successfully")
