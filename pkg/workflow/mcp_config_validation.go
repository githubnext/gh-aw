...
//
// This file validates the tools: frontmatter section and orchestrates MCP server
// configuration validation. It ensures tool names are recognized built-ins and
// that custom MCP server entries have valid configurations.
//
...
//   - getRawMCPConfig() - Extracts raw MCP configuration fields for validation
//   - inferMCPType() - Infers the MCP connection type from fields present in a config map
//
// # Validation Pattern: Tools Section and MCP Config Orchestration
//
// MCP validation uses multiple patterns:
//   - Built-in tool name allowlist: Rejects unknown tool names with helpful error messages
//   - Type inference: Determines MCP type from fields if not explicit
//   - Unknown field detection: Catches typos and deprecated fields in tool configs
//
...
//   - It validates MCP server configuration orchestration
//   - It checks MCP-specific field requirements
//   - It validates MCP type compatibility
//   - It ensures MCP configuration correctness
//
// For type-specific requirements and schema validation, see mcp_schema_validation.go.
