...
func TestParseEngineModelFallback(t *testing.T) {
	t.Run("returns nil when firewall object has no apiProxy key", func(t *testing.T) {
		firewallObj := map[string]any{"other": "value"}
		got := parseEngineModelFallback(firewallObj)
		if got != nil {
			t.Errorf("expected nil, got %+v", got)
		}
	})

	t.Run("returns nil when apiProxy is not a map", func(t *testing.T) {
		firewallObj := map[string]any{"apiProxy": "not-a-map"}
		got := parseEngineModelFallback(firewallObj)
		if got != nil {
			t.Errorf("expected nil, got %+v", got)
		}
	...
	t.Run("returns nil when modelFallback is absent", func(t *testing.T) {
		firewallObj := map[string]any{
			"apiProxy": map[string]any{"otherField": true},
		}
		got := parseEngineModelFallback(firewallObj)
		if got != nil {
			t.Errorf("expected nil, got %+v", got)
	...
	t.Run("returns nil when modelFallback object is empty", func(t *testing.T) {
		firewallObj := map[string]any{
		...
		got := parseEngineModelFallback(firewallObj)
		if got != nil {
			t.Errorf("expected nil for empty modelFallback, got %+v", got)
		}
	})

	t.Run("parses enabled=false", func(t *testing.T) {
		firewallObj := map[string]any{
			"apiProxy": map[string]any{
				"modelFallback": map[string]any{
					"enabled": false,
		...
		if got.Strategy != "" {
			t.Errorf("expected empty Strategy, got %q", got.Strategy)
		}
	})

	t.Run("parses enabled=true", func(t *testing.T) {
		firewallObj := map[string]any{
			"apiProxy": map[string]any{
				"modelFallback": map[string]any{
					"enabled": true,
	...
	t.Run("parses strategy field", func(t *testing.T) {
		firewallObj := map[string]any{
			"apiProxy": map[string]any{
				"modelFallback": map[string]any{
					"enabled":  false,
		...
		if got.Strategy != "middle_power" {
			t.Errorf("expected Strategy=%q, got %q", "middle_power", got.Strategy)
		}
	})

	t.Run("returns non-nil when only strategy is set (no enabled)", func(t *testing.T) {
		firewallObj := map[string]any{
			"apiProxy": map[string]any{
				"modelFallback": map[string]any{
					"strategy": "middle_power",
		...
		got := parseEngineModelFallback(firewallObj)
		if got == nil {
			t.Fatal("expected non-nil result when strategy is set")
		}
		if got.Enabled != nil {
...
func TestExtractEngineConfigModelFallback(t *testing.T) {
	compiler := NewCompiler()

	t.Run("parses engine.firewall.apiProxy.modelFallback enabled=false", func(t *testing.T) {
		frontmatter := map[string]any{
			"engine": map[string]any{
				"id": "copilot",
		...
		if config.ModelFallback == nil {
			t.Fatal("Expected ModelFallback to be set")
		}
		if config.ModelFallback.Enabled == nil {
			t.Fatal("Expected ModelFallback.Enabled to be non-nil")
		}
		if *config.ModelFallback.Enabled != false {
			t.Errorf("Expected ModelFallback.Enabled=false, got %v", *config.ModelFallback.Enabled)
		}
	})

	t.Run("parses engine.firewall.apiProxy.modelFallback with strategy", func(t *testing.T) {
		frontmatter := map[string]any{
			"engine": map[string]any{
				"id": "copilot",
				"firewall": map[string]any{
		...
		if config.ModelFallback.Strategy != "middle_power" {
			t.Errorf("Expected Strategy=%q, got %q", "middle_power", config.ModelFallback.Strategy)
		}
	})

	t.Run("ModelFallback is nil when engine.firewall is absent", func(t *testing.T) {
		frontmatter := map[string]any{
			"engine": map[string]any{
				"id": "copilot",
			},
		...
		if config.ModelFallback != nil {
			t.Errorf("Expected ModelFallback to be nil, got %+v", config.ModelFallback)
		}
	})
}
