...
func (c *Compiler) generateMCPSetup(yaml *strings.Builder, tools map[string]any, engine CodingAgentEngine, workflowData *WorkflowData) {
	...
	containerCmd := fmt.Sprintf("docker run -i --rm -p %d:%d", port, DefaultMCPGatewayContainerPort)
	...
	// MCP_GATEWAY_PORT inside container must match the container port (right side of -p mapping)
	containerCmd += fmt.Sprintf(" -e MCP_GATEWAY_PORT=%d", DefaultMCPGatewayContainerPort)
