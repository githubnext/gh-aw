...
// collectMCPEnvironmentVariables collects all environment variables needed by MCP servers
// This function centralizes the logic for collecting secrets and variables used by both
// the "Setup MCPs" step and the MCP gateway steps
...
func (c *Compiler) generateMCPSetup(yaml *strings.Builder, tools map[string]any, engine CodingAgentEngine, workflowData *WorkflowData) {
	...
	// Collect MCP environment variables (used by both Setup MCPs and gateway steps)
	...
	// Pass the same environment variables to the gateway
