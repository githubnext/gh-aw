...
func (c *Compiler) validateNetworkAllowedDomains(network *NetworkPermissions) error {
	...
	for i, domain := range network.Allowed {
		if err := validateNetworkAllowedDomain(domain, i); err != nil {
...
func validateNetworkAllowedDomain(domain string, index int) error {
	if domain == "*" {
...
func validateDomainPattern(domain string) error {
	...
	domainWithoutProtocol, err := stripAllowedDomainProtocol(domain)
	if err != nil {
		return err
	}

	if err := validateDomainWildcardPattern(domain, domainWithoutProtocol); err != nil {
		return err
	}

	// Validate domain pattern format (without protocol)
	if !domainPattern.MatchString(domainWithoutProtocol) {
		return domainPatternFormatError(domain, domainWithoutProtocol)
	}

	return nil
}

func stripAllowedDomainProtocol(domain string) (string, error) {
...
func validateDomainWildcardPattern(domain, domainWithoutProtocol string) error {
	if domainWithoutProtocol == "*" {
		return NewValidationError("domain", domain, "wildcard-only domain '*' is not allowed", "Use a specific wildcard pattern with a base domain. Examples:\n  - '*.example.com'\n  - '*.github.com'\n  - 'https://*.api.example.com'")
	}
	if domainWithoutProtocol == "*." {
		return NewValidationError("domain", domain, "wildcard pattern must have a domain after '*.'", "Add a base domain after the wildcard. Examples:\n  - '*.example.com'\n  - '*.github.com'\n  - 'https://*.api.example.com'")
	}
	if strings.Count(domainWithoutProtocol, "*") > 1 {
		return NewValidationError("domain", domain, "domain pattern contains multiple wildcards, only one wildcard at the start is allowed", "Use a single wildcard at the start of the domain. Examples:\n  - '*.example.com' ✓\n  - '*.*.example.com' ✗ (multiple wildcards)\n  - 'https://*.github.com' ✓")
	}
	if strings.Contains(domainWithoutProtocol, "*") && !strings.HasPrefix(domainWithoutProtocol, "*.") {
		return NewValidationError("domain", domain, "wildcard must be at the start followed by a dot", "Move the wildcard to the beginning of the domain. Examples:\n  - '*.example.com' ✓\n  - 'example.*.com' ✗ (wildcard in middle)\n  - 'https://*.github.com' ✓")
	}
	return validateWildcardBaseDomain(domain, domainWithoutProtocol)
}

func validateWildcardBaseDomain(domain, domainWithoutProtocol string) error {
	if !strings.HasPrefix(domainWithoutProtocol, "*.") {
		return nil
	}
	baseDomain := domainWithoutProtocol[2:]
	if baseDomain == "" {
		return NewValidationError("domain", domain, "wildcard pattern must have a domain after '*.'", "Add a base domain after the wildcard. Examples:\n  - '*.example.com'\n  - '*.github.com'\n  - 'https://*.api.example.com'")
	}
	if strings.HasPrefix(baseDomain, ".") {
		return NewValidationError("domain", domain, "wildcard pattern has invalid format (extra dot after wildcard)", "Use correct wildcard format. Examples:\n  - '*.example.com' ✓\n  - '*.*.example.com' ✗ (extra dot)\n  - 'https://*.github.com' ✓")
	}
	return nil
}

func domainPatternFormatError(domain, domainWithoutProtocol string) error {
	if strings.HasSuffix(domainWithoutProtocol, ".") {
		return NewValidationError("domain", domain, "domain pattern cannot end with a dot", "Remove the trailing dot from the domain. Examples:\n  - 'example.com' ✓\n  - 'example.com.' ✗\n  - '*.github.com' ✓")
	}
	if strings.Contains(domainWithoutProtocol, "..") {
		return NewValidationError("domain", domain, "domain pattern cannot contain consecutive dots", "Remove extra dots from the domain. Examples:\n  - 'api.example.com' ✓\n  - 'api..example.com' ✗\n  - 'sub.api.example.com' ✓")
	}
	if strings.HasPrefix(domainWithoutProtocol, ".") && !strings.HasPrefix(domainWithoutProtocol, "*.") {
		return NewValidationError("domain", domain, "domain pattern cannot start with a dot (except for wildcard patterns)", "Remove the leading dot or use a wildcard. Examples:\n  - 'example.com' ✓\n  - '.example.com' ✗\n  - '*.example.com' ✓")
	}
	if err := invalidDomainCharacterError(domain, domainWithoutProtocol); err != nil {
		return err
	}
	return NewValidationError("domain", domain, "domain pattern is not a valid domain format", "Use a valid domain format. Examples:\n  - Plain: 'github.com', 'api.example.com'\n  - Wildcard: '*.github.com', '*.example.com'\n  - With protocol: 'https://api.github.com'")
}

func invalidDomainCharacterError(domain, domainWithoutProtocol string) error {
