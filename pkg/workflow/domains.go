...
// Mirrors the provider-aware routing that Pi can use via the AWF LLM gateway.
...
// Engines with model-specific defaults (for example, Pi) are resolved in
...
// Pi domains are model/provider-specific, so they are
...
// For engines with model/provider-specific defaults (such as Pi), pass the
...
// Returns a deduplicated, sorted, comma-separated string suitable for AWF's --allow-domains flag.
// Falls back to an empty default domain list for unknown engines.
// For model/provider-specific engines such as Pi, prefer
