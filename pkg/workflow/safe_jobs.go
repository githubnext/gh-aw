...
func (c *Compiler) buildSafeJobs(data *WorkflowData, threatDetectionEnabled bool) ([]string, error) {
	...
	for jobName, jobConfig := range data.SafeOutputs.Jobs {
		...
		steps = append(steps, "      - name: Configure Safe Outputs Environment Variables\n")
		...
		// "Configure Safe Outputs Environment Variables" step output so user steps can access them.
