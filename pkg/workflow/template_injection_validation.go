...
var (
	...
	// Pattern explanation:
	//   ^\s+run:\s*\|(?:-|)\s*\n((?:[ \t]+.+\n?)+?)\s*(?:^[ \t]*(?:-\s|[a-z-]+:)|\z) - matches multi-line block scalar (run: | or run: |-)
	//     - Matches both run: | (keep) and run: |- (strip) indicators
	//     - Stops at next step (^[ \t]*-\s), next YAML key (^[ \t]*[a-z-]+:), or end of string (\z)
	//   | - OR
