...
func ValidateActionSHAsInLockFile(lockFilePath string, cache *ActionCache, verbose bool) error {
	...
	if err := ValidateDistinctActionSHAs(actions); err != nil {
		return fmt.Errorf("action SHA integrity check failed: %w. Run 'gh aw update-actions' and recompile. If the issue persists, inspect .github/aw/actions-lock.json for incorrect pins", err)
	}
