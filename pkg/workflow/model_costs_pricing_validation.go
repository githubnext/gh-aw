...
func validateDefaultAiCreditsPricing(workflowData *WorkflowData) error {
	...
	if p.Input < 0 {
		return fmt.Errorf("models.default-ai-credits-pricing: input must be a non-negative value (got %g)", p.Input)
	}
	if p.Output < 0 {
		return fmt.Errorf("models.default-ai-credits-pricing: output must be a non-negative value (got %g)", p.Output)
	}
	if p.CachedInput != nil && *p.CachedInput < 0 {
		return fmt.Errorf("models.default-ai-credits-pricing: cache_read must be a non-negative value when set (got %g)", *p.CachedInput)
	}
	if p.CacheWrite != nil && *p.CacheWrite < 0 {
		return fmt.Errorf("models.default-ai-credits-pricing: cache_write must be a non-negative value when set (got %g)", *p.CacheWrite)
	}
