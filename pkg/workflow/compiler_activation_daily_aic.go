...
func (c *Compiler) buildActivationDailyAICGuardrailStep(data *WorkflowData) []string {
	...
	var steps []string
	steps = append(steps, c.buildDailyAICTokenAndCacheSteps(data)...)
	steps = append(steps, c.buildDailyAICGuardrailCheckStep(data)...)
	return steps
}

func (c *Compiler) buildDailyAICTokenAndCacheSteps(data *WorkflowData) []string {
	var steps []string
	if data.MaxDailyAICreditsGitHubApp != nil {
...
func (c *Compiler) buildDailyAICCacheRestoreSteps(data *WorkflowData) []string {
	...
	token := c.resolveDailyAICToken(data)
	return []string{
...
func (c *Compiler) buildDailyAICGuardrailCheckStep(data *WorkflowData) []string {
	token := c.resolveDailyAICToken(data)
	steps := []string{
