...
func generateAgenticWorkflowsInstallStep(c *Compiler, yaml *strings.Builder, hasAgenticWorkflows bool, workflowData *WorkflowData) {
	...
	yaml.WriteString("          if [ -z \"$GH_AW_BIN\" ] && [ -x \"${RUNNER_TEMP}/gh-aw/bin/gh-aw\" ]; then\n")
	yaml.WriteString("            GH_AW_BIN=\"${RUNNER_TEMP}/gh-aw/bin/gh-aw\"\n")
	yaml.WriteString("          fi\n")
	yaml.WriteString("          if [ -z \"$GH_AW_BIN\" ]; then\n")
	yaml.WriteString("            GH_AW_BIN=$(find \"${RUNNER_TEMP}/gh-aw/bin\" -maxdepth 1 -type f -name 'gh-aw*' -perm -111 2>/dev/null | head -1) || true\n")
	yaml.WriteString("          fi\n")
	yaml.WriteString("          if [ -z \"$GH_AW_BIN\" ]; then\n")
	yaml.WriteString("            GH_AW_BIN=$(find \"${HOME}/.local/share/gh/extensions/gh-aw\" -name 'gh-aw*' -type f -perm -111 2>/dev/null | head -1) || true\n")
	yaml.WriteString("          fi\n")
	yaml.WriteString("          if [ -z \"$GH_AW_BIN\" ] && [ -n \"${GH_CONFIG_DIR:-}\" ]; then\n")
	yaml.WriteString("            GH_AW_BIN=$(find \"${GH_CONFIG_DIR}/extensions/gh-aw\" -name 'gh-aw*' -type f -perm -111 2>/dev/null | head -1) || true\n")
	yaml.WriteString("          fi\n")
