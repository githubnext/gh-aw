...
//
// Side effects: acc.mergedMaxTurns, acc.mergedMaxToolFailure, acc.mergedMaxRuns, acc.mergedMaxEffectiveTokens,
...
func (acc *importAccumulator) extractConfigFields(fm map[string]any, fullPath string) {
	...
	acc.extractFirstWinsJSONField(fm, fullPath, "max-tool-failure", &acc.mergedMaxToolFailure)
...
func (acc *importAccumulator) toImportsResult(topologicalOrder []string) *ImportsResult {
	...
	return &ImportsResult{
		...
		MergedMaxToolFailure:          acc.mergedMaxToolFailure,
