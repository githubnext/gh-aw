...
func downloadFileFromGitHub(owner, repo, path, ref string) ([]byte, error) {
	// Use gh CLI to download the file with proper token resolution
	cmd := ghcli.ExecGH("api", fmt.Sprintf("/repos/%s/%s/contents/%s?ref=%s", owner, repo, path, ref), "--jq", ".content")
