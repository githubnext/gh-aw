...
func GetGitHubHost() string {
	...
	for _, envVar := range envVars {
		if value := os.Getenv(envVar); value != "" { //nolint:osgetenvlibrary // GitHub host detection intentionally honors standard host env overrides.
...
func GetGitHubToken() (string, error) {
	...
	if token := os.Getenv("GH_TOKEN"); token != "" { //nolint:osgetenvlibrary // gh CLI auth intentionally honors the user's GH_TOKEN environment.
