...
Defines custom model cost information for legacy ET computation and AI Credits cost ratios. Specified under `engine.token-weights` in workflow frontmatter and stored in `aw_info.json` at runtime.
...
Per-token-class weights for legacy ET computation. Each field corresponds to one token class; a zero value means "use the default weight".
