...
var typeLog = logger.New("typeutil:convert")
...
func ParseIntValue(value any) (int, bool) {
	...
	case uint64:
		...
		if v > uint64(maxInt) {
			typeLog.Printf("uint64 value %d exceeds max int value, returning 0", v)
			return 0, false
		}
		return int(v), true
	case float64:
		...
		if v != float64(intVal) {
			typeLog.Printf("Float value %.2f truncated to integer %d", v, intVal)
		}
		return intVal, true
	default:
		return 0, false
