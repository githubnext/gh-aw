...
func tryDynamicActionResolution(actionRepo, version string, ctx *PinContext, isAlreadySHA bool) (string, bool) {
	...
	return formatPinnedActionWithResolution(actionRepo, sha, version, resolvedVersion), true
}

func resolveFromHardcodedPins(actionRepo, version string, ctx *PinContext, isAlreadySHA bool, matchingPins []ActionPin) (string, bool) {
	...
	if isAlreadySHA {
		...
		return FormatPinnedActionReference(actionRepo, version, version), true
	}
	...
	actionPinsLog.Printf("Using version in non-strict mode: %s@%s (requested) → %s@%s (used)",
		actionRepo, version, actionRepo, selectedPin.Version)
	return formatPinnedActionWithResolution(actionRepo, selectedPin.SHA, version, selectedPin.Version), true
}

func resolveMissingActionPin(actionRepo, version string, ctx *PinContext, isAlreadySHA bool) (string, error) {
	if isAlreadySHA {
		actionPinsLog.Printf("SHA %s not found in hardcoded pins, returning as-is", version)
...
func ResolveActionPin(actionRepo, version string, ctx *PinContext) (string, error) {
	...
	if resolved, ok := tryDynamicActionResolution(actionRepo, version, ctx, isAlreadySHA); ok {
		actionPinsLog.Printf("Returning pinned reference: %s", resolved)
		return resolved, nil
	}
