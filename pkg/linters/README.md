...
| `trimleftright` | Custom `go/analysis` analyzer that flags likely mistaken `strings.TrimLeft`/`TrimRight` calls using a multi-character literal cutset where `TrimPrefix`/`TrimSuffix` was likely intended |
