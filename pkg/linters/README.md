...
- `trimleftright` — reports `strings.TrimLeft` / `strings.TrimRight` calls with a multi-character literal cutset where `strings.TrimPrefix` / `strings.TrimSuffix` was likely intended.
...
| `trimleftright` | Custom `go/analysis` analyzer that flags `strings.TrimLeft` / `strings.TrimRight` calls with a multi-character literal cutset where `strings.TrimPrefix` / `strings.TrimSuffix` was likely intended |
