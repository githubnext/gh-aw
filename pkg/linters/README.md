...
- `trimleftright` — flags `strings.TrimLeft`/`TrimRight` calls with a multi-character literal cutset where `strings.TrimPrefix`/`TrimSuffix` was likely intended.
...
| `trimleftright` | Custom `go/analysis` analyzer that flags `strings.TrimLeft`/`TrimRight` calls with a multi-character literal cutset where `strings.TrimPrefix`/`TrimSuffix` was likely intended |
