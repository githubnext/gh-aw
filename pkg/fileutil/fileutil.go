...
var fileLog = logger.New("fileutil:fileutil")
...
func ValidateAbsolutePath(path string) (string, error) {
	...
	if path == "" {
		fileLog.Print("ValidateAbsolutePath: rejected empty path")
	...
	if !filepath.IsAbs(cleanPath) {
		fileLog.Printf("ValidateAbsolutePath: rejected relative path: %s", path)
		return "", fmt.Errorf("path must be absolute, got: %s", path)
	}

	fileLog.Printf("ValidateAbsolutePath: validated path: %s", cleanPath)
...
func ValidatePathWithinBase(base, candidate string) error {
	fileLog.Printf("ValidatePathWithinBase: checking candidate=%q within base=%q", candidate, base)
	...
	if err != nil || !filepath.IsLocal(rel) {
		fileLog.Printf("ValidatePathWithinBase: path escape detected: candidate=%q base=%q", candidate, base)
		return fmt.Errorf("path %q escapes base directory %q", candidate, base)
	}
	fileLog.Printf("ValidatePathWithinBase: path is safe: candidate=%q (rel=%s) within base=%q", candidate, rel, base)
	return nil
}
	...
	defer func() {
		...
		if removePartial {
			if removeErr := os.Remove(dst); removeErr != nil {
				fileLog.Printf("Failed to remove partial destination file during cleanup: %s", removeErr)
			}
		}
	}()
...
func CopyFile(src, dst string) error {
	fileLog.Printf("Copying file: src=%s, dst=%s", src, dst)
	in, err := os.Open(src)
	if err != nil {
		fileLog.Printf("Failed to open source file: %s", err)
		return err
	}
	defer in.Close()
	...
	if err != nil {
		fileLog.Printf("Failed to create destination file: %s", err)
		return err
	}
	...
	fileLog.Printf("File copied successfully: src=%s, dst=%s", src, dst)
	return nil
}
