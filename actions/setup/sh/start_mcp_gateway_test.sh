...
test_env_var_validation() {
  ...
  if ! MCP_GATEWAY_DOMAIN="localhost" MCP_GATEWAY_API_KEY="test-key" MCP_GATEWAY_DOCKER_COMMAND="docker run -i --rm -p 80:8000 test-image" bash "$SCRIPT_PATH" 2>/dev/null; then
  ...
  if ! MCP_GATEWAY_PORT="8080" MCP_GATEWAY_API_KEY="test-key" MCP_GATEWAY_DOCKER_COMMAND="docker run -i --rm -p 80:8000 test-image" bash "$SCRIPT_PATH" 2>/dev/null; then
  ...
  if ! MCP_GATEWAY_PORT="8080" MCP_GATEWAY_DOMAIN="localhost" MCP_GATEWAY_DOCKER_COMMAND="docker run -i --rm -p 80:8000 test-image" bash "$SCRIPT_PATH" 2>/dev/null; then
...
test_config_not_found() {
  ...
  if ! MCP_GATEWAY_PORT="8080" MCP_GATEWAY_DOMAIN="localhost" MCP_GATEWAY_API_KEY="test-key" MCP_GATEWAY_DOCKER_COMMAND="docker run -i --rm -p 80:8000 test-image" bash "$test_script" < /dev/null 2>/dev/null; then
...
test_container_missing_docker_run() {
  ...
  echo '{"mcpServers":{},"gateway":{"port":80,"domain":"localhost","apiKey":"test-key"}}' > "$fake_home/.copilot/mcp-config.json"
  ...
  if ! MCP_GATEWAY_PORT="8080" MCP_GATEWAY_DOMAIN="localhost" MCP_GATEWAY_API_KEY="test-key" MCP_GATEWAY_DOCKER_COMMAND="test-image" bash "$test_script" < /dev/null 2>/dev/null; then
...
test_container_missing_i_flag() {
  ...
  if ! MCP_GATEWAY_PORT="8080" MCP_GATEWAY_DOMAIN="localhost" MCP_GATEWAY_API_KEY="test-key" MCP_GATEWAY_DOCKER_COMMAND="docker run --rm -p 80:8000 test-image" bash "$test_script" < /dev/null 2>/dev/null; then
...
test_container_missing_rm_flag() {
  ...
  if ! MCP_GATEWAY_PORT="8080" MCP_GATEWAY_DOMAIN="localhost" MCP_GATEWAY_API_KEY="test-key" MCP_GATEWAY_DOCKER_COMMAND="docker run -i -p 80:8000 test-image" bash "$test_script" < /dev/null 2>/dev/null; then
...
# Test 8: Container missing required -p port mapping flag
test_container_missing_port_flag() {
  echo ""
  echo "Test 8: Container missing required -p port mapping flag"
  ...
  # Test with container missing -p port mapping flag
  if ! MCP_GATEWAY_PORT="80" MCP_GATEWAY_DOMAIN="localhost" MCP_GATEWAY_API_KEY="test-key" MCP_GATEWAY_DOCKER_COMMAND="docker run -i --rm test-image" bash "$test_script" < /dev/null 2>/dev/null; then
    print_result "Script rejects container without -p port mapping flag" "PASS"
  else
    print_result "Script should reject container without -p port mapping flag" "FAIL"
  fi
...
test_validation_functions_exist() {
  ...
  # Check for gateway section validation (replaces config file check since stdin is used)
  if grep -q "missing required 'gateway' section" "$SCRIPT_PATH"; then
    print_result "Gateway section validation exists" "PASS"
  else
    print_result "Gateway section validation missing" "FAIL"
  fi
  ...
  # Check for -p port mapping validation
  if grep -q "must include -p PORT:PORT flag" "$SCRIPT_PATH"; then
    print_result "-p port mapping flag validation exists" "PASS"
  else
    print_result "-p port mapping flag validation missing" "FAIL"
  fi
}
