...
if [ "${RUN_DETECTION:-false}" = "true" ] && [ ! -f "${RESULT_FILE}" ]; then
  ...
  if [ -f "${DETECTION_LOG_FILE}" ]; then
    detection_status="$(grep 'THREAT_DETECTION_STATUS:' "${DETECTION_LOG_FILE}" | tail -n 1 || true)"
  fi
  ...
  if [ -n "${detection_status}" ]; then
    ...
    result_message="${result_message}; detection log exists at ${DETECTION_LOG_FILE} but did not include THREAT_DETECTION_STATUS"
  else
