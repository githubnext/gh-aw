...
cd /tmp/gh-aw/safeoutputs || exit 1
...
if [ ! -f mcp-server.cjs ]; then
  echo "ERROR: mcp-server.cjs not found in /tmp/gh-aw/safeoutputs"
  ls -la /tmp/gh-aw/safeoutputs/
  exit 1
fi
if [ ! -f config.json ]; then
  echo "ERROR: config.json not found in /tmp/gh-aw/safeoutputs"
  ls -la /tmp/gh-aw/safeoutputs/
  exit 1
fi
if [ ! -f tools.json ]; then
  echo "ERROR: tools.json not found in /tmp/gh-aw/safeoutputs"
  ls -la /tmp/gh-aw/safeoutputs/
  exit 1
fi
...
if [ ${#MISSING_FILES[@]} -gt 0 ]; then
  echo "ERROR: Missing required dependency files in /tmp/gh-aw/safeoutputs/"
...
mkdir -p /tmp/gh-aw/safeoutputs/logs
...
# Create initial server.log file for artifact upload
{
  ...
  echo "==========================================="
  echo ""
} > /tmp/gh-aw/safeoutputs/logs/server.log

# Start the HTTP server in the background
echo "Starting safe-outputs MCP HTTP server..."
node mcp-server.cjs >> /tmp/gh-aw/safeoutputs/logs/server.log 2>&1 &
SERVER_PID=$!
...
for i in {1..10}; do
  ...
  if ! kill -0 $SERVER_PID 2>/dev/null; then
    ...
    cat /tmp/gh-aw/safeoutputs/logs/server.log
    exit 1
  fi
