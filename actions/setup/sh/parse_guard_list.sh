...
# blocked-users, trusted-users, approval-labels, and disapproval-labels, merges them,
# validates each item, and writes the resulting JSON arrays to $GITHUB_OUTPUT for use in
# the MCP gateway config step.
#
...
#   GH_AW_DISAPPROVAL_LABELS_EXTRA  - Static items or user-expression value for disapproval-labels
#   GH_AW_DISAPPROVAL_LABELS_VAR    - Value of vars.GH_AW_GITHUB_DISAPPROVAL_LABELS (fallback)
#
...
#   disapproval_labels - JSON array, e.g. ["needs-work"] or []
#
# Exit codes:
...
DISAPPROVAL_INPUT=$(combine_inputs "${GH_AW_DISAPPROVAL_LABELS_EXTRA:-}" "${GH_AW_DISAPPROVAL_LABELS_VAR:-}")
...
disapproval_labels_json=$(parse_list "$DISAPPROVAL_INPUT" "disapproval-labels")
...
echo "disapproval_labels=${disapproval_labels_json}" >> "$GITHUB_OUTPUT"
...
echo "  disapproval-labels: ${disapproval_labels_json}"
