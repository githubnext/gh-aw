...
# e.g. Squid runs as UID 13 inside a container, writing files to the host-mounted volume).
...
# This targeted cleanup is defense-in-depth: it removes the specific directory that AWF's
# writeConfigs step will try to populate, so AWF can always create it fresh as the runner user.
if [ -d /tmp/gh-aw/sandbox/firewall ] && [ ! -w /tmp/gh-aw/sandbox/firewall ]; then
