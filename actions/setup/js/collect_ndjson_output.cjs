...
async function main() {
  try {
    ...
    const writeValidatedOutput = validatedOutput => {
    ...
    const emitFallbackNoop = reason => {
      if (!shouldEmitFallbackNoop) {
        return false;
      }
      core.warning(`[INGESTION] ${reason}. Emitting fallback noop output.`);
      const fallbackOutput = {
        items: [{ type: "noop", message: "No safe outputs were generated by the agent; auto-emitted noop." }],
        errors: [],
      };
      writeValidatedOutput(fallbackOutput);
      core.setOutput("raw_output", "");
      core.setOutput("output_types", "noop");
    ...
    if (!fs.existsSync(outputFile)) {
      ...
      if (emitFallbackNoop(`Output file does not exist: ${outputFile}`)) {
        return;
      }
      core.setOutput("output", "");
      core.setOutput("output_types", "");
    ...
    if (parsedItems.length === 0 && errors.length === 0 && shouldEmitFallbackNoop) {
      core.warning("[INGESTION] No valid safe outputs were generated. Emitting fallback noop output.");
      parsedItems.push({
        type: "noop",
        message: "No safe outputs were generated by the agent; auto-emitted noop.",
      });
    }
    ...
    writeValidatedOutput(validatedOutput);
