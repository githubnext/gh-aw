      ...
      try {
        ...
        if (issueIntentStrict) {
          ...
          if (missingMetadataLabels.length > 0) {
            const error = `Label objects must include both "rationale" and "confidence" when issue_intent is explicitly enabled. Missing metadata on: ${missingMetadataLabels.map(l => JSON.stringify(typeof l === "string" ? l : l.name)).join(", ")}`;
            core.warning(error);
            return { success: false, error };
          }
        }
