...
const { normalizeTitleForDedup, findDuplicateByTitle, parseCreateIssueTitleDedupRolloutPercent, getCreateIssueDedupWorkflowSeed, resolveDeduplicateByTitle, dedupRolloutBucket } = require("./issue_title_dedup.cjs");
...
function createHandlers(server, appendSafeOutput, config = {}) {
  ...
  const dedupWorkflowSeed = getCreateIssueDedupWorkflowSeed();
  ...
  try {
    deduplicateByTitle = resolveDeduplicateByTitle(createIssueConfig.deduplicate_by_title, dedupWorkflowSeed, dedupRolloutPercent);
  } catch (error) {
    throw new Error(`${ERR_VALIDATION}: ${getErrorMessage(error)}`);
  }
  if (createIssueConfig.deduplicate_by_title === undefined && dedupWorkflowSeed && dedupRolloutPercent > 0) {
    const rolloutBucket = dedupRolloutBucket(dedupWorkflowSeed);
