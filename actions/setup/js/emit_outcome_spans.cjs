...
async function main() {
  ...
  for (const eval_ of evaluations) {
    ...
    const attributes = [buildAttr("gh-aw.outcome.type", type), buildAttr("gh-aw.outcome.result", result), buildAttr("gh-aw.outcome.workflow", workflow), buildAttr("gh-aw.outcome.run_id", sourceRunId), buildAttr("gh-aw.outcome.repo", repo)];
  ...
  const payload = buildOTLPBatchPayload({
    serviceName,
    scopeVersion: "outcome-collector",
    resourceAttributes,
    spans: allSpans,
  });
