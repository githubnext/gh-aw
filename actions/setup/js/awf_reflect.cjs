...
function resolveProviderEndpointFromReflect(options) {
  ...
  const reflectRecord = reflectData && typeof reflectData === "object" ? /** @type {{ endpoints?: unknown }} */ (reflectData) : null;
  const endpointCandidates = Array.isArray(reflectRecord?.endpoints) ? /** @type {unknown[]} */ (reflectRecord.endpoints) : [];
  const endpoints = /** @type {any[]} */ (endpointCandidates.filter(/** @param {any} ep */ ep => ep && ep.configured === true));
