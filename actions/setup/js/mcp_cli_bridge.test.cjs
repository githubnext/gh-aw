...
import { ensureSafeOutputsTools, formatResponse, getToolCallTimeoutMs, hasStdinJsonPayload, parseToolArgs, readStdinSync, shouldShowToolHelpForEmptyArgs, showHelp, showToolHelp, unescapeCliStringArg, writeStdoutAndFlush } from "./mcp_cli_bridge.cjs";
...
describe("mcp_cli_bridge.cjs", () => {
  ...
  describe("parseToolArgs — string escape unescaping", () => {
    it("unescapes \\n in string-typed CLI flag arguments", () => {
      const schemaProperties = { body: { type: "string" } };
      const { args } = parseToolArgs(["--body", "Hello\\nWorld"], schemaProperties);
      expect(args).toEqual({ body: "Hello\nWorld" });
    });

    it("unescapes \\n in string-typed --key=value arguments", () => {
      const schemaProperties = { body: { type: "string" } };
      const { args } = parseToolArgs(["--body=Hello\\nWorld"], schemaProperties);
      expect(args).toEqual({ body: "Hello\nWorld" });
    });

    it("unescapes \\n when type is ['string', 'null']", () => {
      const schemaProperties = { body: { type: ["string", "null"] } };
      const { args } = parseToolArgs(["--body", "line1\\nline2"], schemaProperties);
      expect(args).toEqual({ body: "line1\nline2" });
    });
    ...
    it("produces actual newlines matching JSON stdin mode behaviour", () => {
