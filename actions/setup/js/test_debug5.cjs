  ...
  if (potentialClosers.length > 0) {
    ...
    if (firstCloser.hasOpenerBetween) {
      ...
      if (directClosers.length > 1) {
        console.log("  → CHECK FOR TRUE NESTING");
        const hasAnyIndirectClosers = potentialClosers.some(c => c.hasOpenerBetween);
        console.log("  hasAnyIndirectClosers: " + hasAnyIndirectClosers);
        if (!hasAnyIndirectClosers) {
          console.log("  → ESCAPE (true nesting - all closers are direct)");
        } else {
          console.log("  → PAIR WITH FIRST (some closers might be for outer blocks)");
        }
