...
async function linearGraphQL(query, variables, token = process.env.GH_AW_LINEAR_TOKEN) {
  ...
  if (Array.isArray(payload.errors) && payload.errors.length > 0) {
    ...
    if (message.toLowerCase().includes(LINEAR_TEAM_NOT_FOUND_MESSAGE)) {
      throw new Error(`${ERR_CONFIG}: Linear could not access the configured team. Verify LINEAR_TEAM_ID or safe-outputs.linear-create-issue.team-id references a team in the workspace authorized by LINEAR_API_KEY`);
    }
