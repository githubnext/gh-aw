...
describe("messages.cjs", () => {
  ...
  describe("getFooterMessage", () => {
    ...
    it("should prepend reduced model identifier to effective tokens when engine model is set", async () => {
      ...
      expect(result).toBe("> Generated by [Test Workflow](https://github.com/test/repo/actions/runs/123) · ● son46 12.5K");
    });

    it("should prefer GH_AW_EFFECTIVE_TOKENS_MODEL over GH_AW_ENGINE_MODEL for the ET model prefix", async () => {
      process.env.GH_AW_EFFECTIVE_TOKENS = "7100000";
      // User configured an alias "agent" but the actual model is claude-sonnet-4-6-20250514
      process.env.GH_AW_ENGINE_MODEL = "agent";
      process.env.GH_AW_EFFECTIVE_TOKENS_MODEL = "claude-sonnet-4-6-20250514";
      ...
      // Should show "son46" (from actual model) not "age00" (from alias "agent")
      expect(result).toBe("> Generated by [Test Workflow](https://github.com/test/repo/actions/runs/123) · ● son46 7.1M");
      expect(result).not.toContain("age00");
    });

    it("should fall back to GH_AW_ENGINE_MODEL when GH_AW_EFFECTIVE_TOKENS_MODEL is absent", async () => {
      process.env.GH_AW_EFFECTIVE_TOKENS = "5000";
      process.env.GH_AW_ENGINE_MODEL = "claude-opus-4.7";
      delete process.env.GH_AW_EFFECTIVE_TOKENS_MODEL;
      ...
      expect(result).toBe("> Generated by [Test Workflow](https://github.com/test/repo/actions/runs/123) · ● opu47 5K");
    });
