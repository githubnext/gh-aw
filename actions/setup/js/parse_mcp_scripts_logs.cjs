...
function generatePlainTextSummary(logEntries) {
  ...
  for (const entry of logEntries) {
    ...
    const invokingMatch = message.match(/\[\s*([^\]]+)\s*\]\s+Invoking handler with args:\s*(\{.*\})$/i);
    if (invokingMatch) {
    ...
    const backendToolMatch = message.match(/toolName=([^\s,]+),\s+args=map\[args:(.+)\]$/i);
    if (backendToolMatch) {
    ...
    const serializedResultMatch = message.match(/\[\s*([^\]]+)\s*\]\s+Serialized result:\s*(.+)$/i);
