 ...
 * Copilot SDK Runner
 *
 * Provides the `runWithCopilotSDK` helper function and its supporting utilities.
 * Extracted into a dedicated file so it can be safely reused by both
 * copilot_sdk_driver.cjs and detection_job_driver.cjs without either file
 * depending on the other.
 *
 * The function connects to an already-running headless Copilot CLI server,
 * creates a session, sends a prompt, waits for the session to go idle, and
 * returns a result shape that mirrors what runProcess() returns so that callers
...
module.exports = { runWithCopilotSDK };
