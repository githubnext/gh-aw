 ...
 * (when preserve-branch-name and reuse-existing-ref are enabled, by force-deleting
 ...
 *
 * The "force-delete then recreate" semantic is gated behind `reuse-existing-ref`
 ...
 * users intend by enabling `reuse-existing-ref` on a reusable branch.
 *
 * When `preserve-branch-name: true` but `reuse-existing-ref: false` (default),
 ...
 * @param {boolean} [options.reuseExistingRef] - Whether reuse-existing-ref is enabled.
 ...
 *   existing remote ref when reuse-existing-ref is enabled.
...
async function handleRemoteBranchCollision(branchName, preserveBranchName, options = {}) {
  ...
  if (preserveBranchName) {
    const { reuseExistingRef, githubClient, owner, repo } = options;
    if (!reuseExistingRef) {
      // preserve-branch-name asked us to keep the exact branch name, but
      // reuse-existing-ref is not enabled, so we cannot silently destroy the
      // existing remote ref. Surface an error so the caller falls back to the
      // configured fallback (e.g. opening an issue).
      throw new Error(
        `Remote branch "${branchName}" already exists and preserve-branch-name is enabled. ` + `Set reuse-existing-ref: true to force-delete and recreate the remote ref, or disable ` + `preserve-branch-name to allow renaming the branch.`
      );
    }
    ...
    // intended behavior when reuse-existing-ref is enabled for long-lived
    // reusable branches whose previous PR was merged.
    if (!githubClient || !owner || !repo) {
      throw new Error(
        `Remote branch "${branchName}" already exists and reuse-existing-ref is enabled, ` +
    ...
    core.warning(`Remote branch ${branchName} already exists - reusing it (reuse-existing-ref enabled, force-deleting remote ref)`);
    try {
      ...
      if (status === 422 && /Reference does not exist/i.test(message)) {
        ...
        throw new Error(`Failed to delete existing remote branch "${branchName}" for reuse with reuse-existing-ref: ${message || String(err)}`);
      }
    }
    return branchName;
  }
...
async function main(config = {}) {
  ...
  const reuseExistingRef = config.reuse_existing_ref === true;
  ...
  return async function handleCreatePullRequest(message, resolvedTemporaryIds) {
    ...
    if (hasBundleFile) {
      ...
      try {
        branchName = await handleRemoteBranchCollision(branchName, preserveBranchName, { reuseExistingRef, githubClient, owner: repoParts.owner, repo: repoParts.repo });
