...
function getFooterWorkflowRecompileMessage(ctx) {
  ...
  const effectiveTokensFormatted = effectiveTokens > 0 ? formatET(effectiveTokens) : undefined;

  // Create context with both camelCase and snake_case keys
  const templateContext = toSnakeCase({ ...ctx, agenticWorkflowUrl, effectiveTokens: effectiveTokens > 0 ? effectiveTokens : undefined, effectiveTokensFormatted });
...
function getFooterAgentFailureIssueMessage(ctx) {
  ...
  // Create context with both camelCase and snake_case keys, including computed history_link and agentic_workflow_url
  const templateContext = toSnakeCase({ ...ctx, historyLink, agenticWorkflowUrl, effectiveTokens: effectiveTokens > 0 ? effectiveTokens : undefined, effectiveTokensFormatted });
