...
async function main(config = {}) {
  ...
  try {
    deduplicateByTitle = resolveDeduplicateByTitle(config.deduplicate_by_title, dedupWorkflowSeed, dedupRolloutPercent);
  } catch (error) {
  ...
  } else if (config.deduplicate_by_title === undefined && dedupWorkflowSeed && dedupRolloutPercent > 0) {
