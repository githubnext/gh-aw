  ...
  return async function handleCreateProject(message, resolvedTemporaryIds) {
    ...
    try {
      ...
      if (configuredViews.length > 0) {
        ...
        // has been deprecated and removed from the GitHub GraphQL API. Users will need to
        // manually delete the default view if desired, or it can remain alongside custom views.
      }
