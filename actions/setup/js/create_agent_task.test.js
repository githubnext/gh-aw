  ...
  (describe("basic functionality", () => {
    (it("should handle missing environment variable", async () => {
      ...
      it("should handle no create_agent_task items", async () => {
        (createAgentOutput([{ type: "create_issue", title: "Test", body: "Content" }]), await runScript(), expect(mockCore.info).toHaveBeenCalledWith("No create-agent-task items found in agent output"));
      }));
  }),
    describe("staged mode", () => {
      (beforeEach(() => {
        ((process.env.GITHUB_AW_SAFE_OUTPUTS_STAGED = "true"), (process.env.GITHUB_AW_AGENT_TASK_BASE = "main"), (process.env.GITHUB_REPOSITORY = "owner/repo"));
      }),
        ...
        it("should handle task without body in staged mode", async () => {
          (createAgentOutput([{ type: "create_agent_task", body: "" }]), await runScript());
          const summaryCall = mockCore.summary.addRaw.mock.calls[0];
          expect(summaryCall[0]).toContain("No description provided");
        }),
        it("should use target repo when specified", async () => {
          ((process.env.GITHUB_AW_TARGET_REPO = "org/target-repo"), createAgentOutput([{ type: "create_agent_task", body: "Test task" }]), await runScript());
          const summaryCall = mockCore.summary.addRaw.mock.calls[0];
    ...
    describe("agent task creation", () => {
      (beforeEach(() => {
        ((process.env.GITHUB_AW_AGENT_TASK_BASE = "develop"), (process.env.GITHUB_AW_SAFE_OUTPUTS_STAGED = "false"));
      }),
        ...
        it("should log agent output content length", async () => {
          (createAgentOutput([{ type: "create_agent_task", body: "Test agent task description" }]),
            await runScript(),
            expect(mockCore.info).toHaveBeenCalledWith(expect.stringContaining("Agent output content length:")),
            expect(mockCore.info).toHaveBeenCalledWith(expect.stringContaining("Found 1 create-agent-task item(s)")));
        }));
    }),
    ...
    describe("edge cases", () => {
      (it("should handle output with no items array", async () => {
        ...
        it("should use default base branch when not specified", async () => {
          (delete process.env.GITHUB_AW_AGENT_TASK_BASE, delete process.env.GITHUB_REF_NAME, (process.env.GITHUB_AW_SAFE_OUTPUTS_STAGED = "true"), createAgentOutput([{ type: "create_agent_task", body: "Test" }]), await runScript());
          const summaryCall = mockCore.summary.addRaw.mock.calls[0];
          expect(summaryCall[0]).toContain("**Base Branch:** main");
        }),
