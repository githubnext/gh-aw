...
/**
 * Resolve the standalone driver's SDK session configuration from environment.
 * In AWF reflect mode, restore the BYOK custom provider/model mapping that the
 * removed inline harness path used to pass directly into the SDK session.
 *
 * @param {{
 *   env?: NodeJS.ProcessEnv,
 *   logger?: (msg: string) => void,
 *   resolveCustomProviderFromReflect?: (options?: { model?: string, logger?: (msg: string) => void }) => { model: string, provider: import("@github/copilot-sdk").ProviderConfig } | null,
 * }} [options]
 * @returns {{ model: string | undefined, provider: import("@github/copilot-sdk").ProviderConfig | undefined }}
 */
function resolveStandaloneSDKSessionConfig(options) {
  const env = options?.env ?? process.env;
  const logger = options?.logger ?? log;
  const model = env.COPILOT_MODEL || undefined;
  if (env.AWF_REFLECT_ENABLED !== "1") {
    return { model, provider: undefined };
  }

  const resolveCustomProviderFromReflect = options?.resolveCustomProviderFromReflect ?? require("./copilot_harness.cjs").resolveCopilotSDKCustomProviderFromReflect;
  const customProviderConfig = resolveCustomProviderFromReflect({ model, logger });
  return {
...
module.exports = { extractPromptFromArgs, resolveStandaloneSDKSessionConfig, runWithCopilotSDK };
...
async function main() {
  ...
  const { model, provider } = resolveStandaloneSDKSessionConfig();
