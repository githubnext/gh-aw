...
5. The system will automatically block `create_pull_request` if this branch already has three or more completed failed workflow runs (currently conclusions `failure`, `timed_out`, `startup_failure`, or `stale`). Treat that as a required human checkpoint before another push, and use `report_incomplete` to explain the repeated branch failures instead of opening or updating the PR.
