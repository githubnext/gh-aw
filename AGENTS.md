...
6. Never attempt to run an agentic workflow check (e.g., `gh aw run`, `gh aw audit`, workflow dispatch) as part of a Copilot cloud agent run. The token does not have the required access. Always fail with an error — do not task the user or ask them to run it manually.
