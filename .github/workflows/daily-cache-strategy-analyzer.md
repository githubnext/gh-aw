...
Read the issue template from `.github/aw/cache-strategy-templates.md` (see "## Issue Template" section) immediately before creating each issue. After creating each issue, record it in `known-issues.json` with the title as key and issue number as value.
...
Read the discussion template from `.github/aw/cache-strategy-templates.md` (see "## Discussion Template" section) immediately before generating the discussion report.
