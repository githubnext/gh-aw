...
2. **Filter assigned issues** - Skip issues that already have assignees (human or agent)
3. **Analyze ALL unassigned issues** - Determine if each unassigned issue is well-suited for AI agent resolution (including issues already on the board - re-evaluate them to keep scores current)
4. **Update project board** - For unassigned issues with AI-Readiness Score ≥ 5, add or update them on project ${{ github.event.inputs.project_url }} with current field assignments
...
2. **Filter Assigned Issues**: Skip any issues that already have assignees (human or agent) - these are already being worked on
3. **Score ALL Unassigned Issues**: Evaluate AI-readiness for ALL unassigned issues, including those already on the project board (this keeps scores current as issues evolve)
4. **Update Project Board**: For each unassigned issue with AI-Readiness Score ≥ 5, output an `update_project` safe-output item with `"project": "${{ github.event.inputs.project_url }}"` (or `"project": "https://github.com/orgs/githubnext/projects/53"` when the input is empty) to add or update it on the project board with current field assignments. Issues with scores < 5 are not added to the board.
5. **Assign High-Readiness Issues**: For unassigned issues with AI-Readiness Score ≥ 9, output an `assign_to_agent` safe-output item to immediately assign the issue to @copilot (this includes issues that improved from lower scores)
...
- **All unassigned issues are re-evaluated every run** to keep scores current as issues evolve (comments add context, acceptance criteria improve, etc.)
- **Only unassigned issues with AI-Readiness Score ≥ 5** are added/updated on the project board (issues with existing assignees or scores < 5 are skipped); routed issues are differentiated via the Status field
- **Issues crossing the 9/10 threshold** automatically get assigned to @copilot (even if they were previously scored lower)
