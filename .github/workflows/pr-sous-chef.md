...
3. **Resolve review threads that already have a response using a safe output**
   ...
   - If resolving one thread fails, append `{pr_number: <N>, skip_reason: "resolve_review_thread_failed", thread_id: "<thread-id>"}` to the run-summary `skipped` array and continue.

4. **Dismiss stale `github-actions[bot]` blocking reviews when all PR review threads are resolved**
   - **Slash-command guard**: Skip this step entirely on `/souschef` slash-command runs — slash-command runs are acknowledgment nudges and must not perform automated review cleanup.
