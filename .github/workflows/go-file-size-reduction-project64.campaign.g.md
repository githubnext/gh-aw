...
2. **Inspect workflow jobs and logs**:
   - Use `list_workflow_jobs` with the workflow run ID to get all jobs in a run
   - Use `get_job_logs` with specific job IDs to retrieve logs from worker executions
