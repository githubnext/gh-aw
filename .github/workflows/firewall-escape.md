...
cat /tmp/gh-aw/repo-memory-default/memory/firewall-escape/escape-attempts.json 2>/dev/null || echo "No prior attempts found"
cat /tmp/gh-aw/repo-memory-default/memory/firewall-escape/techniques-tried.md 2>/dev/null || echo "No techniques log found"
```
...
```bash
# Create/update the escape attempts JSON
mkdir -p /tmp/gh-aw/repo-memory-default/memory/firewall-escape/

# Append your new techniques to the history
cat > /tmp/gh-aw/repo-memory-default/memory/firewall-escape/escape-attempts.json << 'EOF'
{
...
# Also update the human-readable techniques log
cat >> /tmp/gh-aw/repo-memory-default/memory/firewall-escape/techniques-tried.md << 'EOF'
## Run ${{ github.run_id }} - $(date)

- [ ] Technique 1: description (result)
- [ ] Technique 2: description (result)
...
EOF
```
