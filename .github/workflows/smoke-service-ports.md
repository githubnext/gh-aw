...
**Purpose:** Validate that the `--allow-host-service-ports` feature works end-to-end. This workflow opts into `runtime: docker-sudo-iptables` so the compiler can configure AWF to allow traffic to the Redis service port.
