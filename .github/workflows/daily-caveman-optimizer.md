...
- If the file does not exist, count the total number of files in the sorted list and pick a **random** starting index with `$(( RANDOM % total_files ))`. This avoids always processing the same files first when the cache is cold.
