...
on:
  ...
  workflow_dispatch:
concurrency:
  group: "campaign-documentation-tasks-orchestrator-${{ github.ref }}"
...
tools:
    ...
    repo-memory:
        - branch-name: memory/campaigns
          campaign-id: documentation-tasks
          file-glob:
            - documentation-tasks/**
          id: campaigns
---
...
<!-- Source: .github/workflows/documentation-tasks.campaign.md -->

# Campaign Orchestrator

This workflow orchestrates the 'Documentation Quality & Completeness Campaign' campaign.
...
- KPIs:
  - Documentation coverage (primary): baseline 0 → target 95 over 90 days percent
  - Broken links fixed (supporting): baseline 0 → target 0 over 30 days count
  - Documentation freshness (supporting): baseline 0 → target 90 over 60 days percent
- Associated workflows: documentation-tracker
- Memory paths: memory/campaigns/documentation-tasks/**
...
- Governance: max new items per run: 10
- Governance: max discovery items per run: 100
- Governance: max discovery pages per run: 10
- Governance: opt-out labels: no-campaign, no-bot
- Governance: do not downgrade done items: true
- Governance: max project updates per run: 15
- Governance: max comments per run: 15
...
**Cursor file (repo-memory)**: `memory/campaigns/documentation-tasks/cursor.json`
...
**Metrics/KPI snapshots (repo-memory)**: `memory/campaigns/documentation-tasks/metrics/*.json`
...
```json
{
  "campaign_id": "documentation-tasks",
  "date": "YYYY-MM-DD",
  "tasks_total": 0,
  "tasks_completed": 0,
  "tasks_in_progress": 0,
...
- `campaign_id` (string): Must be exactly "documentation-tasks"
...
- Documentation coverage (primary): baseline 0 → target 95 over 90 days (unit: percent) (direction: increase) (source: custom)

- Broken links fixed (supporting): baseline 0 → target 0 over 30 days (unit: count) (direction: decrease) (source: custom)

- Documentation freshness (supporting): baseline 0 → target 90 over 60 days (unit: percent) (direction: increase) (source: custom)
...
**Campaign ID**: Extract from tracker label `campaign:documentation-tasks` (format: `campaign:CAMPAIGN_ID`)
...
```
update-project:
  project: "https://github.com/orgs/githubnext/projects/TBD"
  item_url: "ISSUE_URL"
  status: "Todo"  # or "Done" if issue is already closed
  campaign_id: "CAMPAIGN_ID"  # Required: extract from tracker label campaign:documentation-tasks
```
