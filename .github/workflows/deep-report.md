...
6. **Extract actionable tasks** - Identify exactly 5 specific, high-impact tasks that can be assigned to agents for quick wins
...
**CRITICAL**: Based on your analysis, identify exactly **5 actionable tasks** (quick wins) and **CREATE GITHUB ISSUES** for each one:
...
**CRITICAL**: Identify exactly **5 actionable tasks** that could be immediately assigned to an AI agent to improve the project. Focus on **quick wins** - tasks that are:
...
**Remember**: The maximum is 5 issues. Choose the most impactful tasks.
...
1. **Create GitHub Issues**: For each of the 5 actionable tasks identified (if any), create a GitHub issue using the safe-outputs create-issue capability
