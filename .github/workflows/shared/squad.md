---
# Squad Bootstrap — installs and initializes Squad (https://github.com/bradygaster/squad)
# in the activation job only, then republishes the generated team state to the agent job.
#
# The Squad CLI is never installed or executed in the agent job — only the files it
# produces (`.squad/` team state and `.github/agents/squad.agent.md`) are restored there.
#
# Usage:
#   imports:
#     - uses: shared/squad.md
#       with:
#         squad-cli-version: "0.11.0"           # optional, default: 0.11.0
#         squad-github-token: ${{ secrets.SQUAD_PAT }}                # optional
#         squad-github-app-id: ${{ vars.SQUAD_APP_ID }}               # optional
#         squad-github-app-private-key: ${{ secrets.SQUAD_APP_KEY }}  # optional
#         squad-github-app-owner: "my-org"                            # optional
#
# Importing workflows must still declare their own `engine:` block — `engine:` is not
# a mergeable import field:
#
#   engine:
#     id: copilot
#     agent: squad
#     copilot-sdk: true
#
# Auth precedence for `squad init`: GitHub App installation token (when
# squad-github-app-id/squad-github-app-private-key are set) > squad-github-token >
# the workflow's own default token. Provide the App or token inputs only when Squad
# needs to reach other organizations or private repositories beyond the current one.
import-schema:
  squad-cli-version:
    type: string
    default: "0.11.0"
    description: Pinned npm version of @bradygaster/squad-cli to install for `squad init`.
  squad-github-token:
    type: string
    description: Optional custom GitHub token used only by `squad init` when it needs access to other organizations or private repositories not reachable with the default token.
  squad-github-app-id:
    type: string
    description: Optional GitHub App ID used to mint an installation token for `squad init` (paired with squad-github-app-private-key) when broader org/repo access is required.
  squad-github-app-private-key:
    type: string
    description: Optional GitHub App private key (PEM), paired with squad-github-app-id.
  squad-github-app-owner:
    type: string
    description: Optional installation owner for the Squad GitHub App token (defaults to the current repository owner).

network:
  allowed:
    - node

jobs:
  activation:
    pre-steps:
      - name: Checkout repository for Squad bootstrap
        uses: actions/checkout@v7.0.1
        with:
          persist-credentials: false

      - name: Setup Node.js for Squad
        uses: actions/setup-node@v7.0.0
        with:
          node-version: "22"

      - name: Mint Squad GitHub App token
        id: squad-app-token
        if: ${{ github.aw.import-inputs.squad-github-app-id != '' }}
        uses: actions/create-github-app-token@v2
        with:
          app-id: ${{ github.aw.import-inputs.squad-github-app-id }}
          private-key: ${{ github.aw.import-inputs.squad-github-app-private-key }}
          owner: ${{ github.aw.import-inputs.squad-github-app-owner }}

      - name: Install Squad CLI
        env:
          GH_AW_SQUAD_CLI_VERSION: ${{ github.aw.import-inputs.squad-cli-version }}
        run: |
          set -euo pipefail
          npm install -g "@bradygaster/squad-cli@${GH_AW_SQUAD_CLI_VERSION:-0.11.0}"

      - name: Initialize Squad team
        env:
          GH_TOKEN: ${{ steps.squad-app-token.outputs.token || github.aw.import-inputs.squad-github-token || github.token }}
        run: |
          set -euo pipefail
          squad init --preset default

      - name: Upload Squad state artifact
        if: success()
        uses: actions/upload-artifact@v7.0.1
        with:
          name: squad-state
          include-hidden-files: true
          path: |
            .squad
            .github/agents/squad.agent.md
          if-no-files-found: ignore
          retention-days: 1

steps:
  - name: Restore Squad state from activation artifact
    continue-on-error: true
    uses: actions/download-artifact@v8.0.1
    with:
      name: squad-state
      path: ${{ github.workspace }}
---

<!--

## Squad Bootstrap Component

This shared component moves the entire Squad (https://github.com/bradygaster/squad)
install/init lifecycle out of the agent job:

1. **`jobs.activation.pre-steps`** — checks out the repository, installs the pinned
   `@bradygaster/squad-cli` npm release, optionally mints a GitHub App installation
   token (or uses a supplied PAT) so `squad init` can see other organizations or
   private repositories, runs `squad init --preset default` (idempotent), and uploads
   the resulting `.squad/` team state plus `.github/agents/squad.agent.md` as a
   dedicated `squad-state` artifact — all inside the activation job, alongside the
   rest of the prompt/skills/sub-agent packaging.
2. **`steps:`** (agent job) — downloads the `squad-state` artifact and restores it into
   the checked-out workspace. The Squad CLI itself is never installed here; only the
   files it produced are copied in.

-->

## Working with Squad

Squad's team state (`.squad/`) and its Copilot custom agent
(`.github/agents/squad.agent.md`) were already initialized during activation and
restored into this checkout before you started — do not install Squad or run
`squad init` yourself.

- Verify `.squad/team.md` exists before delegating work to the team. If it is
  missing, the activation-job bootstrap step failed — call `noop` and explain
  why instead of proceeding.
- Coordinate work through the Squad team already defined in `.squad/` rather
  than proposing a brand-new team from scratch.
