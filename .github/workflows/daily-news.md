...
steps:
  - name: Download repository activity data
    ...
    run: |
      ...
      mkdir -p /tmp/gh-aw/repo-memory-default/memory/default/daily-news-data
      
      # Check if cached data exists and is recent (< 24 hours old)
      CACHE_VALID=false
      CACHE_TIMESTAMP_FILE="/tmp/gh-aw/repo-memory-default/memory/default/daily-news-data/.timestamp"
      ...
      if [ "$CACHE_VALID" = true ]; then
        ...
        cp -r /tmp/gh-aw/repo-memory-default/memory/default/daily-news-data/* /tmp/gh-aw/daily-news-data/
        ...
        cp -r /tmp/gh-aw/daily-news-data/* /tmp/gh-aw/repo-memory-default/memory/default/daily-news-data/
...
**Repo-memory is enabled** - You have access to persistent storage at `/tmp/gh-aw/repo-memory-default/memory/default/` that persists across workflow runs:
...
**Example use cases**:
- Save aggregated statistics (e.g., `/tmp/gh-aw/repo-memory-default/memory/default/monthly-stats.json`)
