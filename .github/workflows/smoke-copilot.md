...
15. **Sub-Agent Testing**: Use the `file-summarizer` sub-agent to summarize the file `.github/workflows/smoke-copilot.md`. The sub-agent is defined inline in this workflow and is automatically available in `.agents/agents/file-summarizer.agent.md`. Verify the sub-agent returns a brief summary (2–4 sentences). Mark this test as ❌ if the sub-agent is unavailable or returns an error.
