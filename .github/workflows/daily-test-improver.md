...
1. First check for existing open discussion titled "daily-test" using `list_discussions`. If found, read it and maintainer comments. If not found, then perform Phase 1 and nothing else.
...
2. Create a discussion with title "Research and Plan" that includes:
...
1. Check if an open pull request with title "Updates to complete configuration" exists in this repo. If it does, add a comment to the pull request saying configuration needs to be completed, then exit the workflow.
...
4. Before running any of the steps, make a pull request for the addition of the `action.yml` file, with title "Updates to complete configuration". Encourage the maintainer to review the files carefully to ensure they are appropriate for the project.
...
1. **Decide what to work on**
   ...
   c. Check the most recent pull request with title starting with prefix (it may have been closed) and see what the status of things was there. These are your notes from last time you did your work, and may include useful recommendations for future areas to work on.

   d. Check for existing open pull opened by you starting with title prefix. Don't repeat work from any open pull requests.
