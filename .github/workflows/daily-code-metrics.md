...
**Context**: Fresh clone (no git history). Fetch with `git fetch --unshallow` for churn metrics. Memory: `/tmp/gh-aw/repo-memory-default/memory/default/`
...
Store as JSON Lines in `/tmp/gh-aw/repo-memory-default/memory/default/history.jsonl`:
```json
