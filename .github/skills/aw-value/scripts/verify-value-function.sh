...
[[ $# -eq 1 ]] || fail "usage: verify-value-function.sh <value-function.sh>"

value_function=$1
[[ -f $value_function ]] || fail "value function not found: $value_function"
[[ -x $value_function ]] || fail "value function is not executable: $value_function"
command -v jq >/dev/null 2>&1 || fail "jq is required"
bash -n "$value_function"
...
' >/dev/null || fail "value-function definition is invalid"
...
for example_name in targetAttained targetMissed missing malformed; do
    ...
    result=$(printf '%s\n' "$evidence" | "$value_function" --metric)
