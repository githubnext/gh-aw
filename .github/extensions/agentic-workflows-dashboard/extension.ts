...
import type { ExperimentInfo, PagedResult, UsageSummaryItem, WorkflowDefinition, WorkflowRun } from "./src/models.js";
...
interface RunsResult extends ReportMeta {
  runs: WorkflowRun[];
}

interface UsageResult extends ReportMeta {
...
async function startServer(): Promise<ServerEntry> {
  const server = createServer(async (req, res) => {
    const reqUrl = new URL(req.url ?? "/", "http://localhost");
    const pathname = reqUrl.pathname;

    const sendJson = (payload: JsonValue, status = 200): void => {
...
function paginateDefinitions(definitions: WorkflowDefinition[], page: number, pageSize: number): PagedResult<WorkflowDefinition> {
  return paginate(definitions, page, pageSize);
}

function paginateRuns(runs: WorkflowRun[], page: number, pageSize: number): PagedResult<WorkflowRun> {
  return paginate(runs, page, pageSize);
}

function paginateUsage(items: UsageSummaryItem[], page: number, pageSize: number): PagedResult<UsageSummaryItem> {
  return paginate(items, page, pageSize);
}

function paginateExperiments(items: ExperimentInfo[], page: number, pageSize: number): PagedResult<ExperimentInfo> {
  return paginate(items, page, pageSize);
}
      ...
      actions: [
        {
          ...
          handler: async ctx => {
            const input = (ctx.input ?? {}) as { page?: number; pageSize?: number };
        ...
        },
        {
          ...
          handler: async ctx => {
            const input = (ctx.input ?? {}) as { count?: number; page?: number; pageSize?: number; timeout?: number; window?: string };
        ...
        },
        {
          ...
          handler: async ctx => {
            const input = (ctx.input ?? {}) as { run_id?: number };
        ...
        },
        {
          ...
          handler: async ctx => {
            const input = (ctx.input ?? {}) as { run_id?: string };
