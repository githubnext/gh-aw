...
function paginate(items, page, pageSize) {
  ...
  const end = start2 + pageSize;
  return {
    items: items.slice(start2, end),
    page: safePage,
    pageSize,
    totalItems,
    totalPages,
...
function formatNumber(value, options = {}) {
  ...
  return new Intl.NumberFormat(void 0, options).format(numeric);
}
...
function reportWindowById(windowId) {
  return reportWindows.find((window2) => window2.id === windowId) ?? reportWindows[1];
}
function buildReportMessage(meta, emptyLabel) {
  ...
  return fragments.length > 0 ? fragments.join(" \xB7 ") : emptyLabel;
}
...
module_default.data("dashboardApp", () => ({
  tabs: dashboardTabs,
  reportWindows,
...
gh aw version ${this.cliStatus.version}`;
  ...
  async fetchCliStatus() {
    ...
    try {
      ...
      this.cliStatus = null;
      this.errorCliStatus = `Failed to detect gh aw: ${error2 instanceof Error ? error2.message : String(error2)}`;
    } finally {
  ...
  async fetchDefinitions() {
    ...
    try {
      ...
      this.errorDefinitions = `Failed to load workflows: ${error2 instanceof Error ? error2.message : String(error2)}`;
    } finally {
  ...
  async fetchRuns() {
    ...
    try {
      ...
      this.runsMeta = data2;
      this.runs = Array.isArray(data2.runs) ? data2.runs : [];
      this.loadRunPage(1);
      this.selectedRun = this.runs.find((run) => run.run_id === previousRunId) ?? this.runs[0] ?? null;
    } catch (error2) {
      this.runsMeta = null;
      this.errorRuns = `Failed to load runs: ${error2 instanceof Error ? error2.message : String(error2)}`;
    } finally {
  ...
  async fetchUsage() {
    ...
    try {
      ...
      const data2 = await fetchJson(`/api/usage?${params.toString()}`);
      this.usageMeta = data2;
      this.usage = Array.isArray(data2.items) ? data2.items : [];
      this.loadUsagePage(1);
    } catch (error2) {
      this.usageMeta = null;
      this.errorUsage = `Failed to load usage summary: ${error2 instanceof Error ? error2.message : String(error2)}`;
    } finally {
  ...
  async fetchExperiments() {
    ...
    try {
      ...
      this.errorExperiments = `Failed to load experiments: ${error2 instanceof Error ? error2.message : String(error2)}`;
    } finally {
...
gh aw version ${this.cliStatus.version}`;
  ...
  async loadAudit() {
    ...
    try {
      ...
      this.auditData = null;
      this.errorAudit = `Failed to load audit: ${error2 instanceof Error ? error2.message : String(error2)}`;
    } finally {
