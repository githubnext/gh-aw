import { DEFAULT_LOG_TIMEOUT_MINUTES, DEFAULT_RUN_COUNT, getReportWindow } from "./dashboard-config.js";
...
export function buildLogsArgs(options) {
    ...
    if (options.workflowName)
        args.push(options.workflowName);
    ...
    if (options.branch)
        args.push("--ref", options.branch);
    if (options.beforeRunID > 0)
        args.push("--before-run-id", String(options.beforeRunID));
    if (options.afterRunID > 0)
        args.push("--after-run-id", String(options.afterRunID));
    if (options.artifacts.length > 0)
        args.push("--artifacts", options.artifacts.join(","));
    return args;
}
