...
The documentation supports versioning per minor release (0.28, 0.29, etc.) using the `starlight-versions` plugin.

### About Versioning

The Starlight Versions plugin adds support for versioning documentation pages. Versioning allows maintaining multiple versions of documentation, each with its own pages and assets. This is useful for different release versions but increases complexity, build times, and maintenance overhead.

**Versioning Process:**

The current version always lives in `docs/src/content/docs/`. Creating a new version archives the current documentation so you can continue working on the latest while archived versions remain accessible.
...
The plugin uses **folder-based versioning** where each version is stored in dedicated directories (e.g., `0.28/`, `0.29/`). This approach is simple - cloning the repository includes all versions with no additional knowledge required. However, it creates multiple copies of files, making navigation harder and losing easy change tracking between versions.

### Creating a New Version

Versioning is configured lazily - versions are only created when a minor release is published.
...
2. **Uncomment and configure the starlight-versions plugin** in `docs/astro.config.mjs`:
   ...
   ```
3. **Build the documentation** - The plugin automatically creates the version directory:
   ```bash
...
6. **Build and verify** that both versions render correctly
