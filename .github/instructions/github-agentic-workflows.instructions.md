...
- **`engine:`** - AI processor configuration
  - String format: `"claude"` (default), `"codex"`, `"copilot"`, `"github-models"`, `"custom"` (⚠️ experimental)
  - Object format for extended configuration:
    ```yaml
    engine:
      id: claude                        # Required: coding agent identifier (claude, codex, copilot, github-models, custom)
...
- **Invalid enum values** - e.g., `engine` must be "claude", "codex", "copilot", "github-models" or "custom"
