...
> [!WARNING]
> **Never directly merge Dependabot PRs that modify generated files** in `.github/workflows/` (such as `package.json`, `requirements.txt`, or `go.mod`) or `.github/aw/actions-lock.json`. These files are generated by the `gh aw` compiler and any direct changes will be overwritten on the next compilation.
...
| Manifest | Ecosystem | Location |
|----------|-----------|----------|
| `package.json` / `package-lock.json` | npm | `.github/workflows/` |
| `requirements.txt` | pip | `.github/workflows/` |
| `go.mod` | Go | `.github/workflows/` |
...
Filter for PRs affecting generated workflow manifests (title contains `Bump` or similar, files include `package.json`, `requirements.txt`, `go.mod`, or `actions-lock.json`).
...
This updates all manifest files (`package.json`, `requirements.txt`, `go.mod`) from the updated `.md` file versions.

If `package-lock.json` also needs updating:
...
| Issue | Solution |
|-------|----------|
| `package-lock.json` not updated | Run `cd .github/workflows && npm install --package-lock-only` after compilation |
