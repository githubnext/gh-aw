...
- `jobs.<job-id>.setup-steps` and `jobs.<job-id>.pre-steps` - Merged per job with imported steps first, then main workflow steps; `setup-steps` remains distinct and executes before `pre-steps`.
