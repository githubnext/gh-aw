...
- **`features:`** - Feature flags for experimental or optional features (object)
  ...
  - Known feature flags:
    ...
    - `cli-proxy: true` - **Deprecated.** Equivalent to `tools.github.mode: cli`. Use `tools.github.mode: cli` instead (migrated automatically by `gh aw fix`).
    - `integrity-reactions: true` - Enable reaction-based integrity promotion/demotion. The compiler resolves GitHub access to `cli`. Requires `tools.github.min-integrity` and MCPG >= v0.2.18; incompatible with explicit `mcp-local`, explicit `mcp-remote`, and `sandbox.agent.runtime: cloud-hypervisor`. See [security-profiles.md](security-profiles.md).
...
- **`sandbox:`** - Sandbox configuration for AI engines (string or object)
  ...
  - **`sandbox.agent.runtime`** (string) selects the sandbox security and topology profile: `docker` (default), `docker-sudo-iptables`, `gvisor`, `docker-sbx`, or `cloud-hypervisor`. Omission and explicit `docker` are both valid. See [security-profiles.md](security-profiles.md) for selection and compatibility.
  ...
  - **`sandbox.agent.allow-host-ports`** (integer array) allows access to specific host TCP ports and is valid only with `runtime: docker-sudo-iptables`. Use GitHub Actions `services:` for service containers and reserve this field for other host daemons.
  - Removed `sandbox.agent.sudo` and `sandbox.agent.legacy-security` fields must not be emitted. Run `gh aw fix --write` to migrate them to a runtime profile.
...
- **`tools:`** - Tool configuration for the coding agent (`github`, `agentic-workflows`, `edit`, `web-fetch`, `web-search`, `bash`, `playwright`, custom MCP server names, plus `timeout`/`startup-timeout`/`mcp-mode`). See [syntax-tools-imports.md](syntax-tools-imports.md#tool-configuration) for the full schema (GitHub `mode`/`toolsets`/integrity fields, bash allowlist decision rule, Playwright CLI mode).
