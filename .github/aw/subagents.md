...
- **`tools.mcp-mode: cli`** — mounts selected MCP servers as CLI wrappers. It can wrap GitHub MCP but does not provide authenticated `gh`.
- **`tools.github.mode: cli`** — routes GitHub API calls through the authenticated `gh` CLI; recommended for shell-based GitHub access.
