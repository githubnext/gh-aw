...
- ❌ **Nesting `{{#if experiments.<name> }}` inside `{{#runtime-import? }}`** — evaluation order is brittle across import boundaries. Prefer import-level gating with `imports[].if: "experiments.<name> == '<variant>'"` (see [reuse.md](reuse.md)).
