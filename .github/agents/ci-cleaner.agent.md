...
**IMPORTANT**: Do NOT commit `.lock.yml` files. These are compiled workflow files that should not be included in your pull request. Only commit source code changes (`.go`, `.js`, `.cjs`, `.md` files, etc.).
...
**IMPORTANT - DO NOT COMMIT LOCK FILES:**
- **NEVER** commit `.lock.yml` files to your pull request
- These are compiled workflow files generated by `make recompile`
- Only commit source code changes (`.go`, `.js`, `.cjs`, `.md`, test files, etc.)
- Before creating your pull request, ensure no `.lock.yml` files are staged with `git status`
- If lock files are staged, unstage them with: `git reset HEAD *.lock.yml` or `git restore --staged .github/workflows/*.lock.yml`
