...
refresh-models-json:
	...
	jq '{data: ((.providers // {}) | to_entries | map(select(.key | test("^(anthropic|openai|github-copilot)$$"))) | map(. as $$provider | ($$provider.value.models // {}) | to_entries | map({id: "\((if $$provider.key == "github-copilot" then "github" else $$provider.key end))/\(.key)", pricing: ({prompt: (.value.cost.input // empty), completion: (.value.cost.output // empty), reasoning: (.value.cost.reasoning // empty), input_cache_read: (.value.cost.cache_read // empty), input_cache_write: (.value.cost.cache_write // empty), input_audio: (.value.cost.input_audio // empty), output_audio: (.value.cost.output_audio // empty)} | with_entries(select(.value != null)) | with_entries(.value |= (./1000000 | tostring))) })) | add // [] | sort_by(.id))}' "$$src" > "$$tmp"; \
