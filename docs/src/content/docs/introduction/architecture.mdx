...
Agent runtime is independent from the [GitHub access mode and MCP exposure mode](/gh-aw/reference/security-profiles/). CLI GitHub access uses an internal host policy proxy; `tools.mcp-mode: cli` instead wraps MCP servers as commands and does not select the GitHub transport.
