...
description: Security architecture for GitHub Agentic Workflows (gh-aw), including configurable permissions, sandboxing, threat detection, integrity filtering, network controls, and safe outputs in GitHub Actions.
sidebar:
  order: 3
...
GitHub Agentic Workflows (`gh-aw`) compiles Markdown workflow definitions into GitHub Actions workflows that run AI agents with GitHub Copilot, Claude Code, OpenAI Codex, Google Gemini, or experimental Pi. The default generated workflow uses defense-in-depth controls against prompt injection, rogue MCP servers, and compromised agents. These controls are configurable; disabling isolation or threat detection, granting direct write permissions, or adding custom jobs changes the trust model.

## Security Model

GitHub Agentic Workflows adopts a layered approach that combines substrate-enforced isolation, declarative specification, and staged execution. Each layer enforces distinct security properties under different assumptions and constrains the impact of failures above it.

### Threat Model

The threat model considers an adversary that may compromise untrusted user-level components, such as containers, and cause them to behave arbitrarily within their granted privileges. The adversary may attempt to:
...
The threat model assumes that the adversary does not compromise the underlying hardware or cryptographic primitives. Attacks exploiting side channels and covert channels are also out of scope.

---

### Layer 1: Substrate-Level Trust

Agentic workflows run on GitHub-hosted or Linux self-hosted GitHub Actions runners. Self-hosted deployments can include Actions Runner Controller (ARC) pods that use a privileged Docker-in-Docker (DinD) sidecar. The trusted substrate therefore includes the runner host or Kubernetes cluster, CPU, MMU, kernel, and container runtime. With ARC DinD, the runner container sends Docker API commands to the sidecar daemon, which creates the agent, proxy, and MCP containers and enforces their Docker network boundaries.
...
Safe outputs constrain writes requested through the SafeOutputs subsystem. Custom jobs and direct write permissions are separate trust boundaries and must be reviewed independently.
...
ARC DinD preserves AWF's egress policy through Docker network topology rather than host `iptables`: the DinD daemon creates an internal network with no direct internet route and a dual-homed Squid proxy as the sole egress path. See [Self-Hosted Runners](/gh-aw/reference/self-hosted-runners/) and [How to run GitHub Copilot coding agent on ARC with Docker-in-Docker](/gh-aw/guides/arc-dind-copilot-agent/) for deployment details.
...
The `gh-aw` compiler enforces security constraints at compilation time through schema validation, expression allowlisting, and action pinning. The trusted compiler validates declarative configuration artifacts before they are deployed, rejecting misconfigurations and overly permissive specifications. This layer constrains what components may be loaded and how they may be connected, but it does not constrain runtime behavior.
...
GitHub Agentic Workflows provides observability through GitHub Actions runs and artifacts. Generated workflow artifacts can preserve prompts, outputs, patches, and logs for post-hoc analysis. This observability layer supports debugging, security auditing, and cost monitoring without changing runtime isolation.
...
- **Artifact Preservation**: Generated workflows save diagnostic outputs such as prompts, patches, and logs as downloadable artifacts
...
- **Firewall Logs**: When the firewall is enabled, requests routed through it are logged for security auditing
...
| **Plan** | Threat detection, when enabled | Malicious patches, secret leaks |
| **Plan** | Permission separation through configured safe outputs | Direct write access abuse |
...
- [GitHub Agentic Workflows Quickstart](/gh-aw/setup/quick-start/) - Install `gh-aw` and run a first agentic workflow
...
- [Self-Hosted Runners](/gh-aw/reference/self-hosted-runners/) - Runner selection, ARC DinD topology, and split-daemon configuration
- [ARC with Docker-in-Docker](/gh-aw/guides/arc-dind-copilot-agent/) - Kubernetes runner deployment
...
- [GitHub Agentic Workflows Examples](/gh-aw/examples/) - Task-oriented workflow examples and recipes
- [GitHub Agentic Workflows FAQ](/gh-aw/reference/faq/) - Concise answers about behavior, security, engines, and costs
