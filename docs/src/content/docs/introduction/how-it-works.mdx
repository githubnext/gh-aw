...
Workflows use [tools](/gh-aw/reference/tools/) through the **Model Context Protocol ([MCP](/gh-aw/guides/mcps/))** for GitHub operations, external APIs, file operations, and custom integrations.
...
Use `gh aw compile` to generate `.lock.yml` files from workflow `.md` files. The `.md` file is the editable source of truth, while `.lock.yml` is the compiled GitHub Actions workflow with security hardening. Commit both files.
