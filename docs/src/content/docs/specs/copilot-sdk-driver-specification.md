...
| `GH_AW_MAX_TOOL_FAILURE`      | No       | Maximum repeated tool refusals/failures before aborting inference | Input SHOULD be a positive integer; default `5`; implementations MUST fall back on invalid values |
...
`GH_AW_MAX_TOOL_FAILURE` controls the catastrophic tool-failure
