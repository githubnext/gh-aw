...
When [threat detection](/gh-aw/reference/threat-detection/) is enabled, the detection job defaults to `ubuntu-latest` regardless of `runs-on-slim` or `safe-outputs.runs-on`. Override it with `safe-outputs.threat-detection.runs-on`:
