...
GitHub Agentic Workflows enable you to create powerful automation workflows using natural language through GitHub Copilot. Everything can be done through the **GitHub web interface** - no local development environment or VS Code is required.
...
> [!TIP]
> While VS Code integration is available for advanced workflows, it's entirely optional. You can create and manage all workflows directly through the GitHub web interface using Copilot agent sessions.
...
When working with workflow files (`.md` files), it's important to understand the difference between the two main components:
...
- **Editing**: Can be edited directly without recompilation
- **Example**:
  ```markdown
...
> [!TIP]
> You can edit the workflow prompt directly through the GitHub web interface. Changes to the prompt take effect immediately in the next workflow run - no compilation needed.
...
- **Editing**: **Requires recompilation** after changes
- **Example**:
  ```yaml
  ---
  engine: copilot
...
> [!WARNING]
> After editing frontmatter configuration, you **must recompile** the workflow to regenerate the `.lock.yml` file:
> ```bash
