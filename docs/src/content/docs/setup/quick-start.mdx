    ...
    content: |
      {
        ...
        "step": [
          ...
          },
          {
            ...
            "text": "Run: gh aw add-wizard githubnext/agentics/daily-repo-status — then follow the interactive prompts to choose GitHub Copilot, Claude Code, OpenAI Codex, Google Gemini, or experimental Pi and configure authentication.",
            "position": 2
          },
          {
            "@type": "HowToStep",
...
GitHub Agentic Workflows (`gh-aw`) lets developers define AI-powered repository automation in Markdown with YAML frontmatter and run AI agents through GitHub Actions. By the end of this guide, you will have a recurring agentic workflow running in your repository with GitHub Copilot, Claude Code, OpenAI Codex, Google Gemini, or experimental Pi. The stable-engine path takes about 10 minutes and requires only the GitHub CLI.
...
2. **Select an AI engine** - Choose GitHub Copilot, Claude Code, OpenAI Codex, Google Gemini, or experimental Pi.
3. **Configure AI engine authentication** - Use the recommended [`copilot-requests: write`](/gh-aw/reference/auth/#copilot-requests-write-permission) permission or [`COPILOT_GITHUB_TOKEN`](/gh-aw/reference/auth/#copilot_github_token) for Copilot, [`ANTHROPIC_API_KEY`](/gh-aw/reference/auth/#anthropic_api_key) or [Anthropic WIF](/gh-aw/reference/auth/#anthropic-workload-identity-federation-wif) for Claude, `CODEX_API_KEY` or [`OPENAI_API_KEY`](/gh-aw/reference/auth/#openai_api_key) for Codex, and [`GEMINI_API_KEY`](/gh-aw/reference/auth/#gemini_api_key) or [Google WIF](/gh-aw/reference/auth/#google-workload-identity-federation-wif) for Gemini. Pi uses the authentication required by its selected provider. See [Authentication](/gh-aw/reference/auth/) for setup instructions.
...
<Tabs>
  ...
  <TabItem label="Pi (experimental)">
    ...
    Pi also requires `tools.github.mode: gh-proxy` and `tools.cli-proxy: true`. Follow [Using Pi with GitHub Agentic Workflows](/gh-aw/engines/pi/) for the complete experimental setup.
  </TabItem>
</Tabs>
