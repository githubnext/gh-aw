...
> [!IMPORTANT]
> If a workflow frontmatter omits `engine:`, GitHub Agentic Workflows defaults to Copilot. To use Claude, pass `--engine claude` during add, or set `engine: claude` in workflow frontmatter.
