...
<Tabs>
  ...
  <TabItem label="Claude">
    > [!WARNING]
    > If you use `claude login` locally, its `CLAUDE_CODE_OAUTH_TOKEN` will not work in GitHub Agentic Workflows—it is ignored, causing an API-key authentication error. Set `ANTHROPIC_API_KEY` instead.
