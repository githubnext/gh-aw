...
Use this `gh extension install` command by default — it's the standard way to install `gh-aw` and works for most setups. The curl script and `gh auth login` below are only needed for troubleshooting authentication issues, not as a first choice.
...
<Tabs>
  ...
  <TabItem label="Claude">
    > [!WARNING]
    > If you use `claude login` locally, its `CLAUDE_CODE_OAUTH_TOKEN` will not work in GitHub Agentic Workflows—it is ignored, causing an API-key authentication error. Set `ANTHROPIC_API_KEY` instead, or use [Anthropic WIF](/gh-aw/reference/auth/#anthropic-workload-identity-federation-wif).
