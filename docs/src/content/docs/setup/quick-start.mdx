...
>
> [!WARNING]
> **Claude Code / Max / Pro subscription OAuth tokens are not supported.** gh-aw requires a separate Anthropic API key (billed per use). If you have set `CLAUDE_CODE_OAUTH_TOKEN` as a repository secret, it will be silently ignored — configure `ANTHROPIC_API_KEY` instead.
