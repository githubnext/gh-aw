...
Use this `gh extension install` command by default — it's the standard way to install `gh-aw` and works for most setups. The curl script and `gh auth login` below are only needed for troubleshooting authentication issues.
