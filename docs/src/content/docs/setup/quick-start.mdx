...
GitHub Agentic Workflows lets developers define AI-powered repository automation in Markdown with <a id="frontmatter-definition"></a>YAML frontmatter (the configuration block between the `---` markers at the top of the file) and run AI agents through GitHub Actions. By the end of this guide, you will have a recurring agentic workflow running in your repository with GitHub Copilot, Claude Code, OpenAI Codex, Google Gemini, or Pi. The stable-engine path takes about 10 minutes and requires only the GitHub CLI.
...
> [!TIP]
> If you already have GitHub Copilot, start with the **Copilot** tab. It requires no additional account setup; the other tabs require an API key or provider-specific configuration.
...
3. If you changed the [frontmatter](#frontmatter-definition) (the YAML configuration block between the `---` markers at the top of the file), regenerate the compiled workflow by running:
