...
<Tabs>
  <TabItem label="Copilot">
    ...
    **Why a separate token?** The built-in `GITHUB_TOKEN` that Actions automatically provides is scoped only to the current repository and cannot call the GitHub Copilot API. A Personal Access Token (PAT) created under your user account can be granted the **Copilot Requests** permission that the workflow needs.
