...
- `COPILOT_GITHUB_TOKEN` is a separate fine-grained PAT used by Copilot CLI inference.

`GITHUB_TOKEN` does not include Copilot permissions, so Copilot engine runs need `COPILOT_GITHUB_TOKEN`.
