...
<details>
  ...
  2. From your repository root, run `gh aw add-wizard githubnext/agentics/daily-repo-status`.
  3. Choose an AI engine and set the matching repository secret when prompted.
  4. Let the wizard add `.github/workflows/daily-repo-status.md` and its compiled `.lock.yml`, then optionally trigger the first run.
  5. Open the repository's **Actions** tab to watch the run, then check **Issues** for the generated daily report.
</details>
