...
Use this `gh extension install` command by default — it's the standard way to install `gh-aw` and works for most setups.
...
There are two commands for adding a workflow — pick one:

| Command | Use it when |
|---------|-------------|
| `gh aw add-wizard` | You want guided, interactive prompts for choosing an engine and configuring auth/secrets (used in this guide) |
| [`gh aw add`](/gh-aw/setup/cli/#add) | You already know the engine and secrets and want a single non-interactive command (for example, in scripts or CI) |
