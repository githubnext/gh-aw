...
## Getting Started with SideRepoOps
...
Create a `GH_AW_MAIN_REPO_TOKEN` secret with read access to your main repository and write access for issues and pull requests. See [Authentication](/gh-aw/reference/auth/) for PAT and GitHub App setup options. Store it as a secret:
...
> [!NOTE]
> The default `GITHUB_TOKEN` cannot access other repositories. You must use additional authentication.
...
If the agent cannot read files from the main repository, provide an explicit token if the main repo is private:
...
GitHub Actions only delivers webhook events to workflows in the repository where the event occurred. Since SideRepoOps workflows live in a side repository, events from the main repository never reach them — **`slash_command:` triggers cannot be used directly in a SideRepoOps workflow**.
...
- [ChatOps](/gh-aw/patterns/chat-ops/) — Slash command workflows in the main repo
