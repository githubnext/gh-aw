...
Slash commands (e.g. `/review`, `/deploy`) rely on GitHub Actions events — `issue_comment`, `issues`, `pull_request`, etc. — firing in the repository **where the workflow file lives**. In SideRepoOps, workflows live in the side repository, so events from the main repository never reach them. This means you **cannot use `slash_command:` triggers directly in a SideRepoOps workflow**.

> [!NOTE]
> This is a fundamental GitHub Actions constraint, not a limitation of gh-aw. GitHub only delivers webhook events to workflows in the repository where the event occurred.

### Recommended Bridge Pattern

The recommended approach is to keep a thin relay workflow in the **main repository** that listens for the slash command and forwards it to the side repository via `workflow_dispatch`. The side repository workflow does the actual work.
...
Forward this `/review` command to the automation repository for processing.
```

**Step 2 — Review workflow in the side repository:**
...
on:
  workflow_dispatch:
    inputs:
      source_repo:
        description: "Repository where the command was triggered"
        required: true
      pr_number:
        description: "Pull request number"
        required: true
      comment_body:
        description: "Sanitized comment text"
        required: true
...
Analyze the code changes and post a detailed review comment on the pull request.
```

The relay workflow in the main repo is deliberately minimal — it contains no AI logic and just forwards the event. All automation stays in the side repository.

### Trade-offs

| Approach | Isolation | Simplicity | Event Access |
|----------|-----------|------------|--------------|
...
- **Slash command in main repo** — acceptable when you're comfortable with the workflow living alongside your code and don't need the isolation SideRepoOps provides
- **Bridge pattern** — best when you want slash command interactivity but need to keep AI logic out of the main repository; trade-off is a thin relay workflow in the main repo
- **Scheduled polling** — simplest SideRepoOps approach; use when real-time command response isn't required (e.g., nightly triage instead of on-demand `/triage`)

> [!TIP]
> If you only need a few slash commands, keep them in the main repository and use SideRepoOps only for scheduled and `workflow_dispatch` workflows. This gives you the best of both worlds with minimal complexity.
...
**Patterns**: [MultiRepoOps](/gh-aw/patterns/multi-repo-ops/) · [ChatOps](/gh-aw/patterns/chat-ops/) · [Orchestration](/gh-aw/patterns/orchestration/) · [IssueOps](/gh-aw/patterns/issue-ops/)

**Reference**: [Command Triggers](/gh-aw/reference/command-triggers/) · [Cross-Repository Operations](/gh-aw/reference/cross-repository/) · [Safe Outputs](/gh-aw/reference/safe-outputs/) · [GitHub Tools](/gh-aw/reference/github-tools/) · [Authentication](/gh-aw/reference/auth/) · [Reusing Workflows](/gh-aw/guides/packaging-imports/)
