...
description: Learn how to use the create-agentic-workflow custom agent in VS Code to author agentic workflows in natural language.
sidebar:
  order: 100
---
...
- `.github/agents/create-shared-agentic-workflow.md` - Custom agent for creating reusable workflow components
...
`.github/agents/create-agentic-workflow.md` is a custom agent file that contains the structure and instructions GitHub Copilot will use to generate a workflow markdown file that the `gh aw` CLI understands.
...
In Visual Studio Code with GitHub Copilot Chat, use the `/agent` command and select `create-agentic-workflow` from the list:
...
Assuming you have the GitHub Copilot CLI installed, you can reference the custom agent file using the `--agent` flag:

```bash
copilot --agent .github/agents/create-agentic-workflow.md
```
