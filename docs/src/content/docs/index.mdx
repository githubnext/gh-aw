...
Supported built-in AI engines include GitHub Copilot, Claude Code, OpenAI Codex, and Google Gemini, plus experimental Pi. Choose the engine and authentication method that fit the workflow, then keep the same Markdown-and-frontmatter authoring model.
...
<FeatureGrid columns={3}>
  ...
  <FeatureCard icon="beaker" title="Multiple AI Engines" href="/gh-aw/reference/engines/">
    Built-in support for Copilot, Claude Code, Codex, Gemini, and experimental Pi
  </FeatureCard>
...
flowchart LR
    ...
    TOKENS --> SECRETS["No Repository Secrets by Default"]
...
GitHub Agentic Workflows (`gh-aw`) provides four stable built-in AI engines and experimental Pi. Engine changes may also require a different authentication method or tool configuration; use the linked guide for each engine.
...
<FeatureGrid columns={4}>
  ...
  <FeatureCard title="Pi (Experimental)" href="/gh-aw/engines/pi/">
    Multi-provider experimental engine. Set engine: pi and configure its required proxies and provider authentication.
  </FeatureCard>
</FeatureGrid>
