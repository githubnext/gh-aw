...
<FeatureGrid columns={3}>
  ...
  <FeatureCard icon="beaker" title="Multiple AI Engines" href="/gh-aw/reference/engines/">
    4 primary engines — Copilot, Claude, Codex, and Gemini — plus 3 experimental engines (Crush, OpenCode, Pi)
  </FeatureCard>
...
AI agents can be manipulated by prompt injection, malicious repository content, or compromised tools. GitHub Agentic Workflows uses **6 layered controls** to keep each run contained: read-only token, no secrets in agent, sandbox and network firewall, safe outputs gate, threat detection scan, and scoped write job.
