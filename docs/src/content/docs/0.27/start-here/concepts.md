---
title: Concepts
description: Learn the core concepts and capabilities of GitHub Agentic
  Workflows, combining AI agents with GitHub's collaboration platform for
  Continuous AI.
sidebar:
  order: 300
slug: 0.27/start-here/concepts
---
...
Agentic workflows run with minimal permissions (no write access by default), use tool allowlists to prevent unexpected actions, and process outputs through a safety layer before applying changes. Critical actions can require human approval. See [Security Guide](/gh-aw/0.27/guides/security/) for details.
...
See [Workflow Structure](/gh-aw/0.27/reference/workflow-structure/) and [Frontmatter](/gh-aw/0.27/reference/frontmatter/) for configuration details.

## AI Engines

Workflows support GitHub Copilot CLI (default), Claude Code, and Codex (experimental). Each engine interprets natural language instructions and executes them using configured tools and permissions. See [AI Engines](/gh-aw/0.27/reference/engines/) for details.
...
Workflows use tools through the Model Context Protocol (MCP) for GitHub operations, external APIs, file operations, and custom integrations. Learn more in [Tools](/gh-aw/0.27/reference/tools/) and [MCPs](/gh-aw/0.27/guides/mcps/).

## Best Practices

Start simple and iterate. Write clear, specific instructions. Test with `gh aw compile --watch` and `gh aw run`. Monitor costs with `gh aw logs`. Review AI-generated content before merging. Use [Safe Output Processing](/gh-aw/0.27/reference/safe-outputs/) for controlled creation of issues, comments, and PRs.

## Next Steps

Explore [Workflow Structure](/gh-aw/0.27/reference/workflow-structure/), [Frontmatter](/gh-aw/0.27/reference/frontmatter/), [Tools](/gh-aw/0.27/reference/tools/), [Security Guide](/gh-aw/0.27/guides/security/), and [VS Code Integration](/gh-aw/0.27/tools/vscode/) to build sophisticated workflows that understand context, make intelligent decisions, and take meaningful actions while maintaining GitHub Actions reliability.
