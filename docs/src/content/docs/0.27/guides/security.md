...
description: Important security considerations for GitHub Agentic Workflows,
  including sandboxing, permissions, and best practices for safe agentic
  automation.
sidebar:
  order: 100
slug: 0.27/guides/security
---
...
#### workflow\_run Trigger Security
...
This generates a safety condition that prevents execution if the triggering workflow\_run is from a different repository:
...
Without branch restrictions, workflows emit warnings during compilation (or errors in strict mode). Branch restrictions improve security by limiting which branch workflows can trigger the workflow\_run event.
...
* **Approval gates**: Use the `manual-approval` field to require manual approval before workflow execution. Configure environment protection rules in repository settings to specify required reviewers and approval policies:
  ...
  ```
  See [Manual Approval Gates](/gh-aw/0.27/reference/triggers/#manual-approval-gates-manual-approval) for configuration details.
...
See the [Frontmatter Reference](/gh-aw/0.27/reference/frontmatter/#strict-mode-strict) for complete strict mode documentation.
...
For complete documentation on `stop-after:` configuration and supported formats, see [Trigger Events](/gh-aw/0.27/reference/triggers/#stop-after-configuration-stop-after).
...
* **workflow\_dispatch** is treated as safe only when `write` is in the allowed roles, since workflow\_dispatch can be triggered by users with write access
...
See the [Safe Outputs Reference](/gh-aw/0.27/reference/safe-outputs/) for complete configuration details and available output types.
...
See the [Threat Detection Guide](/gh-aw/0.27/guides/threat-detection/) for comprehensive documentation, configuration examples, and the LlamaGuard integration pattern.
...
See the [Network Reference](/gh-aw/0.27/reference/network/) for detailed configuration options and the [Engine Network Permissions](#engine-network-permissions) section below for engine-specific controls.
...
See [Copilot Engine - Network Permissions](/gh-aw/0.27/reference/engines/#network-permissions) for detailed AWF configuration.
...
* [Threat Detection Guide](/gh-aw/0.27/guides/threat-detection/) - Comprehensive threat detection configuration and examples
* [Safe Outputs Reference](/gh-aw/0.27/reference/safe-outputs/)
* [Network Configuration](/gh-aw/0.27/reference/network/)
* [Tools](/gh-aw/0.27/reference/tools/)
* [MCPs](/gh-aw/0.27/guides/mcps/)
* [Workflow Structure](/gh-aw/0.27/reference/workflow-structure/)
