...
A safe output type that triggers `repository_dispatch` events in external repositories for cross-repository orchestration. Each key under `safe-outputs.dispatch-repository:` defines a named tool exposed to the agent. A tool requires a `workflow` identifier (forwarded in `client_payload` for routing), an `event-type` (alias: `event_type`), and either a static `repository` slug or an `allowed-repositories` list (alias: `allowed_repositories`). GitHub Actions expressions (`${{ ... }}`) are supported in repository fields and are passed through without format validation. See [Safe Outputs Reference](/gh-aw/reference/safe-outputs/#repository-dispatch-dispatch-repository).
