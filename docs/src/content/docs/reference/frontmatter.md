...
| `observability.otlp.if-missing` | string (`error`, `warn`, `ignore`) | Controls behavior when OTLP endpoint/header values resolve to empty values at runtime (for example because a referenced secret is unset): `error` (default) fails startup, `warn` continues with OTLP config, `ignore` logs a warning and skips MCP gateway OTLP configuration. |
