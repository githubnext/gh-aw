...
timeout-minutes: 30                  # Defaults to 60 minutes
```
...
`timeout-minutes` accepts an integer or a GitHub Actions expression string (e.g. `${{ inputs.timeout }}`), letting a reusable `workflow_call` workflow parameterize its own timeout from caller inputs. It bounds both the `agentic_execution` step and generated `agent`/`detection` jobs, including their setup steps. Set `jobs.agent.timeout-minutes` or `jobs.detection.timeout-minutes` to override either job independently. When omitted, `${{ vars.GH_AW_DEFAULT_TIMEOUT_MINUTES }}` is used before the 60-minute fallback. It applies to the workflow being compiled, **not** to plain caller jobs that invoke a reusable workflow with job-level `uses:` — GitHub rejects `timeout-minutes` there.
