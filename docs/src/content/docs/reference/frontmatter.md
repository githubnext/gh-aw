...
| `observability.otlp.ignore-if-missing` | boolean | When `true`, if OTLP endpoint/header values resolve to empty values at runtime (for example because a referenced secret is unset), gh-aw logs a warning and skips MCP gateway OTLP configuration instead of failing the workflow. |
...
| `GH_AW_OTLP_IGNORE_IF_MISSING` | Set to `true` when `observability.otlp.ignore-if-missing` is enabled. Used by runtime gateway setup to downgrade missing OTLP endpoint/header values from errors to warnings. |
