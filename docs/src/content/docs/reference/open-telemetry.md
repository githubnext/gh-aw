...
| `OTEL_SERVICE_NAME` | `gh-aw.<sanitized-workflow-id>` when the workflow ID is available (for example, `Repo Triage/Weekly` → `gh-aw.repo-triage-weekly`); otherwise `gh-aw`. |
