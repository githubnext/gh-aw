...
| `safe-output-github-app` | object | Optional per-checkout GitHub App credentials used exclusively for safe_outputs git operations on this checkout target (`client-id` or `app-id` (deprecated), `private-key`, optional `owner`, `repositories`). Does not change agent/activation checkout auth. When omitted, the safe_outputs job uses `GITHUB_TOKEN` for this checkout — `safe-outputs.github-app` and `safe-outputs.github-token` are never used as checkout tokens and apply only to PR/push operations. Use this field for cross-org workflows where the safe_outputs target org differs from the workflow repository org. |
