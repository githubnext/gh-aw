...
| `safe-output-github-app` | object | Optional safe_outputs-only GitHub App credentials for this checkout target (`client-id` or `app-id` (deprecated), `private-key`, optional `owner`, `repositories`). Does not change agent/activation checkout auth. |
