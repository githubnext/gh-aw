...
**Patch Size Limit**: Use `max-patch-size` to limit the total size of changes in a single push (default: 10KB). The total size of the git diff (all staged changes combined) must not exceed this value. If it does, the push is rejected with an error. Use this to prevent large unintentional memory updates.
