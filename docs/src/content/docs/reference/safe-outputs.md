...
**Note:** The `category` field can be either a category ID (e.g., `"DIC_kwDOGFsHUM4BsUn3"`) or a category name (e.g., `"General"`). The workflow will first try to match against category IDs, then against category names. If no `category` is specified, the workflow will use the first available discussion category in the repository.
