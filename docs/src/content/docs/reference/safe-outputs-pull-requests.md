...
description: Reference for create-pull-request and push-to-pull-request-branch safe outputs, including manifest file protection policy.
sidebar:
  order: 801
---

This page covers the two safe-output types that write code to a repository: [`create-pull-request`](#pull-request-creation-create-pull-request) and [`push-to-pull-request-branch`](#push-to-pr-branch-push-to-pull-request-branch). Both types enforce [Manifest File Protection](#manifest-file-protection) by default.
...
safe-outputs:
  create-pull-request:
    ...
    manifest-files: fallback-to-issue  # push branch, create review issue if manifest files modified
```
...
Both `create-pull-request` and `push-to-pull-request-branch` enforce manifest file protection by default. Patches that modify package manifests, agent instruction files, or repository security configuration are refused unless you explicitly configure a policy.
...
Configure the `manifest-files` field on either safe output:
...
safe-outputs:
  ...
  push-to-pull-request-branch:
    manifest-files: fallback-to-issue  # create issue instead of pushing when manifests change
```

When manifest file protection triggers and is set to `blocked`, the 🛡️ **Manifest File Protection Triggered** section appears in the agent failure issue or comment generated by the conclusion job. It includes the blocked operation, the specific files found, and a YAML remediation snippet showing how to configure `manifest-files: fallback-to-issue`.
