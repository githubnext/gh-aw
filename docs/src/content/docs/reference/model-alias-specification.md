...
- Normative validation rules applied at compile time
...
provider-token    = 1*( ALPHA / DIGIT / "-" / "_" )
...
model-glob-token  = 1*( ALPHA / DIGIT / "-" / "_" / "." / "*" )
...
- Parameter keys and values MUST use only URL-safe characters from the set `[A-Za-z0-9\-_.]`.
...
- The provider prefix is matched exactly (no wildcards in provider segment).
- The first catalog entry that matches the pattern is returned.
...
An implementation MUST maintain a set of in-progress alias names during recursion. If `Resolve` is called with a `base` already in the in-progress set, it MUST immediately report a circular reference error and halt resolution.
...
- **V-MAF-010**: Detect and report circular alias references.
...
- **T-MAF-040**: Circular alias `a: [b]`, `b: [a]` is detected and reported as a compile-time error
- **T-MAF-041**: Unrecognized parameter key `?foo=bar` produces a compile-time warning
- **T-MAF-042**: Alias with no matching catalog entries produces a compile-time warning
...
| Circular alias detection | T-MAF-040 | 3 | Required |
| Unrecognized param warning | T-MAF-041 | 3 | Recommended |
| Empty catalog warning | T-MAF-042 | 3 | Recommended |
