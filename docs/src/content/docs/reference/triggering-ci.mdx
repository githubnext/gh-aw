...
When configured, the token will be used to push an extra empty commit to the PR branch after PR creation. This will trigger `push` and `pull_request` events normally, even when checkout persisted `GITHUB_TOKEN` credentials.
