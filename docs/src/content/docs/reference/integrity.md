...
| `disapproval-labels` | array or expression | No | `[]` | GitHub label names that degrade items to `none` integrity |
...
`blocked-users`, `trusted-users`, `approval-labels`, and `disapproval-labels` can each accept a GitHub Actions expression instead of a literal array. The expression is evaluated at runtime and should resolve to a comma- or newline-separated list of values.
...
4. **Else if the item has a label in `disapproval-labels`**: effective integrity → `none`.
...
Each per-item list (`blocked-users`, `trusted-users`, `approval-labels`, `disapproval-labels`) can also be extended centrally using GitHub repository or organization variables. The runtime automatically unions the per-workflow values with the corresponding variable:
...
| `disapproval-labels` | `GH_AW_GITHUB_DISAPPROVAL_LABELS` |
...
This mechanism allows a security team to maintain a shared blocked-users list or approval-labels/disapproval-labels policy without modifying individual workflow files.
