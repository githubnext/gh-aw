...
**Default Max**: 10  
**Cross-Repository Support**: Yes (via `content_repo` field in agent output; requires `allowed-repos` configuration)  
**Mandatory**: No
...
- Cross-repo support uses `content_repo` in agent output to resolve issues/PRs from other repos; the `allowed-repos` configuration option controls which repos are permitted
