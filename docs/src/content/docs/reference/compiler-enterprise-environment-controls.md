...
| `GH_AW_DEFAULT_MAX_CACHE_MISSES` | Compiler process environment | Default AWF `apiProxy.maxCacheMisses` guardrail | `max-cache-misses` is not set in frontmatter or any imported workflow |
...
`default_`-prefixed keys such as `default_max_ai_credits`, `default_max_cache_misses`, `default_detection_max_ai_credits`, `default_max_daily_ai_credits`, `default_timeout_minutes`,
...
For max cache misses, precedence is:

1. `max-cache-misses` in workflow frontmatter
2. `GH_AW_DEFAULT_MAX_CACHE_MISSES`
3. Built-in constant default: `5`

The compiler emits `apiProxy.maxCacheMisses` directly in the AWF config JSON. When `max-cache-misses` is omitted, the compiler reads `GH_AW_DEFAULT_MAX_CACHE_MISSES` from its process environment and falls back to `5` if the variable is unset or invalid.
...
export GH_AW_DEFAULT_MAX_CACHE_MISSES=7
