...
:::note[Deprecated form]
`${{ needs.activation.outputs.text }}`, `${{ needs.activation.outputs.title }}`, and `${{ needs.activation.outputs.body }}` are deprecated aliases. The compiler auto-rewrites them with a deprecation warning. Use `steps.sanitized.outputs.*` directly in all new and updated workflows.
:::
