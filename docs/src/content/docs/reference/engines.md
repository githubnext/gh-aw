...
description: Complete guide to AI engines (coding agents) usable with GitHub Agentic Workflows, including Copilot, Claude, Codex, Gemini, Antigravity, and Pi with their specific configuration options.
sidebar:
  order: 600
---
...
| Feature | Copilot | Claude | Codex | Gemini | Antigravity | Pi |
|---------|:-------:|:------:|:-----:|:------:|:-----:|:--------:|:--:|
...
Defaults: Claude `60s`, Codex `120s`. Other engines (Copilot, Gemini, Antigravity, Pi) are engine-managed and not enforced by gh-aw. See [Tool Timeout Configuration](/gh-aw/reference/tools/#tool-timeout-configuration) for full documentation including `tools.startup-timeout`.

### Per-Engine Timeout Controls

| Knob | Copilot | Claude | Codex/Gemini/Antigravity/Pi | Purpose |
|---|:---:|:---:|:---:|---|
...
Copilot uses `max-continuations` for autopilot runs; Claude uses `max-turns` to cap iterations. Codex, Gemini, Antigravity, and Pi rely solely on `timeout-minutes` and `tools.timeout`.
