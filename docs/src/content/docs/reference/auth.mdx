...
| **Gemini** (deprecated) | [`GEMINI_API_KEY`](#gemini_api_key) | — | Legacy Google Gemini CLI API key |
| **Antigravity** | [`ANTIGRAVITY_API_KEY`](#antigravity_api_key) | — | API key from Google AI Studio |
...
If using the Antigravity by Google engine, you need to set a GitHub Actions secret `ANTIGRAVITY_API_KEY` with an API key from Google AI Studio.

Gemini remains temporarily supported, but new workflows should use Antigravity instead. Run `gh aw fix --write` to migrate existing Gemini workflows when you're ready.
...
2. Add it to your repository secrets, either by CLI or GitHub UI:
   ...
   ```bash wrap
   gh aw secrets set ANTIGRAVITY_API_KEY --value "YOUR_ANTIGRAVITY_API_KEY"
   ```

See also [AI Engines](/gh-aw/reference/engines/#available-coding-agents) for additional configuration needed when using Antigravity with GitHub MCP.

---

### `GEMINI_API_KEY`

If using the deprecated Gemini by Google engine, you need to set a GitHub Actions secret `GEMINI_API_KEY` with an API key from Google AI Studio.

> [!WARNING]
> `engine: gemini` is deprecated. Prefer `engine: antigravity` with `ANTIGRAVITY_API_KEY` for new workflows.
...
See also [AI Engines](/gh-aw/reference/engines/#available-coding-agents) for migration guidance to Antigravity.
...
**`401 Unauthorized` (Antigravity)**

The `ANTIGRAVITY_API_KEY` secret is missing or invalid. Generate a new key at https://aistudio.google.com/api-keys.
