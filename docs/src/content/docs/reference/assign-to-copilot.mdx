...
When `github-app:` is configured in `safe-outputs`, `assign-to-agent` automatically falls back to using `GH_AW_AGENT_TOKEN` (or the token chain `GH_AW_AGENT_TOKEN || GH_AW_GITHUB_TOKEN || GITHUB_TOKEN`). Make sure `GH_AW_AGENT_TOKEN` is set as a repository secret with the required PAT permissions, or specify an explicit `github-token:` in your `assign-to-agent` config.
:::
