...
**Maintenance workflow generation is lazy**: `agentics-maintenance.yml` is only generated when at least one workflow explicitly opts in — by setting `expires` on `create-issue`, `create-discussion`, or `create-pull-request`, or by explicitly configuring `safe-outputs.noop`. The implicit `noop` fallback that is added automatically when `safe-outputs` is present does not generate a maintenance workflow.
