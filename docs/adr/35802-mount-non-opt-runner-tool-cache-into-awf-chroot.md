...
1. The generated AWF command **MUST** resolve the active tool cache using the following ordered fallback chain: `RUNNER_TOOL_CACHE` when set; otherwise `/opt/hostedtoolcache` when it exists; otherwise `/home/runner/work/_tool` when it exists.
