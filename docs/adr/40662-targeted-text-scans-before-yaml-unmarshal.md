...
### Negative
- The scan logic is more complex: `hasNonAllowedExpressionInRunContent` re-implements `run:` block tracking (indent handling, block vs. inline scalars) in text-scanning code that must stay consistent with the YAML parser's view of the same structure.
...
*This is a DRAFT ADR generated by the [Design Decision Gate](https://github.com/github/gh-aw/actions/runs/27914034292) workflow. The PR author must review, complete, and finalize this document before the PR can merge.*
