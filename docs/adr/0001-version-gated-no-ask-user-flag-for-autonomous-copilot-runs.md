...
We will add a version-gate helper (`copilotSupportsNoAskUser`) that inspects the workflow's `EngineConfig.Version` and returns `true` only when the configured CLI version is ≥ 1.0.19 (the minimum version that accepts `--no-ask-user`). The flag is emitted during argument construction in `GetExecutionSteps` for agent jobs only — detection jobs (identified by `SafeOutputs == nil`) must never receive the flag. Unspecified versions and `"latest"` are treated as always supported; non-semver strings (e.g., branch names) are treated conservatively as unsupported. This pattern mirrors the existing `awfSupportsExcludeEnv` and `awfSupportsCliProxy` helpers, extending the established version-gate convention in this codebase.
...
- 187 lock files were regenerated as part of the dependency graph update triggered by the code change.
...
1. Implementations **MUST** emit `--no-ask-user` to the Copilot CLI argument list for agent jobs when the effective CLI version is ≥ `CopilotNoAskUserMinVersion` (currently `1.0.19`).
2. Implementations **MUST NOT** emit `--no-ask-user` for detection jobs (identified by `WorkflowData.SafeOutputs == nil`).
3. Implementations **MUST NOT** emit `--no-ask-user` when the effective CLI version is a semver string strictly less than `CopilotNoAskUserMinVersion`.
4. Implementations **MUST NOT** emit `--no-ask-user` when the effective CLI version is a non-semver string (e.g., a branch name), treating such cases conservatively as unsupported.
5. Implementations **MUST** treat an unspecified version (nil `EngineConfig` or empty `EngineConfig.Version`) as supported, since `DefaultCopilotVersion` is always ≥ `CopilotNoAskUserMinVersion`.
6. Implementations **MUST** treat the literal string `"latest"` (case-insensitive) as supported, since `latest` always resolves to a current release.
...
An implementation is considered conformant with this ADR if it satisfies all **MUST** and **MUST NOT** requirements above. In particular: `--no-ask-user` **MUST** be gated on both job type (agent vs. detection) and CLI version (≥ 1.0.19 or unspecified/latest). Failure to meet any **MUST** or **MUST NOT** requirement constitutes non-conformance.
