# ADR-30021: Multiple OTLP Endpoints with JSON-Array Fan-Out via GH_AW_OTLP_ENDPOINTS
...
The gh-aw observability pipeline exports OpenTelemetry (OTLP) spans from GitHub Actions jobs. Until this point the system supported exactly one collector endpoint, configured via `OTEL_EXPORTER_OTLP_ENDPOINT` in the environment. Users operating redundant or vendor-differentiated observability stacks (e.g., primary collector + hot-standby, or two different SaaS vendors simultaneously) had no way to fan out spans to more than one destination. An earlier ADR (28524) already extended the endpoint object to support per-endpoint header objects; this change builds on that to allow a list of such objects. Because the Go compiler and the JavaScript action steps run in separate processes, a compact, self-contained serialization format is needed to pass the full endpoint list across the process boundary without requiring code changes in every consuming script.

### Decision

We will introduce a new `endpoints` array in the `observability.otlp` frontmatter field and propagate all endpoint configurations — including those expressed with the legacy scalar `endpoint:` field — as a single JSON-encoded array in the environment variable `GH_AW_OTLP_ENDPOINTS`. JavaScript action scripts will read exclusively from `GH_AW_OTLP_ENDPOINTS` and fan out spans to every listed endpoint concurrently via `Promise.allSettled`. The legacy `OTEL_EXPORTER_OTLP_ENDPOINT` variable is still injected for backward compatibility with the MCP gateway but is no longer used by the JavaScript fan-out logic.

### Alternatives Considered

#### Alternative 1: Separate Indexed Environment Variables (`OTEL_EXPORTER_OTLP_ENDPOINT_0`, `_1`, …)
...
#### Alternative 2: Retain OTEL_EXPORTER_OTLP_ENDPOINT as the Primary Signal and Add a Separate Multi-Endpoint Variable
...
#### Positive
- Workflow authors can now send spans to multiple OTLP collectors simultaneously by listing them under `observability.otlp.endpoints`.
- JavaScript action code is simplified: a single code path (`parseOTLPEndpoints` + `sendOTLPToAllEndpoints`) handles both the single-endpoint and multi-endpoint cases.
...
1. Workflow authors **MAY** specify multiple OTLP endpoints using the `observability.otlp.endpoints` array field, where each element **MUST** contain at minimum a `url` key and **MAY** contain a `headers` map.
2. Workflow authors **MAY** continue to use the legacy `observability.otlp.endpoint` and `observability.otlp.headers` scalar fields; the compiler **MUST** treat them as a single-element endpoint configuration and include them in the serialized `GH_AW_OTLP_ENDPOINTS` array.
3. Both the `endpoint` scalar form and the `endpoints` array form **MAY** be present simultaneously; the compiler **MUST** merge them into a single list with the scalar endpoint normalized first.

### Environment Variable Contract

1. The Go compiler **MUST** inject `GH_AW_OTLP_ENDPOINTS` as a JSON-encoded array of endpoint objects for every workflow that contains at least one OTLP endpoint configuration, regardless of whether one or multiple endpoints are specified.
...
4. Static hostname values extracted from the `endpoints` array **MUST** be added to the network firewall allowlist at compile time; expression-based URL values (`${{ … }}`) **MUST** be skipped.
...
An implementation is considered conformant with this ADR if it satisfies all **MUST** and **MUST NOT** requirements above. Specifically: the Go compiler must always serialize endpoint configurations into `GH_AW_OTLP_ENDPOINTS` as a JSON array; JavaScript consumers must read exclusively from that array and fan out concurrently; and the legacy `OTEL_EXPORTER_OTLP_ENDPOINT` must remain injected for MCP gateway backward compatibility but must not be used as the primary endpoint source by new code.
