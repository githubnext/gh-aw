...
- [ ] A test asserts that the diagnostic log line is emitted when `byok-copilot` implicitly activates `cli-proxy` on a Copilot engine workflow.
- [ ] `byok-copilot` has been added to the end-user workflow authoring reference documentation.
