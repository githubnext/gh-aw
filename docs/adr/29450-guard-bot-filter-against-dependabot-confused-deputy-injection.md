...
#### Negative
- Any legitimate scenario where `github.actor` intentionally differs from the PR or comment author field — for example, a GitHub App that opens PRs on behalf of a human user through the API — will be denied at `check_membership.cjs`. Such scenarios are believed to be non-existent in the gh-aw context, but the denial will require a code change if they materialise.
...
2. For `pull_request` events, the function **MUST** return `true` if and only if `actor` does not strictly equal `payload.pull_request.user.login`.
