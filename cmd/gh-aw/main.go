...
// compileRunFlags holds all flag values read for the compile command.
type compileRunFlags struct {
...
// readCompileFlags reads all compile command flags into a compileRunFlags struct.
func readCompileFlags(cmd *cobra.Command) compileRunFlags {
	var f compileRunFlags
	...
	f.ghAwRef, _ = cmd.Flags().GetString("gh-aw-ref")
...
// buildCompileConfig converts the compile run flags into a cli.CompileConfig.
func buildCompileConfig(args []string, f compileRunFlags) cli.CompileConfig {
	workflowDir := f.dir
	if f.workflowsDir != "" {
...
func runCompileCmd(cmd *cobra.Command, args []string) error {
	f := readCompileFlags(cmd)

	if f.ghAwRef != "" {
		resolvedRef, resolveErr := workflow.ResolveGhAwRef(cmd.Context(), f.ghAwRef)
		if resolveErr != nil {
	...
	if err := validateEngine(f.engineOverride); err != nil {
		return err
	}

	finishCompileUpdateCheck := cli.StartCompileUpdateCheck(cmd.Context(), f.noCheckUpdate, f.verbose)
	...
	config := buildCompileConfig(args, f)
...
// Package-level command vars for commands instantiated during init.
var (
...
// registerCommandGroups adds the five display groups to rootCmd.
...
// configureRootCmdSettings sets global flags, output, silencing, version template,
// and corrects the help/version flag descriptions for the root command.
func configureRootCmdSettings() {
...
// printSubcmdsByGroup prints the available subcommands in groups.
func printSubcmdsByGroup(cmd *cobra.Command, colFmt string) {
	out := cmd.OutOrStderr()
...
// commandUsageFunc is the SetUsageFunc callback that rewrites "gh " → "gh aw "
// so that subcommand usage lines show the correct binary name.
func commandUsageFunc(cmd *cobra.Command) error {
	...
	if cmd.HasAvailableSubCommands() {
		...
		if len(cmd.Groups()) == 0 {
			...
			printSubcmdsByGroup(cmd, colFmt)
		}
	}
...
// setCustomHelpCmd creates and installs the custom help command that supports
// "help all" for a full command reference printout.
func setCustomHelpCmd() {
...
// instantiateAllCmds creates all commands that are constructed via cli.NewXxx helpers.
func instantiateAllCmds() {
...
// registerNewCmdFlags adds flags to newCmd.
...
// registerCompileCmdFlags adds all flags, completions, and mutual-exclusion rules to compileCmd.
func registerCompileCmdFlags() {
...
// registerRemoveCmdFlags adds flags and completions for removeCmd.
...
// registerToggleCmdFlags adds flags and completions for enableCmd and disableCmd.
func registerToggleCmdFlags() {
...
// registerRunCmdFlags adds all flags and completions for runCmd.
func registerRunCmdFlags() {
...
// registerFlagsForCmds sets up all flag registrations for each command.
func registerFlagsForCmds() {
	registerNewCmdFlags()
	registerCompileCmdFlags()
	registerRemoveCmdFlags()
	registerToggleCmdFlags()
	registerRunCmdFlags()
}

// assignGroupIDsToAllCmds sets the GroupID field on every command.
func assignGroupIDsToAllCmds() {
...
// addCmdsToRoot adds all commands to rootCmd.
func addCmdsToRoot() {
...
// fixSubCmdHelpFlags recursively updates the --help flag description for cmd
// and all its subcommands so it reads "Show help for gh aw <cmd>" rather than
// the Cobra default "help for [cmd]".
func fixSubCmdHelpFlags(cmd *cobra.Command) {
	cmd.InitDefaultHelpFlag()
...
func init() {
	...
	configureRootCmdSettings()
	rootCmd.SetUsageFunc(commandUsageFunc)
	setCustomHelpCmd()
	instantiateAllCmds()
	registerFlagsForCmds()
	assignGroupIDsToAllCmds()
	addCmdsToRoot()
