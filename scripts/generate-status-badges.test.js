...
allPassed &= assertContains(
  output,
  '| copilot |',
  'Copilot engine detected in at least one workflow'
);
...
allPassed &= assertContains(
  output,
  '| claude |',
  'Claude engine detected in at least one workflow'
);
...
allPassed &= assertContains(
  output,
  '| codex |',
  'Codex engine detected in at least one workflow'
);
...
allPassed &= assertContains(
  output,
  '.github/workflows/',
  'Workflow links to .github/workflows directory'
);

allPassed &= assertContains(
  output,
  '.md)',
  'Workflow links point to .md files'
);
...
allPassed &= assertContains(
  output,
  'https://github.com/githubnext/gh-aw/actions/workflows/',
...
allPassed &= assertNotContains(
  output,
  '| unknown |',
  'No workflows with unknown engine (should default to copilot)'
);
...
allPassed &= assertContains(
  output,
  'Click on a workflow link to view the source markdown file.',
  'Note mentions workflow links'
);
...
if (tableRowCount >= 50) { // We expect at least 50 workflows
...
allPassed &= assertNotContains(
  output,
  '<CardGrid>',
  'No CardGrid component (should be table now)'
);

allPassed &= assertNotContains(
  output,
  '<Card>',
  'No Card component (should be table now)'
);
