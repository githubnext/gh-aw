...
T9_BASE_REF=$(git -C "$T9" rev-parse HEAD)
...
if (cd "$T9" && bash "$STALE_SCRIPT" --base-ref "$T9_BASE_REF" >"$T9_OUT" 2>&1); then
...
T10_BASE_REF=$(git -C "$T10" rev-parse HEAD)
...
if (cd "$T10" && bash "$STALE_SCRIPT" --base-ref "$T10_BASE_REF" >"$T10_OUT" 2>&1); then
