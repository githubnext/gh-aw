...
Require values that heuristically look like secrets to be passed to the core `setSecret` API. Registering a value with the Actions toolkit masks later occurrences in runner logs.
...
Accepted masking forms include `core.setSecret?.(value)` (the runtime convention), `core.setSecret(value)` when the toolkit is guaranteed, computed access, stable aliases of `core`, and destructured `setSecret` aliases. Passing a derived expression such as `core.setSecret?.(token.trim())` also masks the tracked binding.
