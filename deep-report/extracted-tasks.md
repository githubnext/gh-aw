## Extracted code-quality tasks (2026-09-01, ~18:00Z cycle, baseline #57702, window since 12:47:24Z, 10 new discussions: 57703,57705,57706,57716,57719,57722,57725,57727,57736,57744)

All 5 filed issues live-verified against current repo state (not just report text) before filing:

1. **Filed**: Add `go` ecosystem preset to `.github/workflows/ci-coach.md` `network.allowed` (currently `[defaults, node]`) — Daily Security Observability Report #57736 showed `proxy.golang.org:443` blocked 136x this week (61% of all repo-wide firewall blocks), 100% attributed to CI Optimization Coach; live-verified the workflow's frontmatter lacks the `go` preset and that `pkg/workflow/domains.go` defines one.
2. **Filed**: Append corrective example to glob path validation error message, `pkg/workflow/glob_validation.go:280` — User Experience Analysis Report #57719, live-verified exact line/message text.
3. **Filed**: Standardize 🔬→🧪 emoji in `.github/workflows/test-quality-sentinel.md:210` `run-started` message (workflow's declared `emoji: "🧪"` and `footer` both use 🧪) — same report #57719, live-verified lines 3/209/210.
4. **Filed**: Extract shared `RequireGit`/`RequireDocker` test helpers into existing `pkg/testutil` package, replacing 29+ duplicated git/docker environment-guard skip blocks in `pkg/cli/init_command_test.go` (16 sites) and `pkg/cli/git_test.go` (13 sites) — Repository Quality Improvement Report #57706 Task 3, live-verified site counts and that `pkg/testutil` already exists.
5. **Filed**: Add non-blocking `make lint-unconditional-skips-report` target flagging unconditional `t.Skip` calls — same report #57706 Task 4, live-verified 12+2=14 unconditional `t.Skip("pending...")` sites in `otel_reliability_formal_test.go`/`otel_observability_formal_test.go`, and that the `lint-error-messages-report` non-blocking-target precedent exists at `Makefile:1041-1042`.

**Declined/deferred this cycle** (see [[flagged_items]] for full reasoning): `CLAUDE_CODE_OAUTH_TOKEN` silent-ignore (docs review #57703) — chronic, closed 4-6x, standing policy, not re-filed; `ab.chatgpt.com:443` blocks (#57736) — 2nd+ cycle, previously root-caused as likely analytics-subdomain noise not a missing-preset bug, not a code fix; unlabeled-WIP auto-label-at-creation (daily issues #57716) — chronic, no single attributable source location after real grep, standing decline; CodeQL alert #653 stale dismissal (UK AI Resilience #57727) — already self-filed by that report as #57728; `gateway.jsonl`/DIFC author_login-unknown telemetry gap (security observability #57736) — same family as chronic gateway.jsonl gap, standing policy against re-filing without verified-merged evidence; repository-quality Tasks 1-2 (triage the 14 otel skips themselves) — judgment-heavy (Go vs JS implementation choice), left for the source workflow's own follow-through rather than pre-empting with a vague issue; quick-start.mdx Copilot-framing rebalance — previously NOT_PLANNED 2x (#46478, #53927); 6 "open Dependabot PRs" (Agent Performance Report #57705) — live `gh pr list` showed zero Dependabot PRs currently open, stale snapshot claim, correctly not filed (see [[known_patterns]]); flat-CODEOWNERS structural risk (UK AI Resilience #57727) — source report itself judged below filing threshold.
...
## 2026-09-01 cycle (1 issue filed, baseline #57495, window #57497-57558, 9 new discussions)
