...
|--------------|------------------------------|-------------------------------|
| `1.0.30` | `v0.87.9` (or newer) | No new CTR rules; extends the CTR-001 Privilege Escalation mapping to document the already-implemented, already-tested status-function bypass guard (`guardIfAgainstStatusFuncBypass`, `selectCompilerOwnedNeeds`, `ifExpressionContainsStatusFunc` in `pkg/workflow/compiler_builtin_job_augmentation.go`), which was absent from the Section 7.1 mapping table. This guard rewrites a user-declared `jobs.agent.if` (or another built-in job's `if`) containing a GitHub Actions status function (`always()`, `success()`, `failure()`, `cancelled()`) to add explicit `needs.<job>.result == 'success'` guards for each compiler-owned prerequisite (e.g. `activation`), because GitHub Actions removes the implicit success() check for all `needs` entries the moment any status function appears in an `if` expression; author-owned custom jobs auto-wired as prerequisites are intentionally left unguarded so authors can choose their own result semantics (e.g. an `if: always()` agent analyzing a failing probe job). No `.lock.yml` schema changes. |
...
adds CTR-025; `1.0.26` adds CTR-026; `1.0.27` extends the CTR-006 mapping
with `mcp_renderer_guard.go`; and `1.0.30` extends the CTR-001 mapping with the
status-function bypass guard (`guardIfAgainstStatusFuncBypass`). Versions with no
distinct lock-file impact are grouped above.
...
|---------|-------------------------------|-----------------------|
| CTR-001 Privilege Escalation | `pkg/workflow/*permissions*validation*.go`, `pkg/workflow/strict_mode_permissions_validation.go`, `pkg/workflow/github_app_permissions_validation.go`; compiler-owned prerequisite status-function bypass guard (`pkg/workflow/compiler_builtin_job_augmentation.go`: `guardIfAgainstStatusFuncBypass`, `selectCompilerOwnedNeeds`, `ifExpressionContainsStatusFunc`) preventing a user-declared `always()`/`success()`/`failure()`/`cancelled()` job `if` condition from silently bypassing the implicit success check on compiler-owned prerequisites such as `activation` | `pkg/workflow/*permissions*_test.go`, `pkg/workflow/*dangerous_permissions*_test.go`, `pkg/workflow/compiler_custom_jobs_test.go` (`TestApplyBuiltinJobNeedsAugmentations_StatusFuncAlwaysAddsSuccessGuards`, `TestApplyBuiltinJobNeedsAugmentations_StatusFuncKeepsCustomJobUnguarded`) |
...
Audit result: ✅ all listed `CTR-001` through `CTR-026` rows currently include non-empty implementation references and non-empty test coverage targets; no `TODO` placeholders were found in the mapping table. Review window: daily optimizer cycle 2026-09-02, covering the merged `custom-job-status-func-guard` changeset. Security-relevant items evaluated: (1) **Custom-job status-function bypass guard** (`pkg/workflow/compiler_builtin_job_augmentation.go`: `guardIfAgainstStatusFuncBypass`, `selectCompilerOwnedNeeds`, `ifExpressionContainsStatusFunc`): custom jobs auto-wired as prerequisites of built-in jobs are no longer force-guarded when a built-in job's `if` (e.g. `jobs.agent.if`) uses a status function such as `always()`; only compiler-owned prerequisites (e.g. `activation`) retain the explicit `needs.<job>.result == 'success'` guard GitHub Actions would otherwise silently drop once a status function appears in the expression. This is an already-implemented and already-tested (`pkg/workflow/compiler_custom_jobs_test.go`) privilege-escalation-prevention control that was absent from the Section 7.1 CTR-001 mapping; no new threat class — folded into the existing CTR-001 mapping (see 1.0.30 sync-log entry above). No other security-sensitive compiler or parser diffs, and no new `threat-detection-suppress` annotations, were found in the review window.
