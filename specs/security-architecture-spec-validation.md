...
| Permission Management | T-PM-001 to T-PM-007 | ✅ EVIDENCED | PM-01/PM-02 (permission defaults), PM-08 (fork protection), PM-10/PM-11 (RBAC) verified in earlier sections; T-PM-003 (strict mode) via `TestFormalStrictMode_WritePermissionsRejected` and `TestFormal_CTR001_WritePermissionsRejected`; T-PM-005 (repository validation for `workflow_run`) via `TestFormalRS003_WorkflowRunRepositoryValidation`; T-PM-007 (token validation) via `TestFormalRS005_RuntimeTokenValidation` |
| Sandbox Isolation | T-SI-001 to T-SI-007 | ⚠️ PARTIALLY EVIDENCED | §8c adds compiled-workflow and runtime-script evidence for AWF chrooting, docker-host indirection, environment filtering, MCP/tool mounts, and composed sandbox/firewall operation; direct runtime host-visibility proof remains outstanding (tracked in [#48686](https://github.com/github/gh-aw/issues/48686)); follow-up checkpoint: re-evaluate when [#48686](https://github.com/github/gh-aw/issues/48686) is closed or updated |
| Threat Detection | T-TD-001 to T-TD-007 | ✅ EVIDENCED | T-TD-001 via `TestFormalThreatDetection_EnabledByDefault`; T-TD-002 (prompt injection) via `TestFormalTD002_PromptInjectionDetection`; T-TD-003 (secret leaks) via `TestFormalTD003_SecretLeakDetection`; T-TD-004 (malicious patches) via `TestFormalTD004_MaliciousPatchDetection`; T-TD-005 (custom prompt) via `TestFormalTD005_CustomPromptSupport`; T-TD-006 (engine override) via `TestFormalTD006_EngineConfigOverride`; T-TD-007 (workflow failure on detection) via `TestFormalTD007_WorkflowFailureOnDetection` — all in `pkg/workflow/security_architecture_td_formal_test.go` |
...
1. **Sandbox Isolation (T-SI-001 to T-SI-007)** — Reduced from full gap to partial evidence in §8c. Remaining work is a direct runtime probe for host/socket visibility from inside the AWF sandbox (tracked in [#48686](https://github.com/github/gh-aw/issues/48686)). **Follow-up checkpoint (2026-07-29)**: Issue #48686 was open as of this run. Re-evaluate this gap entry when the issue is resolved or a runtime probe result is recorded. Maintainers SHOULD add a comment on [#48686](https://github.com/github/gh-aw/issues/48686) linking this document so progress on the probe is visible here.

Maintainers SHOULD address remaining gaps in order of risk: the residual sandbox-runtime probe (the only remaining open gap after 2026-07-29 changes).
