...
4. The workflow **SHOULD** open or update a tracking issue when canonical source unavailability persists from one scheduled cron invocation through the next scheduled cron invocation. Manual reruns and ad hoc invocations still perform the refresh and freshness checks above, but do not advance this consecutive-scheduled-run threshold. If the workflow's `cron` schedule changes between the two invocations being compared (for example, the interval is shortened or lengthened), the "next scheduled cron invocation" **MUST** be interpreted as the next invocation actually triggered by the schedule in effect at that time; a schedule change **MUST NOT** by itself reset or skip the persistence threshold, but it also **MUST NOT** be used to count more than one consecutive-scheduled-run advance across the transition. (T-DR-SAFE-004)
